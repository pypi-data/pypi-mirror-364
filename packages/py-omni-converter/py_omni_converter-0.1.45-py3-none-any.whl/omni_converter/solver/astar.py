...
def timeout_manager(duration_sec):
    ...
    dt = end - start
    if dt.total_seconds() > duration_sec:
        raise RuntimeError(f"Operation took {dt.total_seconds():.2f} seconds")

@contextmanager
def warn_on_timeout(duration_sec,msg:str):
    start = datetime.now()
    yield
    end = datetime.now()
    if end - start > timedelta(seconds=duration_sec):
        from loguru import logger
        logger.warning(f"{msg} took longer than {duration_sec} seconds. Consider optimizing the operation.")
OMNI_CONVERTER_TIMEOUT_SEC=60
...
class AstarSolver(ISolver):
    ...
    def _solve(self, start, end, silent) -> Converter:
        ...
        timeout = timedelta(seconds=float(os.environ.get("OMNI_CONVERTER_TIMEOUT_SEC", OMNI_CONVERTER_TIMEOUT_SEC)))
        while to_visit:
            if datetime.now() - search_start >= timeout:
                raise RuntimeError(f"AstarSolver search timed out after {timeout.total_seconds():.2f} seconds. consider changing OMNI_CONVERTER_TIMEOUT_SEC env variable.")
            if not silent:
            ...
            with warn_on_timeout(1,f"getting negihbors for {pos}"):
