...
class ImageGenerator:
    ...
    def create_image(self, file_ident):
        ...
        cmd = ["openscad", "--view", "axes", "--render", "1", "-o", png_path, scad_path]
        try:
            subprocess.run(cmd, env=self._env, check=True)
        except subprocess.CalledProcessError:
            print(
                "Error running OpenSCAD on {scad_path}. This may be a scad syntax error."
            )
