...
class FileProjectRegistry(ProjectRegistry):
    ...
    def _get_project_index_file(self) -> Path:
    ...
    async def _update_task_index(self, project_id: str, user_id: Optional[str], config_path: Optional[str] = None) -> None:
    ...
    async def add_project(self, user_id: str, project_id: str, config_path: Optional[str] = None) -> None:
        ...
        async with self._lock:
            ...
            if project_id not in data["tasks"]:
                data["tasks"].append(project_id)
                await self._write_user_data(user_id, data)
                await self._update_task_index(project_id, user_id, config_path)
                logger.info(f"Added task {project_id} to user {user_id}")
    ...
    async def remove_project(self, user_id: str, project_id: str) -> None:
        ...
        async with self._lock:
            ...
            if project_id in data["tasks"]:
                data["tasks"].remove(project_id)
                await self._write_user_data(user_id, data)
                await self._update_task_index(project_id, None)
                logger.info(f"Removed task {project_id} from user {user_id}")
    ...
    async def user_owns_project(self, user_id: str, project_id: str) -> bool:
        """Check if a user owns a specific task"""
        tasks = await self.get_user_projects(user_id)
        return project_id in tasks
    ...
    async def get_project_owner(self, project_id: str) -> Optional[str]:
        ...
        try:
            async with aiofiles.open(index_file, 'r') as f:
                content = await f.read()
                index = json.loads(content)
                task_data = index.get(project_id)
...
class RedisProjectRegistry(ProjectRegistry):
    ...
    async def add_project(self, user_id: str, project_id: str, config_path: Optional[str] = None) -> None:
        ...
        await r.sadd(f"user:{user_id}:tasks", project_id)
        
        # Add reverse mapping
        await r.set(f"task:{project_id}:owner", user_id)
        
        # Track creation time
        await r.set(f"task:{project_id}:created_at", datetime.now().isoformat())
        
        # Store config path if provided
        if config_path:
            await r.set(f"task:{project_id}:config_path", config_path)
        
        logger.info(f"Added task {project_id} to user {user_id} in Redis")
    ...
    async def remove_project(self, user_id: str, project_id: str) -> None:
        ...
        await r.srem(f"user:{user_id}:tasks", project_id)
        
        # Remove reverse mapping
        await r.delete(f"task:{project_id}:owner")
        await r.delete(f"task:{project_id}:created_at")
        await r.delete(f"task:{project_id}:config_path")
        
        logger.info(f"Removed task {project_id} from user {user_id} in Redis")
    ...
    async def user_owns_project(self, user_id: str, project_id: str) -> bool:
        """Check if a user owns a specific task"""
        r = await self._get_redis()
        return await r.sismember(f"user:{user_id}:tasks", project_id)
    
    async def get_project_owner(self, project_id: str) -> Optional[str]:
        """Get the owner of a task"""
        r = await self._get_redis()
        owner = await r.get(f"project:{project_id}:owner")
    ...
    async def get_project_info(self, project_id: str) -> Optional[dict]:
        ...
        created_at = await r.get(f"task:{project_id}:created_at")
        config_path = await r.get(f"task:{project_id}:config_path")
...
def get_project_registry() -> ProjectRegistry:
    ...
    if os.getenv("REDIS_URL"):
        if not HAS_REDIS:
            logger.warning("REDIS_URL is set but redis.asyncio is not installed. Using file-based registry instead.")
            logger.warning("To use Redis, install it with: pip install redis[asyncio]")
            return FileProjectRegistry()
        logger.info("Using Redis for user-project registry")
        return RedisProjectRegistry(os.getenv("REDIS_URL"))
    else:
        logger.info("Using file-based user-project registry")
        return FileProjectRegistry()