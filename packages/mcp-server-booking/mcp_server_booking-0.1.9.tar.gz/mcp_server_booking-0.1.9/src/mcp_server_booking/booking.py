...
@mcp.tool(
    description="查询指定时间段内可用的场地（重要提示：1. 需要先调用 booking_get_field_info 获取场地信息；2. 查询时间段应该尽可能精确，因为只要有任何重叠的预订都会使场地被判定为不可用；3. 建议查询时间段与实际预订需求一致）",
)
...
@mcp.tool(
    description="预订指定时间段的指定类型场地（重要提示：1. 预订前必须先调用 booking_get_available_places 查询可用场地，确保场地在指定时间段内可用；2. 预订前建议调用 booking_get_field_info 获取场地详细信息；3. 单次预订时长限制为1小时，如需预订超过1小时请分多次预订；4. 预订操作不可撤销，请确认所有信息正确后再执行）",
)
