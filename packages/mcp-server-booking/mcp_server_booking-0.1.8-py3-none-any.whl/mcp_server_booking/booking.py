...
@mcp.tool(
    description="查询指定时间段的指定类型场地信息和预订情况",
)
...
@mcp.tool(
    description="查询指定时间段的指定类型场地可用情况",
)
...
@mcp.tool(
    description="预订指定时间段的指定类型场地(注意: 预订前请先查询可用场地和场地信息, 并确保场地可用。单次预定的时长限制为1小时, 如果预定时长超过1小时, 请分多次预订)",
)
