...
def unique_semaphore_name() -> str:
    return f"/shm_pipe_test_{uuid.uuid4().hex}"


def writer_proc(payloads, name_holder, sem_name=None):
    if sem_name is None:
        sem_name = unique_semaphore_name()
    with SharedMemoryPipe(semaphore_name=sem_name) as pipe:
        name_holder[0] = (pipe.name, sem_name)
        for p in payloads:
            pipe.write(p)


def read_all(name, sem_name, result_holder):
    with SharedMemoryPipe(name=name, semaphore_name=sem_name) as pipe:
        buf = bytearray()
        while True:
            chunk = pipe.read()
            if chunk is None:
...
def roundtrip_reader(name, sem, out_list):
    with SharedMemoryPipe(name=name, semaphore_name=sem) as pipe:
        while True:
            chunk = pipe.read()
            if chunk is None:
                break
...
def pickled_reader_reader(conn, result_holder):
    reader_pipe, sem_name = conn.recv()
    buffer = bytearray()
    while True:
...
def pickled_reader_writer(conn):
    sem = unique_semaphore_name()
    with SharedMemoryPipe(semaphore_name=sem) as pipe:
        conn.send((pipe.reader(), sem))
        pipe.write(b'ping')


def unlink_check_reader(shm_name, sem_name, during_flag, after_flag):
    shm_path = Path('/dev/shm') / shm_name
    with SharedMemoryPipe(name=shm_name, semaphore_name=sem_name) as pipe:
...
def slot_wraparound_writer(payloads, name_holder, sem_name, max_slots, slot_size):
...
def slot_wraparound_reader(shm_name, sem_name, sink, max_slots, slot_size):
    with SharedMemoryPipe(
        name=shm_name,
        semaphore_name=sem_name,
        max_slots=max_slots,
...
class TestSharedMemoryPipe(unittest.TestCase):
    ...
    def test_small_message(self):
        ...
        shm, sem = name_holder[0]
        p_reader = ctx.Process(target=read_all, args=(shm, sem, result))
    ...
    def test_roundtrip_messages(self):
        ...
        shm, sem = name_holder[0]
        p_reader = ctx.Process(target=roundtrip_reader, args=(shm, sem, collected))
        p_reader.start(); p_reader.join()
    ...
    def test_init_requires_semaphore_name(self):
        with self.assertRaises(ValueError):
            SharedMemoryPipe(semaphore_name=None)
    ...
    def test_writer_can_spawn_reader(self):
        sem = unique_semaphore_name()
        with SharedMemoryPipe(semaphore_name=sem) as writer:
    ...
    def test_shared_memory_unlinked_after_reader_close(self):
        ...
        # write in-process
        sem_name = unique_semaphore_name()
        writer_proc([b'cleanup'], name_holder, sem_name)

        shm_name, _ = name_holder[0]
        during = manager.Value('b', False)
        after = manager.Value('b', True)

        p = ctx.Process(target=unlink_check_reader, args=(shm_name, sem_name, during, after))
        p.start(); p.join()
    ...
    def test_slot_index_wraparound(self):
        ...
        p_writer = ctx.Process(
            ...
            args=(payloads, name_holder, sem_name, max_slots, slot_size)
        )
        p_writer.start()
        ...
        p_reader = ctx.Process(
            ...
            args=(shm_name, sem_name, seen_holder, max_slots, slot_size)
        )
        p_reader.start()
