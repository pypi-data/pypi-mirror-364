    ...
    def batch_get(self, images: list[cp.ndarray], MAX_BATCH_SIZE: int = 16) -> list[cp.ndarray]:
        ...
        if batch.shape[0] > MAX_BATCH_SIZE:
            ...
            for i in range(0, len(images), MAX_BATCH_SIZE):
                ...
                sub_preds = self.forward(sub_batch)

                sub_result = self.composite_masks(sub_preds)  # (N, 3, 512, 512)
                sub_result = px.batch_to_images(sub_result, swap_rb=True, layout="NCHW")
            ...
            preds = self.forward(batch)
            result = self.composite_masks(preds)  # (N, 3, 512, 512)
            result = px.batch_to_images(result, swap_rb=True, layout="NCHW")
            return [result]

    def raw_get(self, images: list[cp.ndarray], MAX_BATCH_SIZE: int = 16) -> cp.ndarray:
        """
        Get raw segmentation masks without post-processing.
