...
class BaseFaceDetector(BaseModelLoader):
    """
    ...
    def crop(self, image: cp.ndarray, kps: cp.ndarray, size: int = 512) -> tuple[cp.ndarray, cp.ndarray]:
        ...
        output_image, matrix = px.crop_from_kps(image, kps, size * 2)
...
class OnnxFaceDetector(BaseFaceDetector):
    ...
    def forward(self, image: cp.ndarray, threshold: float) -> tuple[list[cp.ndarray], list[cp.ndarray], list[cp.ndarray]]:
        ...
        batch = px.images_to_batch(image, scalefactor=1.0 / self.input_std, size=self.input_size, mean=self.input_mean)
...
class TrtFaceDetector(BaseFaceDetector):
    ...
    def forward(self, image: cp.ndarray, threshold: float) -> tuple[list[cp.ndarray], list[cp.ndarray], list[cp.ndarray]]:
        ...
        batch: cp.ndarray = px.images_to_batch(image, scalefactor=1.0 / self.input_std, mean=self.input_mean)
