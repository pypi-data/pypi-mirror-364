...
class BaseFaceEnhancer(BaseModelLoader):
    """
    ...
    def get(self, image: cp.ndarray | list[cp.ndarray]) -> cp.ndarray | list[cp.ndarray]:
        """
        Face enhancement inference method.
        ...
        images = px.batch_to_images(preds, scalefactor=1.0 / self.input_std, mean=self.input_mean, swap_rb=True, layout="NCHW")
...
class OnnxFaceEnhancer(BaseFaceEnhancer):
    ...
    def get(self, image: cp.ndarray) -> cp.ndarray:
        ...
        print(f"OnnxGfpgan.get: Processing batch with shape: {batch.shape}")
    ...
    def batch_get(self, images: list[cp.ndarray]) -> list[cp.ndarray]:
        ...
        print(f"OnnxGfpgan.batch_get: Processing batch with shape: {batch.shape}")

        # Forward pass
        preds = self.forward(batch)

        return px.batch_to_images(preds, scalefactor=1.0 / self.input_std, mean=self.input_mean, swap_rb=True, layout="NCHW")


class TrtFaceEnhancer(BaseFaceEnhancer):
    ...
    def initialize(self, model_bytes: bytes) -> None:
        with cp.cuda.Device(self.device_id):
            ...
            print(f"TrtGfpgan initialized with input size: {self.input_size}")
            print(f"Input mean: {self.input_mean}, Input std: {self.input_std}")
    ...
    def get(self, image: cp.ndarray | list[cp.ndarray]) -> cp.ndarray | list[cp.ndarray]:
        with cp.cuda.Device(self.device_id):
            ...
            print(f"TrtGfpgan.get: Processing batch with shape: {batch.shape}")

            # Forward pass
            preds = self.forward(batch)

            images = px.batch_to_images(
                preds, scalefactor=1.0 / self.input_std, mean=self.input_mean, swap_rb=True, layout="NCHW"
            )
    ...
    def batch_get(self, images: list[cp.ndarray]) -> list[cp.ndarray]:
        with cp.cuda.Device(self.device_id):
            ...
            print(f"TrtGfpgan.batch_get: Processing batch with shape: {batch.shape}")

            # Forward pass
            preds = self.forward(batch)

            return px.batch_to_images(
                preds, scalefactor=1.0 / self.input_std, mean=self.input_mean, swap_rb=True, layout="NCHW"
            )
