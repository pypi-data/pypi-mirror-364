...
def main():
    ...
    product_crud = multi_crud.register_model(Product, use_plural=False)
    print(f"   資源名稱: product")
    
    # 3. 自定義資源名稱 (organizations)
    print("3. Company 模型 - 自定義資源名稱")
    company_crud = multi_crud.register_model(Company, resource_name="organizations")
    print(f"   資源名稱: organizations")
    ...
    app = multi_crud.create_fastapi_app(
        title="靈活 URL API",
        description="展示不同 URL 形式的 CRUD API"
    )
    ...
    # 添加產品（單數 URL）
    product1 = multi_crud.create("product", {
        "name": "筆記本電腦",
        "price": 999.99,
        "category": "電子產品"
    })
    ...
    # 添加公司（自定義 URL）
    company1 = multi_crud.create("organizations", {
        "name": "科技公司",
        "industry": "軟體開發",
        "employee_count": 100
    })
    ...
    for route in app.routes:
        if hasattr(route, 'path') and hasattr(route, 'methods') and route.path.startswith('/api'):
