...
class ExecutionContext:
    ...
    def from_request(cls, request, agent_node_id: str) -> 'ExecutionContext':
        ...
        workflow_id = headers.get("x-workflow-id", generate_workflow_id())
        execution_id = headers.get("x-execution-id", generate_execution_id())
        parent_execution_id = headers.get("x-parent-execution-id")
        parent_workflow_id = headers.get("x-parent-workflow-id")
        
        # Extract DID headers
        session_id = headers.get("x-session-id")
        caller_did = headers.get("x-caller-did")
        target_did = headers.get("x-target-did")
        agent_node_did = headers.get("x-agent-node-did")
