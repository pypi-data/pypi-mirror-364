...
#            write_page_index=True, 
            ...
            sorting_columns=[pq.SortingColumn(0), pq.SortingColumn(1), pq.SortingColumn(2)] if not sort_by_field_code else [pq.SortingColumn(0), pq.SortingColumn(0), pq.SortingColumn(1), pq.SortingColumn(2)],
        ) 
...
def convert(input: list[str], format: Literal['marc','pica'], output: str, parquet_compression: str, parquet_compression_level: int, parquet_row_group_size: int, max_rows: int, partition_cols: Iterable[str], sort: bool) -> None:
    if output.endswith('.parquet'):
        ...
        if not partition_cols:
            if not sort:
                with parquet_writer(output, parquet_compression, parquet_compression_level) as pw:
                    for batch in yield_batches(input, format, parquet_row_group_size, schema):
                        pw.write_batch(batch)
            else:
                with parquet_writer(output, parquet_compression, None) as pw:
                    for batch in yield_batches(input, format, parquet_row_group_size, schema):
                        pw.write_batch(batch)
        else:
