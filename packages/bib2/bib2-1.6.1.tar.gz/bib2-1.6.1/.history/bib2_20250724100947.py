            ...
            if not sort:
                ...
                for batch in yield_batches(input, format, parquet_row_group_size, schema):
                    ...
                    if pq_rows_written >= max_rows_per_file:
                        ...
                        pq_file_number += 1
                        if use_duckdb:
                            duckdb.query(f"COPY read_parquet('{output_name}')) TO '{output}.tmp' (FORMAT 'parquet', COMPRESSION '{parquet_compression}', COMPRESSION_LEVEL {parquet_compression_level})").execute()
                            os.remove(output)
