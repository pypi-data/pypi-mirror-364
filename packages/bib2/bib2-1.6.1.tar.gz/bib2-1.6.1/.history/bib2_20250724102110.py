                        ...
                        if batch.num_rows > 0:
                            ...
                            if batch_size >= parquet_row_group_size:
                                ...
                                if pq_rows_written >= max_rows_per_file:
                                    ...
                                    pq_file_number += 1
                                if use_duckdb:
                                    duckdb.query(f"COPY '{cur_output}' TO '{cur_output}.duckdbtmp' (FORMAT 'parquet', COMPRESSION '{parquet_compression}', COMPRESSION_LEVEL {parquet_compression_level})").execute()
                                    os.remove(cur_output)
                                    os.rename(cur_output + ".duckdbtmp", cur_output)
                                cur_output = output.replace(".parquet",f"_{pq_file_number}.parquet")
                                pw = parquet_writer(cur_output, parquet_compression, parquet_compression_level, False)
