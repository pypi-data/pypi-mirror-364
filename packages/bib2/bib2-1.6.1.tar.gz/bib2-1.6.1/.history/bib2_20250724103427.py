...
@click.option("-mr", "--parquet-max-rows-per-file", help="Maximum number of rows per parquet file", type=int, default=1_000_000_000)
@click.option("-s", "--parquet-sort", flag_value=True, default=False, help="Sort parquet output by field_code")
@click.option("-d", "--parquet-use-duckdb", flag_value=True, default=False, help="Write final parquet files using DuckDB for better parquet statistics")
@click.argument('input', nargs=-1)
def convert(input: list[str], format: Literal['marc','pica'], output: str, parquet_compression: str, parquet_compression_level: int, parquet_row_group_size: int, max_rows_per_file: int, sort: bool, use_duckdb: bool) -> None:
