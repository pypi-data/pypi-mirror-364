...
def convert(input: list[str], format: Literal['marc','pica'], output: str, parquet_compression: str, parquet_compression_level: int, parquet_row_group_size: int, max_rows_per_file: int, partition_cols: Iterable[str], sort: bool) -> None:
        ...
        if not partition_cols:
            ...
            if not sort:
                ...
                with parquet_writer(output, parquet_compression, parquet_compression_level, True) as pw:
                    ...
                    if batches:
                        pw.write_batch(pa.concat_batches(batches), row_group_size=parquet_row_group_size)
                duckdb.table(output).query(f"COPY (SELECT * FROM read_parquet('{output}.tmp')) TO '{output}' (FORMAT 'parquet', COMPRESSION '{parquet_compression}', COMPRESSION_LEVEL {parquet_compression_level})").execute()
