    ...
    if output.endswith('.parquet'):
        ...
        if not partition_cols:
            ...
            if not sort:
                ...
                pw = parquet_writer(output, parquet_compression, parquet_compression_level, True)
                pr.sort_by(['field_code', 'record_number', 'field_number', 'subfield_number'], ascending=[True, True, True, True])
                pw.write_table(pr, row_group_size=parquet_row_group_size)
                for batch in yield_batches(input, format, parquet_row_group_size, schema):
                    pw.write_batch(batch)
                pw.close()
        else:
