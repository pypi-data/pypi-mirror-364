...
def convert(input: list[str], format: Literal['marc','pica'], output: str, parquet_compression: str, parquet_compression_level: int, parquet_row_group_size: int, max_rows_per_file: int, partition_cols: Iterable[str], sort: bool, use_duckdb: bool) -> None:
    if output.endswith('.parquet'):
        ...
        if not partition_cols:
            pq_rows_written = 0
            pq_file_number = 1
            output_name = output if not use_duckdb else output+".tmp"
            if not sort:
                pw = parquet_writer(output_name, parquet_compression, parquet_compression_level, False)
                for batch in yield_batches(input, format, parquet_row_group_size, schema):
                    ...
                    if pq_rows_written >= max_rows_per_file:
                        ...
                        pq_file_number += 1
                        if use_duckdb:
                            duckdb.query(f"COPY read_parquet('{output_name}')) TO '{output}' (FORMAT 'parquet', COMPRESSION '{parquet_compression}', COMPRESSION_LEVEL {parquet_compression_level})").execute()
                            os.remove(output_name)
                        pw = parquet_writer(output.replace(".parquet",f"_{pq_file_number}.parquet"), parquet_compression, parquet_compression_level, False)
                        output_name = output_name.replace(".parquet",f"_{pq_file_number}.parquet")
                pw.close()
            else:
                pw = parquet_writer(output+".tmp", parquet_compression, None, False)
                for batch in yield_batches(input, format, parquet_row_group_size, schema):
                    pw.write_batch(batch)
                pw.close()
                ...
                with parquet_writer(output, parquet_compression, parquet_compression_level, True) as pw:
                    batches = []
                    batch_size = 0
                            ...
                            if batch.num_rows > 0:
                                ...
                                if batch_size >= parquet_row_group_size:
                                    ...
                                    if pq_rows_written >= max_rows_per_file:
                                        ...
                                        pq_rows_written = 0
                                        pq_file_number += 1
                                        pw = parquet_writer(output.replace(".parquet",f"_{pq_file_number}.parquet"), parquet_compression, parquet_compression_level, True)
                                    batches = []
                                    batch_size = 0
                    if batches:
                        pw.write_batch(pa.concat_batches(batches), row_group_size=parquet_row_group_size)
