...
@click.option("-r", "--parquet-row-group-size", help="Parquet row group size in bytes", type=int, default=1024*1024)
@click.option("-mr", "--max-rows-per-file", help="Maximum number of rows per parquet file", type=int, default=1_000_000_000)
@click.option("-s", "--sort", flag_value=True, default=False, help="Sort parquet output by field_code")
@click.option("-d", "--use-duckdb", flag_value=True, default=False, help="Write final parquet files using DuckDB for better parquet statistics")
@click.option("-p", "--partition-cols", help="Partition parquet files by these columns", type=str, multiple=True, default=None)
@click.argument('input', nargs=-1)
def convert(input: list[str], format: Literal['marc','pica'], output: str, parquet_compression: str, parquet_compression_level: int, parquet_row_group_size: int, max_rows_per_file: int, partition_cols: Iterable[str], sort: bool, use_duckdb: bool) -> None:
    if output.endswith('.parquet'):
        if parquet_row_group_size > max_rows_per_file:
            print(f"Parquet row group size {parquet_row_group_size:,} is greater than max rows {max_rows_per_file:,}, changing it to {max_rows_per_file:,}.")
            parquet_row_group_size = max_rows_per_file
        if not partition_cols:
            pq_rows_written = 0
            pq_file_number = 1
            cur_output = output
            if not sort:
                pw = parquet_writer(cur_output, parquet_compression, parquet_compression_level, False)
                for batch in yield_batches(input, format, parquet_row_group_size, schema):
                    pw.write_batch(batch)
                    pq_rows_written += batch.num_rows
                    if pq_rows_written >= max_rows_per_file:
                        pw.close()
                        pq_rows_written = 0
                        pq_file_number += 1
                        if use_duckdb:
                            duckdb.query(f"COPY read_parquet('{cur_output}')) TO '{cur_output}.duckdbtmp' (FORMAT 'parquet', COMPRESSION '{parquet_compression}', COMPRESSION_LEVEL {parquet_compression_level})").execute()
                            os.remove(cur_output)
                            os.rename(cur_output + ".duckdbtmp", cur_output)
                            output_or_tmp = output.replace(".parquet",f"_{pq_file_number}.parquet") + ".duckdbtmp"
                        else:
                            output_or_tmp = output.replace(".parquet",f"_{pq_file_number}.parquet")
                        pw = parquet_writer(output_or_tmp, parquet_compression, parquet_compression_level, False)
                pw.close()
            else:
                ...
                output_or_tmp = output if not use_duckdb else output+".duckdbtmp"
                pw = parquet_writer(output_or_tmp, parquet_compression, parquet_compression_level, False)
                batches = []
                batch_size = 0
                        ...
                        if batch.num_rows > 0:
                            ...
                            if batch_size >= parquet_row_group_size:
                                ...
                                if pq_rows_written >= max_rows_per_file:
                                    ...
                                    pq_file_number += 1
                                    if use_duckdb:
                                        duckdb.query(f"COPY read_parquet('{output_or_tmp}')) TO '{output}' (FORMAT 'parquet', COMPRESSION '{parquet_compression}', COMPRESSION_LEVEL {parquet_compression_level})").execute()
                                        os.remove(output_or_tmp)
                                        output_or_tmp = output.replace(".parquet",f"_{pq_file_number}.parquet") + ".tmp"
                                    else:
                                        output_or_tmp = output.replace(".parquet",f"_{pq_file_number}.parquet")
                                    pw = parquet_writer(output_or_tmp, parquet_compression, parquet_compression_level, False)
                                batches = []
            ...
            shutil.rmtree(output, ignore_errors=True)  # Remove existing output directory if it exists
            ds.write_dataset(yield_batches(input, format, parquet_row_group_size, schema), output, schema=schema, format='parquet', file_options=ds.ParquetFileFormat().make_write_options(
                compression=parquet_compression, 
                compression_level=parquet_compression_level,                        
    #            use_byte_stream_split=['record_number', 'field_number', 'subfield_number'], # type: ignore pyarrow import complains: BYTE_STREAM_SPLIT encoding is only supported for FLOAT or DOUBLE data
    #            write_page_index=True, 
            ...
            partitioning=ds.partitioning(schema=pa.schema((schema.field(field) for field in partition_cols)), flavor='hive'))
    else:
