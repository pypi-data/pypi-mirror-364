        ...
        if not partition_cols:
            ...
            if not sort:
                ...
                with parquet_writer(output, parquet_compression, parquet_compression_level, True) as pw:
                    ...
                    if batches:
                        pw.write_batch(pa.concat_batches(batches), row_group_size=parquet_row_group_size)
                duckdb.table(output).table_from_parquet(output+".tmp").to_parquet(output, compression=parquet_compression, compression_level=parquet_compression_level, partition_cols=partition_cols)
