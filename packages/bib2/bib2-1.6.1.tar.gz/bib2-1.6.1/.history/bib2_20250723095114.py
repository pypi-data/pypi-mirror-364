...
def convert(input: list[str], format: Literal['marc','pica'], output: str, parquet_compression: str, parquet_compression_level: int, parquet_row_group_size: int, max_rows: int, partition_cols: Iterable[str], sort: bool) -> None:
    if output.endswith('.parquet'):
        if parquet_row_group_size > max_rows:
            print(f"Parquet row group size {parquet_row_group_size:,} is greater than max rows {max_rows:,}, changing it to {max_rows:,}.")
            parquet_row_group_size = max_rows
        if not partition_cols:
            ...
            if not sort:
                ...
                for batch in yield_batches(input, format, parquet_row_group_size, schema):
                    pw.write_batch(batch)
                pw.close()
                pr = pq.ParquetFile(output+".tmp")
                field_codes = set()
                for batch in pr.iter_batches(columns=['field_code']):
                    field_codes.update(batch.column(0).dictionary.to_pylist())
                with parquet_writer(output, parquet_compression, parquet_compression_level, True) as pw:
                    for field_code in sorted(field_codes):
                        for batch in pr.reader.iter_batches(filters=[('field_code', '==', field_code)], columns=['record_number', 'field_number', 'subfield_number', 'field_code', 'subfield_code', 'value']):
                            if batch.num_rows > 0:
                                pw.write_batch(batch)
                    pw.write_table(pr.sort_by(['field_code', 'record_number', 'field_number', 'subfield_number'], ascending=[True, True, True, True]), row_group_size=parquet_row_group_size)
        else:
