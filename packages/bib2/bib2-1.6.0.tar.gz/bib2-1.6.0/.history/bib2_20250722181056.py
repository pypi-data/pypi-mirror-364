...
def convert(input: list[str], format: Literal['marc','pica'], output: str, parquet_compression: str, parquet_compression_level: int, parquet_batch_size: int, max_rows: int, partition_by: Optional[str]) -> None:
    if output.endswith('.parquet'):
        ...
        pq_rows_written = 0
        pq_file_number = 1
        pq.write_to_dataset(yield_batches(parquet_batch_size, schema), output, partition_cols=[partition_by] if partition_by else None, file_scheme='hive', max_rows=max_rows)
    else:
