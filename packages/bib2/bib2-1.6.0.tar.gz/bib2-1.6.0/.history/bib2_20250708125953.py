...
def yield_from_marc_file(inf: BytesIO) -> Iterator[tuple[int, int, int, str, str, str]]:
    reader = pymarc.MARCReader(inf, to_unicode=True)
    i = 1
    for record in enumerate(reader, start=1):
            if record:
                for row in convert_marc_record(record):
                    yield (i, *row)
...
def yield_from_marcxml_file(inf: OpenFile) -> Iterator[tuple[int, int, int, str, str, str]]:
    ...
    for i, (_, elem) in enumerate(context, start=1):
        for row in convert_marcxml_record(elem):
            yield (i, *row)
        elem.clear()
        while elem.getprevious() is not None:
            del elem.getparent()[0]

def yield_from_picaxml_file(inf: OpenFile) -> Iterator[tuple[int, int, int, str, str, str]]:
    tags = ('{info:srw/schema/5/picaXML-v1.0}record', 'record')
    context = lxml.etree.iterparse(inf, events=('end',), tag=tags)
    for i, (_, elem) in enumerate(context, start=1):
        for row in convert_picaxml_record(elem):
            yield (i, *row)
        elem.clear()
        while elem.getprevious() is not None:
            del elem.getparent()[0]            
...
def convert(input: list[str], format: Literal['marc','pica'], output: str, parquet_compression: str, parquet_compression_level: int, parquet_batch_size: int, max_rows: int) -> None:
    if format == 'marc':
        ...
        convert_record = convert_marcxml_record
    else:
        convert_record = convert_picaxml_record
    ...
    for input_file in input_files:
        ...
        with input_file as oinf:
                ...
                if compression is not None:
                    ...
                    inf = oinf
                if not input_file.path.contains('.xml') and not input_file.path.contains('.mrcx'):
                    def iter_records(inf: OpenFile) -> Iterator[tuple[int, int, int, str, str, str]]:
                            reader = pymarc.MARCReader(inf, force_utf8=True, to_unicode=True)
                            for record in reader:
                                yield from convert_marc_record(record)
                        else:
                            context = lxml.etree.iterparse(inf, events=('end',), tag=tags)
                            for i, (_, elem) in enumerate(context, start=1):
