...
def convert_marc_record(record: pymarc.record.Record) -> Iterator[tuple[int, int, str, str, str]]:
    ...
    for field_number, field in enumerate(record, start=2):
        if field.control_field:
            yield field_number, 1, field.tag, '', cast(str, field.data)
        else:
            for subfield_number, subfield in enumerate(field.subfields, start=1):
                if subfield.text is not None:
                    yield field_number, subfield_number, field.tag, subfield.code, normalize('NFC', subfield.data)
