...
def convert(input: list[str], format: Literal['marc','pica'], output: str, parquet_compression: str, parquet_compression_level: int, parquet_batch_size: int, max_rows: int, partition_cols: Iterable[str]) -> None:
    if output.endswith('.parquet'):
        ...
        ds.write_dataset(yield_batches(input, parquet_batch_size, schema), output, format='parquet', file_options=ds.FileFormat.make_write_options(max_rows_per_file=max_rows, partitioning=partition_cols))
    else:
