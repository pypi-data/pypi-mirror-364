...
def yield_rows(input: list[str]) -> Iterator[tuple[int, int, int, str, str, str]]:
    record_number = 1
...
def yield_batches(input: list[str], parquet_batch_size: int, schema: pa.Schema) -> Iterator[pa.RecordBatch]:
    batch = []
...
@click.option("-p", "--partition-cols", help="Partition parquet files by these columns", type=list[str], default=None)
...
def convert(input: list[str], format: Literal['marc','pica'], output: str, parquet_compression: str, parquet_compression_level: int, parquet_batch_size: int, max_rows: int, partition_cols: Iterable[str]) -> None:
    if output.endswith('.parquet'):
        ...
        ds.write_dataset(yield_batches(input, format, parquet_batch_size, schema), output, format='parquet', file_options=ds.ParquetFileFormat.make_write_options(compression=parquet_compression, compression_level=parquet_compression_level), max_rows_per_file=max_rows, partitioning=ds.partitioning(field_names=partition_cols, flavor='hive') if partition_cols else None)
    else:
