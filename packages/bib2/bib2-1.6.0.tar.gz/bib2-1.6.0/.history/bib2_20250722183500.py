...
def convert(input: list[str], format: Literal['marc','pica'], output: str, parquet_compression: str, parquet_compression_level: int, parquet_batch_size: int, max_rows: int, partition_cols: Iterable[str]) -> None:
    if output.endswith('.parquet'):
        ...
        ds.write_dataset(yield_batches(input, format, parquet_batch_size, schema), output, schema=schema, format='parquet', file_options=ds.ParquetFileFormat().make_write_options(compression=parquet_compression, compression_level=parquet_compression_level), max_rows_per_file=max_rows, partitioning=ds.partitioning(schema=pa.schema((schema.field(field) for field in )), flavor='hive') if partition_cols else None)
    else:
