...
def convert_marc_record(record: pymarc.record.Record) -> Iterable[tuple[int, int, str, str, str]]:
    fields = []
    fields.append((1, 1, 'LDR', '', str(record.leader)))
    for field_number, field in enumerate(record, start=2):
        if field.control_field:
            fields.append((field_number, 1, field.tag, '', field.value()))
        else:
            sf = 1
            if field.indicator1 != ' ':
                fields.append((field_number, sf, field.tag, 'Y', normalize('NFC', field.indicator1)))
                sf += 1
            if field.indicator2 != ' ':
                fields.append((field_number, sf, field.tag, 'Z', normalize('NFC', field.indicator2)))
                sf += 1
            for subfield_number, subfield in enumerate(filter(lambda subfield: subfield.value is not None, field.subfields), start=sf):
                fields.append((field_number, subfield_number, field.tag, subfield.code, normalize('NFC', subfield.value)))
    return fields)
