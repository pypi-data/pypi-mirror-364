...
class SmartOrchestrator:
    ...
    def execute(
        ...
        Args:
            ...
            system_message: System message for agent identity
    ...
    def _direct_response(self, query: str, context: Dict) -> ExecutionResult:
        """Direct response using existing LLM interface with memory context."""
        llm = get_llm()
        
        # Use system message from context if available, otherwise fallback
        system_prompt = context.get('system_message') or "You are a helpful AI assistant. Provide clear, accurate, and concise responses to user queries."
        
        # Build user message with memory context if available
        user_content = query
        memory_context = context.get('memory_context', '').strip()
        if memory_context:
            user_content = f"Context from previous conversation:\n{memory_context}\n\nCurrent query: {query}"
        ...
        return ExecutionResult(
            ...
            confidence=0.9,
            metadata={'strategy': 'direct_response', 'memory_context_used': bool(memory_context)}
        )
    ...
    def _single_tool_execution(
        ...
        # Get tool from context tools first, then registries
        try:
            ...
            if reasoning_result.get('action') == 'final_answer' or reasoning_result.get('ready_for_final'):
                ...
                if tool_name in analysis.required_tools:
                    try:
                        ...
                        if hasattr(tool, 'execute'):
                            tool_result = tool.execute(tool_query)
                        else:
                            tool_result = str(tool)
