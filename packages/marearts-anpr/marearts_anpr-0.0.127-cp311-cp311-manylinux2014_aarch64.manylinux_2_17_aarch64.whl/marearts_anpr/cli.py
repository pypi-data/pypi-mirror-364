...
SERIAL_KEY_PATTERN = re.compile(r'^[A-Za-z0-9\-]+$')
...
def cmd_config(args):
    ...
    try:
        ...
        ret = detector.init(1, user_name, serial_key)
        if ret == 0:
...
def process_image(image_path: str, args) -> List[Dict[str, Any]]:
    ...
    results = marearts_anpr_from_image_file(
        user_name=user_name,
        serial_key=serial_key,
        img_path=image_path,
        detector_model_version=args.detector_model,
        ocr_model_version=args.ocr_model
    )
