...
class LossEQ:
    def __init__(
        ...
        if train_params is not None and "eq" in {key.lower() for key in train_params}:
            eq_acc = torch.tensor(eq_acc, device=DEVICE, dtype=DEFAULT_DTYPE)   # Make sure to set the dtype as needed
    ...
    def lossODE(self):
        ...
        if isinstance(self.eq_acc, torch.Tensor):
            ...
            acc = acc_interpolation.repeat(1, self.DOF) /self.dimensionsless_Pt
        else:
