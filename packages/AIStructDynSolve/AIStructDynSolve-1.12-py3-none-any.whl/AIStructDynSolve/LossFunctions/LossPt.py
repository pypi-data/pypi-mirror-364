...
class LossPt:
    def __init__(
        ...
        if train_params is not None and "pt" in {key.lower() for key in train_params}:
            pt_value = torch.tensor(pt_value, device=DEVICE, dtype=DEFAULT_DTYPE) / self.dimensionsless_unit  # Make sure to set the dtype as needed
            self.pt_value = torch.nn.Parameter(pt_value, requires_grad=True)
            #print(pt_value.size())
        else:
    ...
    def lossODE(self):
        ...
        if isinstance(self.pt_value, torch.Tensor):
            ...
            pt_interp = self.torch_interp_1d(x_query, pt_time_tensor, pt_value_tensor)  # (DOF, N)
            Pt = pt_interp /self.dimensionsless_Pt
        else:
