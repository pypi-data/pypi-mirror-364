...
class LossEQ:
    def __init__(
        ...
        if train_params is not None and "eq" in {key.lower() for key in train_params}:
            eq_acc = torch.tensor(eq_acc, device=DEVICE, dtype=DEFAULT_DTYPE) / self.dimensionsless_unit   # Make sure to set the dtype as needed
            self.eq_acc = torch.nn.Parameter(eq_acc, requires_grad=True)
            #print(eq_acc.size())
        else:
            self.eq_acc = eq_acc
    ...
    def lossODE(self):
        ...
        if isinstance(self.eq_acc, torch.Tensor):
            ...
            acc_interpolation = self.torch_interp_1d(x_query, eq_time_tensor, self.eq_acc).unsqueeze(-1)
            acc = acc_interpolation.repeat(1, self.DOF) * self.dimensionsless_unit /self.dimensionsless_Pt
        else:
