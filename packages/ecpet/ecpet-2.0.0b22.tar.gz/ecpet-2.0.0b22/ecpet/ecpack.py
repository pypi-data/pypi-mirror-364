...
def averag(df):
    ...
    if ec.safe_len(df) == 0:
        ...
        #      ENDDO
        tolcov = pd.DataFrame(
            np.nan, columns=numcols.columns, index=numcols.columns)
        cindep = pd.DataFrame(
            np.nan, columns=numcols.columns, index=numcols.columns)
...
# FIXME warum
    ...
    if qcstruct:
        # FIXME schreiben!
        # struct,dstruct = calcstruct(sample)
...
def process_flux(conf, intervals):
    ...
    if nproc > 1:
        # execute parallel
        pool = Pool(nproc, initializer=ecdb.init_worker_process)
    elif nproc == 1:
        # execute serial
        pool = None
    else:
        # execute parallel
        pool = Pool(initializer=ecdb.init_worker_process)
    #
