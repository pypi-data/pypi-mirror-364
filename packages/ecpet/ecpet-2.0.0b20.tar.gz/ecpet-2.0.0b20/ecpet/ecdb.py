...
tab_head_cols = ['header_id', 'station_name', 'table_name', 'column_count',
                 'logger_name', 'logger_os', 'logger_prog', 'logger_serial',
                 'logger_sig']
...
def table_prepare(cur, tab_name, tab_cols, tab_types):
    ...
    #
    if len(response) == 0:
        logger.info('Create table for file headers: "%s"' % tab_name)
        # create table
...
def ingest_toa5(params):
    ...
    # CRITICAL FIX: Ensure database is properly initialized in worker process
    # This is especially important on Windows where workers start fresh
    try:
        db_prepare(cur)
    except Exception as e:
        logger.error(f'Failed to prepare database in worker: {e}')
        conn.close()
        return
    ...
    if ecfile.toa5_check(infile):
        ...
        #
        dbtable = dbtable_name(header['station_name'], header['table_name'])
        dbcolumns = []
        for i in range(header['column_count']):
            #        dbcolumn  = header['column_names'][i]
            #        if not dbcolumn in ('TIMESTAMP','RECORD'):
            #          dbcolumn += dbsep+header['column_units'][i]
            #          dbcolumn += dbsep+header['column_sampling'][i]
        ...
        if len(response) > 1:
            file_new = False
            # error
            logger.error('duplicate file signatures in database: ' + inhash)
            ...
            file_new = True
            #      # get current max index number and add 1
            #      file_id=db_free_index(cur,tab_file_name,'file_id')
            file_id = free_id
            ...
            for col in tab_file_cols:
                if col == 'file_id':
                    ...
                    values.append('')
            # store data
            sql = 'INSERT INTO "%s" (%s) VALUES (%s);' % (tab_file_name,
                                                          ', '.join(tab_file_cols), ', '.join(values))
            logger.insane('SQL=%s' % sql)
            cur.execute(sql)
            response = cur.fetchall()
            logger.insane('response=%s' % response)
        ...
        if not (file_new or force):
            ...
            if len(response) > 1:
                ...
                # not found --> gt new id and store header
                ...
                for col in tab_head_cols:
                    if col in header:
                        ...
                        values.append('""')
                # store data
                sql = 'INSERT INTO "%s" (%s) VALUES (%s);' % (tab_head_name,
                                                              ', '.join(tab_head_cols), ', '.join(values))
                logger.insane('SQL=%s' % sql)
                cur.execute(sql)
                response = cur.fetchall()
                logger.insane('response=%s' % response)
...
def ingest_df(df, station_name=None, table_name=None, time_column=None):
    ...
    Stores processed results and intermediate data with automatic
    type conversion and missing column handling.
    """
    conn = sqlite3.connect(dbfile, timeout=900)
    cur = conn.cursor()
    # create columns in db table if then don't exist
        ...
        if col not in df.columns:
            if col == 'TIMESTAMP':
                if time_column is not None and time_column in df.columns:
                    df['TIMESTAMP'] = [timestamp_fill(x.strftime('%Y-%m-%d %H:%M:%S.%f'))
                                       for x in df[time_column]]
                else:
                    logger.error(
                        'dataframe to ingest must either have TIMESTAMP or time_column mut be a valid column')
                    raise ValueError
            elif col == 'RECORD':
                df['RECORD'] = [x + 1 for x in range(len(df.index))]
            elif col in ['head_id', 'file_id']:
                df[col] = ""
            else:
                logger.warning(
                    '{:s} not in df but tab_skeleton_cols and not handled')
    #
    # assemble name of db table and columns
    #
    # table name
    dbtable = dbtable_name(station_name, table_name)
    dbcolumns = df.columns
    logger.insane('dbcolumns: %s' % ', '.join(dbcolumns))
    #
    # insert the data
    #
    ...
    response = table_prepare(cur, dbtable, dbcolumns, dbtypes)
    if len(response) > 1:
        logger.critical("ERROR: multiple matching tables. Fix the database.")
        quit()
    #
            ...
            if col == 'head_id':
                ...
                try:
                    ...
                    if row[col] is not None:
                        ...
                        logger.warning(
                            'unknown value to ingest into database: {!s:s}=None'.format(str(col)))
                    values.append('null')
...
def ingest(infiles, force=False, nproc=0, station_name=None, table_name=None, progress=100):
    """
    ...
    if station_name is not None:
        logger.debug(
            'overriding station name in files with {:s}'.format(station_name))
    if table_name is not None:
        logger.debug(
    ...
    for infile in sorted(infiles):
        free_id = get_free_index(cur, tab_file_name, 'file_id', reserved_ids)
...
def db_get_header(cur, opt, dbtable, time_condition):
    ...
    response = exe_sql(cur, 'SELECT %s FROM headers WHERE header_id IN ( %s );'
                       % (cols, ids))
...
def retrieve_df(station_name, table_name, columns=None, tbegin=None, tend=None):
    """
