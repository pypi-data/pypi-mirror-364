...
# Configuration
LIB_VERSION="v0.3.6"
BASE_URL="https://app.brainco.cn/universal/bc-ecap-sdk/libs/${LIB_VERSION}"
...
if [ -f "$VERSION_FILE" ] && grep -F --quiet "$LIB_VERSION" "$VERSION_FILE"; then
  echo_y "[bc-ecap-sdk] (${LIB_VERSION}) is already installed"
  cat "$VERSION_FILE"
  exit 0
fi
...
echo_y "[bc-ecap-sdk] Cleaning up previous distribution..."
...
# Download library
echo_y "[bc-ecap-sdk] Downloading (${LIB_VERSION}) for ${LIB_NAME}..."
...
echo_y "[bc-ecap-sdk] Extracting ${ZIP_NAME}..."
...
# Create VERSION file
echo_y "[bc-ecap-sdk] Creating version file..."
cat >"$VERSION_FILE" <<EOF
[bc-ecap-sdk] Version: ${LIB_VERSION}
Update Time: $(date)
EOF
