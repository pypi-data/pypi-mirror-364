...
let impedanceChecking = false;

/**
 * 切换阻抗监测状态
 */
async function app_start_leadoff() {
  console.log("APP开启阻抗监测", 1);

  if (impedanceChecking) {
    console.warn("阻抗监测已开启，请勿重复开启");
    return;
  }

  // 由于不知道设备状态，提前关闭数据传输
  // target_device.stop_eeg_stream(); // start_leadoff_check 切换检测芯片会先发送停止EEG数据传输, 此处不需要重复发送
  target_device.stop_imu_stream();

  await sleep(1000);
  impedanceChecking = true;
  // 开启 阻抗检测
  target_device.start_leadoff_check();
}

/**
 * 结束阻抗监测
 */
async function app_stop_leadoff() {
  console.log("APP结束阻抗监测", 0);
  ...
  // 结束 阻抗检测
  target_device.stop_leadoff_check();
  await sleep(1000);
  impedanceChecking = false;

  //开启数据传输
  target_device.start_eeg_stream(
    EegSampleRate.SR_250Hz,
    EegSignalGain.GAIN_1,
    EegSignalSource.NORMAL
  );
  await sleep(1000);
  target_device.start_imu_stream(ImuSampleRate.SR_50Hz);
}

async function app_test() {
  for (let i = 0; i < 1000; i++) {
    // app开启阻抗检测
    await app_start_leadoff();
    await sleep(6000);
    // app结束阻抗检测
    await app_stop_leadoff();
    await sleep(6000);
  }
}

async function test_disconnect() {
  await app_start_leadoff();
  await sleep(4000);
  await target_device.disconnect();
}
