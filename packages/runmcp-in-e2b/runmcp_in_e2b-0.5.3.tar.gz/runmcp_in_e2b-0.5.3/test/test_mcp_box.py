...
def merge_tool_input_schema(tool: Tool):
    ...
    if tool.annotations and tool.annotations.model_extra:
        ...
        for parm_name in para_schemas:
            para_desc = para_descs[parm_name]['description']
            para_schemas[parm_name]['description'] = para_desc
        ...
        async with sse_client(url, sse_read_timeout=5) as streams:
            async with ClientSession(*streams) as session:
                ...
                for tool in response.tools:
                    ...
                    print(f"list_tools: TOOL name={tool.name}, description={tool.description}, inputSchema=\n{tool.inputSchema}")


                print("\n=== call mcp box tools ===")
                result = await session.call_tool("getHostFaultCause", {"faultCode": "F02"})
                print(f'call_tool: getHostFaultCause result={result}')

                result = await session.call_tool("getMiddleFaultCause", {"faultCode": "F03"})
                ...
                for tool in response.tools:
                    merge_tool_input_schema(tool)
                    print(
                        f"list_tools: TOOL name={tool.name}, description={tool.description}, inputSchema=\n{tool.inputSchema}")

                print("\n=== call the removed mcp box tool ===")
                result = await session.call_tool("getHostFaultCause", {"faultCode": "F01"})
