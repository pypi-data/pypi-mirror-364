...
class StytchVerifier:
    ...
    def _build_context_from_stytch_data(
        ...
        def normalize_auth_factors(factors):
    ...
    async def get_member_by_email(self, email: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Ensure response_json is a dictionary, not a string
            if isinstance(response_json, str):
                try:
                    response_json = json.loads(response_json)
                except json.JSONDecodeError:
                    logger.error(f"Failed to parse JSON response: {response_json}")
                    raise StytchAPIError("Invalid JSON response from Stytch API")
            ...
            logger.error(
                ...
                extra={"api_response": e.api_response},
            )
