...
SUB_EXCHANGE_MY2KS = {
    'CN_SH': KsExchange.SSE,
    'CN_SZ': KsExchange.SZSE,
    'CN_BJ': KsExchange.BSE,
    'CN_STIB': KsExchange.SSE, # 科创板
    
    'HK_MAINBOARD': KsExchange.SEHK,
    'HK_HKEX': KsExchange.SEHK,
    'HK_GEMBOARD': KsExchange.SEHK, # 创业板
    
    
    'US_NASDAQ': KsExchange.NASDAQ,
    'US_NYSE': KsExchange.NYSE,
    'US_AMEX': KsExchange.AMEX,
    'US_PINK': KsExchange.OTC
}
...
PRODUCT_KS2MY = {
    ...
    KsProduct.INDEX: SecurityType.IDX
}
...
class KsFutuMarketApi(BaseMarketApi):
    ...
    def query_contracts(
        ...
        if df:
            ...
            data['sub_exchange'] = data['exchange_type'].transform(lambda x: SUB_EXCHANGE_MY2KS.get(x, KsExchange.UNKNOW).value)
    ...
    @GlobalRateLimiter(interval_sec=0.5, id='futu_query_quotes')
    def query_quotes(self, vt_symbols: list[str], df: bool = True) -> Union[KsRetCode, list[QuoteData]]:
        ...
        data['total_market_val'] = np.where(data['trust_valid'], data['trust_aum'], data['total_market_val'])
    ...
    # todo 这里默认先不管是不是df都返回df，后续在处理非df 
    @GlobalRateLimiter(interval_sec=0.5, id='futu_query_history')
    def query_history(
            self,
            vt_symbol: str,
            start: datetime,
        ...
        is_first_req = True
        page_req_key = None
        while is_first_req or page_req_key:
            is_first_req = False
            ret_k, data_k_page, page_req_key = self.quote_ctx.request_history_kline(
                my_symbol,
            ...
            if not ret_k == RET_OK:
                return KS_RET_ERROR, self.get_error(vt_symbol=vt_symbol, start=start, end=end, interval=interval, adjustment=adjustment, msg=data_k_page)
            data_k = pd.concat([data_k, data_k_page], ignore_index=True)
        ...
        data_k['volume'] = data_k.volume
        data_k['turnover'] = data_k.turnover
        data_k['turnover_rate'] = data_k.turnover_rate
        data_k['open'] = data_k.open
        data_k['high'] = data_k.high
        data_k['low'] = data_k.low
        data_k['close'] = data_k.close
        
        ret_df = data_k[['vt_symbol', 'datetime', 'interval', 'volume', 'turnover', 'turnover_rate', 'open', 'high', 'low', 'close']]
        return KS_RET_OK, ret_df
