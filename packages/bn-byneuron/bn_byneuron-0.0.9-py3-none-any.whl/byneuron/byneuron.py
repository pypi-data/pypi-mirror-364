...
class ByConfig:
    ...
    def __init__(self, api=None, login=None, id=None, key=None, isUser=True):
        ...
        self.client_id = None if id is None or isUser is False else id
        self.client_key = None if key is None or isUser is False else key
        self.user_id = None if id is None or isUser is True else id
        self.user_key = None if key is None or isUser is True else key
        self.get_env()
    ...
    def valid(self):
        return self.valid_client() or self.valid_user()

    def valid_client(self):
        if None not in {self.client_id, self.client_key}:
            return True
    ...
    def get_env(self):
        ...
        setter('client_key', 'OAUTH2_CLIENT_SECRET')
    ...
    def get_ini(self, filename=None, section=None):
        ...
        if files:
            ...
            if section in cp:
                ...
                setter('client_key', cp, 'client.secret')
...
class Byneuron:
    ...
    def _login(self):
        ...
        } if self.config.valid_client() else {
