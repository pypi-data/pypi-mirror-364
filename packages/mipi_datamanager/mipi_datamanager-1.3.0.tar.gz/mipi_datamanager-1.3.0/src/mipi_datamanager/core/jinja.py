...
def bind_to_default_config_jinja_parameters(func_):
    ...
    def _inner(*args, **kwargs):
        ...
        orig = bound.arguments.get("jinja_parameters_dict", {})
...
class JinjaRepo(JinjaLibrary):
    ...
    def _get_default_jinja_params_from_config(self, inner_path):
        _path = Path(inner_path)
        return self.master_config[str(_path.parent)][str(_path.name)].get("jinja_parameters_dict",{})
    ...
    @bind_to_default_config_jinja_parameters
    def execute_file(self, inner_path: str, connection: connection.Odbc, jinja_parameters_dict: dict = None) -> pd.DataFrame:
        # note: merged dict is now in the right spot
        return super().execute_file(inner_path, connection, jinja_parameters_dict)
