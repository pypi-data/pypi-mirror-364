...
def snapshot_download(
        ...
        *,
        ...
        # 新增一个回调函数
        on_get_repo_info: Optional[callable] = None,
        on_files_filtered: Optional[callable] = None,
    ...
    if not local_files_only:
        ...
        try:
            ...
            if on_get_repo_info and repo_info:
                try:
                    on_get_repo_info(repo_id, repo_info)
                except Exception as e:
                    logger.warning(f"on_get_repo_info 回调函数执行失败: {e}")
    ...
    if on_files_filtered is not None:
        try:
            on_files_filtered(repo_id, filtered_repo_files, repo_info)
        except Exception as e:
    ...
    def _inner_hf_hub_download(repo_file: str):
        if download_callback:
            try:
                download_callback(repo_id, repo_file, "", "", "started", None, None)
            except Exception as e:
                logger.warning(f"download_callback (started) 执行失败: {e}")
        try:
            ...
            if download_callback:
                try:
                    download_callback(repo_id, repo_file, "", result_path, "completed", None, None)
                except Exception as e:
            ...
            if download_callback:
                try:
                    download_callback(repo_id, repo_file, "", "", "failed", str(e), None)
