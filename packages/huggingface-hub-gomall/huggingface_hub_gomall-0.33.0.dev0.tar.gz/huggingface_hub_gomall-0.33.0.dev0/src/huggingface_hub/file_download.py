...
def http_get(
        ...
        *,
        ...
        download_callback_info: Optional[Dict] = None,
) -> None:
    """
    ...
    if download_callback_info:
        callback = download_callback_info.get("callback")
        repo_id = download_callback_info.get("repo_id")
    ...
    def update_progress_callback(bytes_downloaded: int):
        if callback and repo_id:
            try:
                progress_info = {
                    "bytes_downloaded": bytes_downloaded,
                    "total_bytes": total,
                    "progress_percentage": (bytes_downloaded / total * 100) if total else 0
                }
                callback(repo_id, filename, url, temp_file.name, "progress", None, progress_info)
    ...
    with progress_cm as progress:
        if hf_transfer and total is not None and total > 5 * DOWNLOAD_CHUNK_SIZE:
            ...
            try:
                def wrapped_callback(bytes_count):
                    progress.update(bytes_count)
                    update_progress_callback(progress.n)
                ...
                hf_transfer.download(
                    ...
                    parallel_failures=3,
                    max_retries=5,
                    **({"callback": wrapped_callback} if supports_callback else {}),
                )
            except Exception as e:
                raise RuntimeError(
        ...
        try:
            for chunk in r.iter_content(chunk_size=DOWNLOAD_CHUNK_SIZE):
                if chunk:  # filter out keep-alive new chunks
                    ...
                    # 【新增】调用进度回调
                    update_progress_callback(new_resume_size)
            ...
            logger.warning("错误原因是 %s, 可能是网络问题导致的。正在尝试重新下载...", str(e))
            time.sleep(1)
            ...
            return http_get(
                ...
                _tqdm_bar=_tqdm_bar,
                download_callback_info=download_callback_info,
            )
...
def hf_hub_download(
        ...
        *,
        ...
        endpoint: Optional[str] = None,
        # 【新增】回调函数参数
        download_callback: Optional[Callable[[str, str, str, str, str, Optional[str], Any], None]] = None,
        # Deprecated args
        legacy_cache_layout: bool = False,
        resume_download: Optional[bool] = None,
        force_filename: Optional[str] = None,
...
def _hf_hub_download_to_local_dir(
    ...
    if (
        ...
        if download_callback:
            try:
                download_callback(repo_id, filename, "", str(paths.file_path), "skipped", None, "file_already_exists")
            except Exception as e:
                logger.warning(f"download_callback (skipped) 执行失败: {e}")
    ...
    if head_call_error is not None:
        ...
        if not force_download and paths.file_path.is_file():
            ...
            if download_callback:
                try:
                    download_callback(repo_id, filename, "", str(paths.file_path), "cached", None, "using_cached_file")
                except Exception as e:
                    logger.warning(f"download_callback (cached) 执行失败: {e}")
            return str(paths.file_path)
        # Otherwise => raise
        _raise_on_head_call_error(head_call_error, force_download, local_files_only)
    ...
    if not force_download and paths.file_path.is_file():
        ...
        if local_metadata is not None and local_metadata.etag == etag:
            ...
            if download_callback:
                try:
                    download_callback(repo_id, filename, "", str(paths.file_path), "up_to_date", None,
                                      "file_up_to_date")
                except Exception as e:
                    logger.warning(f"download_callback (up_to_date) 执行失败: {e}")
            return str(paths.file_path)
        ...
        if local_metadata is None and REGEX_SHA256.match(etag) is not None:
            ...
            if file_hash == etag:
                ...
                if download_callback:
                    try:
                        download_callback(repo_id, filename, "", str(paths.file_path), "hash_match", None,
                                          "hash_verified")
                    except Exception as e:
                        logger.warning(f"download_callback (hash_match) 执行失败: {e}")
                return str(paths.file_path)
    ...
    if not force_download:
        ...
        if isinstance(cached_path, str):
            ...
            if download_callback:
                try:
                    download_callback(repo_id, filename, "", str(paths.file_path), "copied_from_cache", None,
                                      {"cached_path": cached_path})
                except Exception as e:
                    logger.warning(f"download_callback (copied_from_cache) 执行失败: {e}")
            return str(paths.file_path)
    ...
    try:
        with WeakFileLock(paths.lock_path):
            ...
            _download_to_tmp_and_move(
                ...
                force_download=force_download,
                # 【新增】传递回调信息
                download_callback_info={
                    "repo_id": repo_id,
                    "callback": download_callback
                } if download_callback else None,
            )
        ...
        if download_callback:
            try:
                file_size = os.path.getsize(paths.file_path) if paths.file_path.exists() else 0
                download_callback(repo_id, filename, url_to_download, str(paths.file_path), "completed", None,
                                  {"file_size": file_size})
            except Exception as e:
                logger.warning(f"download_callback (completed) 执行失败: {e}")
        ...
        if download_callback:
            try:
                download_callback(repo_id, filename, url_to_download, str(paths.file_path), "failed", str(e), None)
            except Exception as callback_e:
                logger.warning(f"download_callback (failed) 执行失败: {callback_e}")
        raise
