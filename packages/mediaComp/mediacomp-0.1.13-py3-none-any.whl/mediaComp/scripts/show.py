            ...
            while not self._stop_event.is_set():
                try:
                    ...
                    if control == bytes([0]):
                        ...
                        # Read size - use blocking read but check stop event between bytes
                        size_bytes = b''
                        for i in range(8):
                            if self._stop_event.is_set():
                                break
                            
                            # For size reading, use longer timeout since this is critical data
                            if sys.platform == 'win32':
                                # Wait longer for size data on Windows
                                timeout_count = 0
                                while not msvcrt.kbhit() and timeout_count < 50:  # Wait up to 5 seconds
                                    if self._stop_event.wait(0.1):
                                        break
                                    timeout_count += 1
                                if self._stop_event.is_set() or timeout_count >= 50:
                                    break
                                byte = sys.stdin.buffer.read(1)
                            else:
                                # Wait longer for size data on Unix
                                ready, _, _ = select.select([sys.stdin], [], [], 5.0)
                                if not ready:
                                    if not self._stop_event.is_set():
                                        print("Timeout waiting for size data", file=sys.stderr)
                                    break
                                byte = sys.stdin.buffer.read(1)
                        ...
                        print(f"Reading payload of size: {size}", file=sys.stderr)
                        
                        if size == 0:
                            continue
                        
                        # Read payload - use larger chunks but still check stop event
                        payload = b''
                        bytes_read = 0
                        while bytes_read < size and not self._stop_event.is_set():
                            remaining = size - bytes_read
                            chunk_size = min(4096, remaining)  # Back to larger chunks for efficiency
                            
                            if sys.platform == 'win32':
                                # On Windows, wait for data to be available
                                data_available = False
                                timeout_count = 0
                                while not data_available and timeout_count < 50:  # Wait up to 5 seconds
                                    if msvcrt.kbhit():
                                        data_available = True
                                        break
                                    if self._stop_event.wait(0.1):
                                ...
                                if not data_available:
                                    print(f"Timeout waiting for payload data at {bytes_read}/{size}", file=sys.stderr)
                                    break
                                    
                                chunk = sys.stdin.buffer.read(chunk_size)
                            else:
                                # On Unix-like systems, use select with longer timeout for payload
                                ready, _, _ = select.select([sys.stdin], [], [], 5.0)
                                if not ready:
                                    if not self._stop_event.is_set():
                                        print(f"Timeout waiting for payload data at {bytes_read}/{size}", file=sys.stderr)
                            ...
                            # Occasional stop event check during large payloads
                            if bytes_read % 8192 == 0 and self._stop_event.is_set():
                                break
                        
                        if len(payload) == size and not self._stop_event.is_set():
                            print(f"Successfully read payload of {len(payload)} bytes", file=sys.stderr)
