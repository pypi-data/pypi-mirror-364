...
class CPDLC:
    ...
    Attributes:
        ...
        _thread_pool (concurrent.futures.ThreadPoolExecutor) : thread pool
    ...
    def __init__(self, max_workers: int = 8):
        ...
        self._thread_pool.submit(lambda: self.client)
    ...
    def initialize_service(self):
        ...
        result_1 = self._thread_pool.submit(check_connection)
        result_2 = self._thread_pool.submit(check_service_level)
    ...
    def _message_receiver_callback(self, message: AcarsMessage) -> None:
        ...
        def execute(fn: Callable[[AcarsMessage], None]) -> None:
            try:
        ...
        for callback in self._message_receiver_callbacks:
            self._thread_pool.submit(execute, callback)
    ...
    def _message_sender_callback(self, to: str, message: str) -> None:
        ...
        def execute(fn: Callable[[str, str], None]) -> None:
            try:
                logger.trace(f"Callback executed: {callback.__name__}")
                fn(to, message)
            except Exception as e:
