    ...
    try:
        ...
        if ignore:
            ...
            if 递归:
                ...
                while True:
                    if err.tb_next:
                        ...
                        lno = err.tb_lineno
                        frame = err.tb_frame  # 获取当前帧
                        func_name = frame.f_code.co_name  # 获取函数名
                        break
            ...
            if lno!=e.__traceback__.tb_lineno and 递归:
                pcolor('[{}>{}][{}] 错误: {}'.format(e.__traceback__.tb_next.tb_lineno, lno,func_name, 错误提示 if 错误提示 else e), 'err')
            # pcolor('[错误 {}] : {}'.format(sys.exc_info()[2].tb_next.tb_lineno ,error if error else e.args),'err')
            else:
                pcolor('[{}][{}] 错误: {}'.format(lno,func_name, 错误提示 if 错误提示 else e), 'err')
def 打印错误(e: BaseException, 递归=1):
    if 递归:
        ...
        func_name = frame.f_code.co_name
    if lno!=e.__traceback__.tb_lineno and 递归:
        pcolor('[{}>{}][{}] 错误: {}'.format(e.__traceback__.tb_lineno, lno,func_name, e), 'err')
    else:
        pcolor('[{}][{}] 错误: {}'.format(lno,func_name, e), 'err')
