...
def list_correlate(list_first: list,
                   ...
                   in_both: bool = False) -> tuple:
    """
    Correlate *list1* and *list2* by computing their differences.
            ...
            if len(list_first) <= len(list_second):
                if in_both or only_in_first:
                    for first in list_first:
                        if first in list_second:
                            if in_both:
                                result_both.append(first)
                        elif only_in_first:
                            result_first.append(first)
                if only_in_second:
                    result_second = [second for second in list_second if second not in list_first]
            else:
                if in_both or only_in_second:
                    for second in list_second:
                        if second in list_first:
                            if in_both:
                                result_both.append(second)
                        elif only_in_second:
                            result_second.append(second)
                if only_in_first:
                    result_first = [first for first in list_first if first not in list_second]
