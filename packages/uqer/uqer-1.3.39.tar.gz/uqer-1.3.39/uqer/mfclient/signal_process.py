...
def neutralize(raw_data, target_date, industry_type="SW1", exclude_style_list=[], **kwargs):
    ...
    try:
        r = session.post(BRAIN_HOST + '/neutralize', json=send_data)
    except:
...
def neutralize_pit(raw_data, target_date, industry_type="SW1", exclude_style_list=[], **kwargs):
    '''
    sw21中性化处理函数
    ...
    send_data['config'] = config_dict
    try:
        r = session.post(BRAIN_HOST + '/neutralize_pit', json=send_data)
    except:
        raise MFNetworkError('Neutralize Network Error')
    result = handle_recv_json(r)
