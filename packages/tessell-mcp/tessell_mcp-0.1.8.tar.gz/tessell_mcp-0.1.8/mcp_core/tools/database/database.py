...
This module provides generic, engine-agnostic database tools and dispatchers for the Tessell MCP platform. All engine-specific logic—such as engine implementations, queries, and database operations—must be implemented in their respective engine modules (e.g., postgresql_engine.py, oracle_engine.py). This ensures a clean separation of concerns, maintainability, and extensibility as new database engines are added (e.g., MySQL, SQL Server).
...
Contributors and AI agents: Follow these conventions for all future database tool development.
"""
...
def create_database(connection_name: str, database_name: str):
    ...
    Dispatches to the engine-specific implementation based on the connection type.
    - For PostgreSQL: Creates a new database with the current user as owner. Requires CREATEDB privilege.
...
def get_sample_data(connection_name: str, database: str, schema: str, object_name: str, object_type: str, limit: int = 10) -> Dict[str, Any]:
    ...
    try:
        ...
        if object_type == "table":
            ...
            # Try engine.get_sample_data if implemented, else fallback to SELECT *
            if hasattr(engine, "get_sample_data"):
                desc = engine.get_sample_data(database, f"{schema}.{object_name}", limit)
                # If engine returns error or not implemented, fallback
                if not desc.get("success", True):
                    query = f'SELECT * FROM "{schema}"."{object_name}" LIMIT %s'
                    result = engine.execute_query(query, (limit,))
                    if result["success"]:
                        desc = {
                        ...
                        desc = {"success": False, "error": result.get("error", "Failed to get sample data from view.")}
            else:
                query = f'SELECT * FROM "{schema}"."{object_name}" LIMIT %s'
                result = engine.execute_query(query, (limit,))
                if result["success"]:
