...
class RobotModel(UnmanagedObject):
  ...
  def get_grav_comp_efforts(self,
    ...
    num_frames = self.get_frame_count(FrameType.CenterOfMass)
    masses = self.masses

    wrench = np.zeros((6, num_frames))
    np.outer(gravity, masses, out=wrench[:3, :])
    if jacobians is None:
      jacobians = self.get_jacobians_mat(FrameType.CenterOfMass, positions)

    # jacobians (frames, 6, dof)
    # wrench (6, frames)
    # efforts (dof)
    #np.einsum('ijk,ji->k', jacobians, wrench, out=comp_torque)
    comp_torque[:] = np.tensordot(jacobians, wrench, axes=([0, 1], [1, 0]))
  ...
  def get_dynamic_comp_efforts(self,
    ...
    cmd_accel_dt_s = .5*cmd_accels*dt_s
    ...
    # get positions at +/- dt
    cmd_positions_last = cmd_positions-cmd_v_dt+cmd_accel_dt_s
    cmd_positions_next = cmd_positions+cmd_v_dt+cmd_accel_dt_s

    masses = self.masses

    # Get Forward kinematics for all 3 sets of angles
    # (#frames x 3)
    cmd_frames_last = self.get_forward_kinematics_mat(FrameType.CenterOfMass, cmd_positions_last)[:, :3, 3]
    cmd_frames_now = self.get_forward_kinematics_mat(FrameType.CenterOfMass, cmd_positions)[:, :3, 3]
    cmd_frames_next = self.get_forward_kinematics_mat(FrameType.CenterOfMass, cmd_positions_next)[:, :3, 3]

    # Build wrench vector and calculate compensatory torques
    if jacobians is None:
      jacobians = self.get_jacobians_mat(FrameType.CenterOfMass, fbk_positions)
    ...
    efforts[:] = np.tensordot(jacobians, wrench, axes=([0, 1], [0, 1]))
