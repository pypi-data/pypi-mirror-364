...
from service.aui_test_agent.hyperjump_copilot.chat_prompt import PROMPT, TOOLS
...
CHAT_NAMESPACE = '/hyperjump/copilot/chat'
...
class ChatResponseManager:
    ...
    def chat_once(self, data, role='user'):
        model = Friday(model=self.model, max_tokens=2048, functions=TOOLS)
        self.chat_storage.chat_history.append(ChatMessage(role=role, content=data))
        messages = [{'role': 'system', 'content': PROMPT}] + self.chat_storage.get_history_message()
        completion = model(messages)
        return completion, model.last_response
    ...
    def save_ui_case(self,mmcd_url=None,case_name=None):
        self.send_chat_message("用例保存成功！"+mmcd_url+case_name)
        sceneid=self.chat_storage.state_memory.sceneid
        parentid=extract_page_id(mmcd_url)
...
def manage_event(
    ...
    try:
        ...
        chat_resp_manager = ChatResponseManager(chat_storage, device_storage, socket, task_queue)
