from service.app_traversal.mss_client import s3_client
from service.app_traversal.image_utils import get_center_pos
import copy
import traceback
import numpy as np
import requests
...
config.read('config/visionui_client.conf', encoding='utf-8')

class ClientHorus(object):
    def __init__(self):
        self.ocr_host = config['default']['ocr_host']
        self.host_online = config['default']['host_online']
        # vision-ui服务，plus 不支持ubuntu，手动用vm mac部署
        self.vision_ui_infer_url = config['default']['vision_ui_infer_url']
        self.vision_ui_semantic_url = config['default']['vision_ui_semantic_url']

    def qr_reader(self, url):
        host = self.host_online+'qrReader'
        max_try = 3
        payload = {'codeType': 'qrcode', 'url': url, 'version': 3}
        header = {'Content-Type': 'application/json;charset=utf-8'}
        result = {'code': 0, 'data': ''}
        resp = type('Resp', (object,), dict(text=''))()
        for i in range(max_try):
            time.sleep(1)
            resp = requests.request('POST', url=host, data=json.dumps(payload), headers=header)
            if resp.status_code == 200:
                resp_obj = json.loads(resp.text)
                if resp_obj.get('errorCode') == 0 or resp_obj.get('errorCode') == 5:
                    result['data'] = resp_obj['results'][0]['decodeRes']
                    break
        else:
            result['code'] = 1
    ...
    def get_inspect(self, image_source: str, func_type: str):
        ...
        image_url = s3_client.save_to_mss(image_source, s3_path_name)
    ...
    def semantic_search(self, payload):
        ...
        for i in range(max_try):
            try:
                resp = requests.request('POST', url=self.vision_ui_semantic_url, data=json.dumps(payload), headers=header, timeout=90)
                if resp.status_code == 200:
                    resp_obj = json.loads(resp.text)
                    if resp_obj.get('code') == 0:
                        result['data'] = resp_obj['data']
                        break
                time.sleep(3)
    ...
    def get_ui_infer(self, image: numpy.ndarray = None, image_path: str = None, offset_y: int = 0,
        ...
        if image is not None:
            ...
            s3_path_name = f'resource/{ts}-{image_name}'
            image_url = s3_client.save_to_mss(image_path, s3_path_name)
    ...
    def get_ocr_from_horus(self, image_source, business=''):
        ...
        try:
            ...
            horus_ocr_reporter_publish(time_used_millis, success, cause, exception_msg, '', business)
    ...
    def find_elem_by_image_semantic(self, target_img_path, source_img_path, desc, top_k, confidence_score,
        ...
        try:
            if target_img_path:
                target_img_source = cv2.imread(target_img_path)
                image_b_t = base64.b64encode(cv2.imencode('.png', target_img_source)[1])
                target_img_source = bytes.decode(image_b_t)
                image_alpha = 1.0
            else:
                target_img_source = np.zeros([100, 100, 3], dtype=np.uint8) + 255
                image_b_t = base64.b64encode(cv2.imencode('.jpg', target_img_source)[1])
                target_img_source = bytes.decode(image_b_t)
                image_alpha = 0.0

            source_img_source = cv2.imread(source_img_path)
            image_b_s = base64.b64encode(cv2.imencode('.png', source_img_source)[1])
...
def horus_ocr_reporter_publish(time_used_millis: float, success: bool, cause: str, exception_msg: str, trace, business):
    body = {
        "step": "HorusOCR",
        "jobId": "0",
        "type": "hyperjump",
        "caseId": "0",
        "business": business,
        "timeUsedMillis": time_used_millis,
        "imageSize": '-1',
        "success": str(success),
        "exceptionDetail": {
            "message": exception_msg if exception_msg is not None else '',
            "cause": cause if cause is not None else '',
            "trace": trace if trace is not None else ''
        }
    }
    try:
        resp = requests.post(config['default']['ocr_es_publish_url'], json=body, timeout=5)
    except Exception:
        print(traceback.format_exc())
        resp = None
    if resp and resp.status_code != 200:
        print(f'EagleReporter failed. {resp.text}')
    return resp and resp.status_code == 200

visionui_client = ClientHorus()