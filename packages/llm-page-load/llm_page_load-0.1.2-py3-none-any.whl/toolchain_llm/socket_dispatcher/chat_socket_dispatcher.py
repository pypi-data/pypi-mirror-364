...
class ChatSocketDispatcher(BaseSocketDispatcher):
    '''
    聊天消息的Socket Dispatcher
    每个 ChatSocketDispatcher 实例确实对应一个唯一的 socket.sid，这是由 Flask-SocketIO 的 request.sid 机制保证的。
    '''
    ...
    def on_message(self, message):
        ...
        try:
            ...
            logger.error('ChatSocketDispatcher Error', e)
    ...
    def on_connect(self):
        ...
        current_sid = request.sid
        event = SocketEvent(
            # event_name='chat', direction='Server2Frontend', event_data='请向我提问，可以提问“帮我申请一个设备”或者“手机白屏怎么办”')
