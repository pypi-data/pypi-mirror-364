from coral8.injector import inject_function
import typer
...
def graft(
    ...
    into: str = typer.Option(None, "--into", help="Target .py file to insert into"),
):
    """
    Extract and print a function or class, or inject into a target file using --into.
    """
    try:
        if into:
            from_path = DATA_DIR / filename
            to_path = Path(into).resolve()
            message = inject_function(from_path, to_path, symbol)
            typer.echo(message)
        else:
