...
def main(transport='stdio', port=6000):
    # ...existing setup...
    if transport == 'http':
...
async def old():
    tools = [
        find_exact_poi_from_user_input,
        find_exact_category_from_user_input,
        generate_qr_code_for_specific_poi,
        generate_qr_code_for_category
    ]
    agent = Agent(name="Pointr Indoor Location Assistant", instructions=SYSTEM_PROMPT, tools=tools, model="gpt-4o-mini")
    with trace("Ask indoor location"):
        result =  await Runner.run(agent, "where is Azure Meeting Room?")
