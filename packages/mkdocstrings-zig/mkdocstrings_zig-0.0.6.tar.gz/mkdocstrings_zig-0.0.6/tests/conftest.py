...
if TYPE_CHECKING:
    ...
    from mkdocstrings_handlers.zig.handler import ZigHandler
...
def fixture_mkdocs_conf(request: pytest.FixtureRequest, tmp_path: Path) -> Iterator[config.Config]:
    ...
    conf_dict = {
        ...
        "plugins": [{"mkdocstrings": {"default_handler": "zig"}}],
...
def fixture_handler(plugin: MkdocstringsPlugin, ext_markdown: Markdown) -> ZigHandler:
    ...
    Returns:
        A handler instance.
    """
    handler = plugin.handlers.get_handler("zig")
