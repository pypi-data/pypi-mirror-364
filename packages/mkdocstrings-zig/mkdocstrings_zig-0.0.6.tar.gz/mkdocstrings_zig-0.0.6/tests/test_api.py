...
from mkdocstrings_handlers import zig
...
def _fixture_loader() -> griffe.GriffeLoader:
    ...
    loader.load("mkdocstrings_handlers.zig")
...
def _fixture_internal_api(loader: griffe.GriffeLoader) -> griffe.Module:
    return loader.modules_collection["mkdocstrings_handlers.zig._internal"]


@pytest.fixture(name="public_api", scope="module")
def _fixture_public_api(loader: griffe.GriffeLoader) -> griffe.Module:
    return loader.modules_collection["mkdocstrings_handlers.zig"]
...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `mkdocstrings_handlers.zig`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in zig.__all__ or not hasattr(zig, obj.name)
    ]
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("mkdocstrings_handlers.zig")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and _module_or_child("mkdocstrings_handlers.zig", item.name)
        ):
