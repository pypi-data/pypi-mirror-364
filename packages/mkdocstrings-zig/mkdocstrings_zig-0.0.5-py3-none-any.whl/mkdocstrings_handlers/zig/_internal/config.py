...
try:
    ...
    if sys.version_info < (3, 10):
        try:
            ...
            _logger.debug(
                ...
                "Deactivating Pydantic validation for Zig handler options.",
            )
            raise
    ...
    _base_url = "https://mkdocstrings.github.io/mkdocstrings-zig/usage"
...
# YORE: EOL 3.9: Replace `**_dataclass_options` with `frozen=True, kw_only=True` within line.
@dataclass(**_dataclass_options)
class ZigOptions(ZigInputOptions):  # type: ignore[override,unused-ignore]
...
# YORE: EOL 3.9: Replace `**_dataclass_options` with `frozen=True, kw_only=True` within line.
@dataclass(**_dataclass_options)
class ZigInputConfig:
    """Zig handler configuration."""

    # We want to validate options early, so we load them as `ZigInputOptions`.
    options: Annotated[
        ZigInputOptions,
...
# YORE: EOL 3.9: Replace `**_dataclass_options` with `frozen=True, kw_only=True` within line.
@dataclass(**_dataclass_options)
class ZigConfig(ZigInputConfig):  # type: ignore[override,unused-ignore]
    """Zig handler configuration."""
