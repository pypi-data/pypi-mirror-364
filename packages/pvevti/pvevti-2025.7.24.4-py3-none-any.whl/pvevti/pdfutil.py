    ...
    "pages": [
        {
            ...
            "plots": [
                {
                    "plotTitle": "Speed and Distance",
                    "plotType" : "line",
                    "xData"    : "Cumulative Distance",
                    "yData"    : ["isp_vehicle_speed"],
                    "filterLength": 40
                },
                {
                    "plotTitle": "Altitude",
        ...
        },
        {
            ...
            "plots": [
                ...
                },
                {
                    ...
                    "plotType" : "scatter",
                    "xData"    : "VehSpd_Cval_CPC",
                    "yData"    : "TransCurrentGear"
                },
                {
                    "plotTitle": "Dual Line Plot Test",
                    "plotType" : "dualline",
                    "xData"    : "t",
                    "yData"    : ["boostpress_cval_pt", "ATS_TO"],
                    "filterLength": 180
                }
            ]
        }
...
class Plot:
    def __init__(self, type="line", data=[], legend = [], xlabel = "", ylabel = "", columns = [], infer_names = False, title="Unnamed Plot", filterLength=0):
        """
    ...
    def render(self):
        ...
        match self.type:
            ...
            case "scatter" | "scatterplot":
                for i in range(1, len(self.data)):
                    if i <= len(cols):
                        plt.scatter(self.data[0], self.data[i], color=getCol(i-1)+"D0", s=3)
                    else:
            ...
            case "dual" | "dualline":
                ...
                ax2.set_position([pos.x0, pos.y0, pos.width * 0.925, pos.height])
                ax1.plot(self.data[0], self.data[1], color=getCol(0))
                ax2.plot(self.data[0], self.data[2], color=getCol(1))
                ax1.set_ylabel(self.columns[1], color=getCol(0))
                ax2.set_ylabel(self.columns[2], color=getCol(1))
                ax1.tick_params(axis="y", colors=getCol(0))
                ax2.tick_params(axis="y", colors=getCol(1))
                ax1.set_xticklabels(labels=[], color=getCol(0))
                ax2.set_xticklabels(labels=[], color=getCol(1))
                ax1.grid(True, 'major', 'y', color=getCol(0), linestyle="--")
