...
def most_recent_csv(directory, ignore=""):
    ...
    csv_files = [directory + file for file in files if ".csv" in file and (ignore not in file or ignore == "")]
...
def all_csvs(directory, cascade = False):
    """
    ...
    if cascade:
        for path in os.listdir(directory):
            ...
            if os.path.isdir(fullpath):
                csv_files = csv_files + all_csvs(fullpath, True)
    if len(csv_files) >= 1:
        return csv_files
    else:
        return []
