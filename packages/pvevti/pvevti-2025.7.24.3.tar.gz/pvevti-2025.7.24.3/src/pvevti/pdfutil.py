...
class Plot:
    def __init__(self, type="line", data=[], legend = [], xlabel = "", ylabel = "", columns = [], infer_names = False, title="Unnamed Plot"):
        """
    ...
    for (i, page) in enumerate(cfg['pages']):
        ...
        for (x, plot) in enumerate(page['plots']):
            ...
            cd_plot = Plot(type=plot["plotType"], data=toPlot, 
                           columns=([plot["xData"]]+gu.parseNames(data.columns, plot['yData'])), 
