...
class display:
    ...
    def set_window(size=(800, 600), title="GameX!"):
        ...
        ic = pygame.image.load(os.path.join(BASE_DIR, "gameX_icon.jpg"))
...
class Role(pygame.sprite.Sprite):
    ...
    def collide(self, target):
        ...
        if isinstance(target, Role):
            ...
            # 计算目标相对于当前角色的坐标偏移
            x_offset = int(target.position.x - self.position.x)
            y_offset = int(target.position.y - self.position.y)
            ...
            for sprite in target:
                sprite.mask = pygame.mask.from_surface(_get_image(sprite))
                x_offset = int(sprite.position.x - self.position.x)
                y_offset = int(sprite.position.y - self.position.y)
...
class Pen:
    ...
    def __init__(self, width=None, high=None):
        ...
        BASE_DIR = os.path.dirname(os.path.abspath(__file__))
        self.font = os.path.join(BASE_DIR,"My_font.ttf")
...
def help(cont=None):
    ...
    if cont == 'all' or cont is None:
        ...
        print(__key__)
        print(__role__)
    elif cont == 'display': print(__display__)
    elif cont == 'background': print(__background__)
    elif cont == 'mouse': print(__mouse__)
    elif cont == 'sound': print(__sound__)
    elif cont == 'pen': print(__pen__)
    elif cont == 'key': print(__key__)
    elif cont == 'role': print(__role__)
    else:
