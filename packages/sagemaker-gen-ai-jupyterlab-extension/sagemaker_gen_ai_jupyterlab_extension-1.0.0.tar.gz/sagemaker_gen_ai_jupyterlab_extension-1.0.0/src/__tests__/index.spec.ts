...
jest.mock('../icons/MynahIconBlack', () => ({
  MynahIconBlack: 'mock-icon'
}));
jest.mock('../webview', () => ({
...
describe('index.ts', () => {
  ...
  describe('connectWebSocket', () => {
    it('should create WebSocket connection with correct URL', () => {
      ...
      (URLExt.join as jest.Mock).mockReturnValue('ws://localhost:8888/sagemaker_gen_ai_jupyterlab_extension/ws');
    ...
    it('should use default empty endpoint', () => {
      ...
      const mockDocManager = { services: {} };
      (ServerConnection.makeSettings as jest.Mock).mockReturnValue(mockSettings);
      (URLExt.join as jest.Mock).mockReturnValue('ws://localhost:8888/sagemaker_gen_ai_jupyterlab_extension/');
      (ServerConnection.makeSettings as jest.Mock).mockReturnValue(
        mockSettings
      );
      (URLExt.join as jest.Mock).mockReturnValue(
  ...
  describe('plugin configuration', () => {
    ...
    it('should test plugin activation function', async () => {
      ...
      plugin.activate(mockApp as any, mockLabShell as any, mockDocManager as any, mockNotebookTracker as any, mockSettingsRegistry as any);
  ...
  describe('Error Handling', () => {
    ...
    it('should test actual plugin activation with error handling', async () => {
      ...
      (ServerConnection.makeSettings as jest.Mock).mockReturnValue({ wsUrl: 'ws://test' });
      ...
      const mockRegisterContextMenuActions = require('../contextMenu').registerContextMenuActions;
      ...
      plugin.activate(mockApp as any, mockLabShell as any, mockDocManager as any, mockNotebookTracker as any);
      
      expect(consoleSpy).toHaveBeenCalledWith('Failed to register context menu actions:', expect.any(Error));
    });
    ...
    it('should test actual plugin activation with WebSocket failure', async () => {
      ...
      const result = await plugin.activate(mockApp as any, mockLabShell as any, mockDocManager as any, mockNotebookTracker as any);
    ...
    it('should test setTimeout error handling in plugin', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Failed to create initial Q panel');
