...
class SimpleField:
    ...
    def _raise_attr_type_error(self) -> None:
        raise FieldValueError(f'Key {self.attr_name} must be {self.attr_type}.')

    def _raise_choices_error(self, value: Any) -> None:
        raise FieldValueError(f"The value '{value}' for field '{self.attr_name}' must be in this list {self.choices}.")

    def _raise_readonly_error(self) -> None:
        raise FieldValueError(f"The field '{self.attr_name}' value cannot be changed.")

    def _raise_required_error(self) -> None:
        raise RequiredError(f'The {self.attr_name} attribute is required.')
