    ...
    def generate_schema(
        ...
        with get_cursor(conn) as cursor:
            for table, col, typ in filtered_columns:
                ...
                # Strategy 2: Simple random sampling without expensive COUNT
                if len(samples) < num_examples:
                    try:
                        # Just get a reasonable number of distinct values without COUNT
                        cursor.execute(
                            f"SELECT DISTINCT `{col}` FROM `{table}` WHERE `{col}` IS NOT NULL LIMIT 1000"
                        )
                        all_samples = [row[0] for row in cursor.fetchall()]
                        
                        if len(all_samples) <= num_examples:
                            # If we got fewer than requested, use them all
                            samples = all_samples
                        else:
                            # Take random sample from this set
                            import random
                            samples = random.sample(all_samples, num_examples)
                        logger.debug(f"Used simple sampling for {table}.{col}")
                    except mysql.connector.Error:
                        pass
