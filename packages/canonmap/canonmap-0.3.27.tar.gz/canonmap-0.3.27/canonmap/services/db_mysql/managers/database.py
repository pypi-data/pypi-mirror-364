    ...
    def generate_schema(
        ...
        with get_cursor(conn) as cursor:
            for table, col, typ in filtered_columns:
                ...
                # Strategy 2: Use RAND() with a smaller subset if TABLESAMPLE fails
                if len(samples) < num_examples:
                    try:
                        ...
                        if total_distinct <= num_examples:
                            # If we have fewer distinct values than requested, get them all
                            cursor.execute(
                                f"SELECT DISTINCT `{col}` FROM `{table}` WHERE `{col}` IS NOT NULL",
                            )
                            samples = [row[0] for row in cursor.fetchall()]
                        else:
                            # Use RAND() but with a reasonable limit to avoid full table scan
                            # Sample from first 10,000 rows instead of entire table
                            cursor.execute(
                                f"SELECT DISTINCT `{col}` FROM `{table}` WHERE `{col}` IS NOT NULL ORDER BY RAND() LIMIT 10000"
                            )
                            all_samples = [row[0] for row in cursor.fetchall()]
                            # Take random sample from this smaller set
                            import random
                            samples = random.sample(all_samples, min(num_examples, len(all_samples)))
                        logger.debug(f"Used RAND() sampling for {table}.{col}")
                    except mysql.connector.Error:
                        pass
