...
class MatcherManager:
    ...
    def match(self, request: MatchEntityRequest, top_n=20, threshold=None, weights=None) -> list[tuple[str, float]]:
        ...
        for field in request.select_fields:
            ...
            logger.debug(f"Field {field.field_name}: phonetic={len(phonetic_candidates)}, soundex={len(soundex_candidates)}, initialism={len(initialism_candidates)}, exact={len(exact_candidates)}")
        
        logger.debug(f"Total candidates found: {len(candidates)}")
        
        # If we don't have enough candidates, expand the search
        if len(candidates) < top_n:
            logger.debug(f"Only found {len(candidates)} candidates, expanding search to get more candidates")
            ...
            for field in request.select_fields:
                additional = _get_more_candidates(conn, normalized_entity_name, field.table_name, field.field_name, top_n - len(candidates))
        ...
        # Apply threshold logic: always return top_n results, but prioritize threshold-passing ones
        if threshold is not None:
            # Split results into above-threshold and below-threshold
            above_threshold = [(name, score) for name, score in ranked if score >= threshold]
            below_threshold = [(name, score) for name, score in ranked if score < threshold]
            
            # If we have enough above-threshold results, return top_n of those
            if len(above_threshold) >= top_n:
                return above_threshold[:top_n]
            
            # Otherwise, fill with below-threshold results to reach top_n
            result = above_threshold.copy()
            remaining_slots = top_n - len(result)
            result.extend(below_threshold[:remaining_slots])
            return result
        
        # If no threshold specified, return top_n as before
        return ranked[:top_n]
