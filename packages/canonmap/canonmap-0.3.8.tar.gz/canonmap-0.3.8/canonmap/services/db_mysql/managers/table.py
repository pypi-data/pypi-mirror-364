...
from canonmap.services.db_mysql.schemas import Field, FieldTransform
...
class TableManager:
    ...
    def create_table_fields(
        ...
        """
        For each Field(table_name, field_name, field_transform) create a derived column:
        ...
        for table_name, flist in by_table.items():
            ...
            # Split work
            sql_jobs: list[tuple[Field, str]] = []
            py_jobs:  list[tuple[Field, str]] = []
    ...
    def drop_table_fields(self, fields: list["Field"]) -> dict[str, list[str]]:
        """
        Drop the derived columns referenced by Field objects.
        Uses Field.field_transform to infer the column name:
