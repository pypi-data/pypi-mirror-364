    ...
    def generate_schema(
        ...
        with get_cursor(conn) as cursor:
            for table, col, typ in filtered_columns:
                ...
                # Strategy 2: Simple and fast - get more values than needed and sample randomly
                if len(samples) < num_examples:
                    try:
                        # Get more distinct values than needed to increase randomness
                        # This is fast and gives reasonable randomness
                        cursor.execute(
                            f"SELECT DISTINCT `{col}` FROM `{table}` WHERE `{col}` IS NOT NULL LIMIT 5000"
                        )
                        all_samples = [row[0] for row in cursor.fetchall()]
                        
                        # Take random sample from what we got
                        import random
                        if len(all_samples) <= num_examples:
                            samples = all_samples
                        else:
                            samples = random.sample(all_samples, num_examples)
                        logger.debug(f"Used simple sampling for {table}.{col} (got {len(all_samples)} distinct values)")
                    except mysql.connector.Error:
                        pass
