    ...
    def generate_schema(
        ...
        with get_cursor(conn) as cursor:
            if tables_to_include:
                # Build a query with specific table names
                placeholders = ', '.join(['%s'] * len(tables_to_include))
                cursor.execute(
                    f"SELECT TABLE_NAME, COLUMN_NAME, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=%s AND TABLE_NAME IN ({placeholders})",
        ...
        with get_cursor(conn) as cursor:
            for table, col, typ in filtered_columns:
                # Try multiple efficient random sampling strategies
                samples = []
                
                # Strategy 1: Use TABLESAMPLE if available (MySQL 8.0+)
                try:
                    cursor.execute(
                        f"SELECT DISTINCT `{col}` FROM `{table}` TABLESAMPLE SYSTEM(1) WHERE `{col}` IS NOT NULL LIMIT %s",
                        (num_examples,)
                    )
                    samples = [row[0] for row in cursor.fetchall()]
                    if len(samples) >= num_examples:
                        logger.debug(f"Used TABLESAMPLE for {table}.{col}")
                except mysql.connector.Error:
                    pass
                
                # Strategy 2: Truly random sampling using multiple random offsets
                if len(samples) < num_examples:
                    try:
                        # Get multiple random samples from different parts of the table
                        all_samples = set()
                        attempts = 0
                        max_attempts = 10  # Try up to 10 different random offsets
                        
                        while len(all_samples) < num_examples and attempts < max_attempts:
                            # Use a random offset to sample from different parts of the table
                            import random
                            random_offset = random.randint(0, 100000)  # Random starting point
                            
                            cursor.execute(
                                f"SELECT DISTINCT `{col}` FROM `{table}` WHERE `{col}` IS NOT NULL LIMIT 200 OFFSET %s",
                                (random_offset,)
                            )
                            batch_samples = [row[0] for row in cursor.fetchall()]
                            all_samples.update(batch_samples)
                            attempts += 1
                        
                        # Convert to list and take random sample
                        all_samples_list = list(all_samples)
                        if len(all_samples_list) <= num_examples:
                            samples = all_samples_list
                        else:
                            samples = random.sample(all_samples_list, num_examples)
                        logger.debug(f"Used multi-offset sampling for {table}.{col} (attempts: {attempts})")
                    except mysql.connector.Error:
                        pass
                
                # Strategy 3: Fallback to first N distinct values
                if len(samples) < num_examples:
                    cursor.execute(
                        f"SELECT DISTINCT `{col}` FROM `{table}` WHERE `{col}` IS NOT NULL LIMIT %s",
                        (num_examples,)
                    )
                    samples = [row[0] for row in cursor.fetchall()]
                    logger.debug(f"Used fallback sampling for {table}.{col}")
