    ...
    def generate_schema(
        ...
        with get_cursor(conn) as cursor:
            for table, col, typ in filtered_columns:
                # true random sampling at the SQL level
                try:
                    cursor.execute(
                        f"SELECT DISTINCT `{col}` FROM `{table}` "
                        "WHERE `{col}` IS NOT NULL ORDER BY RAND() LIMIT %s",
                        (num_examples,),
                    )
                    samples = [row[0] for row in cursor.fetchall()]
                    logger.debug(f"Used ORDER BY RAND() for {table}.{col}")
                except mysql.connector.Error as e:
                    logger.warning(f"Random sampling failed for {table}.{col}: {e}")
                    samples = []
