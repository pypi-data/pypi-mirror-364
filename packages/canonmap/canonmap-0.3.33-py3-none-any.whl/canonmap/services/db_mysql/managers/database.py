    ...
    def generate_schema(
        ...
        with get_cursor(conn) as cursor:
            for table, col, typ in filtered_columns:
                ...
                # attempt primary-key range sampling for performance
                try:
                    cursor.execute(
                    ...
                    if pk_info:
                        ...
                        if pk_type.upper() in ("INT", "BIGINT", "SMALLINT", "MEDIUMINT", "TINYINT"):
                            cursor.execute(f"SELECT MIN(`{pk_col}`), MAX(`{pk_col}`) FROM `{table}`")
                            min_id, max_id = cursor.fetchone()
                            import random
                            trials = 0
                            while len(samples) < num_examples and trials < num_examples * 10:
                                ...
                                trials += 1
                            logger.debug(
                                f"Used PK-range sampling for {table}.{col} "
                                f"with {trials} trials and {len(samples)} samples"
                            )
                        else:
                            raise ValueError("Non-integer PK")
                    else:
                    ...
                    samples = reservoir
                    logger.debug(
                        f"Used fallback reservoir sampling for {table}.{col} "
