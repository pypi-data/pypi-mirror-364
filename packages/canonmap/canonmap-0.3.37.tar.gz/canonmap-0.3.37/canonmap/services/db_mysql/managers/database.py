        ...
        try:
            with get_cursor(conn) as cursor:
                ...
                if cursor.fetchone()[0] == 0:
                    # Drop any existing stub and create the function in multi-statement mode
                    cursor.execute(f"DROP FUNCTION IF EXISTS `{initialism_fn}`;", multi=True)
                    create_fn_sql = f"""
                    ...
                    END;
                    """
                    cursor.execute(create_fn_sql, multi=True)
                    conn.commit()
