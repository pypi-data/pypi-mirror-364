...
class ChattyAIAgent(CompanyAssetModel):
    ...
    default_follow_up_strategy_id: Optional[StrObjectId] = Field(default=None, description="Default follow-up strategy id")
    ...
    @property
    def follow_up_strategy_id_default(self) -> StrObjectId:
        """Get the default follow up strategy"""
        if self.default_follow_up_strategy_id is None:
            raise ValueError(f"Chatty AI Agent {self.id} has no default follow up strategy")
