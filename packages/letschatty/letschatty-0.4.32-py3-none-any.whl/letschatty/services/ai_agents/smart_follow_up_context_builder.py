...
class SmartFollowUpContextBuilder(ContextBuilder):
    ...
    def build_prompt(chat_smart_follow_up_assigned: SmartFollowUpState, follow_up_strategy: FollowUpStrategy, company_info: EmpresaModel, chatty_ai_agent: ChattyAIAgent, chat: Chat, mode_in_chat: ChattyAIMode) -> str:
        ...
        context += ContextBuilder.relevant_contexts(chatty_ai_agent, chat)
        ...
        context += f"And here are some real business applications of the follow up strategy: {ContextBuilder.relevant_examples(follow_up_strategy.contexts, chat)}"
        return context
    ...
    def follow_up_strategy_context(chat_smart_follow_up_assigned: SmartFollowUpState, follow_up_strategy: FollowUpStrategy) -> str:
        ...
        if datetime.now(ZoneInfo('UTC')).weekday() >= 5:
            raise PostponeFollowUp("The follow up is postponed because it's a weekend")
