...
class Deck(PClass):
    ...
    deck_cards = field(type=PVector)  # List[DeckCard]
...
class State(PClass):
    ...
    def __todict__(self):
        return {
            ...
            "decks": [x.__todict__() for x in self.decks],
...
def getinitialstate(game_config):
    ...
    state = State(
        player_idxs=pvector(range(game_config.num_players)),
        decks=pvector([]),  # No decks initialized yet
...
def shuffle_player_order(game):
    ...
    game.rng.shuffle(shuffled_idxs)
    
    return game.set(player_idxs=v(shuffled_idxs))
...
"""
    ...
    try:
        ...
        if log:
            print(f"****************************** Running initialization hook 4: {hook.uuid} {result.player_idxs}")
...
def getpublicstate(s):
    return PublicState(
        player_idxs=list(s.player_idxs),
...
def get_imagined_state(static_board_config, player_state):
    ...
    return State(
        player_idxs = pvector(public_state.player_idxs),
        decks = pvector([]), # Use public state, eventually
        game_config = game_config,
