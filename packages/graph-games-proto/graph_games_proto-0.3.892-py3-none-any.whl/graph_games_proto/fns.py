...
class Deck(PClass):
    ...
    deck_cards = field(type=list)  # List[DeckCard]
    def __todict__(self):
        return {
            "uuid": self.uuid,
            "cards": [card.__todict__() for card in self.cards],
            "deck_cards": [deck_card.__todict__() for deck_card in self.deck_cards],
        }
    @staticmethod
    def __fromdict__(d):
        return Deck(
            uuid=d["uuid"],
            cards=[], # TODO: Card.__fromdict__(card) for card in d["cards"]
            deck_cards=[DeckCard.__fromdict__(deck_card) for deck_card in d["deck_cards"]]
        )




class DeckCard(PClass):
    uuid = field(type=str)
    is_wild = field(type=bool)
    card_idx = field(type=int)
    def __todict__(self):
        return {
            "uuid": self.uuid,
            "is_wild": self.is_wild,
    ...
    state = State(
        ...
        decks=[
            Deck(
                ...
                cards=[],
                deck_cards=[
                    DeckCard(uuid=str(uuid4()), is_wild=False, card_idx=i) for i in range(gettotaldeckcards(fig))
                ],
            ),
            Deck(
                uuid=str(uuid4()),
                cards=[],
                deck_cards=[
                    DeckCard(uuid=str(uuid4()), is_wild=False, card_idx=i) for i in range(getnumroutecards(fig))
                ],
            )
        ],
...
def shuffle_all_decks(game):
    return game.set(decks=[shuffle_deck(game, i) for i in range(len(game.decks))])
...
def shuffle_deck(game, deck_idx):
    ...
    if not deck or not deck.deck_cards:
        return game
    
    rng = game.rng
    shuffled_deck_cards = list(deck.deck_cards)
    rng.shuffle(shuffled_deck_cards)
    
    shuffled_deck = deck.set(deck_cards=shuffled_deck_cards)
