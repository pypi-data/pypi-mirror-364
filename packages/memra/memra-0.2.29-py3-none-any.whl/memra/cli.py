...
def run_ai_job_portal_demo():
    """Run the AI Job Portal demo"""
    ...
    # Extract bundled files
    if not extract_bundled_files(demo_dir):
        return False
    ...
    # Run the AI Job Portal workflow
    if not run_ai_job_portal_workflow(demo_dir):
        return False
    
    return True

def run_ai_job_portal_workflow(demo_dir):
    """Run the AI Job Portal workflow using Memra composition language"""
    try:
        # Import the AI Job Portal workflow module
        import memra.demos.ai_job_portal.ai_job_portal_demo as ai_job_module
        ...
        try:
            if hasattr(ai_job_module, 'main'):
                ai_job_module.main()
                return True
            else:
                print("❌ AI Job Portal module does not have a main function")
                return False
        finally:
            os.chdir(original_cwd)
            
    except Exception as e:
        print(f"❌ Error running AI Job Portal workflow: {e}")
        return False
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  memra ai_job_portal - Run the AI Job Portal demo (natural language to agents)")
    ...
    if command == "demo":
        ...
        run_kforce_demo()
    elif command == "ai_job_portal":
        run_ai_job_portal_demo()
        ...
        print("  ai_job_portal     - Run the AI Job Portal demo (natural language to agents)")
