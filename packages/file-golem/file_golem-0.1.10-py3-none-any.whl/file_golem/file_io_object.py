...
class FileGolem:
    ...
    def fetch_config_field(self,config_or_config_name, subconfig_keys = [],is_required=True,default_value=None):
        subconfig = self.fetch_subconfig(config_or_config_name,subconfig_keys[:-1])
        if not subconfig:
            if is_required:
                raise Exception(f'No subconfig found for keys {subconfig_keys} in config {config_or_config_name}')
            else:
        ...
        if field is None:
            if is_required:
                raise Exception(f'No field found for keys {subconfig_keys} in config {config_or_config_name}')
            else:
                return default_value
        return field
    

    def fetch_class_from_config(self,config_or_config_name,subconfig_keys,is_required=True):
        model_class = self.fetch_config_field(config_or_config_name, subconfig_keys=subconfig_keys,is_required=is_required)
