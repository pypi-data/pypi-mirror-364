...
class PostHogClient:
    ...
    def _direct_capture(self, event: ProductTelemetryEvent) -> None:
        try:
            if os.environ.get('TRUST3_CLIENT_DEPLOYMENT', 'dev') != "test":
                posthog.capture(
                    event.user_id,
                    event.name,
                    event.properties
...
def capture_setup_event(frameworks):
    ...
    trust3_common_version = get_module_version('trust3-common')
    if trust3_common_version:
        event_data['trust3-common_version'] = trust3_common_version
