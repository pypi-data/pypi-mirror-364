...
from idepy_next.dom import ManipulationMode
...
class DOM:
    ...
    def create_element(self, html: str, parent: Union[Element, str]=None, mode=ManipulationMode.LastChild) -> Element:
        ...
        node_id = self.__window.evaluate_js(f"""
            ...
            idepy._insertNode(newElement, element, '{mode.value}')
            idepy._getNodeId(newElement);
        """)
    ...
    def get_elements(self, selector: str) -> List[Element]:
        code = f"""
            ...
            for (var i = 0; i < elements.length; i++) {{
                var nodeId = idepy._getNodeId(elements[i]);
                nodeIds.push(nodeId);
            }}
