...
class SpringBootAnalyzer(BaseAnalyzer):
    ...
    def _analyze_controller(self, file_path: Path) -> List[ApiEndpoint]:
        ...
        # Find all method mappings - comprehensive patterns
        method_patterns = [
            # Mappings with value= or path= attributes
            ...
            # Mappings with direct string (no attribute name)
            (r'@GetMapping\s*\(\s*["\']([^"\']*)["\'].*?\)[\s\S]*?public\s+[\w<>.,\s]+\s+(\w+)\s*\(', 'GET'),
            (r'@PostMapping\s*\(\s*["\']([^"\']*)["\'].*?\)[\s\S]*?public\s+[\w<>.,\s]+\s+(\w+)\s*\(', 'POST'),
            (r'@PutMapping\s*\(\s*["\']([^"\']*)["\'].*?\)[\s\S]*?public\s+[\w<>.,\s]+\s+(\w+)\s*\(', 'PUT'),
            (r'@DeleteMapping\s*\(\s*["\']([^"\']*)["\'].*?\)[\s\S]*?public\s+[\w<>.,\s]+\s+(\w+)\s*\(', 'DELETE'),
            (r'@PatchMapping\s*\(\s*["\']([^"\']*)["\'].*?\)[\s\S]*?public\s+[\w<>.,\s]+\s+(\w+)\s*\(', 'PATCH'),
            # Mappings without parentheses (empty path)
            (r'@GetMapping\s*[\s\S]*?public\s+[\w<>.,\s]+\s+(\w+)\s*\(', 'GET'),
            (r'@PostMapping\s*[\s\S]*?public\s+[\w<>.,\s]+\s+(\w+)\s*\(', 'POST'),
            (r'@PutMapping\s*[\s\S]*?public\s+[\w<>.,\s]+\s+(\w+)\s*\(', 'PUT'),
            (r'@DeleteMapping\s*[\s\S]*?public\s+[\w<>.,\s]+\s+(\w+)\s*\(', 'DELETE'),
            (r'@PatchMapping\s*[\s\S]*?public\s+[\w<>.,\s]+\s+(\w+)\s*\(', 'PATCH'),
        ]
        
        for pattern, http_method in method_patterns:
            matches = re.finditer(pattern, content, re.MULTILINE | re.DOTALL)
            for match in matches:
                # Check number of groups to determine pattern type
                if len(match.groups()) == 2:
                    # Pattern with path: @GetMapping("/path") or @GetMapping(path="/path")
                    path = match.group(1)
                    method_name = match.group(2)
                elif len(match.groups()) == 1:
                    # Pattern without path: @GetMapping (no parentheses or empty parentheses)
                    path = ""
    ...
    def _extract_parameters(self, content: str, match: re.Match, endpoint: ApiEndpoint):
        ...
        # Look for @RequestBody with various validation patterns - Enhanced for better DTO analysis
        request_body_patterns = [
            # @RequestBody final Type name
            r'@RequestBody\s+final\s+(\w+(?:\.\w+)*)\s+(\w+)',
            # @RequestBody @Valid Type name
            r'@RequestBody\s+@Valid\s+(\w+(?:\.\w+)*)\s+(\w+)',
            # @RequestBody @Validated Type name  
            r'@RequestBody\s+@Validated\s+(\w+(?:\.\w+)*)\s+(\w+)',
            # @Valid @RequestBody Type name
            r'@Valid\s+@RequestBody\s*(?:\([^)]*\))?\s+(\w+(?:\.\w+)*)\s+(\w+)',
            # @Validated @RequestBody Type name
            r'@Validated\s+@RequestBody\s*(?:\([^)]*\))?\s+(\w+(?:\.\w+)*)\s+(\w+)',
            # @RequestBody Type name (no validation)
            r'@RequestBody\s*(?:\([^)]*\))?\s+(\w+(?:\.\w+)*)\s+(\w+)'
        ]
        ...
        for match in request_body_matches:
            ...
            endpoint.request_body = {
                "type": param_type_raw,
                "example": dto_example or self._generate_example_for_type(param_type_raw)
            }
    ...
    def _parse_dto_class(self, class_name: str) -> Optional[dict]:
        ...
        try:
            # Common DTO search patterns in Spring Boot projects
            search_patterns = [
                f"**/{class_name}.java",
                f"**/dto/**/{class_name}.java",
                f"**/dto/{class_name}.java",
                f"**/model/**/{class_name}.java",
                f"**/domain/**/{class_name}.java",
            ...
            if not dto_file_path:
                console.print(f"[yellow]DTO class {class_name} not found in codebase[/yellow]")
                return None
            ...
            return self._parse_java_class_fields(dto_content, class_name)
    ...
    def _parse_java_class_fields(self, class_content: str, class_name: str) -> dict:
        ...
        # Extract field declarations - comprehensive patterns
        field_patterns = [
            ...
            r'@(?:NotNull|NotEmpty|Valid|NotBlank)[^\n]*\n[^\n]*private\s+(\w+(?:<[^>]+>)?)\s+(\w+)\s*;'
        ]
        ...
        for pattern in field_patterns:
            ...
            for match in matches:
                ...
                if '@JsonIgnore' in field_context:
                    continue
    ...
    def _generate_field_example(self, field_type: str, field_name: str, context: str) -> any:
        ...
        if field_type in ['String', 'string']:
            if 'email' in field_name_lower:
                ...
                return "John Doe" if 'account' in field_name_lower else "Example Name" 
                ...
                return "CODE123"
            elif 'bicfi' in field_name_lower:
                return "ABNANL2A"
