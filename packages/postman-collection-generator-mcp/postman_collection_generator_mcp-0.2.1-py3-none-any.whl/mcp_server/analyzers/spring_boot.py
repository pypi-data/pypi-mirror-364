...
class SpringBootAnalyzer(BaseAnalyzer):
    ...
    def _analyze_controller(self, file_path: Path) -> List[ApiEndpoint]:
        ...
        method_patterns = [
            ...
            (r'@GetMapping\s*(?:\(\s*(?:value\s*=\s*|path\s*=\s*)?["\']([^"\']*)["\'][^)]*\)|(?!\s*\())[\s\S]*?public\s+[\w<>.,\s]+\s+(\w+)\s*\(', 'GET'),
            (r'@PostMapping\s*(?:\(\s*(?:value\s*=\s*|path\s*=\s*)?["\']([^"\']*)["\'][^)]*\)|(?!\s*\())[\s\S]*?public\s+[\w<>.,\s]+\s+(\w+)\s*\(', 'POST'),
            (r'@PutMapping\s*(?:\(\s*(?:value\s*=\s*|path\s*=\s*)?["\']([^"\']*)["\'][^)]*\)|(?!\s*\())[\s\S]*?public\s+[\w<>.,\s]+\s+(\w+)\s*\(', 'PUT'),
            (r'@DeleteMapping\s*(?:\(\s*(?:value\s*=\s*|path\s*=\s*)?["\']([^"\']*)["\'][^)]*\)|(?!\s*\())[\s\S]*?public\s+[\w<>.,\s]+\s+(\w+)\s*\(', 'DELETE'),
            (r'@PatchMapping\s*(?:\(\s*(?:value\s*=\s*|path\s*=\s*)?["\']([^"\']*)["\'][^)]*\)|(?!\s*\()))[\s\S]*?public\s+[\w<>.,\s]+\s+(\w+)\s*\(', 'PATCH'),
        ]
