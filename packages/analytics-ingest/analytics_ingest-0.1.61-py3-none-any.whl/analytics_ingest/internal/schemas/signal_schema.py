...
from analytics_ingest.internal.utils.time_utils import parse_time_val
...
class SignalSchema(BaseModel):
    ...
    def parseTime(self, entry):
        ...
        if isinstance(ts, (int, float)):
            return parse_time_val(ts)[:-3] + "Z"  # Trim microseconds for clean format
        elif isinstance(ts, str):
            try:
                dt = datetime.fromisoformat(ts.replace("Z", ""))
                return dt.strftime("%Y-%m-%dT%H:%M:%SZ")
            except Exception:
                raise ValueError(f"Invalid ISO timestamp string: {ts}")
        else:
