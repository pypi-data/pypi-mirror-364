...
def read_directory(
    ...
    *,
    ...
    files= make_list(root_path)
    prefix_len = len(root_path.rstrip(os.sep)) + 1
    if os.path.isdir(root_path):
        prefix_len = len(root_path.rstrip(os.sep)) + 1
        root_paths = glob.glob(os.path.join(f"{root_path}","**"),recursive=True)
        directories = [item for item in root_paths if os.path.isdir(item)]
        files = [item for item in root_paths if os.path.isfile(item)]
