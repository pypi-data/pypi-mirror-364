...
class FileDropArea(QtWidgets.QWidget):
    ...
    def __init__(self, log_widget: QtWidgets.QTextEdit,view_widget=None, parent=None):
        ...
        self.unallowed_extentions = DEFAULT_UNALLOWED_EXTS
    ...
    def set_filter_config(
        ...
        *,
        ...
        unallowed_exts:      Optional[List[str]] = None,
        exclude_dirs:        Optional[List[str]] = None,
        exclude_file_patterns: Optional[List[str]] = None,
    ) -> None:
        """Replace any of the four filter‐sets with exactly what the user passed in."""
        if allowed_exts is not None:
            self.allowed_extensions   = {e.lower() for e in allowed_exts}
        if unallowed_exts is not None:
            self.unallowed_extentions = {e.lower() for e in unallowed_exts}
        if exclude_dirs is not None:
            self.exclude_dirs         = set(d.lower() for d in exclude_dirs)
        if exclude_file_patterns is not None:
            self.exclude_file_patterns = set(p.lower() for p in exclude_file_patterns)
    # ────────────────────────────────────────────────────────────────────────
