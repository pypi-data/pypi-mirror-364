...
class FileDropArea(QtWidgets.QWidget):
    ...
    def dropEvent(self, event: QtGui.QDropEvent):
        try:
            ...
            item_paths = [url.toLocalFile() for url in make_list(urls)]
            dirs = [item_path for item_path in item_paths if os.path.isdir(item_path)]
            paths = [item_path for item_path in item_paths if os.path.isfile(item_path)]
            for directory in dirs:
                directory = eatOuter(directory,['/'])
                items = glob.glob(os.path.join(f"{directory}/*",'**'),recursive=True)
                paths+=[item_path for item_path in items if os.path.isfile(item_path) and item_path not in paths]
