...
from xync_client.Bybit.etype.order import StatusChange, CountDown, SellerCancelChange, Read, Receive
                ...
                if data := json.loads(resp):
                    match data.get("topic"):
                        case "OTC_ORDER_STATUS":
                            match data["type"]:
                                case "STATUS_CHANGE":
                                    StatusChange.model_validate(data["data"])
                                case "COUNT_DOWN":
                                    CountDown.model_validate(data["data"])
                                case _:
                                    self.listen(data)
                        case "OTC_USER_CHAT_MSG":
                            match data["type"]:
                                case "RECEIVE":
                                    Receive.model_validate(data["data"])
                                case "READ":
                                    Read.model_validate(data["data"])
                                case "CLEAR":
                                    pass
                                case _:
                                    self.listen(data)
                        ...
                        case "SELLER_CANCEL_CHANGE":
                            SellerCancelChange.model_validate(data["data"])
                        case None:
