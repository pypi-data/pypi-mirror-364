...
async def main():
    ...
    async with FileClient(TOKEN) as b:
        ...
        await gather(
            # cl.get_api_orders(11),  # 10, 1738357200000, 1742504399999
            cl.battle(usdt, rub, False, ["volet"], 80.44),  # гонка в стакане покупки - мы продаем
            cl.battle(usdt, rub, True, ["volet"], 80.9),  # гонка в стакане продажи - мы покупаем
            cl.battle(usdt, rub, False, ["payeer"], 81.2),  # гонка в стакане покупки - мы продаем
            cl.battle(usdt, rub, True, ["payeer"], 80.2),  # гонка в стакане продажи - мы покупаем
