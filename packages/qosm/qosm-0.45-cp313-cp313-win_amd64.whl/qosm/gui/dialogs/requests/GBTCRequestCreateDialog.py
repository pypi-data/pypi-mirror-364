...
from qosm.gui.managers.GBTCSiimulationManager import GBTCSimulationManager
            ...
            if isinstance(data['calibration'], str):
                ...
                if calib_method == 'trl' and 'trl' in data['calibration']:
                    ...
                    if 'line_offset' in trl_params:
                        self.form['trl_line_offset'].setValue(trl_params['line_offset'])
...
def get_gbtc_request_parameters(form):
    ...
    if calibration_method == 'trl':
        calibration = {
            'method': 'trl',
            'trl': {
                'line_offset': form['trl_line_offset'].value(),
                'type_reflector': 'cc'
            }
        }
    else:
...
def setup_gbtc_request_parameters(layout) -> dict:
    ...
    # Set locale for frequency spinboxes
    form['freq_start'].setLocale(QLocale.c())
    form['freq_stop'].setLocale(QLocale.c())
    form['trl_line_offset'].setLocale(QLocale.c())
