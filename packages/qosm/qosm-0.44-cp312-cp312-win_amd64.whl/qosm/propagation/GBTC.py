...
def create_mut(config, frequency_GHz):
    ...
    # First interface
    s_first = Surface(id=surface_id_sample, centre=Vec3(0, 0, current_z_sample), normal=Vec3(0, 0, -1),
    ...
    for i, layer in enumerate(config['mut']):
        ...
        z_normal = -1
        s = Surface(id=surface_id_sample, centre=Vec3(0, 0, current_z_sample), normal=Vec3(0, 0, z_normal),
                    ior1=ior_list[i], ior2=next_ior, curvature=nan, max_radius=1.0,
                    allow_reflection=True, allow_refraction=True)
...
def simulate(config, frequency_GHz):
    ...
    s11_refl, _, _, _ = gbtc_compute_coupling(beams=beams_s11_refl, w0_tx=port1['w0'], w0_rx=port1['w0'],
                                              r_rx=pos1, u_rx=u1, max_radius=0.1, id_beam=0)

    # Coupling s22_reflect, id_beam=0
    s22_refl, _, _, _ = gbtc_compute_coupling(beams=beams_s22_refl, w0_tx=port2['w0'], w0_rx=port2['w0'],
                                              r_rx=pos2, u_rx=u2, max_radius=0.1, id_beam=0)

    # Coupling s21_thru, id_beam=0
    s21_thru, _, _, _ = gbtc_compute_coupling(beams=beams_s21_thru, w0_tx=port1['w0'], w0_rx=port2['w0'],
                                              r_rx=pos2, u_rx=u2, max_radius=0.1, id_beam=0)

    # Coupling s11, s21 with id_beam=0
    s11_raw, _, _, _ = gbtc_compute_coupling(beams=beams_system, w0_tx=port1['w0'], w0_rx=port1['w0'],
                                             r_rx=pos1, u_rx=u1, max_radius=0.1, id_beam=0)

    s21_raw, _, _, _ = gbtc_compute_coupling(beams=beams_system, w0_tx=port1['w0'], w0_rx=port2['w0'],
                                             r_rx=pos2, u_rx=u2, max_radius=0.1, id_beam=0)

    # Coupling s22, s12 with id_beam=1
    s22_raw, _, _, _ = gbtc_compute_coupling(beams=beams_system, w0_tx=port2['w0'], w0_rx=port2['w0'],
                                             r_rx=pos2, u_rx=u2, max_radius=0.1, id_beam=1)

    s12_raw, _, _, _ = gbtc_compute_coupling(beams=beams_system, w0_tx=port2['w0'], w0_rx=port1['w0'],
                                             r_rx=pos1, u_rx=u1, max_radius=0.1, id_beam=1)
    ...
    if calibration['method'] == 'trl' and  not np.isnan(s21_thru) and not np.isnan(s11_refl):
        ...
        pos2_line = pos2 - u2 * line_offset
        s21_line, _, _, _ = gbtc_compute_coupling(beams=beams_s21_line, w0_tx=port1['w0'], w0_rx=port2['w0'],
                                                  r_rx=pos2_line, u_rx=u2, max_radius=0.1, id_beam=0)
