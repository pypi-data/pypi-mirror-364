...
class AuthClient:
    ...
    def force_get_qq(self, method: str, content: str | int) -> dict:
        ...
        with Get(endpoint=f"/user/bind/qq/admin/{method}/{content}", base_url=self._base_url) as api:
            result = api.get(json_data=payload)
            logger.trace(f"获取qq绑定信息；result:{result}")
            return result
    ...
    def generate_token(self, app_name: str, expire_minutes: int = None) -> dict:
        ...
        with Post(endpoint=f"/user/application/token", base_url=self._base_url) as api:
            result = api.post(json_data=payload)
            logger.trace(f"获取qq绑定信息；result:{result}")
            return result
    ...
    def get_token_list(self) -> dict:
        ...
        with Get(endpoint=f"/user/application/token", base_url=self._base_url) as api:
            result = api.get(json_data=payload)
    ...
    def delete_token(self, token_id: int) -> dict:
        ...
        with Delete(endpoint=f"/user/application/token/{id}", base_url=self._base_url) as api:
            result = api.delete(json_data=payload)
