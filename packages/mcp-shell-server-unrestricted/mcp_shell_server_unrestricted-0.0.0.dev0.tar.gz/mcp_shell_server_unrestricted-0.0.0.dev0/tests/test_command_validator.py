...
def test_get_allowed_commands(validator, monkeypatch):
    ...
    # Now always returns empty list since no restrictions are enforced
    assert validator.get_allowed_commands() == []


def test_is_command_allowed(validator, monkeypatch):
    clear_env(monkeypatch)
    monkeypatch.setenv("ALLOW_COMMANDS", "allowed_cmd")
    # Now all commands are allowed
    assert validator.is_command_allowed("allowed_cmd")
    assert validator.is_command_allowed("disallowed_cmd")
...
def test_validate_pipeline(validator, monkeypatch):
    ...
    # Valid pipeline - all commands are now allowed
    ...
    validator.validate_pipeline(["any_cmd", "|", "any_other_cmd", "test"])
...
def test_validate_command(validator, monkeypatch):
    ...
    # All commands are now allowed
    validator.validate_command(["any_cmd", "-arg"])  # Should not raise
    validator.validate_command(["rm", "-rf"])  # Should not raise
    validator.validate_command(["sudo", "reboot"])  # Should not raise
