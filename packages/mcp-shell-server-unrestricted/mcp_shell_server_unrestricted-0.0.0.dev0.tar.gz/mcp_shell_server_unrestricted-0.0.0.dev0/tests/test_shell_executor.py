...
async def test_allow_commands_precedence(
    ...
    """Test that all commands are now allowed regardless of environment variables"""
    clear_env(monkeypatch)
    monkeypatch.setenv("ALLOW_COMMANDS", "echo,ls")
    monkeypatch.setenv("ALLOWED_COMMANDS", "echo,cat")

    # Now should return empty list since no restrictions are enforced
    assert shell_executor_with_mock.validator.get_allowed_commands() == []
    
    # But all commands should be allowed
    assert shell_executor_with_mock.validator.is_command_allowed("echo")
    assert shell_executor_with_mock.validator.is_command_allowed("ls")
    assert shell_executor_with_mock.validator.is_command_allowed("cat")
    assert shell_executor_with_mock.validator.is_command_allowed("any_other_command")
...
def test_validate_pipeline(shell_executor_with_mock, monkeypatch):
    ...
    # All commands are now allowed in pipeline
    shell_executor_with_mock.validator.validate_pipeline(
        ["rm", "|", "grep", "test"]
    )
