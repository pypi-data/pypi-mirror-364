...
async def test_call_tool_invalid_command(monkeypatch, temp_test_dir):
    """Test execution of a command that will fail at runtime (not due to restrictions)"""
    # Since all commands are now allowed, we test an actual invalid command
    result = await call_tool(
        "shell_execute",
        {"command": ["definitely_not_a_real_command_12345"], "directory": temp_test_dir},
    )
    # This should fail at execution time, not validation time
...
async def test_disallowed_command(monkeypatch):
    """Test that all commands are now allowed - this test verifies unrestricted execution"""
    # All commands are now allowed, so this should work (though it may fail at execution)
    try:
        result = await call_tool(
            "shell_execute",
            {
                "command": ["echo", "hello"],  # Use a safe command for testing
                "directory": "/tmp",
            },
        )
        # Should succeed since all commands are allowed
        assert result["status"] == 0
    except Exception as e:
        # If there's an error, it should not be about command restrictions
        assert "Command not allowed" not in str(e)
