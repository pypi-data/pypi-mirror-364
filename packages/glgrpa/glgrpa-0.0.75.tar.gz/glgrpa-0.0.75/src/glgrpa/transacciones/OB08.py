...
class OB08(SAP):
    ...
    def entradas_nuevas(self) -> bool:
        ...
        for intento in range(max_intentos):
            ...
            if intento == 0:
                # Estrategia 1: F5 simple
                self.mostrar("📋 Estrategia 1: Presionando F5")
                self.enviar_tecla_ventana('F5')
            elif intento == 1:
                # Estrategia 2: F5 con demora adicional
                self.mostrar("📋 Estrategia 2: F5 con demora extendida")
                self.demora(2)
                self.enviar_tecla_ventana('F5')
            elif intento == 2:
                # Estrategia 3: Doble F5
                self.mostrar("📋 Estrategia 3: Doble F5")
                self.enviar_tecla_ventana('F5')
                self.demora(1)
                self.enviar_tecla_ventana('F5')
            elif intento == 3:
                # Estrategia 4: Enter seguido de F5
                self.mostrar("📋 Estrategia 4: Enter + F5")
                self.enviar_tecla_ventana('ENTER')
                self.demora(1)
                self.enviar_tecla_ventana('F5')
            else:
                # Estrategia 5: Click en elemento + F5
                self.mostrar("📋 Estrategia 5: Click en ventana + F5")
