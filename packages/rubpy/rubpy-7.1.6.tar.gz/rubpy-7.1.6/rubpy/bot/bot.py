...
from rubpy.bot.enums import ButtonTypeEnum, ChatKeypadTypeEnum, UpdateTypeEnum
from rubpy.bot.filters import Filter
from rubpy.bot.models import Keypad, Button, InlineMessage, KeypadRow, Update, Message
        ...
        try:
            ...
            for item in response.get("updates", []):
               ...
               if update:
                    ...
                    if int(last_time) and now - int(last_time) > 5:
                        continue

                    updates.append(update)

            self.next_offset_id = response.get("next_offset_id")
