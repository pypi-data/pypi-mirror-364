...
from raphson_mp.track import FileTrack, Track
...
def SubsonicAlbumID3WithSongs(conn: Connection, album: Album) -> dict[str, Any]:
    ...
    data["song"] = [
        SubsonicChild(conn, FileTrack(conn, path), detailed=True) for path, in conn.execute(query, params)
    ]
    return data
...
async def stream(request: web.Request, conn: Connection, _user: User):
    ...
    assert isinstance(relpath, str)
    track = FileTrack(conn, relpath)
    return await track.get_audio(AudioFormat.WEBM_OPUS_HIGH)
