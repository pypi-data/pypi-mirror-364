...
def generate_zip(queue_io: AsyncQueueIO, user_id: int):
    with db.connect(read_only=True, no_cache=True) as conn:
        queries: list[ExportQuery] = [
            ...
            ExportQuery(
                "shares", "SELECT share_code, create_timestamp FROM share WHERE user = ?", (user_id,), False
            ),
            ExportQuery(
                "shares_tracks", "SELECT share.share_code, track_code, track FROM share_track JOIN share WHERE user = ?", (user_id,), False
            ),
