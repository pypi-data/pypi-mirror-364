...
class Track(TrackBase):
    ...
    async def get_cover_image(self, client: RaphsonMusicClient, quality: ImageQuality, format: ImageFormat, meme: bool) -> bytes:
        async with client.session.get(
    ...
    async def get_lyrics_json(self, client: RaphsonMusicClient) -> Lyrics | None:
        async with client.session.get("/track/" + urlencode(self.path) + "/lyrics") as response:
            return lyrics.from_dict(await response.json())
