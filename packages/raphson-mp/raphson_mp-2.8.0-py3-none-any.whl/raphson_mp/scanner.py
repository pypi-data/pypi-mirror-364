...
from raphson_mp import db, event, ffmpeg, track, settings
...
def _update_db(loop: asyncio.AbstractEventLoop, conn: Connection, user: User | None, path: Path, relpath: str, mtime: int, update: bool) -> bool:
    ...
    if meta is None:
        if update:
            ...
            _log(loop, conn, event.FileChangeEvent(FileAction.DELETE, relpath, user))
        return False
    ...
    if update:
        conn.execute(
            ...
            WHERE path=:path
            """,
            (meta.duration, meta.title, meta.album, meta.album_artist, meta.track_number, meta.year, meta.lyrics, meta.video, mtime, relpath),
        )
    else:
        conn.execute(
            """
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (relpath, playlist, meta.duration, meta.title, meta.album, meta.album_artist, meta.track_number, meta.year, meta.lyrics, meta.video, mtime, ctime),
        )
    ...
    conn.executemany("INSERT INTO track_artist (track, artist) VALUES (?, ?)", [(relpath, artist) for artist in meta.artists])
    conn.executemany("INSERT INTO track_tag (track, tag) VALUES (?, ?)", [(relpath, tag) for tag in meta.tags])

    conn.execute("COMMIT")

    if update:
        _log(loop, conn, event.FileChangeEvent(FileAction.UPDATE, relpath, user))
    else:
        _log(loop, conn, event.FileChangeEvent(FileAction.INSERT, relpath, user))
