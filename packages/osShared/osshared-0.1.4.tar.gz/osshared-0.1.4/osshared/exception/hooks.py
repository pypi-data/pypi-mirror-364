...
from osshared.response.responseBuilder import classResponseBuilder
from osshared.exception.exception import classBaseException
import traceback
import json


def methodExceptionHandler(exc: Exception, context: Dict[str, Any]) -> Response:
    print("🔥 예외 진입 확인:", type(exc), str(exc)) 

    # 커스텀 예외 처리
    if isinstance(exc, classBaseException):
        ...
        return Response(classResponseBuilder.methodBuildExceptionDict(
