...
class Status(BaseModel):
    ...
    """
    A dict of the text of the status to display in different languages.
    """
    replace: bool = False
    """
    ...
    def validate_text(cls, v):
        if isinstance(v, str):
            return {cls.DEFAULT_LANGUAGE: v}
        if isinstance(v, dict) and not v:
            raise ValueError("text must be a string or a dict with at least one language")
        return v

    def get_text(self, language: str) -> str:
        """
        Get the text of the status to display in the given language.
        If the text is not available in the given language, return the text in the default language (German/de).
        If the text is not available in the preferred or default language, return a random text from the available texts.
        """
        text = self.text.get(language, None)
        if text is None:
            text = self.text.get(self.DEFAULT_LANGUAGE, None)

        if text is None:
            text = next(iter(self.text.values()))
        return text
...
StreamableType = Union[str, Source, Status, Attachment, GeneratedImage, Plan, HiddenContext]
...
class Event(BaseModel):
    ...
    type: Optional[
        Literal["chunk", "source", "status", "attachment", "generated_image", "plan", "hidden_context"]
    ] = None
