            ...
            if config_mark:
                if isinstance(config_mark, str):
                    if config_mark.startswith('runtime('):
                        marks.append(config_mark)
                    else:
                        marks.extend([m.strip() for m in config_mark.split(',')])
                elif isinstance(config_mark, list):
                    marks.extend(config_mark)
            ...
            if case_mark:
                if isinstance(case_mark, str):
                    if case_mark.startswith('runtime('):
                        marks.append(case_mark)
                    else:
                        marks.extend([m.strip() for m in case_mark.split(',')])
                elif isinstance(case_mark, list):
                    marks.extend(case_mark)
            ...
            if marks:
                ...
                for mark in marks:
                    if mark.startswith('runtime('):
                        # 处理runtime标记
                        f = pytest.mark.runtime(eval(mark[8:-1]))(f)
                    else:
                        # 处理普通标记
    ...
    def execute_mysql(self):
        ...
        for attr_name in dir(env_obj):
            ...
            if isinstance(attr_value, ConnectMysql):
                try:
                    ...
                    log.debug(f"Using existing database connection for {attr_name}")
                except Exception as msg:
        ...
        if hasattr(env_obj, 'MYSQL_HOST') or hasattr(env_obj, 'DB_INFO'):
            try:
                ...
                log.debug("Added default database functions to context")
            except Exception as msg:
            ...
            log.warning("No valid database configuration found")
    ...
    def execute_redis(self):
        ...
        for attr_name in dir(env_obj):
            ...
            if isinstance(attr_value, ConnectRedis):
                try:
                    ...
                    log.debug(f"Using existing redis connection for {attr_name}")
                except Exception as msg:
        ...
        if hasattr(env_obj, 'REDIS_HOST') or hasattr(env_obj, 'REDIS'):
            try:
                ...
                log.debug("Added default redis functions to context")
            except Exception as msg:
