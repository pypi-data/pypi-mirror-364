...
class AsyncS3Client:
    ...
    async def download_to_pyarrow(self, object_name, bucket_name=None, file_format='parquet'):
        ...
        Args:
            ...
            bucket_name: Имя бакета (если None, используется self.bucket_name)
            file_format: Формат файла ('parquet' или 'csv')

        Returns:
            pyarrow.Table: Данные из файла

        Raises:
            ValueError: Если указан неподдерживаемый формат файла
            ClientError: Если произошла ошибка при доступе к S3 (включая отсутствие ключа)
        """
        async with self.session.create_client('s3', **self.config) as client:
            ...
            try:
                ...
                raise Exception(f"Object '{object_name}' not found in bucket '{_bucket_name}'") from e
            except Exception as e:
                raise Exception(f"Error accessing object '{object_name}' in bucket '{_bucket_name}': {str(e)}") from e
            ...
            async with response['Body'] as stream:
                ...
                try:
                    ...
                    raise Exception(f"Error reading file '{object_name}' as {file_format}: {str(e)}") from e
