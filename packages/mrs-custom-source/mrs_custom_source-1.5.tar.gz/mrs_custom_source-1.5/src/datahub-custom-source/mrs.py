...
@dataclass
class MrsSourceReport(StaleEntityRemovalSourceReport):
...
class MrsConfig(StatefulIngestionConfigBase, EnvConfigMixin, PlatformInstanceConfigMixin):
    # See the Superset /security/login endpoint for details
    # https://superset.apache.org/docs/rest-api
    connect_uri: str = Field(default="http://localhost:8088", description="Superset host URL.")
    ...
    ingest_dashboards: bool = Field(default=True, description="Enable to ingest dashboards.")
    ingest_charts: bool = Field(default=True, description="Enable to ingest charts.")
    ingest_datasets: bool = Field(default=False, description="Enable to ingest datasets.")
    ...
    dataset_data_file_path: str = Field(
        default=None,
        description="Path to the file containing dataset data. Used for testing purposes.",
    )

    # TODO： ================================
    parse_owner_info: str = Field(
        default=None,
        description="Path to the file containing dataset data. Used for testing purposes.",
    )
...
@platform_name("MRS")
@config_class(MrsConfig)
# @support_status(SupportStatus.CERTIFIED)
@capability(SourceCapability.DELETION_DETECTION, "Enabled by default via stateful ingestion")
@capability(SourceCapability.DOMAINS, "Enabled by `domain` config to assign domain_key")
@capability(SourceCapability.LINEAGE_COARSE, "Supported by default")
class MrsSource(StatefulIngestionSourceBase):
    """
    ...
    config: MrsConfig
    report: MrsSourceReport
    platform = "mrs"

    def __hash__(self):
        return id(self)

    def __init__(self, ctx: PipelineContext, config: MrsConfig):
        super().__init__(config, ctx)
        self.config = config
        self.report = MrsSourceReport()
    ...
    #     用户id int：用户邮箱 str
    def parse_owner_info(self) -> Dict[int, Any]:
        with open(self.config.parse_owner_info, "r", encoding="utf8") as f:
            owner_info = json.loads(f.read())
            return {int(owner_id): email for owner_id, email in owner_info.items()}
    ...
    def get_dataset_info(self, dataset_id: int) -> dict:
        file_path = self.config.dataset_info % dataset_id
        if os.path.exists(file_path):
            with (open(file_path, "r", encoding="utf8") as fp):
                return json.load(fp).get("result", {})
        else:
            with open(self.config.dataset_data_file_path, "r", encoding="utf8") as fp:
                dataset_info_all = json.load(fp)
                for dataset_info in dataset_info_all.get("result", []):
                    if dataset_info.get("id") == dataset_id:
                        return dataset_info
        # If not found, return an empty dictionary
        logger.warning(f"Dataset with ID {dataset_id} not found in the response.")
        return {}

    def get_datasource_urn_from_id(self, dataset_response: dict, platform_instance: str) -> str:
        schema_name = dataset_response.get("schema")
        table_name = dataset_response.get("table_name")
        database_id = dataset_response.get("database", {}).get("id")
        database_name = dataset_response.get("database", {}).get("database_name")
        ...
        if table_name and database_id:
            return make_dataset_urn(
                ...
                name=".".join(name for name in [database_name, schema_name, table_name] if name),
                env=self.config.env,
            )

        raise ValueError("Could not construct dataset URN")

    def construct_dashboard_from_api_data(self, dashboard_data: dict) -> DashboardSnapshot:
        ...
        modified_actor = (
            f"urn:li:corpuser:{self.owner_info.get((dashboard_data.get('changed_by') or {}).get('id', -1), 'unknown')}"
        )
        now = datetime.now().strftime("%I:%M%p on %B %d, %Y")
        modified_ts = int(dp.parse(dashboard_data.get("changed_on_utc", now)).timestamp() * 1000)
        ...
        change_audit_stamps = ChangeAuditStamps(created=None, lastModified=last_modified)
        ...
        if dashboard_data.get("certified_by"):
            ...
            custom_properties["CertificationDetails"] = str(dashboard_data.get("certification_details"))
    ...
    def _process_dashboard(self, dashboard_data: Any) -> Iterable[MetadataWorkUnit]:
        ...
        yield from self._get_domain_wu(title=dashboard_title, entity_urn=dashboard_snapshot.urn)

    def emit_dashboard_mces(self) -> Iterable[MetadataWorkUnit]:
        dashboard_data_list = [
            (dashboard_data,) for dashboard_data in self.paginate_entity_api_results("dashboard/", PAGE_SIZE)
        ]
    ...
    def construct_chart_cll(
        ...
        chart_column_data: List[Tuple[str, bool]] = [
            (column, False) if isinstance(column, str) else (column.get("label", ""), True) for column in column_data
        ]
    ...
    def construct_chart_from_chart_data(self, chart_data: dict) -> Iterable[MetadataWorkUnit]:
        ...
        modified_actor = (
            f"urn:li:corpuser:{self.owner_info.get((chart_data.get('changed_by') or {}).get('id', -1), 'unknown')}"
        )
        now = datetime.now().strftime("%I:%M%p on %B %d, %Y")
        modified_ts = int(dp.parse(chart_data.get("changed_on_utc", now)).timestamp() * 1000)
        ...
        filters = [get_filter_name(filter_obj) for filter_obj in params.get("adhoc_filters", [])]
        ...
        input_fields = self.construct_chart_cll(chart_data, datasource_urn, datasource_id)

        if input_fields:
            yield MetadataChangeProposalWrapper(
                entityUrn=chart_urn,
                aspect=InputFields(fields=sorted(input_fields, key=lambda x: x.schemaFieldUrn)),
            ).as_workunit()
    ...
    def _process_chart(self, chart_data: Any) -> Iterable[MetadataWorkUnit]:
        ...
        try:
            ...
            if not self.config.chart_pattern.allowed(chart_name):
                self.report.report_dropped(f"Chart '{chart_name}' (id: {chart_id}) filtered by chart_pattern")
                return
            ...
            if self.config.database_pattern != AllowDenyPattern.allow_all():
                ...
                if datasource_id:
                    if datasource_id in self.filtered_dataset_to_database:
                        ...
                        is_filtered = not self.config.database_pattern.allowed(database_name)
                        ...
                        if database_name:
                            ...
                            if is_filtered:
                                self.filtered_chart_to_database[chart_id] = database_name
                                self.filtered_dataset_to_database[datasource_id] = database_name
            ...
            self.report.warning(
                ...
                context=str(dict(chart_id=chart_id, chart_name=chart_name, error=str(e))),
    ...
    def gen_metric_schema_fields(self, metric_data: List[Dict[str, Any]]) -> List[SchemaField]:
        schema_fields: List[SchemaField] = []
    ...
    def generate_virtual_dataset_lineage(
        ...
        cll = parsed_query_object.column_lineage if parsed_query_object.column_lineage is not None else []
        ...
        for cll_info in cll:
            ...
            upstreams = [
                make_schema_field_urn(column_ref.table, column_ref.column) for column_ref in cll_info.upstreams
            ]
    ...
    def construct_dataset_from_dataset_data(self, dataset_data: dict) -> DatasetSnapshot:
        dataset_response = self.get_dataset_info(dataset_data.get("id"))
        dataset = MrsDataset(**dataset_response)
        datasource_urn = self.get_datasource_urn_from_id(dataset_response, self.platform)
        dataset_url = f"{self.config.display_uri}/explore/?datasource_type=table&datasource_id={dataset.id}"
        modified_actor = (
            f"urn:li:corpuser:{self.owner_info.get((dataset_data.get('changed_by') or {}).get('id', -1), 'unknown')}"
        )
        now = datetime.now().strftime("%I:%M%p on %B %d, %Y")
        modified_ts = int(dp.parse(dataset_data.get("changed_on_utc", now)).timestamp() * 1000)
        last_modified = AuditStampClass(time=modified_ts, actor=modified_actor)
        upstream_warehouse_platform = dataset_response.get("database", {}).get("backend", "mysql")
        upstream_warehouse_db_name = dataset_response.get("database", {}).get("database_name")
        # if we have rendered sql, we always use that and defualt back to regular sql
        sql = dataset_response.get("rendered_sql") or dataset_response.get("sql")
        ...
        upstream_dataset = self.get_datasource_urn_from_id(dataset_response, upstream_warehouse_platform)
        ...
        if sql == "null" or not sql:
            ...
            upstream_lineage = self.generate_virtual_dataset_lineage(parsed_query_object, datasource_urn)
    ...
    def _process_dataset(self, dataset_data: Any) -> Iterable[MetadataWorkUnit]:
        logger.warning(f"Processing dataset data: {dataset_data}")
        dataset_name = ""
        try:
    ...
    def emit_dataset_mces(self) -> Iterable[MetadataWorkUnit]:
        with open(self.config.dataset_data_file_path, "r", encoding="utf8") as fp:
            dataset_info_all = json.load(fp)
        dataset_data_list = [(dataset_data, ) for dataset_data in dataset_info_all.get("result", [])]
        yield from ThreadedIteratorExecutor.process(
    ...
    def _get_domain_wu(self, title: str, entity_urn: str) -> Iterable[MetadataWorkUnit]:
        ...
        for domain, pattern in self.config.domain.items():
            if pattern.allowed(title):
                domain_urn = make_domain_urn(self.domain_registry.get_domain_urn(domain))
                break
