...
class MrsConfig(
    ...
    owner_info_data_file_path: str = Field(description="User ID and user email correspondence")
    dataset_data_file_path: str = Field(description="dataset data file path")
...
class MrsSource(StatefulIngestionSourceBase):
    ...
    def parse_owner_info(self) -> Dict[int, Any]:
        with open(self.config.owner_info_data_file_path, "r", encoding="utf-8") as file:
            return {int(owner_id): email for owner_id, email in json.load(file).items()}
    ...
    def get_dataset_info(self, dataset_data: dict) -> dict:
        return {"result": dataset_data}
    ...
    def construct_dataset_from_dataset_data(
        ...
        dataset_response = self.get_dataset_info(dataset_data)
        dataset = MrsDataset(**dataset_response["result"])

        datasource_urn = self.get_datasource_urn_from_id(
            dataset_response, self.platform
        )
        # todo: explore_url OR format display_uri==============================================
    ...
    def _process_dataset(self, dataset_data: Any) -> Iterable[MetadataWorkUnit]:
        ...
        try:
            ...
            self.report.info(
                f"success. Dataset name: {dataset_data.get('table_name')}. Error: \n{e}"
            )
        except Exception as e:
            self.report.warning(
    ...
    def emit_dataset_mces(self) -> Iterable[MetadataWorkUnit]:
        with open(self.config.dataset_data_file_path, "r", encoding="utf-8") as file:
            dataset_data_list = [(dataset_data,) for dataset_data in json.load(file).get("result", [])]
        yield from ThreadedIteratorExecutor.process(
