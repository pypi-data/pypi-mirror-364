...
[red]🧨 Delete all sessions:[/red] [bold]gemini-wt clean --all[/bold]"""
    ...
    # Launch Gemini
    # shutil.which("gemini")
    gemini_cmd = ["npx", "https://github.com/google-gemini/gemini-cli"]

    console.print(f"[cyan]🚀 Launching Gemini from:[/cyan] {gemini_cmd}")
...
def resume(branch_name: str):
    ...
    try:
        ...
        full_branch_name = f"refs/heads/gemini-wt-{branch_name}"
        result = subprocess.run(
...
def clean(
    ...
    try:
        ...
        if branch_name:
            ...
            full_branch_name = f"gemini-wt-{branch_name}"
            ...
            with console.status("[bold cyan]Cleaning all gemini sessions..."):
                ...
                try:
                    ...
                    for wt in worktrees:
                        ...
                        if branch_name.startswith("refs/heads/gemini-wt-"):
                            try:
                                subprocess.run(
                                    [
                                        "git",
                ...
                try:
                    ...
                    console.print(
                        "  [yellow]No gemini-wt-* branches found[/yellow]")
...
def list():
    ...
    try:
        ...
        gemini_worktrees = [
            wt for wt in worktrees if wt.get("branch", "").startswith("refs/heads/gemini-wt-")
        ]
        ...
        for wt in sorted(gemini_worktrees, key=lambda x: x.get("branch", "")):
            branch_name = wt.get("branch", "")
            suffix = branch_name.replace("refs/heads/gemini-wt-", "")
            wt_path = wt["path"]
...
def version():
    ...
    try:
        ...
        version = importlib.metadata.version("gemini-wt")
        console.print(f"gemini-wt {version}")
    except ImportError:
        console.print("gemini-wt Unavailable")
