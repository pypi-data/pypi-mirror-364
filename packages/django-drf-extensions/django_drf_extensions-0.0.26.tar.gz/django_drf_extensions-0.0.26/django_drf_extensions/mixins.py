...
class OperationsMixin:
    ...
    def _perform_sync_upsert(
        ...
        queryset = self.get_queryset()  # Get queryset once to avoid recursion
        ...
        for index, item_data in enumerate(data_list):
            try:
                # First, check if this is a create or update scenario using raw data
                unique_filter = {}
                missing_fields = []
                ...
                existing_instance = queryset.filter(**unique_filter).first()
                ...
                if existing_instance:
                    ...
                    # Create new record - use upsert serializer to avoid unique validation errors
                    serializer = self._create_upsert_serializer(
                ...
                # Build lookup filter for the validated data
                lookup_filter = {}
                for field in unique_fields:
        ...
        existing_instances = {}
        if combined_filter:
            for instance in queryset.filter(combined_filter):
