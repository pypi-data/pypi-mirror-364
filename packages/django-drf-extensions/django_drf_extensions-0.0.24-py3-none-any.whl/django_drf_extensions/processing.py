...
def create_upsert_serializer(serializer_class, data):
    """
        ...
        for field_name, field in serializer.fields.items():
            if hasattr(field, "validators"):
                ...
                field.validators = [
                    ...
                    or v.__name__ not in ["UniqueValidator", "UniqueTogetherValidator"]
                ]
...
def async_create_task(
    ...
    try:
        ...
        if instances_to_create:
            ...
            model_class = (
                import_string(model_class_path)
                if model_class_path
                else serializer_class.Meta.model
            )
            new_instances = [
    ...
    try:
        ...
        for index, item_data in enumerate(data_list):
            try:
                ...
                serializer = create_upsert_serializer(serializer_class, item_data)
                if serializer.is_valid():
                    ...
                    if unique_filter:
                        all_lookup_filters.append(
                            (index, lookup_filter, validated_data)
                        )
        ...
        OperationCache.set_task_progress(
            task_id, len(data_list), len(data_list), "Checking existing instances..."
        )
