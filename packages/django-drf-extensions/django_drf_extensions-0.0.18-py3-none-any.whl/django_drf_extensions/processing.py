...
def async_upsert_task(
    ...
    try:
        ...
        # Get actual model fields (excluding properties)
        model_fields = {f.name for f in model_class._meta.get_fields()}

        # Validate all items first
        OperationCache.set_task_progress(task_id, 0, len(data_list), "Validating data...")
        
        # Collect all unique filters for bulk lookup
        all_lookup_filters = []
        validated_items = []
            ...
            try:
                ...
                if not serializer.is_valid():
                    ...
                    if non_unique_errors:
                        result.add_error(index, str(non_unique_errors), item_data)
                        continue
                    else:
                        # If only unique field errors, treat as valid
                        validated_data = serializer.initial_data
                else:
                    validated_data = serializer.validated_data
                    
                # Filter out properties from validated data
                validated_data = {
                ...
                for field in unique_fields:
                    if field in validated_data:
                        ...
                        elif hasattr(model_class, field) and hasattr(getattr(model_class, field), 'field'):
        ...
        OperationCache.set_task_progress(task_id, len(data_list), len(data_list), "Checking existing instances...")
        
        # Build a comprehensive filter for all items
        ...
        if combined_filter:
            for instance in model_class.objects.filter(combined_filter):
        ...
        try:
            with transaction.atomic():
                ...
                if to_update:
                    ...
                    for lookup_key, (instance, validated_data) in to_update.items():
                        ...
                        if update_fields:
                            instance_update_fields = [f for f in update_fields if f in validated_data and f in model_fields]
                        else:
                            instance_update_fields = [f for f in validated_data.keys() if f not in unique_fields and f in model_fields]
                    ...
                    for update_fields_group, group_data in update_groups.items():
                        ...
                        for instance, item_data in zip(instances, data):
                            for field in update_fields_group:
                                if field in model_fields:  # Only set actual model fields
            ...
            result.add_error(None, str(e), None)
