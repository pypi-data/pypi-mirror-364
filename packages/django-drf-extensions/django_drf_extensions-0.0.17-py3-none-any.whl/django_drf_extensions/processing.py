    ...
    try:
        ...
        if combined_filter:
            for instance in model_class.objects.filter(combined_filter):
                ...
                instance_key = tuple(getattr(instance, field) for field in unique_fields if hasattr(instance, field))
        ...
        for index, validated_data, lookup_filter in validated_items:
            ...
            item_key = tuple(lookup_filter.get(f"{field}_id", lookup_filter.get(field)) for field in unique_fields)
