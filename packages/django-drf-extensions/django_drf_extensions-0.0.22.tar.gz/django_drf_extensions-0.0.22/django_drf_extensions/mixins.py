...
class OperationsMixin:
    ...
    def bulk_create(self, request):
        ...
        user_id = request.user.id if request.user.is_authenticated else None
        task = async_create_task.delay(serializer_class_path, data_list, user_id, model_class_path)
    ...
    def bulk_update(self, request):
        ...
        user_id = request.user.id if request.user.is_authenticated else None
        task = async_update_task.delay(serializer_class_path, data_list, user_id, model_class_path)
    ...
    def bulk_replace(self, request):
        ...
        user_id = request.user.id if request.user.is_authenticated else None
        task = async_replace_task.delay(serializer_class_path, data_list, user_id, model_class_path)
    ...
    def _bulk_upsert(self, request, data_list, unique_fields_param):
        ...
        task = async_upsert_task.delay(
            serializer_class_path, data_list, unique_fields, update_fields, user_id, model_class_path
        )
