...
class OrchestraAdapter:
            ...
            for argname, argtype in hints.items():
                if argname == "return":
                    ...
                    if argtype != inspect._empty and not issubclass(
                        ...
                        raise ValueError(
                            f"Function '{fn.__name__}': Argument '{argname}' must be a AvAspect but is {argtype}"
                        )
    ...
    def invoke_callback(self, args: av.InvokeArgs) -> av.AvValue:
        for route in self._routes.values():
            ...
            try:
                assert self._hns is not None, "should be initialized by the time we adapt"
