...
def test_all_models():
    ...
    challenge.fill_problem_with_fair_odds() # we do this since GJO dataset has no built-in market odds
    ...
    irt_result = irt_model.fit(challenge.forecast_problems, method="SVI", config=svi_config, include_scores=False)

    # testing market earning
    market_earning = MarketEarning()
    market_earning_result = market_earning.fit(challenge.forecast_problems, include_scores=False)
    ...
    problem_discriminations = [problem_discrimination_dict[problem.problem_id] for problem in challenge.forecast_problems]
    ...
    all_results = [brier_result, irt_result, market_earning_result, weighted_brier_result, bt_result]
    all_results_names = ["brier_result", "irt_result", "market_earning_result", "weighted_brier_result", "bt_result"]
    
    # compute all pairwise correlations
    for i in range(len(all_results)):
        for j in range(i + 1, len(all_results)):
            print(f"Spearman correlation between {all_results_names[i]} and {all_results_names[j]}: {spearman_correlation(all_results[i], all_results[j])}")
            print(f"Kendall correlation between {all_results_names[i]} and {all_results_names[j]}: {kendall_correlation(all_results[i], all_results[j])}")
            print("-" * 100)
