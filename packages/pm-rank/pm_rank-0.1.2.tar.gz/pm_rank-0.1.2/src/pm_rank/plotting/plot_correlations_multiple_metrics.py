...
def _get_all_rankings():
    ...
    for problem in problems:
        problem.odds = [1 / len(problem.options)] * len(problem.options) # type: ignore

    # get the IRT rankings
    device = "cpu" # change to "cuda" if you have a GPU
    svi_config = SVIConfig(optimizer="Adam", num_steps=5000, learning_rate=0.005, device=device)
    irt_model = IRTModel()
    irt_rankings = irt_model.fit(problems, method="SVI", config=svi_config, include_scores=False)
    ...
    market_earning_rankings = MarketEarning().fit(problems, include_scores=False)

    # get the Bradley-Terry rankings
    bt_rankings = GeneralizedBT(method="MM", num_iter=500).fit(problems, include_scores=False)

    # get the weighted Brier rankings
    problem_discrim_dict, _ = irt_model.get_problem_level_parameters()
    problem_discriminations = np.array([problem_discrim_dict[problem.problem_id] for problem in problems])
    weighted_brier_rankings = BrierScoringRule().fit(problems, problem_discriminations=problem_discriminations, include_scores=False)

    return {
        "IRT": irt_rankings,
        "Brier": brier_rankings,
        "Market Earning": market_earning_rankings,
...
def plot_correlation_grid(rankings_dict, output_file="correlation_grid.png"):
    ...
    for i in range(N):
        for j in range(N):
            if i == j:
                continue
            spearman_vals[i, j] = spearman_correlation(rankings_dict[methods[i]], rankings_dict[methods[j]])
            kendall_vals[i, j] = kendall_correlation(rankings_dict[methods[i]], rankings_dict[methods[j]])
    ...
    for i in range(N):
        for j in range(N):
            if i == j:
                ...
                ax.text(j, i, methods[i], ha='center', va='center', fontsize=14, fontweight='bold', color='black')
            elif i > j:
                # Lower triangle: Spearman
                val = spearman_vals[i, j]
                color = cmap_spearman(0.5 + 0.5*val)  # val in [-1,1]
                ax.add_patch(Rectangle((j-0.5, i-0.5), 1, 1, color=color, alpha=0.7, zorder=0))
                ax.text(j, i, f"{val:.2f}\nS", ha='center', va='center', fontsize=13, color='black', fontweight='semibold')
            elif i < j:
                # Upper triangle: Kendall
                val = kendall_vals[i, j]
                color = cmap_kendall(0.5 + 0.5*val)
                ax.add_patch(Rectangle((j-0.5, i-0.5), 1, 1, color=color, alpha=0.7, zorder=0))
                ax.text(j, i, f"{val:.2f}\nK", ha='center', va='center', fontsize=13, color='black', fontweight='semibold')
    ...
    # Title and layout
    fig.suptitle('Correlation Grid between Ranking Methods', fontsize=20, fontweight='bold', y=1.05)
    plt.tight_layout()
