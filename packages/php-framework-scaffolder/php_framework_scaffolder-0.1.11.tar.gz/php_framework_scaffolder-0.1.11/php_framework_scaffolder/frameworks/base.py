...
class BaseFrameworkSetup:
    ...
    def setup(self, repository_path: Path, apk_packages: List[str] = [], php_extensions: List[str] = [], pecl_extensions: List[str] = []) -> None:
        composer_data = read_composer_json(os.path.join(self.target_folder, "src", "composer.json"))
        ...
        pecl_extensions = list(set(pecl_extensions + [
            "rdkafka",
            "redis",
        ]))
        pecl_extensions.sort()
