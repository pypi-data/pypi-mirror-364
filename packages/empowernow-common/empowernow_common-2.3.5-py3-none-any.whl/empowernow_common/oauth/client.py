...
class PrivateKeyJWTConfig:
    ...
    def to_jwt(self, client_id: str, token_url: str) -> str:
        ...
        return jose_jwt.encode(payload, self.signing_key, algorithm=self.signing_alg)
...
class HardenedOAuth:
    ...
    async def secure_authorization_flow_with_jar_and_jarm(
        ...
        **kwargs,
        ...
        authorization_url = f"{self.config.authorization_url}?client_id={self.config.client_id}&request={request_object}"
        ...
        async def _request_token() -> HardenedToken:
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    headers = self._get_security_headers()
                    headers = self._dpop_manager.add_dpop_header(
                        headers, "POST", self.config.token_url
                    )
    ...
    def configure_private_key_jwt(
        self, signing_key, signing_alg: str = "RS256", assertion_ttl: int = 300
    ):
    ...
    async def introspect_token(self, token: str) -> Dict[str, Any]:
        ...
        response = await client.post("", data=data, headers=headers, auth=auth)
        response.raise_for_status()
