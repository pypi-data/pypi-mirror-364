...
class _Saver:
    ...
    def save(
        ...
        **kwargs: ValueOrLambda,
        ...
        # If filters are specified, apply them BEFORE creating Activation objects
        # to avoid expensive tensor operations on filtered-out activations
        if self._filters is not None:
            filtered_kwargs = {}
            for name, value in kwargs.items():
                # Build the full activation name including prefixes for filtering
                full_name = ".".join(self._prefixes_fn() + [name.replace("/", ".")])

                # Only include activations that match at least one filter
                if any(fnmatch.fnmatch(full_name, f) for f in self._filters):
                    filtered_kwargs[name] = value
            kwargs = filtered_kwargs

        # Build activations only for the filtered subset
        activations = Activation.from_dict(kwargs)

        for activation in activations:
            # Save the current activation (no need to re-check filters)
