 ...
 "basic_events": [ // 细粒度的基本事件列表,每个事件都是一个原子信息单元
  {
   ...
   "sentence": "string", // 抽取出的精炼句子,要求保存信息量,减少非必要的形容词。移除所有特殊字符（如Markdown语法、图片链接等）,只保留文本和基本标点。 注意与entities的概念统一, 处理好aliases, 同时代词要替换为明确的词
...
1. **文章标题提取**:直接从文章顶部提取标题。**严格确保不含特殊字符。如有特殊字符, 更换表述方式**
...
2. **`sections`构建**:
  ...
  *  **分级标题处理**:
    ...
    *  `title` 字段填入原始标题文本。**严格确保不含特殊字符。如有特殊字符, 更换表述方式**
...
from text2kgh_prompts.concat import concats
from text2kgh_prompts.concat_postprocess import concat_postproces
...
def main():
    ...
    with open("demo/result1.txt", "w") as f:
        f.write(extract_python_code(result))
    print(22)
    with open("demo/demo2.txt", "r") as f:
        text = f.read()
    result = ""
    for result_i in bx.product_stream(prompt + "\n" + text):
        result += result_i
    with open("demo/result2.txt", "w") as f:
        f.write(extract_python_code(result))
    print(23)
    concats()
    concat_postproces()
