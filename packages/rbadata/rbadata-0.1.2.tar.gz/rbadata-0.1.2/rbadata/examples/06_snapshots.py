...
def main():
    ...
    for _, row in indicators.iterrows():
        print(
            f"{row['indicator']:<20} {row['value']:<10} {row['unit']:<15} {row.get('previous', 'N/A'):<10}"
        )
    ...
    if "previous" in indicators.columns:
        ...
        indicators_with_change["abs_change_pct"] = indicators_with_change[
            "change_pct"
        ].abs()
    ...
    major_sectors = composition[composition["share"] > 5].sort_values(
    ...
    for _, row in payments.iterrows():
        print(
            f"{row['method']:<15} {row['share']:<14}% {row['transactions']:<19}% {row['year']}"
        )
    ...
    if "share" in payments.columns:
        ...
        print(
            f"Least used payment method: {least_used['method']} ({least_used['share']}% by value)"
        )
    ...
    if not dashboard_data["payments"].empty:
        ...
        print(
            f"Most used payment: {popular_payment['method']} ({popular_payment['share']}% by value)"
        )
