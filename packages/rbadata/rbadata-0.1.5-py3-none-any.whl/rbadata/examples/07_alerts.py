...
def main():
    ...
    alert_id1 = alerts.add_alert(table_no="G1", name="CPI Data Release")
    print(f"Created alert for CPI data: {alert_id1}")

    # Alert for unemployment data
    alert_id2 = alerts.add_alert(series_id="GLFSURSA", name="Unemployment Rate Update")
    print(f"Created alert for unemployment rate: {alert_id2}")

    # Alert for Chart Pack releases
    alert_id3 = alerts.add_alert(
        release_type="chart-pack", name="New Chart Pack Available"
    )
    ...
    quick_alert = rbadata.create_alert(table_no="F1", name="Interest Rates Update")
    ...
    if not all_alerts.empty:
        ...
        for _, alert in all_alerts.iterrows():
            alert_type = (
                "Table"
                if alert.get("table_no")
                else "Series" if alert.get("series_id") else "Release"
            )
            created_date = pd.to_datetime(alert["created"]).strftime("%Y-%m-%d")
            print(
                f"{alert['name']:<30} {alert_type:<15} {str(alert['enabled']):<10} {created_date}"
            )
    ...
    for _, release in schedule.iterrows():
        print(
            f"{release['release']:<35} {release['frequency']:<20} {release['typical_day']:<25}"
        )
    ...
    callback_alert = alerts.add_alert(
        table_no="G1", name="CPI with Auto-Download", callback=on_cpi_release
    )
