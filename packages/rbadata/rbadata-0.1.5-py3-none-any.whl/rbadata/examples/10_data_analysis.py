...
def analyze_monetary_policy():
    ...
    try:
        ...
        # Recent changes
        rate_3m_ago = (
            cash_rate.iloc[-3]["value"] if len(cash_rate) > 3 else current_rate
        )
...
def calculate_financial_conditions_index():
    ...
    try:
        ...
        components["cash_rate"] = (current_cash - historical_avg) / cash_rate[
            "value"
        ].std()
...
def evaluate_forecast_accuracy():
    ...
    try:
        ...
        forecasts["horizon_days"] = (
            forecasts["date"] - forecasts["forecast_date"]
        ).dt.days
        one_year = forecasts[
            (forecasts["horizon_days"] > 300) & (forecasts["horizon_days"] < 400)
        ]
        ...
        for variable in ["gdp_change", "cpi_annual", "unemp_rate"]:
            ...
            if errors:
                results[variable] = {
                    ...
                    "hit_rate": sum(1 for e in errors if abs(e) < 0.5)
                    / len(errors)
                    * 100,
                }
...
def analyze_economic_cycles():
    ...
    try:
        ...
        last_change_idx = (
            phase_changes[phase_changes != 0].index[-1]
            if any(phase_changes != 0)
            else gdp_ts.index[0]
        )
        ...
        for i in range(2, len(gdp_ts) - 2):
            if (
                gdp_ts.iloc[i] > gdp_ts.iloc[i - 1]
                and gdp_ts.iloc[i] > gdp_ts.iloc[i + 1]
            ):
                turning_points.append(
                    {"date": gdp_ts.index[i].strftime("%Y-%m"), "type": "Peak"}
                )
            elif (
                gdp_ts.iloc[i] < gdp_ts.iloc[i - 1]
                and gdp_ts.iloc[i] < gdp_ts.iloc[i + 1]
                ...
                turning_points.append(
                    {"date": gdp_ts.index[i].strftime("%Y-%m"), "type": "Trough"}
                )
    ...
    try:
        ...
        if "unemployment" in aligned.columns and "inflation" in aligned.columns:
            ...
            for lag in range(-4, 5):
                if lag < 0:
                    corr_lag = (
                        aligned["unemployment"]
                        .iloc[:lag]
                        .corr(aligned["inflation"].iloc[-lag:])
                    )
                elif lag > 0:
                    corr_lag = (
                        aligned["unemployment"]
            ...
            relationships["Phillips Curve"] = {
                ...
                "interpretation": (
                    "Negative relationship" if corr < 0 else "Positive relationship"
                ),
            }
        ...
        if "cash_rate" in aligned.columns and "inflation" in aligned.columns:
            ...
            relationships["Policy Response"] = {
                ...
                "interpretation": (
                    "Strong positive"
                    if corr > 0.7
                    else "Moderate positive" if corr > 0.3 else "Weak"
                ),
            }
...
def create_economic_dashboard_summary():
    ...
    dashboard = {"current_conditions": {}, "outlook": {}, "risks": []}
