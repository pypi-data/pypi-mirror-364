...
class ChartPack:
    ...
    def get_charts_by_category(
        ...
        if category_lower not in [
            c.lower().replace(" ", "-") for c in self._categories
        ]:
            raise RBADataError(
                f"Category '{category}' not found. Available: {self._categories}"
            )
        ...
        return [
            ...
            if c["category"].lower().replace(" ", "-") == category_lower
        ]
    ...
    def _scrape_chart_pack_page(self):
        ...
        if date_text:
            try:
                self._last_update = pd.to_datetime(
                    date_pattern.search(date_text).group(1)
                )
            except Exception:
                self._last_update = None
    ...
    def _guess_category(self, title: str) -> str:
        ...
        if any(word in title_lower for word in ["house", "property", "dwelling"]):
            ...
            word in title_lower for word in ["credit", "lending", "debt", "money"]
        ):
            ...
            word in title_lower
            for word in ["unemployment", "labour", "labor", "employment"]
        ):
            return "Labour Market"
        elif any(
            word in title_lower for word in ["interest", "rate", "yield", "cash rate"]
        ):
    ...
    def _get_default_charts(self) -> List[Dict[str, str]]:
        ...
        return [
            {
                "title": "GDP Growth - Selected Economies",
                "category": "World Economy",
                "id": "gdp-growth",
                "url": "",
            },
            {
                "title": "Unemployment Rates - Selected Economies",
                "category": "World Economy",
                "id": "unemployment-global",
                "url": "",
            },
            {
                "title": "Australian GDP Growth",
                "category": "Australian Growth",
                "id": "aus-gdp",
                "url": "",
            },
            {
