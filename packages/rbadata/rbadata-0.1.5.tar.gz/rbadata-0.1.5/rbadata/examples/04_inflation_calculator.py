...
def main():
    ...
    print(
        f"${original_amount} in {from_year} is equivalent to ${adjusted_value:.2f} in {to_year}"
    )
    print(
        f"That's an increase of {adjusted_value - original_amount:.2f} ({(adjusted_value/original_amount - 1)*100:.1f}%)"
    )
    ...
    print(
        f"Required wage to maintain 2010 purchasing power: ${required_wage_2023:,.2f}"
    )
    ...
    inflation_adjusted = calc.calculate_value(
        investment_amount, investment_year, current_year
    )
    ...
    print(
        f"Value needed to maintain purchasing power in {current_year}: ${inflation_adjusted:,.2f}"
    )
    print("\nInvestment outcomes:")
    ...
    for scenario, final_value in investment_scenarios:
        ...
        if real_return > 0:
            print(
                f"  Real return: ${real_return:,.2f} ({real_return_pct:.1f}% above inflation)"
            )
        else:
            print(
                f"  Real loss: ${-real_return:,.2f} ({-real_return_pct:.1f}% below inflation)"
            )
