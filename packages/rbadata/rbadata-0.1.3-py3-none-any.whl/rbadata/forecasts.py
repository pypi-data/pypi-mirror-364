...
def rba_forecasts(all_or_latest: Literal["all", "latest"] = "all") -> pd.DataFrame:
    """
    ...
    all_forecasts = all_forecasts.drop_duplicates(
        subset=["date", "series", "forecast_date"], keep="last"
    )

    # Add year_qtr column
    all_forecasts["year_qtr"] = (
        all_forecasts["date"].dt.year + (all_forecasts["date"].dt.quarter - 1) * 0.25
    )
