...
def main():
    ...
    print(
        f"Date range: {all_forecasts['forecast_date'].min()} to {all_forecasts['forecast_date'].max()}"
    )
    ...
    for series in all_forecasts["series"].unique():
        series_desc = all_forecasts[all_forecasts["series"] == series][
            "series_desc"
        ].iloc[0]
    ...
    print(
        f"Number of unique forecast dates: {inflation_forecasts['forecast_date'].nunique()}"
    )
    ...
    if not forecasts_2024.empty:
        for _, row in forecasts_2024.tail(5).iterrows():
            print(
                f"  Forecast made on {row['forecast_date'].strftime('%Y-%m-%d')}: {row['value']}%"
            )
    ...
    if not revisions.empty:
        ...
        for _, row in revisions.iterrows():
            print(
                f"{row['forecast_date'].strftime('%Y-%m-%d')}      |  {row['value']:.1f}%"
            )
    ...
    gdp_forecasts["target_period"] = (
        gdp_forecasts["date"].dt.year.astype(str)
        + "-Q"
    ...
    gdp_pivot = gdp_forecasts.pivot_table(
        index="target_period", columns="forecast_date", values="value"
    )
