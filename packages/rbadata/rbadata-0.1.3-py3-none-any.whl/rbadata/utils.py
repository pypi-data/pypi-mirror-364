...
from .data import get_series_list, get_table_list
...
def get_rba_urls(table_nos: List[str], cur_hist: str = "current") -> List[str]:
    ...
    # Filter by current/historical if column exists
    if "current_or_historical" in table_list.columns:
        table_list = table_list[table_list["current_or_historical"] == cur_hist]
    else:
        # If column doesn't exist, try to refresh the table list
        table_list = get_table_list(refresh=True)
        if "current_or_historical" in table_list.columns:
            table_list = table_list[table_list["current_or_historical"] == cur_hist]
    ...
    for table_no in table_nos:
        ...
        if "url" not in matches.columns:
            raise RBADataError(
                f"URL information not available for table '{table_no}'. "
                "Try refreshing the table list."
            )
