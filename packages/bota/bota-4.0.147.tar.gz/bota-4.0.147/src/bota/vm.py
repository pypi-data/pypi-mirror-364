...
def get_package_name_from_debian_url(debian_installer_url):
    ...
    temp_filename = f"temp_{default_name}"
...
def uninstall_desktop_app_in_vm(debian_installer_url, package_name):
    """
    ...
    try:
        ...
        err = e.stderr.decode('utf-8') if e.stderr is not None else ""
        if "Unable to locate package" in str(err):
            pass
        else:
            print(err)
            sys.exit(1)
    ...
    # except subprocess.CalledProcessError as e:
    #     click.echo(f"Warning: Could not remove package {final_package_name}: {e}")
    #     click.echo("The package may not be installed or may have already been removed.")
    if path:
