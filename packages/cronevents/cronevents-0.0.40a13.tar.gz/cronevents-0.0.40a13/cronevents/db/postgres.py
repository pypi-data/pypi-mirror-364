...
import cronevents.db.persistqueue
...
class Postgres:
    ...
    def download_table(self, table_name: str = None, columns='*', suffix='', sql=None, stream=False, callback=None, queue: bool = False, row_transform: callable = None) -> Union[list[dict], Generator, cronevents.db.persistqueue.JsonlPersistentQueue]:
        """
        ...
        with psycopg2.connect(
            ...
            if not stream and callback is None and not queue:
                ...
                if queue:
                    q = cronevents.db.persistqueue.JsonlPersistentQueue()
    ...
    ) -> Union[List[Dict], AsyncGenerator, cronevents.db.persistqueue.JsonlPersistentQueue]:
        """
