...
class PostgresLogger(LoggerBase):
    table_name: str = 'cronevents_log'

    def __init__(self, trigger_id):
        self.table_name = PostgresLogger.table_name

        self.trigger_id = trigger_id
        self.db = get_postgres_from_env()
    ...
    def upload_to_db(self, table):
        try:
            upload_logs(self.table_name, self.db, self.trigger_id, table)
        except:
            self.db = get_postgres_from_env()
            upload_logs(self.table_name, self.db, self.trigger_id, table)
    ...
    def list(self, stream: bool = False) -> list[Log] | Generator[Log, None, None]:
        q = f"select * from {self.table_name} where trigger_id = $id${self.trigger_id}$id$ order by index asc;"
        if stream:
            return (Log.from_row(row) for row in self.db.download_table(sql=q, stream=True))
        return [Log.from_row(row) for row in self.db.download_table(sql=q)]


def upload_logs(table_name, db, trigger_id, logs):
    if logs:
        kwargs = {}
        index_query = f'create index if not exists event_logs_event_id_idx on {table_name} (trigger_id);'
        if isinstance(db, Postgres):
            index_query = f'create index if not exists event_logs_event_id_idx on {table_name} using hash (trigger_id);'
            kwargs['partition'] = 'trigger_id'
            kwargs['partition_query'] = f'''CREATE TABLE if not exists "{table_name}_{trigger_id}" 
                                    PARTITION OF "{table_name}" FOR VALUES IN ('{trigger_id}');'''
