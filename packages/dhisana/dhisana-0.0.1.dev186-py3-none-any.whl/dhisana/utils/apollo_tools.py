...
from dhisana.schemas.sales import LeadsQueryFilters
...
async def search_leads_with_apollo_page(
    ...
    leads. Pagination details (``next_page`` and ``per_page``) are returned so
    callers can easily request the following page on the next invocation.
    """
    ...
    if example_url:
        ...
        page_list = query_params.get("page", ["1"])
        per_page_list = query_params.get("per_page", [str(default_per_page)])
        ...
        if "per_page" not in query_params:
            dynamic_payload["per_page"] = default_per_page
    ...
    per_page_val = int(dynamic_payload.get("per_page", default_per_page))
    if per_page is not None:
        try:
            per_page_val = int(per_page)
        except ValueError:
            pass
        dynamic_payload["per_page"] = per_page_val

    page_number = int(dynamic_payload.get("page", 1))
    if page is not None:
        try:
            page_number = int(page)
        except ValueError:
            pass
        dynamic_payload["page"] = page_number

    page_payload = dict(dynamic_payload)
    page_payload["page"] = page_number
    page_payload["per_page"] = per_page_val

    page_results = await search_people_with_apollo(tool_config=tool_config, dynamic_payload=page_payload)
    if not page_results:
        return {"next_page": page_number + 1, "per_page": per_page_val, "results": []}

    selected_people = page_results

    leads: List[Dict[str, Any]] = []
    for person_data in selected_people:
        input_user_properties: Dict[str, Any] = {}
        additional_props = input_user_properties.get("additional_properties") or {}
        input_user_properties = fill_in_properties_with_preference(input_user_properties, person_data)
        person_data = cleanup_properties(person_data)
    ...
    logger.info(f"Converted {len(leads)} Apollo records into dictionaries (single page mode).")
    return {"next_page": page_number + 1, "per_page": per_page_val, "results": leads}
