...
class HMI:
    ...
    This class handles both SDL2 controller and keyboard inputs to generate
    ...
    def __init__(self, dead_zone: float = 0.1) -> None:
        ...
        self.phi: float = 0.0           # Pitch
        self.theta: float = 0.0         # Roll
        self.psi: float = 0.0           # Yaw angle (degrees)
        self.psi_velocity: float = 0.0  # Yaw velocity
    ...
    def get_connected_controllers(self) -> List[str]:
        """Gets a list of names of the currently connected controllers.

        Returns:
            A list of controller name strings.
        """
        names = []
        for idx, ctrl in self.controllers.items():
            if ctrl.attached():
                name = controller.name_forindex(idx) or f"Controller {idx}"
                names.append(name)
        return names
    ...
    def __call__(self) -> Tuple[List[float], bool, bool]:
        ...
        self.psi += self.psi_velocity * 2.0  # Scale factor for yaw rate
        self.psi = self.psi % 360.0
