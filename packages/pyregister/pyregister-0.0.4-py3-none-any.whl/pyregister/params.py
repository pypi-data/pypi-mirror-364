...
class RegisterParamError(Exception):
    pass
...
class Params(MutableMapping):
    ...
    parameter passing and validation for topolearning.
    ...
    .. note::
        ...
        The convention for using a `Params` object in topolearning is that you will consume the parameters
    ...
    def pop(self, key: str, default: Any = DEFAULT, keep_as_dict: bool = False) -> Any:
        ...
        `RegisterParamError`, instead of the typical `KeyError`.
        """
    ...
    def pop_choice(
        ...
        Parameters:
            ...
            choices (List[Any]): A list of valid options for values corresponding to
                `key`. For example, if you're specifying the type of encoder to use
                for some part of your model, the choices might be the list of encoder
                classes we know about and can instantiate.  If the value we find in
                the param dictionary is not in `choices`, we raise a `RegisterParamError`,
                because the user specified an invalid value in their parameter file.
            default_to_first_choice (bool, optional): If this is `True`, we allow the
                `key` to not be present in the parameter dictionary. If the key is
                not present, we will use the return as the value the first choice in
                the `choices` list.  If this is `False`, we raise a `RegisterParamError`,
                because specifying the `key` is required (e.g., you `have` to specify
                your model class when running an experiment, but you can feel free to
                use default settings for encoders if you want).
                (default = :obj:`False`)
            allow_class_names (bool, optional): If this is `True`, then we allow unknown
                choices that look like fully-qualified class names. This is to allow
                e.g. specifying a model type as my_library.my_model.MyModel and importing
                it on the fly. Our check for "looks like" is extremely lenient
        ...
        if value not in choices and not ok_because_class_name:
            ...
            raise RegisterParamError(message)
        return value
    ...
    def as_dict(self, quiet: bool = False, infer_type_and_cast: bool = False):
        ...
        Parameters:
            quiet (bool, optional): Whether to log the parameters before returning
                them as a dict.
                (default = :obj:`False`)
            infer_type_and_cast (bool, optional): If True, we infer types and cast
    ...
    def assert_empty(self, class_name: str):
        """
        Raises a `RegisterParamError` if `self.params` is not empty.  We take `class_name` as
        an argument so that the error message gives some idea of where an error happened, if there
        was one.  `class_name` should be the name of the `calling` class, the one that got extra
        parameters (if there are any).
        """
        if self.params:
            raise RegisterParamError(
    ...
    def from_file(
        ...
        Parameters:
            ...
            params_overrides (Union[str, Dict[str, Any]], optional): A dict of
                overrides that can be applied to final object. e.g.
                `{"model.embedding_dim": 10}` will change the value of "embedding_dim"
                within the "model" object of the config to 10. If you wanted to
                override the entire "model" object of the config, you could do
                `{"model": {"type": "other_type", ...}}`.
                (default = :obj:`""`)
            ext_vars (dict, optional): Our config files are Jsonnet, which allows
                specifying external variables for later substitution. Typically
                we substitute these using environment variables; however, you can
                also specify them here, in which case they take priority over
                environment variables. e.g. {"HOME_DIR": "/Users/topolearning/home"}
        """
        if ext_vars is None:
            ext_vars = {}
    ...
    def from_jsonstr(
        ...
        Parameters:
            jsonstr (str): The json str to the configuration.
            params_overrides (Union[str, Dict[str, Any]], optional): A dict of
                overrides that can be applied to final object. e.g.
                `{"model.embedding_dim": 10}` will change the value of "embedding_dim"
                within the "model" object of the config to 10. If you wanted to
        ...
        file_dict = json.loads(jsonstr) if isinstance(jsonstr, str) else jsonstr
    ...
    def as_ordered_dict(self, preference_orders: List[List[str]] = None) -> OrderedDict:
        ...
        Parameters:
            preference_orders (List[List[str]], optional): `preference_orders` is
                list of partial preference orders. ["A", "B", "C"] means "A" > "B" > "C".
                For multiple preference_orders first will be considered first.
                Keys not found, will have last but alphabetical preference. Default
                Preferences: `[["dataset_reader", "iterator", "model", "train_data_path",
                "validation_data_path", "test_data_path", "trainer"], ["type"]]`
        """
...
def pop_choice(
    ...
    history, so you'll have to fix that in the log if you want to actually recover 
