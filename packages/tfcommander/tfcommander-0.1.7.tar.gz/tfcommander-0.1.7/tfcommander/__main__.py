...
def register_service(public_ip, service_id, service_name, address, port, domains, middlewares=None, https_insecure=False, custom_rule=None, disable_tls=False):
    ...
    tags.append(f"traefik.http.services.{service_id}.loadbalancer.server.port={port}")
...
Examples:
  ...
  tf register jim proxmox Proxmox 192.168.19.10 8006 prox.example.com --https-insecure # this option forces Traefik to communicate to upstream over TLS, but doesn't require validated certificate
  tf register jim proxmox Proxmox 192.168.19.10 8006 prox.example.com --disable-tls # this option disables TLS termination on Traefik
...
"""
    ...
    reg_parser.add_argument("--disable-tls", action="store_true", help="Disable TLS termination in Traefik")
