...
mcp_client_baseurl = 'http://10.10.3.119:25989'
...
def boot(app: FastAPI):
    ...
    async def post_task(abstract: str, description: str, verification: str):
        ...
        # 启动后台任务
        asyncio.create_task(run_background_task(task_id, abstract, description, verification))
        return {"task_id": task_id}

    @app.get("/get/task/status")
    async def get_task_status():
        tasks = []
        for key in task_manager.keys():
            graph_name = task_manager[key]['graph_name']
            graph_context = ""
            if os.path.exists(graph_name):
                with open(graph_name, 'r') as f:
                    graph_context = f.read()
            tasks.append(Task(token=key, status=task_manager[key]["status"], graph_context=graph_context,
                              abstract=task_manager[key]["abstract"], description=task_manager[key]["description"],
                              verification=task_manager[key]["verification"],
                              context=task_manager[key]["mcp_client"].context))
        return tasks
