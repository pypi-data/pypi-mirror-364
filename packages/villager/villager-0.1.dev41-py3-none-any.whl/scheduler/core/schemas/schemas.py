...
def merge_task_model_out(input: List[TaskModelOut]) -> TaskModelOut:
    _sum_result = ''
    _sum_result_abstract = ''
    _split_chat = ''
    if len(input) > 1:
        _split_chat = ';'
    for it in input:
        _sum_result += it.result + _split_chat
        _sum_result_abstract += it.result_abstract + _split_chat
    return TaskModelOut(result_abstract=_sum_result_abstract, result=_sum_result)


if __name__ == '__main__':
    TMO_A = TaskModelOut(result_abstract='haha', result='haha')
    TMO_B = TaskModelOut(result_abstract='hahaB', result='hahaB')
    print(merge_task_model_out([TMO_A, TMO_B]))
    """
    result_abstract='haha;hahaB;' result='haha;hahaB;'
    """
