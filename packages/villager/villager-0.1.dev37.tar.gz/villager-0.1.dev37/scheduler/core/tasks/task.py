...
from scheduler.core.schemas.schemas import TaskModel, NeedBranchModel, TaskExecuteStatusModel, \
    TaskModelOut, TaskStatus, TaskModelOutList
...
class TaskNode(Node):
    ...
    def branch_and_execute(self, branch_requirement) -> List[TaskModelOut]:
        """
        ...
        tasks_classed = []
        task_chain_output: List[TaskModelOut] | None = None
    ...
    def execute(self, rebranch_prompt='') -> TaskModelOut:
        ...
        if len(branch_requirement.task_chain.tasks) > 1:
            try:
                return self.digest_task_model_out(self.branch_and_execute(branch_requirement))
            except TaskImpossibleException as e:
    ...
    def digest_task_model_out(self, input_task_model_out_list: List[TaskModelOut]) -> TaskModelOut:
        ...
        promptTemplate = ChatPromptTemplate.from_messages([
            ("system", "{format_instructions}"
                       "你是一名助手，请根据用户提供的任务输出列表整合弄错成父节点所需要的任务返回结果"
                       "请注意:"
                       "1. 不要尝试去实际执行任务!"
             ),
            ("user",
