    ...
    functions["diffToFile"] = {
        ...
        "func": async (args: any, streaming: boolean = false, call_id: string = undefined): Promise<string> => {
            ...
            try {
                ...
                if (streaming && streamingContent.has(call_id)) {
                    // Use cached content for subsequent streaming operations
                    originalContent = streamingContent.get(call_id);
                    console.log(`📋 Using cached content for streaming operation (${originalContent.length} chars)`);
                } else {
                    // Read from disk for first operation or non-streaming operations
                    let fileContent;
                    try {
                    ...
                    try {
                        ...
                        // Cache content for streaming operations
                        if (streaming) {
                            streamingContent.set(call_id, originalContent);
                            streamingCallIds.set(filePath, call_id);
                            console.log(`💾 Cached content for streaming (${originalContent.length} chars)`);
                        }
                    } catch (err) {
                ...
                // Update cached content for streaming operations
                if (streaming) {
                    streamingContent.set(call_id, diffResult);
                    console.log(`🔄 Updated cached content for streaming (${diffResult.length} chars)`);
                }
