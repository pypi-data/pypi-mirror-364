    ...
    functions["diffToFile"] = {
        ...
        "func": async (args: any, streaming: boolean = false, call_id: string = undefined): Promise<string> => {
            ...
            try {
                ...
                if (streamingContent.has(call_id)) {
                    // Use cached content for any operation that has a cached state (streaming or final)
                    originalContent = streamingContent.get(call_id);
                    console.log(`📋 Using cached content for ${streaming ? 'streaming' : 'final'} operation (${originalContent.length} chars)`);
                } else {
                    ...
                    try {
                        ...
                        // Cache content for any operation (streaming or not)
                        streamingContent.set(call_id, originalContent);
                        streamingCallIds.set(filePath, call_id);
                        console.log(`💾 Cached content for ${streaming ? 'streaming' : 'single'} operation (${originalContent.length} chars)`);
                    } catch (err) {
                ...
                // Update cached content for all operations (streaming and final)
