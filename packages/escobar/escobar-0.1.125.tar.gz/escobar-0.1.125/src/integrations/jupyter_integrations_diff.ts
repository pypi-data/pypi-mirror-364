...
function validateFileContent(fileContent: any, filePath: string): string {
    console.log(`=== DETAILED FILE ANALYSIS FOR ${filePath} ===`);
    console.log(`Raw fileContent keys:`, Object.keys(fileContent || {}));
    console.log(`Full fileContent object:`, JSON.stringify(fileContent, null, 2));
    ...
    console.log(`=== DECISION LOGIC FOR ${filePath} ===`);
    console.log(`mimetype: "${mimetype}" (type: ${typeof mimetype})`);
    console.log(`fileType: "${fileType}" (type: ${typeof fileType})`);
    console.log(`format: "${format}" (type: ${typeof format})`);
    console.log(`content type: ${typeof fileContent.content}`);
    console.log(`mimetype.startsWith('text/'): ${mimetype.startsWith('text/')}`);
    console.log(`mimetype === 'application/x-python-code': ${mimetype === 'application/x-python-code'}`);
    ...
    if (mimetype.startsWith('text/') || mimetype === 'application/x-python-code') {
        console.log(`✅ TAKING TEXT FILE PATH for ${filePath} with mimetype: ${mimetype}`);
        ...
        if (typeof fileContent.content === 'string') {
            ...
            console.log(`Notebook content structure:`, JSON.stringify(fileContent.content, null, 2));
            try {
                const extractedSource = extractSourceFromNotebook(fileContent.content);
                console.log(`✅ Successfully extracted source (${extractedSource.length} chars):`, extractedSource.substring(0, 200) + '...');
                return extractedSource;
            } catch (err) {
                console.error(`❌ Failed to extract source from Jupytext file ${filePath}:`, err);
                throw new Error(`Failed to extract source from Jupytext file ${filePath}: ${err.message}`);
            }
        } else {
            console.error(`❌ Text file ${filePath} has unexpected content type: ${typeof fileContent.content}`);
    ...
    if (fileType === 'notebook' &&
        ...
        console.log(`Jupytext metadata:`, JSON.stringify(fileContent.content.metadata.jupytext, null, 2));
        console.log(`Notebook content structure:`, JSON.stringify(fileContent.content, null, 2));

        try {
            const extractedSource = extractSourceFromNotebook(fileContent.content);
            console.log(`✅ Successfully extracted source from Jupytext (${extractedSource.length} chars):`, extractedSource.substring(0, 200) + '...');
            return extractedSource;
        } catch (err) {
            console.error(`❌ Failed to extract source from Jupytext file with null mimetype ${filePath}:`, err);
            throw new Error(`Failed to extract source from Jupytext file ${filePath}: ${err.message}`);
        }
    }
    ...
    if ((fileType === 'notebook' || mimetype === 'application/x-ipynb+json') &&
        ...
        console.log(`✅ TAKING NOTEBOOK FILE PATH for ${filePath} (actual notebook, not Jupytext)`);
        if (fileContent.content === null) {
            return '';
        }
        try {
            const jsonString = JSON.stringify(fileContent.content, null, 2);
            console.log(`✅ Successfully stringified notebook content (${jsonString.length} chars)`);
            return jsonString;
        } catch (err) {
            console.error(`❌ Failed to stringify notebook content for ${filePath}:`, err);
            throw new Error(`Failed to stringify notebook content for ${filePath}: ${err.message}`);
        }
    }

    console.log(`⚠️ No specific path matched, continuing to fallback logic...`);
