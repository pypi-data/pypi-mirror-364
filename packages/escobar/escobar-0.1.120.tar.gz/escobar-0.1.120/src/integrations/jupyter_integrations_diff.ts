...
function validateFileContent(fileContent: any, filePath: string): string {
    ...
    if (typeof fileContent.content !== 'string') {
        throw new Error(`Expected string content for ${filePath}, got ${typeof fileContent.content}`);
    }
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (typeof result.content !== 'string') {
                throw new Error(`Invalid file content type on attempt ${attempt}: ${typeof result.content}`);
            }
