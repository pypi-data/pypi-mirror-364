...
async function readFileWithReadyFlags(contents: any, filePath: string): Promise<any> {
    // Wait for service manager to be ready
    if (app.serviceManager && app.serviceManager.ready) {
        await app.serviceManager.ready;
    }

    // Now read the file - no retries needed since services are ready
    const result = await contents.get(filePath);

    if (!result) {
        throw new Error(`File read returned null/undefined for ${filePath}`);
    }
        ...
        "func": async (args: any, streaming: boolean = false, call_id: string = undefined): Promise<string> => {
            ...
            try {
                ...
                if (fileBaselines.has(filePath)) {
                    ...
                    let fileContent;
                    try {
                        fileContent = await readFileWithReadyFlags(contents, filePath);
                    } catch (err) {
