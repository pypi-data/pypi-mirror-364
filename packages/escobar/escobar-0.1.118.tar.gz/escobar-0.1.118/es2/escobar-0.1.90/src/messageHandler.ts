  ...
  public async sendMessage(content: string, mode: string, selectedProvider?: string): Promise<ResponseMessage> {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
      try {
        ...
        // Get Google ID token if available
        const googleIdToken = (window as any).escobarGoogleIdToken || null;
        ...
        const userMessage: IUserMessageRequest = {
          ...
          google_id_token: googleIdToken // Include Google ID token if available
        };
