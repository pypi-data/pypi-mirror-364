...
export function createTopButtons(
  ...
  // Create Google Auth button with vertical key icon
  ...
  googleAuthButton.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!googleClientId) {
        ...
        alert('Google Client ID not configured. Please set it in the settings first.');
        return;
      }
      ...
      // Attempt login using Google Identity Services (preferred method)
      const result: IGoogleAuthResult = await authManager.loginWithGSI();
      ...
      if (result.success && result.idToken) {
        // Comprehensive token logging in UI button handler
        console.log('=== UI BUTTON: GOOGLE AUTHENTICATION SUCCESS ===');
        console.log('Authentication result:', result);
        console.log('ID Token received:', result.idToken);
        console.log('Access Token received:', result.accessToken || 'Not provided');
        console.log('User info received:', result.userInfo);
        console.log('=== STORING TOKENS GLOBALLY ===');

        // Store the token globally for use in backend requests
        (window as any).escobarGoogleIdToken = result.idToken;
        (window as any).escobarGoogleUserInfo = result.userInfo;

        console.log('Stored escobarGoogleIdToken:', (window as any).escobarGoogleIdToken);
        console.log('Stored escobarGoogleUserInfo:', (window as any).escobarGoogleUserInfo);
        console.log('===============================================');

        // Update button tooltip to show authenticated state (no visual styling changes)
        googleAuthButton.title = `Authenticated as ${result.userInfo?.email || 'Google User'}`;

        // Log success without showing visual notification
        console.log(`Successfully authenticated as ${result.userInfo?.email || 'Google User'}`);
