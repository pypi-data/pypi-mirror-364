...
function validateFileContent(fileContent: any, filePath: string): string {
    ...
    // Handle notebook files (format: 'json', content: object)
    if (fileContent.format === 'json' && typeof fileContent.content === 'object') {
        console.log(`Processing notebook file ${filePath} with JSON content`);
        ...
        try {
            ...
            throw new Error(`Failed to stringify JSON content for ${filePath}: ${err.message}`);
        }
    }
    ...
    if (fileContent.format === 'text' && typeof fileContent.content === 'string') {
    ...
    if (typeof fileContent.content === 'string') {
        console.log(`Processing file ${filePath} with string content (format: ${fileContent.format})`);
        return fileContent.content;
    }

    // Handle case where content is object but not a notebook
    if (typeof fileContent.content === 'object' && fileContent.content !== null) {
        console.warn(`Unexpected object content for ${filePath}, attempting to stringify`);
        try {
            return JSON.stringify(fileContent.content, null, 2);
        } catch (err) {
    ...
    throw new Error(
        ...
        `type=${fileContent.type}, format=${fileContent.format}, ` +
        `contentType=${typeof fileContent.content}. ` +
        `Expected string content or valid JSON/base64 format.`
    );
}
