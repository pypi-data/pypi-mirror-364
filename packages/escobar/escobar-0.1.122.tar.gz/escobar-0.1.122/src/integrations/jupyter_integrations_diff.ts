...
function validateFileContent(fileContent: any, filePath: string): string {
    ...
    // Handle text files based on mimetype (most reliable indicator)
    if (mimetype.startsWith('text/') || mimetype === 'application/x-python-code') {
        ...
        if (typeof fileContent.content === 'string') {
            ...
            // This is likely a Jupytext file - extract source from notebook cells
            console.log(`Extracting source from Jupytext file ${filePath}`);
            try {
    ...
    // Handle actual notebook files
    if (fileType === 'notebook' || mimetype === 'application/x-ipynb+json') {
        console.log(`Processing notebook file ${filePath}`);
