...
// Simple function to validate and extract file content
function validateFileContent(fileContent: any, filePath: string): string {
    ...
    // Use the simple text extraction function
    try {
        return extractTextContent(fileContent, filePath);
    } catch (err) {
        console.error(`Content extraction failed for ${filePath}:`, err.message);
        throw new Error(`Failed to extract content from ${filePath}: ${err.message}`);
    }
}
    ...
    return createFileOperationLock(filePath, async () => {
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                console.error(`❌ File read attempt ${attempt} failed for ${filePath}:`, err.message);
            ...
            try {
                ...
                if (fileBaselines.has(filePath)) {
                    ...
                    try {
                        ...
                        console.error(`File read failed for ${filePath}:`, err.message);
                    ...
                    try {
                        originalFileContent = extractTextContent(fileContent, filePath);
                        ...
                        console.error(`File content extraction failed for ${filePath}:`, err.message);
                        return JSON.stringify({
                            "status": "fail",
                        ...
                        if (targetContext) {
                            ...
                            if (targetContext.model && typeof targetContext.model.fromString === 'function') {
                                ...
                                try {
                                    ...
                                    console.error(`❌ Context save failed for ${filePath}:`, saveError.message);
                                    throw saveError;
                                }
                            }
                        }
