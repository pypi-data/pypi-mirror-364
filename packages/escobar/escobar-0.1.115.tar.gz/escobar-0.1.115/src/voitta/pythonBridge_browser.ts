  ...
  return new Promise((resolve, reject) => {
    ...
    if (ws && isConnected && ws.readyState === WebSocket.OPEN) {
      ...
      if (waitForResponse) {
        ...
        pendingRequests.set(call_id, {
          resolve: (value: any) => {
            ...
            console.log(`[callPython] ${timestamp} Stop button decision for call_id: ${call_id}`, {
              finishReason: value.finish_reason,
              shouldHideStopButton
            });

            if (shouldHideStopButton) {
              console.log(`[callPython] ${timestamp} Dispatching END event (hiding stop button) for call_id: ${call_id}`);
              window.dispatchEvent(new CustomEvent(PYTHON_CALL_EVENTS.END));
            } else {
              console.log(`[callPython] ${timestamp} NOT dispatching END event (keeping stop button) for call_id: ${call_id}`);
            }
          ...
          reject: (reason?: any) => {
            ...
            console.log(`[callPython] ${timestamp} Dispatching END event due to rejection for call_id: ${call_id}`);
            window.dispatchEvent(new CustomEvent(PYTHON_CALL_EVENTS.END));
            reject(reason);
          },
          timeout
        ...
        pendingRequests.delete(call_id);
        console.log(`[callPython] ${timestamp} Dispatching END event (no response wait) for call_id: ${call_id}`);
        window.dispatchEvent(new CustomEvent(PYTHON_CALL_EVENTS.END));
      }
      ...
      try {
        ...
        const compressed_payload = compressString(payload)
        ws.send(compressed_payload);

        console.log(`[callPython] ${timestamp} WebSocket send SUCCESS for call_id: ${call_id}`, {
          payloadLength: payload.length,
