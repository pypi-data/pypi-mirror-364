...
class CreateSecretInputDto(BaseModel):
    ...
    """ # noqa: E501
    aws_secret_input: AWSSecretInputDto = Field(description="AWS-specific input details, including access key, secret key, profile name and region.", alias="awsSecretInput")
    configuration_secret_input: ConfigurationSecretInputDto = Field(description="Configuration-specific input details, including value and filename.", alias="configurationSecretInput")
    environment_secret_input: EnvironmentSecretInputDto = Field(description="Environment-specific input details, including value and environment key.", alias="environmentSecretInput")
    git_secret_input: GitSecretInputDto = Field(description="Git-specific input details, including repository URL, username and password.", alias="gitSecretInput")
    gpg_secret_input: GPGSecretInputDto = Field(description="SSH-specific input details, providing the private key value.", alias="gpgSecretInput")
    name: StrictStr = Field(description="Name of the secret.")
    ssh_secret_input: SshSecretInputDto = Field(description="GPG-specific input details, containing the GPG key data.", alias="sshSecretInput")
    type: SecretLocalType
    __properties: ClassVar[List[str]] = ["awsSecretInput", "configurationSecretInput", "environmentSecretInput", "gitSecretInput", "gpgSecretInput", "name", "sshSecretInput", "type"]
