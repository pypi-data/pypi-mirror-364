...
class SpanBackendWriter(SpanWriter):
    ...
    def write(self, spans: list[dict[str, Any]]) -> None:
        ...
        # Sort spans so that those containing a non-empty session name are handled first. This
        # guarantees the backend upsert logic sees a meaningful name the first time it encounters
        # a new session ID.
        spans_sorted = sorted(
            spans,
            key=lambda s: (s.get("session_id") is not None and s.get("session_name")),
            reverse=True,
        )
