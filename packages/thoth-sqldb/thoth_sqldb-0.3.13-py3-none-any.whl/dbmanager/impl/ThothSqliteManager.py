...
class ThothSqliteManager(ThothDbManager):
    ...
    def get_example_data(self, table_name: str, number_of_rows: int = 30) -> Dict[str, List[Any]]:
        ...
        Modifications:
        1. Limits to 10000 records for frequency calculation
        ...
        with self.engine.connect() as connection:
            for col_info in columns:
                ...
                # Handle text type columns differently - extract 30 non-null, non-blank values
                if 'text' in column_type:
                    logging.info(f"Processing text column: {column_name}")
                    
                    # Query to get 30 non-null, non-blank text values without sorting
                    text_query_str = f"""
                    ...
                    """
                    try:
                        result = connection.execute(text(text_query_str))
                        values = [row[0] for row in result]
                        most_frequent_values[column_name] = values
                    except SQLAlchemyError as e:
                        logging.error(f"Error fetching text values for {column_name} in {table_name} ({self.db_id}.sqlite): {e}")
                        most_frequent_values[column_name] = []
                    continue

                # Query to get most frequent values from a limited dataset of 10000 records
                query_str = f"""
                    SELECT {quoted_column_name}
                    FROM (
