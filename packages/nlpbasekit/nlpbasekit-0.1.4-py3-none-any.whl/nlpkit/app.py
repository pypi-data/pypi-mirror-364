...
class LogisticClassifier:
  """Logistic regression text classifier with optional preprocessing and
    bag-of-words feature extraction.

    Additional parameters (beyond sklearn's):
    
      embeding (str, default='count'):
          Choose 'count' or 'tfidf'. Other strings print an error.
      
      n (int, default=1):
          N-gram width for CountVectorizer (ngram_range=(n, n)), must be ≥ 1.
      
      stop (str, default='english'):
          Language for NLTK stopword removal.
      
      punc (bool, default=True):
          Remove punctuation when preprocessing.
      
      extraction (bool, default=True):
          Expand contractions via contractions.fix().
      
      rare_words (float or int, default=0):
          Proportion threshold for removing infrequent words.
      
      normalization_method (str or None, default=None):
          None, 'stemming', or 'lemitization'; anything else raises ValueError.

    All other arguments are passed directly to sklearn.linear_model.LogisticRegression.
  """  
  ...
  def preprocessing(self,x,y):
    ...
    try:
      if self.normalization_method=="lemitization":
          self.res=self.lemitization(self.res)
      elif self.normalization_method =="stemming":
  ...
  def fit(self,x,y):
    ...
    # train the model
    self.clf=LogisticRegression(penalty=self.penalty, dual=self.dual, tol=self.tol, C=self.C,fit_intercept=self.fit_intercept, intercept_scaling=self.intercept_scaling, class_weight=self.class_weight,random_state=self.random_state, solver=self.solver, max_iter=self.max_iter, multinomial=self.multi_class, verbose=self.verbose,warm_start=self.warm_start, n_jobs=self.n_jobs, l1_ratio=self.l1_ratio,).fit(self.res,self.y)
  ...
  def getPrecisionScore(self):
    try:
      ...
      return precision_score(self.actual_labels.tolist(),self.predicted_labels,pos_label='positive')
    except Exception as e:
...
class SupportVectorClassifier:
  """
    Text-based SVM classifier built on scikit-learn's SVC, with optional
    text preprocessing and bag-of-words feature extraction.

    Additional parameters (beyond sklearn's):

      embeding (str, default='count'):
          'count' or 'tfidf'. Others print an error.
      ...
      rare_words (float or int, default=0):
          Threshold proportion for removing infrequent words.

      normalization_method (str or None, default=None):
          None, 'stemming', or 'lemitization'; anything else raises ValueError.

    All other arguments are forwarded to sklearn.svm.SVC and behave exactly
    as in scikit-learn.
    """
  ...
  def getClassificationReport(self):
    try:
      return classification_report(self.actual_labels,self.predicted_labels)
    except Exception as e:
      print("Error during generating Classification Report:\n",str(e))
  ...
  def getPrecisionScore(self):
    try:
      ...
      if len(set(self.actual_labels)) > 2:
        # multiclassification problem
        return precision_score(self.actual_labels,self.predicted_labels,average='weighted')  
      
      return precision_score(self.actual_labels,self.predicted_labels,pos_label='positive')
    except Exception as e:
      print("Error in getting precision score:\n",str(e))
...
class DTC:
  ...
  def getConfusionMatrix(self):
    try:
      return confusion_matrix(self.actual_labels,self.predicted_labels)
    except Exception as e:
      print("Error during generating confusion matrix:\n",str(e))
...
# KMeans
class NMeans():
  def __init__(self,n_clusters=8, *, init='k-means++', n_init='auto', max_iter=300, tol=0.0001, verbose=0, random_state=None, copy_x=True, algorithm='lloyd',  embeding='count',n=1,stop='english', 
  punc=True, extraction=True,rare_words=0,normalization_method=None):
