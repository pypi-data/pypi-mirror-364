...
def auto_dwcr(file_obj, dh_no, completion_no=None, db=None):
    ...
    field_values = {
        ...
        "date_completed": cevent.completion_date.strftime("%d/%m/%Y")
...
def replace_unit_code_as_hyperlink(unit_code):
    ...
    if "%" in unit_code and not endpoint.endswith("s"):
        endpoint += "s"
    return f"<a href='/app/{endpoint}?{query_param}={unit_code}&env=prod'>{unit_code}</a>"
...
def generate_aquifer_code_index(reports=None, add_hyperlinks=False, sagd_conn=None):
    ...
    if reports is None:
        reports = pd.read_excel(r"r:\dfw_cbd\projects\projects_gw\state\groundwater_toolbox\aquifer_database\reports_index.xlsx")
    ...
    for idx, row in reportsx.iterrows():
        ...
        for row_unit in row_units:
            if "%" in row_unit:
                ...
                matched_aquifer_codes += [c for c in aquifer_codes if row_unit_re.match(c)]
        ...
        if add_hyperlinks:
            ...
            for u in all_codes + row_units:
                if u in comment:
                    comment = re.sub(f"([^a-zA-Z]|^)({u})([^%&a-zA-Z]|$)", f"\\1{replace_unit_code_as_hyperlink(u)}\\3", comment)
            reportsx.loc[idx, "comments"] = comment.replace("\n", "<br />")
            reportsx.loc[idx, "aquifers_strat_units"] = ", ".join([replace_unit_code_as_hyperlink(u) for u in row_units])
