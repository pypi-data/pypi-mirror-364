...
class RainfallStations(BaseModel):
    ...
    def find_stations(self):
        ...
        if self.station_id:
            ...
            gsites = gpd.GeoDataFrame(sites, geometry=gpd.points_from_xy(sites.lon, sites.lat), crs="epsg:7844")
            gsites = gsites.to_crs("epsg:8059")
            wpoint = gpd.GeoDataFrame({"geometry": [well_point] * len(gsites)}, crs="epsg:7844")
            ...
            query_params = [f"nearest_to_well={self.nearest_to_well}", f"sagd_env={self.sagd_env}"]
