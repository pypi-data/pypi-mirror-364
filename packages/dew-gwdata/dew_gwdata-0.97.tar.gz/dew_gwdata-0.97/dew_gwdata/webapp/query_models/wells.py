...
class Wells(BaseModel):
    ...
    def find_wells(self):
        ...
        if self.idq.strip():
            ...
            logger.debug(f"Running Wells query for last edited by {self.edit_by} on {self.edit_timestamp}")
        ...
        if self.filter_aquifer_code:
            logger.debug(f"Filtering to wells containing aquifers {self.filter_aquifer_code}")
        ...
        if self.filter_latest_tds_above:
            logger.debug(f"Filtering to latest_tds_above {self.filter_latest_tds_above}")
            logger.debug(f"- before: {len(wells)} records")
        ...
        if self.filter_latest_tds_below:
            logger.debug(f"Filtering to latest_tds_below {self.filter_latest_tds_below}")
            logger.debug(f"- before: {len(wells)} records")
        ...
        if self.filter_latest_sal_date_since:
            logger.debug(f"Filtering to latest salinity date >= {self.filter_latest_sal_date_since}")
            logger.debug(f"- before: {len(wells)} records")
        ...
        if self.filter_log_types.strip():
            logger.debug(f"Filtering to wells containing log types {self.filter_log_types}")
