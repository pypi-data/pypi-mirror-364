...
def strat_units(
    ...
    details["agso_number"] = details.agso_number.apply(
        lambda n: f"<a href='https://asud.ga.gov.au/search-stratigraphic-units/results/{n}'>{n:.0f}</a>"
        if not pd.isnull(n)
        else ""
    )
...
def strat_unit(request: Request, strat_unit_no: int, env: str = "PROD"):
    ...
    lookups, reports_all = gd.generate_aquifer_code_index(add_hyperlinks=True, sagd_conn=db)
    lookup = lookups[(lookups.code == details.map_symbol) & (lookups.code_type == "map_symbol")]
    reports = reports_all[reports_all.filename.isin(lookup.report_filename)]
    reports["filename"] = reports.filename.apply(lambda fn: f"<a href='/api/aquifer_database_file?filename={fn}'>{fn}</a>")
...
def aquifer_unit(request: Request, aquifer_code: str, env: str = "PROD"):
    ...
    details["linked_strat_agso_number"] = details.linked_strat_agso_number.apply(
        lambda n: f"<a href='https://asud.ga.gov.au/search-stratigraphic-units/results/{n}'>{n}</a>"
        if not n is None
        else ""
    )
    ...
    lookups, reports_all = gd.generate_aquifer_code_index(add_hyperlinks=True, sagd_conn=db)
    lookup = lookups[(lookups.code == details_series.aquifer_code) & (lookups.code_type == "aquifer_code")]
    reports = reports_all[reports_all.filename.isin(lookup.report_filename)]
    reports["filename"] = reports.filename.apply(lambda fn: f"<a href='/api/aquifer_database_file?filename={fn}'>{fn}</a>")
