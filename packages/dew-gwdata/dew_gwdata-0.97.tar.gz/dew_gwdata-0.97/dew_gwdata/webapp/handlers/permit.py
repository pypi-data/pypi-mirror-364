...
def permits(
    ...
    permits["permit_no"] = permits.permit_no.apply(
        lambda p: f"<a href='/app/permit?permit_no={p}'>{p}</a>"
    )
    ...
    permits["existing_unit_hyphen"] = permits.existing_unit_hyphen.apply(
        lambda unit_hyphen: f"<a href='/app/well_summary?dh_no={wells[unit_hyphen]}&env={query.env}'>{unit_hyphen}</a>"
...
def permit(request: Request, permit_no: str, env: str = "PROD"):
    ...
    other["permit_no_only"] = other.permit_no_only.apply(
        lambda pn: f"<a href='/app/permit?permit_no={pn}&env={env}'>{int(pn)}</a>"
        if not np.isnan(pn)
        else ""
    )
