...
def usage(query: query_models.Wells = Depends(), format: str = "json"):
    ...
    df2 = pd.merge(df, sm[["dh_no", "unit_hyphen", "latitude", "longitude"]], on="unit_hyphen", how="left")
    return apply_format(
        df2,
        format=format,
...
def drillhole_geophysical_log_files(
    ...
    fn = gd.produce_geophysical_log_files_zip(
        name,
        files, exclude_extensions=("tif", "tiff", "mp4", "mpg", "mpeg", "avi", "vob")
    )
    with open(fn, mode="rb") as f:
        data = f.read()
    os.remove(fn)
...
def geophysical_logging_job_files(
    ...
    fn = gd.produce_geophysical_log_files_zip(
        name, files, exclude_extensions=("tif", "tiff", "mp4", "mpg", "mpeg", "avi", "vob")
    )
    with open(fn, mode="rb") as f:
        data = f.read()
    os.remove(fn)
...
def aquifer_database_file(
    ...
    aquifer_db = Path(r"r:\dfw_cbd\projects\projects_gw\state\groundwater_toolbox\aquifer_database")
