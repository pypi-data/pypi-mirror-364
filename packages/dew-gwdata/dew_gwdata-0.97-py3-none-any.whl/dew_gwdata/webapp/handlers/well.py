...
def well_summary(request: Request, dh_no: int, env: str = "prod") -> str:
    ...
    const["auto_dwcr"] = const.apply(
        lambda row: f"<a href='/api/well_auto_dwcr?dh_no={row.dh_no}&completion_no={row.completion_no}&env={env}'>Auto DWCR</a>"
        if row.event_type == "C"
        else "",
        axis=1,
    )
    ...
    const["permit_no"] = const.permit_no.apply(
        lambda pn: f"<a href='/app/permit?permit_no={pn}&env={env}'>{pn}</a>"
        if pn
        else ""
    )
    ...
    well["nearest_rainfall_site"] = f"<a href='/app/rainfall_stations?nearest_to_well={well.unit_hyphen}'><img src='/static/external-link.svg' width=12 height=12 /></a>"
    well["aquifer_link"] = ""
    if well["aquifer"]:
        well["aquifer_link"] = f"<a href='/app/aquifer_unit?aquifer_code={well.aquifer}&env={env}'>{well.aquifer}</a>"
...
def well_geophysical_logs(
    ...
    files_cols = ["logged_date", "job_no", "filename", "file_type", "file_size", "csv_download", "max_depth_las", "log_types", "mnemonics"]
