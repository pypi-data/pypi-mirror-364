...
def prep_table_for_html(df, query):
    ...
    df.insert(7, "rf_site", df.unit_hyphen.apply(lambda n: f"<a href='/app/rainfall_stations?nearest_to_well={n}'><img src='/static/external-link.svg' width=12 height=12 /></a>"))
    df = df.drop(["unit_hyphen", "obs_no"], axis=1)
    if "aquifer" in df:
        df["aquifer"] = df.apply(
...
def load_rainfall_from_db(station_id, conn):
    ...
    return ausweather.RainfallStationData.from_data(station_id, df)
