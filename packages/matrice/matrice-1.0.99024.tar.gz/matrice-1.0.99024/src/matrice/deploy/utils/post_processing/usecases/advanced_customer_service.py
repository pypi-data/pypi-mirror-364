        ...
        try:
            ...
            if getattr(config, "enable_smoothing", False):
                if not hasattr(self, "smoothing_tracker") or self.smoothing_tracker is None:
                    # Use config's smoothing params if available, else defaults
                    smoothing_config = getattr(config, "smoothing_config", None)
                    if smoothing_config is None:
                        from ..utils import BBoxSmoothingConfig, BBoxSmoothingTracker
                ...
                from ..utils import bbox_smoothing
