...
try:
    from genai_processors import processor, ProcessorPart, streams  # type: ignore[import-not-found]
except ImportError:
    processor = None
    ProcessorPart = None
    streams = None
...
class ContextEnrichmentProcessor(processor.PartProcessor):
    ...
    def match(self, part: ProcessorPart) -> bool:
        ...
        return hasattr(part, "json") and "prompt" in part.json
    ...
    async def call(self, part: ProcessorPart) -> AsyncIterable[ProcessorPart]:
        ...
        if hasattr(part, "json") and part.json:
            enriched_request = part.json.copy()
            ...
            yield ProcessorPart(json=enriched_request)
        else:
            yield part


class ValidationResultAggregator(processor.PartProcessor):
    ...
    def match(self, part: ProcessorPart) -> bool:
        ...
        return hasattr(part, "json")

    async def call(self, part: ProcessorPart) -> AsyncIterable[ProcessorPart]:
        """Aggregate validation results"""
        if hasattr(part, "json") and part.json:
            # Aggregate results
            self.results.update(part.json)

            # Determine overall approval
            if "approved" in part.json:
            ...
            yield ProcessorPart(json=self.results)
        else:
            yield part


class ValidationStatusProcessor(processor.PartProcessor):
    ...
    async def call(self, part: ProcessorPart) -> AsyncIterable[ProcessorPart]:
        ...
        # Then yield status
        yield processor.status(self.status_message)
...
class HybridValidationPipeline:
    ...
    async def validate_file_operation(
        ...
        # Create input stream
        input_stream = streams.stream_content([ProcessorPart(json=validation_request)])
        ...
        async for part in pipeline(input_stream):
            if hasattr(part, "json") and part.json:
                final_result.update(part.json)
            elif hasattr(part, "status") and part.status:
                print(f"Pipeline status: {part.status}")
