...
from claude_code_adk_validator.config import PROTECTED_BRANCHES, ENFORCE_ISSUE_WORKFLOW
...
class TestBranchValidation(unittest.TestCase):
    ...
    def test_is_protected_branch_main(self, mock_get_branch):
    ...
    @patch.object(SecurityValidator, "_get_current_branch")
    def test_is_protected_branch_feature(self, mock_get_branch):
    ...
    def test_is_allowed_file_on_main_readme(self):
    ...
    def test_is_allowed_file_on_main_docs(self):
        """Test allowed files on main - docs directory"""
        self.assertTrue(self.validator._is_allowed_file_on_main("docs/guide.md"))
        self.assertTrue(self.validator._is_allowed_file_on_main("docs/api/reference.md"))

    def test_is_allowed_file_on_main_code(self):
        """Test disallowed files on main - code files"""
        self.assertFalse(self.validator._is_allowed_file_on_main("src/main.py"))
        self.assertFalse(self.validator._is_allowed_file_on_main("tests/test_example.py"))
    ...
    @patch("subprocess.run")
    def test_validate_issue_exists_success(self, mock_run):
    ...
    @patch("subprocess.run")
    def test_validate_issue_exists_not_found(self, mock_run):
    ...
    @patch("subprocess.run")
    def test_validate_issue_exists_gh_not_installed(self, mock_run):
    ...
    def test_check_branch_validation_on_main_with_code(
        self, mock_get_branch, mock_is_protected
    ):
    ...
    @patch.object(SecurityValidator, "_is_protected_branch")
    def test_check_branch_validation_on_main_with_docs(self, mock_is_protected):
    ...
    @patch.object(SecurityValidator, "_is_protected_branch")
    def test_check_branch_validation_on_feature_branch(self, mock_is_protected):
    ...
    def test_validate_file_operation_with_branch_block(self, mock_check_branch):
        ...
        result = self.validator.validate_file_operation({
    ...
    @patch("claude_code_adk_validator.config.ENFORCE_ISSUE_WORKFLOW", False)
    def test_branch_validation_disabled(self):
