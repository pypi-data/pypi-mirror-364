...
from claude_code_adk_validator.config import STRICT_TEMPLATE_VALIDATION
...
def test_template_file_detection():
    ...
    for file_path in template_files:
        assert FileContextAnalyzer.is_template_file(file_path), f"{file_path} should be detected as template"
        
        context = FileContextAnalyzer.categorize_file(file_path)
        assert context["category"] == "template", f"{file_path} should be categorized as template"
        assert context["requires_strict_security"] == False
        print(f"✓ {file_path} correctly identified as template")


def test_non_template_files():
    ...
    for file_path in non_template_files:
        assert not FileContextAnalyzer.is_template_file(file_path), f"{file_path} should NOT be a template"
        
        context = FileContextAnalyzer.categorize_file(file_path)
        assert context["category"] != "template", f"{file_path} should NOT be categorized as template"
        print(f"✓ {file_path} correctly NOT identified as template")


def test_template_validation_with_strict_disabled():
...
<body>
    ...
    assert result["approved"] == True, f"Template should be approved but got: {result.get('reason')}"
    print("✓ HTML template with external resources correctly approved")


def test_template_validation_blocks_critical_secrets():
...
<body>
    ...
    tool_input = {
        "file_path": "templates/config.html",
        "content": template_with_secret
    }
    
    result = validator.validate("Write", tool_input, "")
    
    assert result["approved"] == False, "Template with real secrets should be blocked"
    assert "AWS access key" in result["reason"] or "Stripe live secret" in result["reason"]
    print("✓ Template with real secrets correctly blocked")


def test_template_xss_detection_with_strict_enabled():
    """Test XSS detection in templates when strict validation is enabled"""
    # Mock the config to enable strict validation
    with patch('claude_code_adk_validator.security_validator.STRICT_TEMPLATE_VALIDATION', True):
        validator = SecurityValidator()
...
<body>
    ...
    <div>{{ content }}</div>
        ...
        tool_input = {
            "file_path": "templates/unsafe.html",
            "content": unsafe_template
        }
        
        result = validator.validate("Write", tool_input, "")
        
        assert result["approved"] == False, "Template with |safe filter should be blocked in strict mode"
        assert "Unescaped output" in result["reason"]
        print("✓ Template with |safe filter correctly blocked in strict mode")


def test_template_allows_safe_patterns():
...
<body>
    ...
    tool_input = {
        "file_path": "templates/safe.html", 
        "content": safe_template
    }
    
    result = validator.validate("Write", tool_input, "")
    
    assert result["approved"] == True, f"Safe template should be approved but got: {result.get('reason')}"
...
def test_template_file_categorization():
    ...
    for path in template_paths:
        ...
        assert context["category"] == "template", f"{path} should be categorized as template"
        assert context["requires_strict_security"] == False, f"{path} should not require strict security"
