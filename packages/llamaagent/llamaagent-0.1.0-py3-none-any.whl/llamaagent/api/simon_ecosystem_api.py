...
class ServiceDiscoveryRequest(BaseModel):
    ...
    exclude_nodes: List[str] = Field(default_factory=list, description="Nodes to exclude")
...
class TaskCollaborationRequest(BaseModel):
    ...
    coordination_strategy: str = Field("parallel", description="Task coordination strategy")
...
class TaskCollaborationResponse(BaseModel):
    ...
    participating_nodes: List[str] = Field(..., description="Nodes participating in task")
...
class ResourceSharingRequest(BaseModel):
    ...
    resource_data: Optional[Dict[str, Any]] = Field(None, description="Resource data for sharing")
...
def get_ecosystem_registry():
    ...
    class MockEcosystemRegistry:
        ...
        def discover_services(self, service_type: Optional[str] = None, capabilities: List[str] = None) -> List[Dict[str, Any]]:
        ...
        def get_ecosystem_health(self) -> Dict[str, Any]:
            ...
            return {
                ...
                "service_health": {
                    "agents": len([n for n in self.nodes.values() if n["node_type"] == "agent"]),
                    "services": len([n for n in self.nodes.values() if n["node_type"] == "service"]),
...
def get_task_coordinator():
    ...
    class MockTaskCoordinator:
        ...
        async def coordinate_task(self, request: TaskCollaborationRequest) -> Dict[str, Any]:
...
def get_resource_manager():
    ...
    class MockResourceManager:
        ...
        def share_resource(self, resource_type: str, resource_data: Dict[str, Any]) -> str:
...
async def discover_services(request: ServiceDiscoveryRequest):
    ...
    try:
        ...
        ecosystem_nodes = [
            EcosystemNode(**service) for service in services
...
async def get_ecosystem_health(
    ...
    try:
        ...
        overall_status = "healthy" if health_data["unhealthy_nodes"] == 0 else "degraded"
        ...
        response = EcosystemHealthResponse(
            ...
            resource_status={"shared_resources": 5, "active_tokens": 12} if include_resources else {},
            performance_metrics=health_data["performance_metrics"] if include_metrics else {},
