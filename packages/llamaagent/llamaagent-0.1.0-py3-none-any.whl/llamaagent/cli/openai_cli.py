...
try:
    ...
    OpenAIIntegrationConfig = None
    create_openai_integration = None
...
@click.option("--mode", type=click.Choice(["openai", "local", "both"]), default="openai", help="Agent mode")
...
def run(task: str, model: str, mode: str, budget: float, tools: List[str], agent_type: str, budget_per_task: Optional[float]) -> None:
...
async def _run_task(
    ...
    if estimated_cost > budget:
        console.print(
            f"[red]Warning: Estimated cost (${estimated_cost:.4f}) exceeds budget (${budget})[/red]"
        )
    ...
    if "python" in tools or "all" in tools::
        tool_registry.register_tool("python", PythonREPLTool())
        ...
        if hasattr(result, 'metadata') and result.metadata:
            ...
            if "usage" in metadata:
                ...
                if total_tokens > 0:
                    ...
                    console.print(
                        f"[dim]Tokens used: {total_tokens}, Estimated cost: ${actual_cost:.4f}[/dim]"
                    )
    else:
        console.print(
            Panel(
...
def configure(openai_key: Optional[str], default_model: Optional[str], default_budget: Optional[float], ollama_url: Optional[str]) -> None:
...
def status() -> None:
    ...
    table.add_row("OpenAI API Key", "Set" if cli_config.get("openai_api_key") else "Not Set")
    ...
    table.add_row("OpenAI Agents", "Available" if OPENAI_AGENTS_AVAILABLE else "Not Available")
...
def experiment(experiment_file: str, output_dir: str) -> None:
    ...
    console.print(f"[green]Running experiment: {experiment_config.get('name', 'Unnamed')}[/green]")
    ...
    # Save results
    results_file = output_path / f"experiment_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with open(results_file, "w") as f:
        json.dump(results, f, indent=2)
    
    console.print(f"[green]Experiment completed. Results saved to: {results_file}[/green]")


@cli.command()
@click.option("--model", help="Model to test")
@click.option("--mode", type=click.Choice(["openai", "local", "both"]), default="both", help="Test mode")
