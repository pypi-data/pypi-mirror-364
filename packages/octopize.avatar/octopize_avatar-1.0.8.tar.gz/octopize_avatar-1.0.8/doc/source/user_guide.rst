...
You can easily generate plots to visualize and better understand the
avatarization process. These plots compare the original vs avatar data,
helping you analyze :

- correlations (``PlotKind.CORRELATION`` and
  ``PlotKind.CORRELATION_DIFFERENCE``).
- distributions of your variables (``PlotKind.DISTRIBUTION``).
- variable contributions in the avatar model
  (``PlotKind.CONTRIBUTION``).
- data projections throughout the anonymization workflow
  (``PlotKind.PROJECTION_2D`` and ``PlotKind.PROJECTION_3D``).
- summary table with mean and std of the first 10 columns
  (``PlotKind.AGGREGATE_STATS``).
