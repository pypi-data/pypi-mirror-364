...
class MachineStats(ServiceAbstract):
    ...
    def on_data_arrive(self, size: int):
        try:
            self.logger.debug(f"Machine stats: on_data_arrive called with size {size} bytes")
            if not self._ensure_connection() or not self.rmq_channel:
                raise Exception("Could not establish connection")
    ...
    def on_data_publish(self, size: int):
        try:
            self.logger.debug(f"Machine stats: on_data_publish called with size {size} bytes")
            if not self._ensure_connection() or not self.rmq_channel:
                raise Exception("Could not establish connection")
            body = json.dumps(
                {"size": -int(size)}
    ...
    def _update_buffer_untill_success(self, total_enqued_size: int, total_dequed_size: int):
        """
        ...
        while self.is_running and loopcount > 0:
            try:
                with self.buffer_update_lock:
                    data_size, data_size_uploaded = self._get_current_buffer()
                    # new stats
                    new_data_size = data_size + total_enqued_size
                    new_data_size_uploaded = data_size_uploaded + total_dequed_size
                    ...
                    self.logger.debug(
                        f"Processed {total_enqued_size} arrivals, {total_dequed_size} bytes added to buffer"
                    )
                    return
                loopcount -= 1
            except Exception as e:
    ...
    def _process_buffer_array(self):
        while self.is_running:
            try:
                if not self._ensure_connection() or not self.rmq_channel:
                    self.logger.error("Could not ensure RabbitMQ connection")
                    time.sleep(5)
                    continue
                total_size_enques = 0
                total_size_deques = 0
                for _ in range(10):
                    try:
                        ...
                        self.logger.error(f"IndexError in buffer array processing: {ie}")
                        break
                    except pika.exceptions.StreamLostError as sle:
                        self.logger.error(f"StreamLostError in buffer array processing: {sle}")
                        self.rmq_conn = None
                        self.rmq_channel = None
                        break
                    except Exception as e:
                ...
                if total_size_enques > 0 or total_size_deques > 0:
                    self._update_buffer_untill_success(total_size_enques, total_size_deques)
            except Exception as e:
                self.logger.error(f"Error processing buffer array: {str(e)}")
            time.sleep(5)
