...
class EventBuffer:
        ...
        with self.lock:
            ...
            if len(self.events) >= self.config.buffer_size:
                ...
                try:
                    ...
                    # No event loop running, try to flush synchronously
                    logger.warning("No event loop running, attempting synchronous flush")
                    self._flush_sync()
    ...
    def _flush_sync(self) -> bool:
        """Synchronous flush implementation for when no event loop is available."""
        events_to_send = []
        ...
        try:
            # Use requests or similar synchronous HTTP client
            import requests
            import json
            
            # Build the API endpoint
            api_url = f"{self.config.api_base_url}/events"
            ...
            if response.status_code == 200:
                ...
                with self.lock:
                    self.events.extend(events_to_send)
                logger.log_flush_error(len(events_to_send), f"HTTP {response.status_code}")
                return False
