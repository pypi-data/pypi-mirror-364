...
class TeraaceAPIClient:
    ...
    async def send_events(self, events: List[AgentEvent]) -> bool:
        ...
        logger.log_api_call(self.config.api_endpoint)
            ...
            try:
                async with self.session.post(
                    ...
                    if response.status == 200:
                        logger.info(f"Successfully sent {len(events)} events to API")
                        return True
                    elif response.status >= 400 and response.status < 500:
                        # Client error - don't retry
                        error_text = await response.text()
                        logger.error(f"Client error {response.status}: {error_text}")
                        logger.error(f"Failed to send {len(events)} events due to client error")
                        return False
                    else:
                        # Server error - retry
                        error_text = await response.text()
                        logger.warning(f"Server error {response.status} (attempt {attempt + 1}/{self.config.max_retries}): {error_text}")
                        if attempt == self.config.max_retries - 1:
                            logger.error(f"Failed to send {len(events)} events after {self.config.max_retries} attempts")
