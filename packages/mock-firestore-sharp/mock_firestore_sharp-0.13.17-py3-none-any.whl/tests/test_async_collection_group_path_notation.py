...
class TestAsyncCollectionGroupPathNotation(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_parent_collection_not_included_in_collection_group(self):
        """
        ...
        await self.mock_db.collection('standalone_reviews').document('review_standalone').set({
        ...
        await self.mock_db.collection('products').document('product3').collection('standalone_reviews').document('review_nested').set({
            'user': 'User Nested',
            'rating': 4,
