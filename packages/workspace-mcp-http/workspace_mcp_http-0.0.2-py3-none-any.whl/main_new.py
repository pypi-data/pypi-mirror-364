...
async def main():
    ...
    tool_imports = {
        ...
        'drive': 'gdrive.drive_tools_new',  # These would need similar updates
    ...
    tools_to_import = args.tools if args.tools is not None else ['gmail', 'drive', 'calendar']  # Enable updated tools
    ...
    try:
        ...
        for tool in tools_to_import:
            try:
                if tool == 'gmail':
                    # Import the updated Gmail tools
                    __import__('gmail.gmail_tools_new')
                    safe_print(f"   {tool_icons[tool]} {tool.title()} - Google {tool.title()} API integration (✅ Token Auth)")
                elif tool == 'drive':
                    # Import the updated Drive tools
                    __import__('gdrive.drive_tools_new')
                    safe_print(f"   {tool_icons[tool]} {tool.title()} - Google {tool.title()} API integration (✅ Token Auth)")
                elif tool == 'calendar':
                    # Import the updated Calendar tools
                    __import__('gcalendar.calendar_tools_new')
                    safe_print(f"   {tool_icons[tool]} {tool.title()} - Google {tool.title()} API integration (✅ Token Auth)")
                else:
                    # For other tools, use original imports (they would need updating)
                    __import__(tool_imports[tool])
                    safe_print(f"   {tool_icons[tool]} {tool.title()} - Google {tool.title()} API integration (⚠️  OAuth)")
