...
from lambda_happy_validation.fista_solver import FistaTestRunner
...
def parse_arguments():
    ...
    parser.add_argument(
        "--compute_fista_solver",
        action="store_true",
        help="Run a FistaSolver test with synthetic data",
    )

    parser.add_argument(
        "--compute_multi_fista_solver",
        action="store_true",
        help="Run multiple FistaSolver tests over various p and seeds",
    )
...
def main():
    ...
    bench = LambdaValidation(estimator, interactive=args.interactive)
    ...
    if args.compute_fista_solver:
        print(
            f"> Running FistaSolver with n={args.n}, p={args.p}, device={device}, dtype={dtype}, version={args.version}"
        )
        runner = FistaTestRunner(device_type=device, dtype=dtype, version=args.version)
        result = runner.run_test(n=args.n, p=args.p, seed=0)
        print("FistaSolver Test Result:")
    ...
    if args.compute_multi_fista_solver:
        print("> Running multiple FistaSolver tests over various p and seeds...")
        fistaTestRunner = FistaTestRunner(
        ...
        fistaTestRunner.run_all(p_list, seeds)
        any_action = True
