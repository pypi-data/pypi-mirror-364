...
class ShowCassandraVersion(Command):
    ...
    def run(self, cmd: str, state: ReplState):
        ...
        user, pw = get_user_pass(state.sts if state.sts else state.pod, state.namespace, secret_path='cql.secret')
        command = f'cqlsh -u {user} -p {pw} -e "show version"'

        if state.pod:
            return cassandra_pod_exec(state.pod, state.namespace, command)
        else:
            return cassandra_nodes_exec(state.sts, state.namespace, command, action='cql')
