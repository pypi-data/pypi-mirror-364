...
class Bash(Command):
    ...
    def run(self, cmd: str, s0: ReplState):
        ...
        if state.in_repl:
            ...
            if state.pod:
                cassandra_pod_exec(state.pod, state.namespace, command, show_out=True)
            elif state.sts:
                cassandra_nodes_exec(state.sts, state.namespace, command, action='bash', show_out=True)
    ...
    def exec_with_dir(self, state: ReplState, args: list[str]):
        ...
        if state.pod:
            ...
            rs = cassandra_nodes_exec(state.sts, state.namespace, command, action='bash', show_out=not session_just_created)
