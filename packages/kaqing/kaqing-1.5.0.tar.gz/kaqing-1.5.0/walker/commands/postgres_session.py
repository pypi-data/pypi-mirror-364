...
from walker.k8s_utils import create_pod, delete_pod, get_secret_data, in_cluster, in_cluster_namespace, is_pod_completed, list_secrets, pod_exec, wait_for_pod
...
class PostgresSession:
    ...
    def hosts(state: ReplState):
        ...
        ss = list_secrets(state.namespace, name_pattern=Config().get('pg.name-pattern', '^{namespace}.*k8spg.*'))
    ...
    def run_sql(self, sql: str, show_out = True):
        ...
        if in_cluster():
            ...
            try:
                create_pod(ns, pod_name, image, ['sleep', f'{timeout}'], env={'NAMESPACE': ns}, sa_name='c3')
            ...
            return pod_exec(pod_name, pod_name, ns, cmd, show_out=show_out)

    def endpoint(self):
        if not self.conn_details:
            self.conn_details = get_secret_data(self.namespace, self.host)
