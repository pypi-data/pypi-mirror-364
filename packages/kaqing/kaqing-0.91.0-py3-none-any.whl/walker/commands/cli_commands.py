...
from walker.k8s_utils import get_secret_data, in_cluster_namespace, list_pods, list_secrets
...
class CliCommands:
    ...
    def values(state: ReplState, collapse = False):
        ...
        pgs = list_secrets(state.namespace, name_pattern=Config().get('pg.name-pattern', '^{namespace}.*k8spg.*'))
        data = {pg: get_secret_data(state.namespace, pg) for pg in pgs}
