...
from walker.commands.postgres_use import UsePostgres
...
def repl_cmd_list() -> list[Command]:
    return [Bash(), Cd(), Check(), ClipboardCopy(), Cqlsh(), GetParam(), Help(), Issues(), Ls(), NodeTool()
            ] + Postgres.cmd_list() + [Postgres(), Processes()] + Reaper.cmd_list() + Repair.cmd_list() + [
            Report(), Restart(), RollingRestart(), SetParam()] + Show.cmd_list() + [
            Status(), Storage(), UsePostgres(), Watch(), Exit()]