...
def get_headers():
	return {'Authorization': f'Bearer {config.read("api_access_token")}'}
...
def sync(install_id: Annotated[str | None, typer.Option()] = None):
	...
	resp = requests.post(f'{Config.API_ADDRESS}/sync/initial', json=payload, timeout=10, headers=get_headers())
