...
from liminal import atuin, __version__, config
...
from liminal.logging import INSTALL_LOG_PATH, LOGGER
...
class Server: # SyncAPI
	...
	def authenticate_user_provided_key(self, install_token: str) -> InstallationSession:
		...
		api_access_token = resp_body['cli_api_access_token']
		config.write('api_access_token', api_access_token)
		LOGGER.debug(f'Validated the user id as liminal user {user_id} {resp_body=}')
		return InstallationSession(**resp_body)
...
def test_correctly_setup():
	...
	full_command, cmd_output = run_test_login_command(
		Shell().exec_path,
...
def _main():
	...
	LOGGER.info(f'Validated {INSTALL_SESSION.username}')
...
def main():
	...
	with (LIMINAL_DIR / 'nohup_sync.out').open(mode='w') as output_file:
...
def report_install_issue(exception: Exception):
	...
	print(f'Please copy all the text output that comes from running the command listed below, and and send it in an email to {EMAIL_CONTACT}:')
	print(f'\tcat {INSTALL_LOG_PATH}\n')
