...
def test_deprecation_warnings():
    ...
    # Test log_parameter deprecation
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        exp.log_parameter("old_param", "value")
        assert len(w) == 1
        assert issubclass(w[0].category, DeprecationWarning)
        assert "log_global_parameter" in str(w[0].message)

    # Test log_object deprecation
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        exp.log_object("old_object", {"key": "value"})
        assert len(w) == 1
        assert issubclass(w[0].category, DeprecationWarning)
        assert "log_global_config" in str(w[0].message)

    # Test log_problem deprecation
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        problem = knapsack_problem()
        ...
        assert len(w) == 1
        assert issubclass(w[0].category, DeprecationWarning)
        assert "log_global_problem" in str(w[0].message)
...
def test_backward_compatibility():
    """Test that old interface still works (with warnings)."""
    ...
    with warnings.catch_warnings():
        ...
        exp_store = exp.dataspace.experiment_datastore
        assert exp_store.parameters["old_style_param"] == 42
        assert exp_store.objects["old_style_object"]["data"] == "test"
        assert jm.is_same(exp_store.problems["old_style_problem"], problem)
