...
def test_new_explicit_interface(tmp_path):
    ...
    exp.log_parameter("dataset_name", "knapsack_test")
...
def test_run_without_context_manager(tmp_path):
    """Test that runs can be used without context manager but must be closed manually."""
...
def test_experiment_level_only_operations(tmp_path):
    ...
    exp.log_parameter("global_config", "production")
    exp.log_parameter("dataset_version", "v2.1")
    exp.log_object("metadata", {"created_by": "test", "purpose": "validation"})
