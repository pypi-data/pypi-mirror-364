...
class MintoLogger:
    ...
    def log_experiment_end(
        ...
        """
        message = f"Experiment '{experiment_name}' completed: {run_count} runs, total time: {duration:.1f}s"
        formatted = self.formatter.format_message(
    ...
    def log_sampleset(
        ...
        if min_energy is not None:
            message = f"SampleSet '{name}': {num_samples} samples, min_energy={min_energy:.3f}"
        else:
