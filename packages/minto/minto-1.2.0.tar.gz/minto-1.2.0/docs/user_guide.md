...
experiment.log_parameter("algorithm", "greedy")
experiment.log_parameter("max_iterations", 1000)
...
experiment.log_object("genetic_config", algorithm_config)
```
...
for temp in temperatures:
    run = experiment.create_run()  # Create explicit run object
...
experiment.log_parameter("learning_rate", 0.001)      # float
experiment.log_parameter("population_size", 100)      # int
experiment.log_parameter("algorithm", "genetic")      # str

# Collections
experiment.log_parameter("layer_sizes", [64, 128, 64])           # list
experiment.log_parameter("hyperparams", {"lr": 0.01, "decay": 0.9})  # dict

# NumPy arrays
import numpy as np
experiment.log_parameter("weights", np.array([0.1, 0.5, 0.4]))   # ndarray
...
experiment.log_problem("knapsack", knapsack_problem)
experiment.log_instance("test_case", problem_instance)
...
experiment.log_parameter("genetic_algorithm_population_size", 100)
experiment.log_solution("clarke_wright_routes", cw_solution)

# Include units when relevant
experiment.log_parameter("time_limit_seconds", 300)
experiment.log_parameter("memory_limit_mb", 2048)

# Use consistent prefixes for related parameters
experiment.log_parameter("sa_temperature", 1000)
experiment.log_parameter("sa_cooling_rate", 0.95)
experiment.log_parameter("sa_min_temperature", 0.01)
```
...
```python
# OLD: Confusing implicit context behavior
experiment.log_parameter("global_param", "value")  # Experiment level

with experiment.run():
    experiment.log_parameter("run_param", "value")  # Run level due to context

# NEW: Explicit and clear separation
experiment.log_parameter("global_param", "value")  # Always experiment level
...
experiment.log_problem("facility_location", milp_problem)
experiment.log_instance("northeast_facilities", problem_instance)

time_limits = [60, 300, 1800]  # 1min, 5min, 30min

for time_limit in time_limits:
    with experiment.run():
        experiment.log_parameter("time_limit_seconds", time_limit)
        experiment.log_parameter("solver", "cplex")
        ...
        experiment.log_solution("cplex_solution", solution)
        experiment.log_parameter("objective", solution.objective)
        experiment.log_parameter("solve_status", str(solution.status))
        experiment.log_parameter("gap", solution.mip_gap)
```
...
def run_experiment(temperature):
    with experiment.run():
        experiment.log_parameter("temperature", temperature)
        
        solution = simulated_annealing(problem, temperature)
        experiment.log_solution("sa_solution", solution)
        experiment.log_parameter("objective", solution.objective)
