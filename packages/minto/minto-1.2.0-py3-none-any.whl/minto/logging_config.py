...
class LogConfig:
    ...
    Attributes:
        ...
        format (LogFormat): Output format style.
        show_timestamps (bool): Whether to include timestamps.
        show_icons (bool): Whether to use emoji icons.
        show_colors (bool): Whether to use colored output.
        indent_size (int): Number of spaces for indentation.
        max_parameter_length (int): Maximum length for parameter values display.
    """
...
class LogFormatter:
    ...
    def format_message(
        ...
        if self.config.format == LogFormat.MINIMAL:
            ...
            if prefix:
                return f"{prefix} {level_str} {indent}{icon}{color_start}{message}{color_end}"
            else:
    ...
    def format_environment_summary(self, env_info: dict[str, typ.Any]) -> str:
        ...
        cpu_info = f"{env_info.get('cpu_info', 'Unknown')} ({env_info.get('cpu_count', 'Unknown')} cores)"
        ...
        return f"OS: {os_info}, CPU: {cpu_info}, Memory: {memory}, Python: {python_version}"
    ...
    def format_environment_info(
        ...
        lines.append(
            self.format_message(
                LogLevel.INFO, "Environment Information", indent_level, "environment"
            )
        )
        ...
        lines.append(
            self.format_message(
                LogLevel.INFO,
                f"OS: {env_info.get('os_name', 'Unknown')} {env_info.get('os_version', '')}",
                indent_level + 1,
            )
        )
        lines.append(
        ...
        lines.append(
            self.format_message(
                LogLevel.INFO,
                f"CPU: {env_info.get('cpu_info', 'Unknown')} ({env_info.get('cpu_count', 'Unknown')} cores)",
                indent_level + 1,
            )
        )
        lines.append(
