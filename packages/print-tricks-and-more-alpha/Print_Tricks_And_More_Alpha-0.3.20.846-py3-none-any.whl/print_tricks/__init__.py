...
## pt_line = f"{NAM.inst_nam}.props({C.t1}{formatted_args}{C.er}) - (Line :{str(line_no)}, {func_name_fmt}{file_name})\n
    ...
    def t2(
        *sequences,
        ...
        ## if sequences is blank tuple, set it to a list of one item, making the default sequence 'pt.t2()'
        if sequences == ():
            sequences = (f"{NAM.inst_nam}.t2()",)
        ...
        for sequence in sequences:
            ...
            if sequence not in pt.sequences_dict:
                ...
                if len_sequences == 1:
                    if print_this == True:
                        print(
                            f"{C.t1}{my_string}{C.er}{filler}{NAM.inst_nam}.t2({formatted_args}): "
                ...
                ## Check to see if they specified arguments on the secondary/tertiary pt.t2() statements. If so, send a message that this is not allowed, and to pass all arguments
                ...
                if (
                    ...
                    pt.c(
                        f"{C.t2}<<<Error>>: {C.er}You cannot pass arguments on line {line_no}: {NAM.inst_nam}.t2({C.t2}{formatted_args}) ({func_name_fmt}{file_name}). \n"
                ...
                ## recover the arguments from the first instance of this pt.t2(), so that we don't have match it at the end.
                ...
                ### - Accounting for the extra pt.t2() processing time.
                ...
                if (
                    sum == True
                ):  ## KEEP this as == and not "sum is True" or I can't do shortcuts like 'pt.t2(sum=1)'
                    tot_time = 0.0
    ...
    def t(
        *sequences,
        ...
        - print the time between any number of statements with the same name. 
