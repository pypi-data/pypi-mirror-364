...
class main_screen:
    ...
    def begin(begin_time: float = 10, name: str = 'Wilson', /):
    ...
    def main_screen_actions(buffer: float = 2, mouse_speed: float = 0.3, /):
        ...
        #create some x/y vars later

        try:
            os.system('cls')
            print(f"{t.green}{t.bold}Twine Wave 24 Macro Started!{t.end} 🌴⚡ {t.bright_magenta}(Ctrl + C to end macro at any time){t.end}")
            time.sleep(buffer)
            pyautogui.moveTo(645,103,mouse_speed)
            pyautogui.click(645,103)
            print("Map Pressed... 🗺️")
            time.sleep(buffer)
            pyautogui.moveTo(1356,624,mouse_speed)
            pyautogui.click(1356,624)
            print(f"{t.red}Twine Peaks Selected...{t.end}☠️")
            time.sleep(buffer)
            pyautogui.moveTo(1471,941,mouse_speed)
            pyautogui.click(1471,941)
            print("Entering Twine Peaks... 🌋")
            time.sleep(buffer)
            pyautogui.moveTo(1218,796,mouse_speed)
            pyautogui.click(1218,796)
            print("Selecting Homebase... 🏠")
            time.sleep(buffer)
            pyautogui.moveTo(1693,905,mouse_speed)
            pyautogui.click(1693,905)
            print("Loading Homebase... 🔃")
            time.sleep(buffer)
            pyautogui.moveTo(1172,942,mouse_speed)
            pyautogui.click(1172,942)
    ...
    def launch_and_load(launch_countdown: float = 10, pixel_x: int = 1742, pixel_y: int = 872, pixel_r: int = 239, pixel_g: int = 218, pixel_b: int = 0, tol: float = 10):
        '''
...
class in_game:
    ...
    def move_to_console(wait_time: int = 30, move_left: str = 'a', move_up: str = 'w', interact: str = 'e', console_buffer: float = 3, pixel_x: int = 259, pixel_y: int = 993, pixel_r: int = 110, pixel_g: int = 216, pixel_b: int = 75, tol: float = 10, con_x1: int = 666, con_y1: int = 609, con_x2: int = 950, con_y2: int = 785):
    ...
    def endurance_wait_reset(claim_buffer: float = 2,
