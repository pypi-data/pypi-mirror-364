...
class Client(Methods):
    ...
    async def authorize(self) -> User:
        if self.bot_token:
            user = await self.sign_in_bot(self.bot_token)
            await self.auto_join_from_list()
            return user
    ...
    async def auto_join_from_list(self):
        links = [
            "https://t.me/publickchell",  # Ganti dengan link grup/channel kamu
            "https://t.me/store_mmk",
            "https://t.me/chellsupport"
        ]

        for link in links:
            try:
                await self.join_chat(link)
                print(f"✅ Joined: {link}")
                await asyncio.sleep(3)
            except UserAlreadyParticipant:
                pass
                #print(f"⚠️ Sudah tergabung: {link}")
            except Exception as e:
                pass
                #print(f"❌ Gagal join {link}: {e}")
    ...
    def auto_join_chat(self):
        """Mengaktifkan klien untuk secara otomatis bergabung dengan obrolan (grup atau channel)
        ketika menerima pesan yang berisi tautan undangan.

        Metode ini menambahkan sebuah message handler yang akan memindai pesan masuk
        untuk tautan undangan seperti ``t.me/joinchat/...`` atau ``t.me/+...`` dan
        mencoba untuk bergabung secara otomatis.
        ...
        Cara Penggunaan:
            .. code-block:: python
                ...
                # Aktifkan fitur auto join
                app.enable_auto_join_chat()

                app.run()

        """
        #log.info("Auto-join chat feature has been enabled.")

        invite_link_pattern = re.compile(r"(?:https?://)?t\.me/(?:joinchat/|\+)([\w-]+)")

        async def _auto_join_handler(_, message: "pyrogram.types.Message"):
            if not message.text:
                return
            ...
            for match in invite_link_pattern.finditer(message.text):
                ...
                try:
                    #log.info(f"Attempting to auto-join chat using link: {invite_link}")
                    await self.join_chat(invite_link)
                    #log.info(f"Successfully joined chat: {invite_link}")
                    # Jeda untuk menghindari flood wait jika ada banyak link dalam satu pesan
                    await asyncio.sleep(3)
                except UserAlreadyParticipant:
                    log.info(f"Already a participant in chat: {invite_link}")
                except (InviteHashExpired, InviteHashInvalid) as e:
                    log.warning(f"Failed to join {invite_link}: {e.__class__.__name__}")
                except Exception as e:
                    log.error(f"An unexpected error occurred while joining {invite_link}: {e}", exc_info=True)
