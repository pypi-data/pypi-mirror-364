...
def test_foreach_registry_syntax(split, kwargs):
    ...
    # Create a mock dataset instead of using real GSM8K
    from unittest.mock import patch

    from doteval.datasets.base import Dataset, DatasetRegistry

    class MockSyntaxDataset(Dataset):
        name = "mock_syntax"
        splits = ["train", "test"]
    ...
    registry = DatasetRegistry()
    registry.register(MockSyntaxDataset)

    with patch("doteval.core._registry", registry):
        # Should create decorator without error
        decorator = getattr(foreach, "mock_syntax")(split, **kwargs)
...
def test_integration_foreach_with_registered_dataset():
    ...
    class MockIntegrationDataset(Dataset):
    ...
    registry = DatasetRegistry()
    registry.register(MockIntegrationDataset)

    with patch("doteval.core._registry", registry):
        decorator = foreach.mock_integration("test")
