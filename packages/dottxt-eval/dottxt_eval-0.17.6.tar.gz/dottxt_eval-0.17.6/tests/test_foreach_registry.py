...
def test_foreach_getattr_with_registered_dataset():
    """Test foreach works with any registered dataset."""

    # Create a mock dataset that behaves like GSM8K but is fast
    ...
    registry = DatasetRegistry()
    registry.register(MockGSM8K)

    with patch("doteval.core._registry", registry):
        decorator = foreach.mock_gsm8k("test")
