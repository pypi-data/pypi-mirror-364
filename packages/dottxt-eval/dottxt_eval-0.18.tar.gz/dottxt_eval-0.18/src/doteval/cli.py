...
from doteval.runner import run_concurrent, run_sequential
...
@click.option(
    "-n",
    "--numprocesses",
    help="Number of concurrent evaluations. Use 'auto' for unlimited, or a number. Default is sequential.",
)
@click.option("--samples", type=int, help="Limit number of dataset samples")
@click.option("--experiment", help="Experiment name for results")
@click.option("-k", "--keyword", help="Only run evaluations matching given substring")
@click.option("-m", "--marker", help="Only run evaluations with given marker")
def run(path, numprocesses, samples, experiment, keyword, marker):
    """Run evaluations in a directory."""
    if numprocesses:
        if numprocesses == "auto":
            max_concurrent = None  # Unlimited
        else:
            try:
                max_concurrent = int(numprocesses)
                if max_concurrent <= 0:
                    console.print(
                        "[red]Error: -n must be a positive integer or 'auto'[/red]"
                    )
                    return
            except ValueError:
                console.print(
        ...
        asyncio.run(
            _run_concurrent_wrapper(
                path, samples, experiment, keyword, marker, max_concurrent
            )
        )
    else:
        asyncio.run(run_sequential(path, keyword, marker, samples, experiment))


async def _run_concurrent_wrapper(
    path, samples, experiment, keyword, marker, max_concurrent
):
    """Async wrapper for concurrent execution."""

    await run_concurrent(path, keyword, marker, samples, experiment, max_concurrent)
