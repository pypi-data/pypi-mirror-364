...
def main():
    ...
    add_parser.add_argument("--level", choices=["facile", "moyen", "difficile"], default="facile", help="Niveau de difficulté")
    ...
    if args.command == "add":
        app.add_card(args.question, args.answer, level=args.level)
        print(f"✅ Carte ajoutée ! (niveau : {args.level})")
    elif args.command == "quiz":
        app.quiz()
    elif args.command == "list":
        cards = app.get_all_cards()
        for i, card in enumerate(cards):
            level = card.get("level", "facile")
            print(f"{i}. ❓ {card['question']} -> ✅ {card['answer']} (niveau : {level})")
