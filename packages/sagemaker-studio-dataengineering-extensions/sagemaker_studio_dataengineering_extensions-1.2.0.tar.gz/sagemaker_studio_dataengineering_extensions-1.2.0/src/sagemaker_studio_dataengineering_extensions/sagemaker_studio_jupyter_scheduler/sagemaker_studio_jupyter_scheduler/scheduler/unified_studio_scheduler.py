...
class SageMakerUnifiedStudioScheduler(BaseScheduler):
    ...
    async def createDataZoneAsset(self, domain_identifier: str, project_identifier: str, schedule_arn: str, training_job_input: Dict) -> Dict:
        ...
        create_asset_request = {
            ...
            "formsInput": [
                ...
                {
                    "formName": f"{SAGEMAKER_SCHEDULE_FORM_TYPE_NAME}_{project_identifier}",
                    "typeIdentifier": f"{domain_identifier}.{SAGEMAKER_SCHEDULE_FORM_TYPE_NAME}_{project_identifier}",
                    "content": json.dumps({
            ...
            "typeIdentifier": f"{SAGEMAKER_SCHEDULE_ASSET_TYPE}_{project_identifier}",
        }
    ...
    async def deleteDataZoneAsset(self, domain_identifier: str, project_identifier: str, schedule_name: str) -> Dict:
        ...
        # Search for the asset by schedule name
        search_params = {
            "domainIdentifier": domain_identifier,
            "owningProjectIdentifier": project_identifier,
            "searchScope": "ASSET",
    ...
    async def _update_datazone_asset_status(self, job_definition_id: str, status: str):
        ...
        try:
            ...
            for form in asset_forms:
                if form.get("formName") == f"{SAGEMAKER_SCHEDULE_FORM_TYPE_NAME}_{project_id}":
                    schedule_form = form
                elif form.get("formName") == ASSET_COMMON_DETAILS_FORM_NAME:
            ...
            try:
                ...
                # Add schedule form with updated status
                forms_input.append({
                    "formName": f"{SAGEMAKER_SCHEDULE_FORM_TYPE_NAME}_{project_id}",
                    "typeIdentifier": f"{domain_id}.{SAGEMAKER_SCHEDULE_FORM_TYPE_NAME}_{project_id}",
    ...
    async def list_job_definitions(
        ...
        search_params = {
            ...
            "searchIn": [
                ...
                {
                    "attribute": f"{SAGEMAKER_SCHEDULE_FORM_NAME}_{project_id}.{search_attribute}"
                }
            ],
        ...
        for item in asset_items:
            ...
            # Verify this is a schedule asset by checking type identifier
            type_identifier = asset_item.get("typeIdentifier", "")
            if not type_identifier.startswith(f"{SAGEMAKER_SCHEDULE_ASSET_TYPE}_{project_id}"):
                continue
