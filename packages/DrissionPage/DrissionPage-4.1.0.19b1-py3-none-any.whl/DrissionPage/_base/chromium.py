...
class Chromium(object):
    ...
    def __new__(cls, addr_or_opts=None, session_options=None):
        ...
        is_headless, browser_id, is_exists, is_local = run_browser(opt)
        with cls._lock:
    ...
    def __init__(self, addr_or_opts=None, session_options=None):
        ...
        self._ws_address = (self._chromium_options.address if self._chromium_options.address.startswith('ws:')
...
def run_browser(chromium_options):
    if chromium_options.address.startswith('ws:'):
        d = Driver('', chromium_options.address)
        browser_id = d.run('Target.getTargets')['targetInfos'][0]['browserContextId']
        is_headless = 'headless' in d.run('Browser.getVersion')['userAgent'].lower()
        d.stop()
        return is_headless, browser_id, True, False

    is_exists, is_local = connect_browser(chromium_options)
    try:
        s = Session()
        s.trust_env = False
        s.keep_alive = False
        ...
        raise BrowserConnectError(_S._lang.BROWSER_CONNECT_ERR2)
    return is_headless, browser_id, is_exists, is_local
