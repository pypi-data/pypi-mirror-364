...
class Ifile
{
  public:
    ...
    template <class TTT> friend std::istream &operator>>(Ifile &in, TTT &x) { return (*(in.s) >> x); }
...
class Ofile
{
  public:
    ...
    template <class T> friend std::ostream &operator<<(Ofile &out, const T &x) { return (*(out.s)) << x; }
