...
def benchmark_models(
    ...
    bench_start_time = time.perf_counter_ns()
    time_spans = []
    for i in range(num_models):
        if device == "pci:0":
            time_span = benchmark_model(
                model_paths[i], num_inferences, device, percentile
            )
        else:
            raise Exception(f"Invalid device type: {device}")
        time_spans.append(time_span)

    bench_time_span = time.perf_counter_ns() - bench_start_time
...
def benchmark_model(
    ...
    interpreter = make_interpreter(model_path, device=device)
    ...
    for _ in range(num_inferences):
        ...
        interpreter.invoke()
        time_span = (time.perf_counter_ns() - start_time) / (1_000_000)
        time_spans.append(time_span)
