...
class OutputGenerator:
    ...
    def generate_code_security_analysis(self) -> Path:
        ...
        # Get best_practices from DCP (legacy storage - now handled by evidence-based system)
        try:
            from coppersun_brass.core.dcp_adapter import DCPAdapter
            dcp = DCPAdapter()
            all_observations = dcp.get_observations()
            best_practices = [obs for obs in all_observations if obs.get('type') == 'best_practice'][:50]
            logger.info(f"OUTPUT_GENERATOR: Retrieved {len(best_practices)} best_practice observations from DCP")
        except Exception as e:
            logger.warning(f"OUTPUT_GENERATOR: Failed to get best_practices from DCP: {e}")
            best_practices = self.storage.get_observations_by_type('best_practice', limit=50)
        ...
        content = self._build_all_data_types_analysis(
            ...
            code_entities, code_metrics, persistent_issues, evolution_report, best_practices
        )
    ...
    def _build_all_data_types_analysis(self, todos, code_issues, security_issues, 
                                     entities, metrics, persistent, evolution, best_practices):
    ...
    def _generate_strategic_recommendations(self, security, todos, code_issues, entities, persistent, best_practices):
        ...
        logger.info(f"_generate_strategic_recommendations: received {len(best_practices)} best_practices")
        for i, bp in enumerate(best_practices):
            logger.info(f"  BP {i}: keys = {list(bp.keys())}")
            if 'data' in bp:
                logger.info(f"    data keys = {list(bp['data'].keys())}")
                logger.info(f"    top_recommendations = {len(bp['data'].get('top_recommendations', []))}")
            if 'details' in bp:
                logger.info(f"    details keys = {list(bp['details'].keys())}")
                logger.info(f"    top_recommendations = {len(bp['details'].get('top_recommendations', []))}")
        
        # Best Practices recommendations (OWASP/NIST) with deduplication
        seen_recommendations = set()
        if best_practices:
            logger.info(f"STRATEGIC: Processing {len(best_practices)} best_practices observations")
            for i, bp in enumerate(best_practices):
                # Handle complex nested data structures
                if 'data' in bp:
                    bp_data = bp['data']
                    # Check if data has nested details
                    if 'details' in bp_data:
                        bp_data = bp_data['details']
                elif 'details' in bp:
                    bp_data = bp['details']
                else:
                    bp_data = {}
                top_recommendations = bp_data.get('top_recommendations', [])
                logger.info(f"STRATEGIC: Obs {i}: top_recommendations = {len(top_recommendations)}")
                
                # Process each sophisticated recommendation with full details and deduplication
                for rec in top_recommendations[:5]:  # Limit to top 5 recommendations
                    rec_title = rec.get('title', 'Best Practice Recommendation')
                    rec_description = rec.get('description', '')
                    rec_implementation = rec.get('implementation_guide', '')
                    rec_rationale = rec.get('rationale', '')
                    rec_references = rec.get('references', [])
                    rec_severity = rec.get('severity', 'medium')
                    
                    # Create unique key for deduplication based on title and description
                    rec_key = f"{rec_title}:{rec_description}"
                    if rec_key in seen_recommendations:
                        logger.info(f"DEDUPLICATION: Skipping duplicate {rec_title}")
                        continue
                    seen_recommendations.add(rec_key)
                    logger.info(f"DEDUPLICATION: Adding {rec_title} (total seen: {len(seen_recommendations)})")
                    
                    # Map severity to priority for display
                    if rec_severity == 'critical':
                        priority = 90
                        icon = "🛡️"
                    elif rec_severity == 'important':
                        priority = 70
                        icon = "🔒"
                    else:
                        priority = 50
                    ...
                    # Add sophisticated OWASP/NIST recommendations with full details
                    if priority >= 70:
                        rec_line = f"{icon} **{rec_title}** (Priority {priority})"
                        if rec_description:
                            rec_line += f"\n  - *Description*: {rec_description}"
                        if rec_implementation:
                            rec_line += f"\n  - *Implementation*: {rec_implementation}"
                        if rec_rationale:
                            rec_line += f"\n  - *Why*: {rec_rationale}"
                        if rec_references:
                            ref_links = ", ".join(rec_references)
                            rec_line += f"\n  - *References*: {ref_links}"
                        recommendations.append(rec_line)
                
                # Fallback for old format if no top_recommendations
                if not top_recommendations:
                    logger.warning(f"STRATEGIC: Obs {i}: FALLBACK TRIGGERED - no top_recommendations found")
                    logger.warning(f"STRATEGIC: bp_data keys = {list(bp_data.keys())}")
                    priority = bp_data.get('priority', 50)
                    title = bp_data.get('title', 'Best Practice Recommendation')
                    if priority >= 70:
                        recommendations.append(f"🛡️ **{title}** (Priority {priority})")
                    elif priority >= 50:
                        recommendations.append(f"🔒 **{title}** (Priority {priority})")
