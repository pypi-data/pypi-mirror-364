...
# Best practices engine integration
try:
...
class ContextManager:
    ...
    def __init__(self, project_root: Path = Path.cwd()):
        ...
        # Initialize best practices engine if available
        self.best_practices_engine = None
        if BEST_PRACTICES_AVAILABLE:
            try:
                self.best_practices_engine = BestPracticesEngine(str(project_root))
                logger.debug("BestPracticesEngine initialized for ContextManager")
            except Exception as e:
                logger.warning(f"Could not initialize BestPracticesEngine: {e}")
                self.best_practices_engine = None
    ...
    def generate_insights(self):
        ...
        # Best practices - use intelligent engine if available
        best_practices_content = self._generate_best_practices_content()
        content.extend(best_practices_content)
        
        # Write insights file
        with open(self.insights_file, 'w') as f:
            f.write('\n'.join(content))
    
    def _generate_best_practices_content(self) -> List[str]:
        """Generate best practices content using the intelligent engine or fallback."""
        content = []
        
        if self.best_practices_engine:
            try:
                # Get mock observations to analyze current project
                mock_observations = self._create_mock_observations()
                
                # Generate intelligent recommendations
                recommendations = self.best_practices_engine.generate_recommendations(
                    observations=mock_observations,
                    limit=8  # Reasonable number for insights display
                )
                
                if recommendations:
                    content.append("## 📚 Best Practices Recommendations")
                    content.append("")
                    ...
                    for category, recs in by_category.items():
                        ...
                        for rec in recs:
                            ...
                            implementation = rec.get('recommendation', '')
                            ...
                            content.append(f"- {severity_badge} **{title}**: {description}")
                            if implementation:
                                content.append(f"  - *Implementation*: {implementation}")
                        
                        content.append("")
                    
                    content.append("*Recommendations generated by Copper Sun Brass AI*")
                    content.append("")
                    
                    logger.info(f"Generated {len(recommendations)} intelligent best practices recommendations")
                    return content
                
            except Exception as e:
                logger.warning(f"Failed to generate intelligent best practices: {e}")
        
        # Fallback to hardcoded practices
    ...
    def _create_mock_observations(self) -> List[Dict[str, Any]]:
        """Create mock observations based on project analysis for best practices engine."""
        observations = []
        
        # Analyze project structure to create relevant observations
        tech_stack = self._identify_tech_stack()
        file_stats = self._get_file_statistics()
        
        # Add tech stack observations
        for tech, description in tech_stack.items():
            observations.append({
                'type': 'tech_analysis',
                'summary': f'Project uses {tech}: {description}',
                'technology': tech
            })
        
        # Add file type observations
        for ext, count in file_stats.items():
            if ext in ['.py', '.js', '.ts', '.jsx', '.tsx', '.java', '.go', '.rs']:
                observations.append({
                    'type': 'code_analysis', 
                    'summary': f'Found {count} {ext} files',
                    'file_type': ext,
                    'count': count
                })
        
        # Add security-focused observation if we have many files
        total_files = sum(file_stats.values())
        if total_files > 50:
            observations.append({
                'type': 'security_analysis',
                'summary': f'Large codebase with {total_files} files - security review recommended'
            })
        
        # Add testing observation based on test file presence
        ...
        if not has_tests:
            observations.append({
                'type': 'test_analysis',
                'summary': 'No test files detected - test coverage improvement needed'
            })
