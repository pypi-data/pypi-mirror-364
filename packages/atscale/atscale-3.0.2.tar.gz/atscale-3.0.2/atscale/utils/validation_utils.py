...
def validate_role_play_format(
    ...
    Returns:
        None.
    """
    # role play should be of pattern `.*\{0\}.*`
    role_play_regex_pattern = ".*\{0\}.*"

    role_play_is_valid = bool(
        search(
            pattern=role_play_regex_pattern,
            string=role_play,
        )
    )

    if not role_play_is_valid:
        raise ValueError(
            f'`role_play` value: `{role_play}` is invalid; it must have the form {role_play_regex_pattern} (i.e., it must contain the substring `"{{0}}"`)'
        )
