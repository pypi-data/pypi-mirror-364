...
def _create_managed_logger(execution_id: str):
    ...
    The logger is equipped with two handlers:
    1. A StreamHandler so that logs still appear on stdout/stderr
    2. An ExecutionLogHandler that forwards the message to
    the backend when we have a managed, non-test execution
    """

    logger = logging.getLogger(f"va.execution.{execution_id}")

    if not is_test_execution(execution_id):
        logger.addHandler(ExecutionLogHandler(execution_id))
