...
class TestAppConfig(BaseModel):
    ...
    logging: TestLoggingConfig = TestLoggingConfig()
    debug: bool = False
...
class TestConfigManager:
    ...
    def config_manager(self, temp_config_dir):
        ...
        manager = ConfigManager[TestAppConfig](
            config_model=TestAppConfig, program_name="TEST_APP", config_paths=config_paths, env="test"
        )
    ...
    def test_env_vars(self):
        ...
        with patch.dict(os.environ, {"TEST_APP_DATABASE_HOST": "env.db", "TEST_APP_DEBUG": "true"}):
            config_manager = ConfigManager[TestAppConfig](
                config_model=TestAppConfig, program_name="TEST_APP", config_paths=[], env="test"
            )
    ...
    def test_loads_toml_file_config(self, temp_config_dir):
        ...
        config_manager = ConfigManager[TestAppConfig](
            config_model=TestAppConfig, program_name="TEST_APP", config_paths=[config_file], env="test"
        )
    ...
    def test_has_config_method(self, config_manager):
        ...
        assert config_manager.has_config(TestDatabaseConfig) is True
        assert config_manager.has_config(TestLoggingConfig) is True
    ...
    def test_get_config_method(self, config_manager):
        ...
        db_config = config_manager.get_config(TestDatabaseConfig)
        assert isinstance(db_config, TestDatabaseConfig)
        assert db_config.host == "localhost"

        logging_config = config_manager.get_config(TestLoggingConfig)
        assert isinstance(logging_config, TestLoggingConfig)
        assert logging_config.level == "INFO"

    def test_config_reload(self, temp_config_dir, config_manager):
    ...
    def test_nullable_string_validator(self):
