...
def reset_time_left(request, group=None, fn=None, key=None, rate=None, method=ALL, time_left=None):
    ...
    print(f"Reset time_left - Window: {window}, Current time: {int(time.time())}")
    print(f"Cache key: {cache_key}")
    print(f"Setting time_left to: {time_left}")

    try:
        current_count = cache.get(cache_key, 0)
        print(f"Current count before reset: {current_count}")
        cache.set(cache_key, current_count, int(time_left) + EXPIRATION_FUDGE)
        verify_count = cache.get(cache_key, 'NOT_FOUND')
        print(f"Count after reset: {verify_count}")
        return {
            'count': current_count,
            'limit': limit,
...
def set_count(request, group=None, fn=None, key=None, rate=None, method=ALL, count=None):
    ...
    try:
        cache.set(cache_key, count, period + EXPIRATION_FUDGE)
        time_left = window - int(time.time())
