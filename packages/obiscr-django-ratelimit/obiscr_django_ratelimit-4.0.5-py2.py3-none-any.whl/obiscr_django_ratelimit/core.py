...
from obiscr_django_ratelimit import ALL, UNSAFE


__all__ = ['is_ratelimited', 'get_usage', 'reset_time_left', 'set_count']
...
def reset_time_left(request, group=None, fn=None, key=None, rate=None, method=ALL, time_left=None):
    if group is None and fn is None:
        raise ImproperlyConfigured('reset_time_left must be called with either '
                                   '`group` or `fn` arguments')
    ...
    if time_left is None:
        raise ValueError("time_left parameter is required")

    if not isinstance(time_left, (int, float)) or time_left < 0:
        raise ValueError("time_left must be a non-negative number")

    window = _get_window(value, period)
    cache_name = getattr(settings, 'RATELIMIT_USE_CACHE', 'default')
    cache = caches[cache_name]
    old_cache_key = _make_cache_key(group, window, rate, value, method)

    current_count = cache.get(old_cache_key, 0)
    cache.delete(old_cache_key)

    new_window = int(time.time()) + time_left
    new_cache_key = _make_cache_key(group, new_window, rate, value, method)

    try:
        cache.set(new_cache_key, current_count, time_left + EXPIRATION_FUDGE)

        return {
            'count': current_count,
            'limit': limit,
            'should_limit': current_count > limit,
            'time_left': time_left,
        }
    except Exception:
        if getattr(settings, 'RATELIMIT_FAIL_OPEN', False):
...
def set_count(request, group=None, fn=None, key=None, rate=None, method=ALL, count=None):
    if group is None and fn is None:
        raise ImproperlyConfigured('set_count must be called with either '
                                   '`group` or `fn` arguments')
    ...
    old_window = _get_window(value, period)
    cache_name = getattr(settings, 'RATELIMIT_USE_CACHE', 'default')
    cache = caches[cache_name]
    old_cache_key = _make_cache_key(group, old_window, rate, value, method)

    cache.delete(old_cache_key)

    new_window = int(time.time()) + period
    new_cache_key = _make_cache_key(group, new_window, rate, value, method)

    try:
        cache.set(new_cache_key, count, period + EXPIRATION_FUDGE)
        time_left = new_window - int(time.time())
