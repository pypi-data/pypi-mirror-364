...
def is_ratelimited(request, group=None, fn=None, key=None, rate=None,
                   method=ALL, increment=False, reset_time_left=None, set_count=None):
    usage = get_usage(request, group, fn, key, rate, method, incremen, reset_time_left, set_count)
    if usage is None:
        return False
...
def get_usage(request, group=None, fn=None, key=None, rate=None, method=ALL,
              increment=False, reset_time_left=None, set_count=None):
    ...
    if reset_time_left is not None:
        time_left = reset_time_left
    else:
        time_left = window - int(time.time())

    if set_count is not None:
        try:
            cache.set(cache_key, set_count, period + EXPIRATION_FUDGE)
            count = set_count
        except:
            pass
