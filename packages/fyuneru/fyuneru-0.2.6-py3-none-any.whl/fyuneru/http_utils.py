...
def write_content(
    ...
    """写入内容"""
    while (content := get_content(url, session, proxies)) is None:
        continue
...
def batch_download(
    ...
    host_grouped_urls = defaultdict(list)
    for url in urls.values():
        host_grouped_urls[urlparse(url).hostname].append(url)
    for host, urls in host_grouped_urls.items():
        ...
        Parallel(n_jobs=n_jobs, backend="threading")(
            ...
            for write_path, url in urls.items()
        )
