...
def run(script_path, python_path, debug, cloud_config):
    """Run the workflow on your script"""
    try:
        # TODO: the is taken from databricks config file as the starting script dir... this is then set into a job... and run as usual...
        ...
        if cloud_config:
            ...
            baseline_code = get_code_from_databricks_config(config['code_path']) #TODO: check over.
            project_path = config['volume_uri'] #TODO:  check over. not needed for POC.
        elif script_path:
        ...
        with yaspin(text="Initializing workflow...", color="magenta") as spinner:
            asyncio.run(
                workflow_runner.run_workflow(
                    baseline_code, python_path, project_path, config, spinner, debug
                )
            )
        click.echo()
    except Exception as e:
