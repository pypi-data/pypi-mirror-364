...
(function() {
     ...
     * 驗證 base64 數據
     */
    FileUploadManager.prototype.validateBase64 = function(base64Data, fileName) {
        if (!base64Data || typeof base64Data !== 'string') {
            throw new Error(`${fileName}: Base64 數據無效`);
        }

        const trimmed = base64Data.trim();
        if (trimmed.length === 0) {
            throw new Error(`${fileName}: Base64 數據為空`);
        }

        // 檢查 base64 格式
        const base64Regex = /^[A-Za-z0-9+/]*={0,2}$/;
        if (!base64Regex.test(trimmed)) {
            throw new Error(`${fileName}: Base64 格式無效`);
        }

        // 檢查長度是否合理（base64 長度應該是 4 的倍數）
        if (trimmed.length % 4 !== 0) {
            throw new Error(`${fileName}: Base64 長度無效`);
        }
        ...
        Promise.all(promises)
            .then(function(base64Results) {
                base64Results.forEach(function(base64, index) {
                    ...
                    try {
                        // 驗證 base64 數據
                        const validatedBase64 = self.validateBase64(base64, file.name);
                        ...
                        console.log('✅ 檔案已添加:', file.name, '大小:', file.size, 'Base64長度:', validatedBase64.length);

                        if (self.onFileAdd) {
                            self.onFileAdd(fileData);
                        }
                    } catch (error) {
                        console.error('❌ 檔案驗證失敗:', error.message);
            ...
            .catch(function(error) {
                ...
                // 提供更詳細的錯誤信息
                let errorMessage = '檔案處理失敗，請重試';
                if (error.message) {
                    if (error.message.includes('base64')) {
                        errorMessage = '圖片格式無效或損壞，請嘗試其他圖片';
                    } else if (error.message.includes('FileReader')) {
                        errorMessage = '檔案讀取失敗，請檢查檔案是否完整';
                    } else {
                ...
                const message = window.i18nManager ?
                    window.i18nManager.t('fileUpload.processingFailed', errorMessage) :
                    errorMessage;
        ...
        return new Promise(function(resolve, reject) {
            ...
            reader.onload = function() {
                try {
                    ...
                    console.log('✅ Base64 轉換成功:', file.name, '長度:', base64Data.length);
                    resolve(base64Data);
                } catch (error) {
                    console.error('❌ Base64 轉換失敗:', file.name, error);
                    reject(new Error(`Base64 conversion failed for ${file.name}: ${error.message}`));
                }
            };
            reader.onerror = function(error) {
                console.error('❌ FileReader 錯誤:', file.name, error);
                reject(new Error(`FileReader error for ${file.name}: ${error.message}`));
            };
