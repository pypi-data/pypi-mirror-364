        ...
        for img in images:
            try:
                ...
                if isinstance(img["data"], str):
                    try:
                        # 先驗證 base64 格式
                        base64_data = img["data"].strip()
                        if not base64_data:
                            debug_log(f"圖片 {img['name']} base64 數據為空")
                            continue
                        ...
                        if not base64_pattern.match(base64_data):
                            debug_log(f"圖片 {img['name']} base64 格式無效")
                            continue
                        ...
                        debug_log(f"Base64 數據長度: {len(img['data']) if img['data'] else 0}")
                        debug_log(f"Base64 數據預覽: {img['data'][:50] if img['data'] else 'None'}...")
                        continue
                else:
                    image_bytes = img["data"]

                if len(image_bytes) == 0:
                    debug_log(f"圖片 {img['name']} 解碼後數據為空，跳過")
                    continue
