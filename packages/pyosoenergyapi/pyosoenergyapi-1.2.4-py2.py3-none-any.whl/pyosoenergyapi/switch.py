...
class OSOEnergySwitch:
    ...
    def enable_holiday_mode(self, device: OSOEnergySwitchData, period_days: int = 365):
        ...
        try:
            ...
            resp = self.session.api.enable_holiday_mode(device.device_id, start_date, end_date)
    ...
    def disable_holiday_mode(self, device: OSOEnergySwitchData):
        ...
        try:
            resp = self.session.api.disable_holiday_mode(device.device_id)
            if resp["original"] == 200:
                final = True
                self.session.get_devices()
...
class Switch(OSOEnergySwitch):
    ...
    def get_switch(self, device: OSOEnergySwitchData) -> OSOEnergySwitchData:
