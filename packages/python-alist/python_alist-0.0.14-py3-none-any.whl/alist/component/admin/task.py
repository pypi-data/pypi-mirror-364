...
class AlistUploadTaskList:
    ...
    def get(
        ...
        **request_kwargs, 
        ...
        "获取某个任务信息"
        def gen_step():
            resp = yield self.client.task_info(
                tid, 
                category=type(self).__category__, 
                request=self.async_request if async_ else self.request, 
                async_=async_, 
    ...
    def list_done(
        ...
        **request_kwargs, 
        ...
        "列出所有已完成任务"
        def gen_step():
            resp = yield self.client.task_done(
                category=type(self).__category__, 
                request=self.async_request if async_ else self.request, 
                async_=async_, 
                **request_kwargs, 
    ...
    def list_undone(
        ...
        **request_kwargs, 
        ...
        "列出所有未完成任务"
        def gen_step():
            resp = yield self.client.task_undone(
                category=type(self).__category__, 
                request=self.async_request if async_ else self.request, 
                async_=async_, 
                **request_kwargs, 
    ...
    def list(
        ...
        **request_kwargs, 
        ...
        "列出所有任务"
        def gen_step():
            undone = yield self.list_undone(
                async_=async_, 
                **request_kwargs, 
            )
            tasks = yield self.list_done(
                async_=async_, 
                **request_kwargs, 
            )
            if not tasks:
