...
int Chunks::copyDataIntoReadyCK(const void* data, uint32_t length) {
  ...
  DataChunk* ck = ready_cks.back();

  char* p_ck_r = (char*)ck + sizeof(*ck) + ck->r_ofs;
...
int Chunks::copyDataIntoFreeCK(const void* data, uint32_t length) {
  ...
  while (iter != this->free_cks.end() && length > 0) {
    DataChunk* ck = *iter;
    ++iter;
    char* f_buf_start = (char*)ck + sizeof(*ck) + ck->r_ofs;
...
int Chunks::copyDataIntoNewChunk(const void* data, uint32_t length) {
  int mem_size = ck_ceil_to_k(sizeof(DataChunk) + length);

  // new a chunk
  DataChunk* ck = (DataChunk*)malloc(mem_size);
  if (ck == NULL) {
    return -1;
  }
  ck->block_size = mem_size - sizeof(DataChunk);
  memcpy(ck + 1, data, length);
  ck->r_ofs = length;
  ck->l_ofs = 0;
...
void Chunks::reduceFreeCK() {
  if (!this->free_cks.empty()) {
    DataChunk* c = *this->free_cks.begin();
    this->free_cks.pop_front();
    this->ck_alloc_size -= (c->block_size + sizeof(DataChunk));
...
void Chunks::checkWaterLevel() {
  while (this->c_resident_size < this->ck_alloc_size && !this->free_cks.empty()) {
    this->reduceFreeCK();
  }
}

uint32_t Chunks::getAllocSize() const { return this->ck_alloc_size; }
...
Chunks::~Chunks() {
  while (!this->ready_cks.empty()) {
    DataChunk* c = this->ready_cks.front();
    free(c);
    ready_cks.pop_front();
  }
...
bool Chunks::useExistingChunk(uint32_t length) const {
  ...
  if (!this->ready_cks.empty()) {
    ...
    uint32_t available = c->block_size - c->r_ofs;
    if (available < length) {
      return false;
    } else {
      length -= available;
...
int Chunks::copyDataIntoChunks(const void* data, uint32_t length) {
  ...
  if (copyDataIntoNewChunk(p_in, length) == -1) {
    // exhausting memory
    return -1;
  }
...
int Chunks::drainOutWithPipe(std::function<int(const char*, uint32_t)> in_pipe_cb) {
  ...
  while (this->iter != this->ready_cks.end()) {
    DataChunk* cur = *iter;
    char* cur_buf = (char*)cur + sizeof(DataChunk) + cur->l_ofs;
    uint32_t cur_size = cur->r_ofs - cur->l_ofs;
    // call in_pipe_cb flush the data out
    int ret = in_pipe_cb(cur_buf, cur_size);
    if (ret <= 0) {
      return ret;
    } else if (ret == (int)cur_size) { // current chunk is done, drop current chunk
      // reset read offset
      cur->l_ofs = 0;
      // reset write offset
      cur->r_ofs = 0;
    ...
    } else if (ret < (int)cur_size) { // some are failed, resend at next time
...
void Chunks::resetChunks() {
  while (!this->ready_cks.empty()) {
    DataChunk* c = *this->ready_cks.begin();
    c->l_ofs = 0;
    c->r_ofs = 0;
    this->ready_cks.pop_front();
    this->ck_free_ck_capacity += c->block_size;
