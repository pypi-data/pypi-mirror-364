...
class CustomUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        # This mapping is for compatibility with old versions of sklearn models
        # Ensures that some user's old model files still work after sklearn upgrade
        mappings = {
            ('sklearn.linear_model.ridge', 'Ridge'): ('sklearn.linear_model', 'Ridge'),
            ('sklearn.preprocessing.data', 'MinMaxScaler'): ('sklearn.preprocessing', 'MinMaxScaler'),
            ...
            ('sklearn.cluster.k_means_', 'KMeans'): ('sklearn.cluster', 'KMeans'),
        ...
        if (module, name) in mappings:
            module, name = mappings[(module, name)]
        return super().find_class(module, name)


def read_with_pickle_from_stream(stream, check_version=True):
    obj = CustomUnpickler(stream).load()
    if check_version:
