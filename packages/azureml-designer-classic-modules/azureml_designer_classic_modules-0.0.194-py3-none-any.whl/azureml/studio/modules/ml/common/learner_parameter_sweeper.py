    ...
    def train(self, training_data: DataTable, label_column_selection, valid_data=None):
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                elif 'overflow encountered in square' in str(e):
                    ErrorMapping.throw(InvalidTrainingDatasetError(
                        action_name='fit',
                        data_name=training_data.name,
                        reason=f'Overflow encountered during training. {ErrorMapping.get_exception_message(e)}',
                        troubleshoot_hint=(
                            'Please check the range of values in the dataset to ensure there are no extreme values.'
                        )
                        ))
                else:
                    raise e
