...
class fastKNN:
    def __init__(self,
        ...
        self.k_df = k_df # assume it has label
        self.nn_df = nn_df

        # keep a copy for validation
        self.k_df_raw = k_df.copy()
        self.nn_df_raw = nn_df.copy()  
        self.sourced_nn_df_with_label = None,
        ...
        self.sourced_nn_df = pd.DataFrame()  # Initialize as empty DataFrame
    ...
    def indexing(self):
        ...
        try:
            ...
            # faiss
            index = faiss.IndexFlatL2(self.k_df.shape[1])
            index.add(self.nn_df)
            
            # Ensure k doesn't exceed database size
            effective_k = min(self.k, len(self.nn_df))
            self.D, self.I = index.search(self.k_df, effective_k)
    ...
    def fastKNN(self):
        ...
        self.indexing()
        self.sourced_nn_df = self.nn_df.iloc[self.indices]
        self.sourced_nn_df_with_label = self.nn_df_raw.iloc[self.indices]
        # self.sourced_nn_df = self.sourced_nn_df.drop_duplicates()
        return self.sourced_nn_df, self.sourced_nn_df_with_label

    def validate(self):
        if self.label not in self.nn_df_raw.columns:
        ...
        try:
            ...
            # labels in db_db
            self.raw_label_number = self.nn_df_raw[self.nn_df_raw[self.label]==1].shape[0]
            
            self.sourced_label_number = self.sourced_nn_df_with_label[self.sourced_nn_df_with_label[self.label]==1].shape[0] 

            print("Label before fastKNN: " + str(self.raw_label_number))
            print("Label after fastKNN: " + str(self.sourced_label_number))

            print("Number of rows before fastKNN: " + str(self.nn_df_raw.shape[0]))
            print("Number of rows after fastKNN: " + str(self.sourced_nn_df.shape[0]))
