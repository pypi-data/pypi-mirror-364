...
class FormConfigs:  # pylint: disable=[R0902]
    ...
    def editor_config(self) -> None:
        ...
        try:
            ...
            self.errors.raise_error_message(f"External editor failed with exit code {e.returncode}", exit_status=e.returncode)
        except FileNotFoundError:
            logger.error("External editor '%s' not found. Please ensure it is installed and in your PATH.", editor, exc_info=True)
            self.errors.raise_error_message(f"External editor '{editor}' not found.", exit_status=1)
        except Exception as e:  # pylint: disable=[W0718]
            logger.critical("An unexpected error occurred while opening external editor: %s", e, exc_info=True)
            self.errors.raise_error_message(f"An unexpected error occurred with editor: {e}", exit_status=1)
    ...
    def read_configs(self) -> None:
        ...
        try:
            ...
            self.errors.raise_error_message(f"Configuration file '{self.config_file}' not found.", exit_status=1)
        except Exception as e:  # pylint: disable=[W0718]
            logger.critical("Error reading original config file '%s': %s", self.config_file, e, exc_info=True)
            self.errors.raise_error_message(f"Error reading configuration file: {e}", exit_status=1)
    ...
    def write_configs(self, tags: list[str]) -> None:
        ...
        for key, new_value_str in zip(self.original_configs['CONFIGS'].keys(), tags):
            ...
            if new_value_str.lower() == 'true':
                ...
                try:
                    ...
                    self.errors.raise_error_message(f"Error parsing config file for key '{key}'. Invalid list format: '{new_value_str}'", 1)
        ...
        try:
            ...
            self.errors.raise_error_message(f"Error writing configuration file: {e}", exit_status=1)
