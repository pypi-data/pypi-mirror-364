...
class Md5sum:
    ...
    def read_binary_file(self, filename: Path) -> bytes:
        ...
        try:
            ...
            # This return is technically unreachable because raise_error_message calls sys.exit(1)
            # but it's kept for mypy's type checking.
            return b''
        except IOError as e:
            logger.critical("IOError: Failed to read binary file '%s': %s", filename, e, exc_info=True)
            self.errors.raise_error_message(f"Error reading file '{filename}': {e}", exit_status=1)
            return b''
        except Exception as e:  # pylint: disable=[W0718]
            logger.critical("An unexpected error occurred while reading binary file '%s': %s", filename, e, exc_info=True)
            self.errors.raise_error_message(f"An unexpected error occurred while reading file '{filename}': {e}", exit_status=1)
            return b''
