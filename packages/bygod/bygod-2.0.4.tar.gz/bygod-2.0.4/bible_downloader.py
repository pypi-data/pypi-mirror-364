...
License: MIT
Version: 2.0.4 - True Async Edition
"""
...
from meaningless.utilities.common import remove_superscript_numbers_in_passage, unicode_to_ascii_punctuation
...
class AsyncBibleDownloader:
    ...
    async def _discover_chapter_count(self, book: str) -> int:
        ...
        try:
            ...
            if matches:
                ...
                self.logger.debug(f"Found {max_chapter} chapters via text pattern for {book}")
                return max_chapter
            ...
            if book in known_chapters:
                self.logger.debug(f"Using known chapter count for {book}: {known_chapters[book]}")
    ...
    def _extract_verses_from_html(self, html_content: str) -> List[str]:
        """
        Extract verses from BibleGateway HTML content using meaningless library utilities.
        ...
        """
        try:
            # Use BeautifulSoup to extract text, then clean with meaningless utilities
            ...
            # Clean up verses using meaningless library utilities
            cleaned_verses = []
            for verse in verses:
                # Use meaningless library's superscript number removal
                cleaned = remove_superscript_numbers_in_passage(verse)
                
                # Remove verse numbers at the beginning (e.g., "1In the beginning...")
                cleaned = re.sub(r"^\d+", "", cleaned.strip())
                
                # Remove cross-references in parentheses (e.g., "(A)", "(B)", etc.)
                cleaned = re.sub(r"\([A-Z]\)", "", cleaned)
                
                # Remove footnote markers (e.g., "[a]", "[b]", etc.)
                cleaned = re.sub(r"\[[a-z]\]", "", cleaned)
                
                # Remove embedded verse numbers (e.g., "2The earth was...")
                cleaned = re.sub(r"(\d+)([A-Z][a-z])", r"\2", cleaned)
                ...
                # Fix spacing issues (e.g., "In thebeginning" -> "In the beginning")
                cleaned = re.sub(r"([a-z])([A-Z])", r"\1 \2", cleaned)
                
                # Fix additional spacing issues (e.g., "thebeginning" -> "the beginning")
            ...
            self.logger.error(f"Error extracting verses with meaningless library utilities: {e}")
            return []
