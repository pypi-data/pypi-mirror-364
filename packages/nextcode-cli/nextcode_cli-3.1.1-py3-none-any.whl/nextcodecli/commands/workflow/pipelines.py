...
def pipelines(ctx, pipeline_name, is_raw, is_details, is_manifest):
    ...
    if is_manifest:
        ...
        for p in pipelines:
            pipeline_options = {}
            for opt in p.get('options', []) if p.get('options') else []:
                pipeline_options[opt['name']] = opt['value']

            print_tab("Pipeline Name", p['name'])
            print_tab("Description", p['description'])
            print_tab("Script", p['script'])
            print_tab("Revision", p.get('revision', 'latest'))

            storage_type = pipeline_options.get("storage_type") or p.get('storage_type', 'shared')
            print_tab("Storage Type", storage_type)

            if storage_type == "dedicated":
                print_tab("Dedicated storage", pipeline_options.get("dedicated_storage_size", "1.2Ti"))
            if storage_type == "dedicated-basic":
                print_tab("Dedicated storage", pipeline_options.get("dedicated_storage_size", "50Gi"))

            print_tab("Local storage", pipeline_options.get("enable_fast_local_storage", "No"))
