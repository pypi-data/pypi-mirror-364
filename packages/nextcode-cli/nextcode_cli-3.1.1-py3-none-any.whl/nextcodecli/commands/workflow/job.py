...
@job.command()
@argument('link_type', required=True, type=click.Choice(['kibana', 'grafana', 'datadog']))
@pass_context
def link(ctx, link_type):
    ...
    if link_type == 'datadog':
        cluster_name = ctx.obj.session.root_info['cluster_name']
        start_time = int(start_date.replace(second=0, microsecond=0).timestamp() * 1000)
        end_time = int(end_date.replace(second=0, microsecond=0).timestamp() * 1000)
        if live:
            datadog_link = f'https://app.datadoghq.com/logs?query=cluster_name%3A{cluster_name}%20job-id%3A{job_id}&from_ts={start_time}&live=true'
        else:
            datadog_link = f'https://app.datadoghq.com/logs?query=cluster_name%3A{cluster_name}%20job-id%3A{job_id}&from_ts={start_time}&to_ts={end_time}&live=false'
        echo(datadog_link)
        click.launch(datadog_link)
