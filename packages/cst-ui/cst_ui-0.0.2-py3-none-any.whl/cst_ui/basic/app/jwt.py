...
from .data_admin import DataAdmin, evaluate_secret_key
from .my_types import Msg
from .my_types import _decode_payload_async
...
async def _handle_decode_errors(data: DataAdmin, key_login: str) -> Union[Dict[str, Any], bool]:
    ...
    try:
        ...
        decode = await _decode_payload_async(
            ...
            key_login=key_login,
            secret_key=(
                data.secret_key.secret if data.secret_key.secret is not None else data.secret_key.pem_key.public
            ),
...
def decode(key_login: str, data: DataAdmin) -> Union[Dict[str, Any], bool]:
    """decodes the jwt and updates the browser sessions.

    ### Parameters to use:
    * `key_login` : key used to store data in the client, also used in the `login` method of `DataAdmin`.
    * `data` : Instance object of the `DataAdmin` class.
    """
    assert not data._login_async, "Use the 'decode_async' method instead of 'decode'."
    assert data.secret_key is not None, "set the 'secret_key' in the class parameter (App)."

    return data.page.run_task(_handle_decode_errors, data, key_login).result()


async def decode_async(key_login: str, data: DataAdmin) -> Union[Dict[str, Any], bool]:
    """decodes the jwt and updates the browser sessions.
