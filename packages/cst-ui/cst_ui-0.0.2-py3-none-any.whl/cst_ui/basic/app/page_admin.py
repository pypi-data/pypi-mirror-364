...
MiddlewareHandler = Callable[[DataAdmin], Optional[Redirect]]
Middleware = List[MiddlewareHandler]
ViewHandler = Callable[[DataAdmin], View]
...
class PageAdmin:
    ...
    * `middleware` : It acts as an intermediary between different software components, intercepting and processing requests and responses. They allow adding functionalities to an application in a flexible and modular way. (optional)
    ...
    Example:
    ```python
    PageAdmin("/test/{id:d}/user/{name:l}", test_page, protected_route=True)
    ```
    """
...
class AddPageAdmin:
    """Creates an object to then add to the list of the `add_routes` method of the `FletApp` class.
    ...
    * route_prefix: text string that will bind to the url of the `page` decorator, example(`/users`) this will encompass all urls of this class. (optional)
    ...
    Example:
    ```python
    users = fs.AddPageAdmin(route_prefix="/user")
    ...
    @users.page("/task")
    async def task_page(data: fs.DataAdmin):
        page = data.page
    ...
    async def information_page(data: fs.DataAdmin):
        page = data.page
    ...
    def __decorator(self, data: Dict = None):
        def decorator(func: Callable):
            ...
            route = (
                (self.route_prefix if data.get("route") == "/" else self.route_prefix + data.get("route"))
    ...
    def page(
        ...
        * `middleware` : It acts as an intermediary between different software components, intercepting and processing requests and responses. They allow adding functionalities to an application in a flexible and modular way. (optional)
        ...
        counter = fs.AddPageAdmin(route_prefix="/counter")


        @counter.page("/", title="Counter")
        async def counter_page(data: fs.DataAdmin):
            view = data.view
