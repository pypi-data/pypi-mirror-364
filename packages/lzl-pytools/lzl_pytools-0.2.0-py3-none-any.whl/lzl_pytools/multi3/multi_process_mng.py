...
class TaskMonitorInfo:
    def __init__(self, obj=None) -> None:
        if obj is None:
            self.task_info = {'begin':0, 'end':0, 'success':0, 'fail':0, 'except':0, 'time':0, 'msg':0}
        else:
            self.task_info = obj
    def rec_start(self):
        self.task_info['begin'] += 1
    def rec_end(self, rslt, cost_time):
        self.task_info['end'] += 1
        if rslt in ['success', 'fail', 'except']:
            self.task_info[rslt] += 1
        self.task_info['time'] += cost_time
    def reverse_start(self):
        self.task_info['begin'] -= 1
    def rec_msg(self):
        self.task_info['msg'] += 1
    def clear(self):
        self.task_info = {'begin':0, 'end':0, 'success':0, 'fail':0, 'except':0, 'time':0, 'msg':0}
    def __str__(self):
        info = self.task_info
        avg_time = info['time'] / info['success'] if info['success'] > 0 else 0
        return f"msg/start/stop={info['msg']}/{info['begin']}/{info['end']},s/f/e={info['success']}/{info['fail']}/{info['except']},avg_time={avg_time:.3f}"
    def add_one(self, task_monitor_info):
        for k in self.task_info.keys():
            self.task_info[k] += task_monitor_info.task_info[k]
    def get_data(self):
...
class ProcessProc:
    ...
    def put_counter(self, force=False):
        ...
        if cur_time - self.put_counter_time >= self.counter_interval or force:
            self.put_msg('_counter', self.counter_info.get_data_and_clear())
    ...
    def run(self, max_task_num=-1, run_time_len=-1, callback=None, callback_interval=1, msg_callback=None):
        ...
        while True:
            ...
            if run_time_len > 0 and cur_time - start_time >= run_time_len:
                self.stop_event.set()
                break
            if max_task_num > 0 and self.consumer_counter.get_data()['end'] >= max_task_num:
                self.stop_event.set()
                break
        ...
        try:
            ...
            if msg_type == '_counter':
                if msg['p_type'] == PROC_TYPE_PRODUCER:
                    self.producer_counter.add_one(TaskMonitorInfo(msg['data']))
                elif msg['p_type'] == PROC_TYPE_CONSUMER:
                    self.consumer_counter.add_one(TaskMonitorInfo(msg['data']))
            elif msg_type == '_log':
