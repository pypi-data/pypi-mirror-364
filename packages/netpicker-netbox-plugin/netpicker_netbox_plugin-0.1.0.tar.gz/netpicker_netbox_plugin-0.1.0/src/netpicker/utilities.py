...
def traverse(obj, path):
    ...
    for step in steps:
        ...
        if not isinstance(result, (Manager, QuerySet)):
            ...
            backref = next((rel for rel in relations if rel.field.attname == attr and isinstance(result[0], rel.related_model)), None)
            if backref:
                linkage = backref.accessor_name
            else:
                fwd_items = target_model._meta._forward_fields_map.items()
                linkage = next((k for k, v in fwd_items if v.related_model and isinstance(result[0], v.related_model)), None)
