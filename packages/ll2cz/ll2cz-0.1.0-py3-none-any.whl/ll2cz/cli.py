...
def transform(
    ...
    limit: Annotated[int, typer.Option("--limit", help="Limit number of records to transform for screen output")] = 50,
) -> None:
...
def _show_all_tables_data(database: LiteLLMDatabase, limit: int) -> None:
    """Show data from all three LiteLLM tables individually."""
    ...
    for table_type, table_name, emoji in tables_to_show:
        if breakdown[f'{table_type}_spend'] > 0:
            ...
            _show_single_table_data(database, table_type, limit)
        else:
            console.print(f"\n[bold yellow]{emoji} {table_name}[/bold yellow]")
            console.print(f"[dim]No records found in {table_name}[/dim]")


def _show_single_table_data(database: LiteLLMDatabase, table_type: str, limit: int) -> None:
    """Show data from a specific LiteLLM table."""
    import polars as pl
    ...
    query = f'SELECT * FROM "{table_name}" ORDER BY date DESC, created_at DESC'
    if limit:
        query += f" LIMIT {limit}"
    ...
    try:
        ...
        if remote_check and cache_info.get('server_freshness'):
            ...
            if freshness.get('server_available', True):
                ...
                console.print(f"  Connection: Active")
...
def _display_test_payloads(cbf_data: "pl.DataFrame") -> None:
    """Display JSON payloads that would be sent to CloudZero API in test mode."""
    if cbf_data.is_empty():
        console.print("[yellow]No CBF data to display[/yellow]")
        return
    
    console.print(f"\n[bold yellow]🧪 Test Mode - JSON Payloads ({len(cbf_data)} records)[/bold yellow]")
    console.print("[dim]These are the JSON payloads that would be sent to CloudZero AnyCost API:[/dim]\n")
    
    # Convert to dicts for JSON display
    records = cbf_data.to_dicts()
    
    for i, record in enumerate(records, 1):
        console.print(f"[bold cyan]Record {i}:[/bold cyan]")
        
        # Pretty print JSON with proper formatting
        import json
        json_payload = json.dumps(record, indent=2, default=str)
        console.print(f"[white]{json_payload}[/white]")
        console.print()  # Add spacing between records
    
    # Show summary
    total_cost = sum(record.get('cost/cost', 0) for record in records)
    total_tokens = sum(record.get('usage/amount', 0) for record in records)
    
    console.print(f"[bold green]📋 Test Summary[/bold green]")
    console.print(f"  Records: {len(records)}")
    console.print(f"  Total Cost: ${total_cost:.6f}")
    console.print(f"  Total Tokens: {total_tokens:,}")
    console.print(f"\n[dim]💡 Remove --test flag to actually transmit these payloads to CloudZero[/dim]")
