...
class DataAnalyzer:
    ...
    def analyze(self, limit: int = 100, force_refresh: bool = False) -> dict[str, Any]:
    ...
    def _print_cbf_examples(self, cbf_examples: list[dict[str, Any]]) -> None:
        ...
        for cbf_record in cbf_examples:
            ...
            date_full = str(cbf_record.get('usage_start', 'N/A'))
            # Extract just the date part but show it completely
            date = date_full.split('T')[0] if 'T' in date_full else date_full
            cost = f"${cbf_record.get('cost', 0):.6f}"
            resource_id = str(cbf_record.get('resource_id', 'N/A'))  # Show full resource_id
            
            tokens = f"{cbf_record.get('usage_amount', 0):,}"
            
            # Extract dimensions
            dimensions = cbf_record.get('dimensions', {})
            entity_type = dimensions.get('entity_type', 'N/A')
            entity_id = dimensions.get('entity_id', '')
            entity = f"{entity_type}\n{entity_id}" if entity_id else entity_type
                
            model = dimensions.get('model', 'N/A')  # Show full model name
            provider = dimensions.get('provider', 'N/A')
    ...
    def czrn_analysis(self, limit: int | None = 100, force_refresh: bool = False) -> None:
    ...
    def _print_czrn_component_analysis(self, czrn_results: list[dict[str, Any]]) -> None:
        ...
        self.console.print(f"[dim]🔍 Use generated CZRNs as resource_id values in CloudZero CBF records[/dim]")
    ...
    def spend_analysis(self, limit: int | None = 100, force_refresh: bool = False) -> None:
    ...
    def _analyze_spend_trends(self, data: pl.DataFrame) -> None:
        ...
        self.console.print(f"[green]📊 Trend Summary[/green]")
