...
logger = CustomAdapter(original_logger, {'prefix': 'DH'})

"""
주식 데이터를 다루는 기본 클래스"""

from mystockutil.etc import remove_unnecessary_symbols

from mydatahandler.handler.handler_functions import get_recent_df
from mydatahandler.handler.update_upsert_df import update_df_with_another_df, upsert_df_with_similar_df
...
class StockDataHandler_property(StockDataHandler_sdh):
    # Fixme: 오늘이 휴일인 날짜에 대해서 처리할 점에 대해 고려할 것.
    def _check_today_is_last_day(self):
        if self.df.empty:
            logger.warning("DataFrame is empty. Cannot check if today is the last day.")
            return False
        # df의 인덱스의 마지막날짜가 오늘인지를 확인.
        if self.df.index.get_level_values('일자').unique()[-1] == pd.Timestamp.today().normalize():
            return True
        else:
            logger.warning(f"데이터의 마지막날짜와 오늘날짜가 다릅니다. {self.df.index.get_level_values('일자').unique()[-1]}")
            return False

    @staticmethod
    def check_today_is_last_day(func):
        """✅ 데코레이터: 메서드 실행 전 '오늘이 마지막 날인지' 확인"""
        @wraps(func)
        def wrapper(self, *args, **kwargs):
            self._check_today_is_last_day()  # 공통 검사
            return func(self, *args, **kwargs)  # 원래 메서드 실행
        return wrapper
...
class _StockDataHandler_base_ims(StockDataHandler_property):
    pass

class _StockDataHandler_manage(_StockDataHandler_base_ims):
...
class _StockDataHandler_add_del(_StockDataHandler_recent):
    ...
    def del_date(self, date:pd.Timestamp):
        ...
        self.set_data(df)
        logger.info(f"날짜 {date}에 해당하는 데이터를 삭제했습니다.")
...
if __name__ == "__main__":
    from mydataprovider.client.single_table_client import SingleTableClient
    
    client = SingleTableClient('rdf')
    res = client.fetch_from('2025-02-01')
    data = StockDataHandler()
    data.df = res
    StockDataHandler.tdf