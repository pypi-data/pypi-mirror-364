...
class AgentMarketplace:
    ...
    def post_offer(self, agent: Web3Agent, service_type: ServiceType,
                   description: str, price_eth: float, **kwargs) -> str:
    ...
    def post_request(self, agent: Web3Agent, service_type: ServiceType,
                    description: str, max_price_eth: float, **kwargs) -> str:
    ...
    def _create_match(self, offer: ServiceOffer, request: ServiceRequest):
        ...
        print(f"Match created: {offer.agent_name} -> {request.requester_name} for {agreed_price} ETH")
    
    def complete_match(self, match_id: str, result: Dict[str, Any],
                      attestation: Optional[Dict[str, Any]] = None):
    ...
    def _clean_expired(self):
        ...
        # Clean requests
        expired_requests = [
            rid for rid, request in self.requests.items()
            if request.is_expired()
        ]
        for rid in expired_requests:
            del self.requests[rid]
    
    def get_active_offers(self, service_type: Optional[ServiceType] = None) -> List[ServiceOffer]:
    ...
    def get_active_requests(self, service_type: Optional[ServiceType] = None) -> List[ServiceRequest]:
    ...
    def get_matches_for_agent(self, agent_address: str) -> List[ServiceMatch]:
        ...
        for match in self.matches.values():
            if (match.offer.agent_address == agent_address or
                match.request.requester_address == agent_address):
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "unique_providers": len(set(
                o.agent_address for o in self.offers.values()
            )),
            "unique_requesters": len(set(
                r.requester_address for r in self.requests.values()
            ))
        }
