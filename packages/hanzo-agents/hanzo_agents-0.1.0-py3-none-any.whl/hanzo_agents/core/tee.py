    ...
    def get_attestation(self) -> AttestationReport:
        ...
        return AttestationReport(
            ...
            metadata={
                ...
                ).hexdigest() if self.last_result else None
            }
        )
...
class ConfidentialAgent:
    ...
    def execute_confidential(
        ...
        """
        # Execute in TEE
        result = self.tee_executor.execute(
            task_code,
            inputs,
            self.tee_config
        )
...
def create_attestation_verifier_tool():
    ...
    class AttestationVerifierTool(Tool):
        ...
        async def compare_attestations(
            self,
            attestation1: Dict[str, Any],
            attestation2: Dict[str, Any]
        ) -> Dict[str, Any]:
    ...
    def _try_match_offers(self):
        ...
        for req_id, request in list(self.requests.items()):
            for offer_id, offer in list(self.offers.items()):
                ...
                if (offer.price_per_second <= request.max_price_per_second and
                    offer.max_duration >= request.required_duration and
                    (not request.required_provider or 
                     request.required_provider in offer.supported_providers)):
                    ...
                    match = {
                        ...
                        "total_price": offer.price_per_second * request.required_duration,
                        "timestamp": time.time()
                    }
