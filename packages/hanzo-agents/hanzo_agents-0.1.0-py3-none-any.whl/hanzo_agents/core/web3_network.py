...
from .wallet import generate_shared_mnemonic, derive_agent_wallet, WalletConfig
from .marketplace import AgentMarketplace, ServiceType, marketplace
...
class Web3Network(Network[S], Generic[S]):
            ...
            if isinstance(agent, Web3Agent):
                if not agent.wallet and agent.web3_config.wallet_enabled:
                    ...
                    wallet = derive_agent_wallet(
                        ...
                        agent_index=i,
                        network_rpc=agent.web3_config.wallet_config.network_rpc
                        if agent.web3_config.wallet_config else "mock://localhost"
                    )
                    agent.wallet = wallet
    ...
    async def run(self) -> S:
        ...
        try:
            ...
            while self.call_count < self.max_steps:
                ...
                if (self.deterministic_config.checkpoint_every_n_steps > 0 and
                    step % self.deterministic_config.checkpoint_every_n_steps == 0):
                ...
                pre_state = self.state.to_dict() if hasattr(self.state, 'to_dict') else str(self.state)
    ...
    def _record_execution(self, agent: Agent, result: InferenceResult, pre_state: Any):
        ...
        record = {
            ...
            "post_state": self.state.to_dict() if hasattr(self.state, 'to_dict') else str(self.state)
        }
    ...
    async def _handle_agent_economics(self, agent: Web3Agent, result: InferenceResult):
        ...
        task_fee = max(
            ...
            agent.web3_config.task_price_eth * self.network_economics.network_fee_percent
        )
    ...
    async def _checkpoint(self, name: str):
        ...
        # Save checkpoint
        checkpoint_file = self.checkpoint_dir / f"checkpoint_{name}_{int(time.time())}.json"
        ...
        for name, agent in self._agent_instances.items():
            if isinstance(agent, Web3Agent):
                ...
                stats[name] = {
                    ...
                    "type": "standard",
                    "calls": sum(
                        1 for entry in self.execution_log
                        if entry["agent"] == name
                    )
                }
    ...
    for agent in agents:
        if isinstance(agent, Web3Agent):
            ...
            config = Web3AgentConfig(
                wallet_enabled=enable_wallets,
                tee_enabled=enable_tee
            )
