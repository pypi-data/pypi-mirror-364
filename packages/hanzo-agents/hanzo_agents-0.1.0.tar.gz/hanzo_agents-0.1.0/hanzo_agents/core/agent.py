...
class Agent(ABC, Generic[S]):
    ...
    @abstractmethod
    async def run(self, state: S, history: "History", network: "Network[S]") -> InferenceResult:
...
def create_agent(name: str, description: str, tools: List[Tool] = None, 
                model: str = None, system_prompt: str = None) -> Type[Agent]:
