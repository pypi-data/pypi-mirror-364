...
from hanzo_agents import (
    Web3Agent, Web3AgentConfig, WalletConfig,
    Web3Network, State, ServiceType,
    AgentMarketplace, generate_shared_mnemonic,
    create_web3_network
)
from hanzo_agents.core.router import Router, create_router
...
class ProductManagerAgent(Web3Agent):
    ...
    async def _run_impl(self, state: State, history, network) -> Any:
        ...
        # Check team budget
        total_cost = architecture.get("estimated_cost", 0.5) + 0.2  # Add design/marketing
...
"""
            ...
            print(f"🔒 Secure build completed: {secure_build.get('result', {}).get('build_hash', 'N/A')}")
...
async def run_startup_team_demo():
    ...
    print(f"Team mnemonic generated (first 3 words): {' '.join(mnemonic.split()[:3])}...")
    ...
    print(f"Architecture: {final_state.get('architecture', {}).get('backend')} + {final_state.get('architecture', {}).get('frontend')}")
    print(f"Design: {final_state.get('design_system', {}).get('pages_designed', 0)} pages designed")
    print(f"Frontend: {final_state.get('frontend_progress', {}).get('pages_completed', 0)} pages built")
    print(f"Backend: {final_state.get('backend_progress', {}).get('endpoints_created', 0)} endpoints")
    print(f"Deployment: {final_state.get('deployment_status', {}).get('health', 'Unknown')}")
    ...
    for agent_name, agent_stats in stats.items():
        if "balance_eth" in agent_stats:
            print(f"{agent_name}: {agent_stats['balance_eth']:.3f} ETH "
