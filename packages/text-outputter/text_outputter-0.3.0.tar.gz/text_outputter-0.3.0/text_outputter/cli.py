...
def main():
    ...
    parser.add_argument("-a", nargs='?', const=True, help="输出或写入文本 A（可指定文件路径）")
    parser.add_argument("-b", nargs='?', const=True, help="输出或写入文本 B")
    parser.add_argument("-c", nargs='?', const=True, help="输出或写入文本 C")
    ...
    if args.a:
        ...
        if isinstance(args.a, str):
            write_text_file(args.a, text)
        else:
            print(text)
    elif args.b:
        text = TEXTS['b']
        if isinstance(args.b, str):
            write_text_file(args.b, text)
        else:
            print(text)
    elif args.c:
        text = TEXTS['c']
        if isinstance(args.c, str):
            write_text_file(args.c, text)
        else:
            print(text)
    else:
        print("请使用 -a、-b 或 -c 参数输出或写入指定文本")
