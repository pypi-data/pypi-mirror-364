...
class Logger(logging.Logger):
    ...
    def create_logger(**kwargs):
        ...
        if kwargs.get('object', {}).get('developer_email'):
            print("Please change developer_email to developer_email_address in the logger object of component_id=" + str(kwargs['object']['component_id']) + " component_name=" + kwargs['object']['component_name'])  # noqa E501
            kwargs['object']['developer_email_address'] = kwargs['object'].pop('developer_email')

        # TODO Shall we support also camelCase?
        mandatory_fields = ('component_id', 'component_name',
                            'component_category', 'developer_email_address')
        ...
        if not all(k in kwargs.get('object', {}) for k in mandatory_fields):
            # TODO When running in debug_mode, a more specific error message, which field is missing
    ...
    def insert_to_payload_extra_vars(self, **kwargs):
        try:
            ...
            self.exception("Error while trying to login using user identification and password", exception)  # noqa
    ...
    def set_is_write_to_sql(self, is_write_to_sql: bool) -> None:
        ...
        self.info(f"_is_write_to_sql={self._is_write_to_sql}")
