...
@mcp.tool()
async def search_institutions(name: str, page: int = 0, per_page: int = 10) -> List[Dict[str, Any]]:
    """
    ...
    """
    return Institutions().search(name).paginate(page=page, per_page=per_page)

@mcp.tool()
async def search_authors(name: str, page: int = 0, per_page: int = 10) -> List[Dict[str, Any]]:
    """
    ...
    """
    return Authors().search(name).paginate(page=page, per_page=per_page)
...
async def search_works(query: str, page: int = 0, per_page: int = 10) -> List[Dict[str, Any]]:
    ...
    """
    return Works().search(query).paginate(page=page, per_page=per_page)
