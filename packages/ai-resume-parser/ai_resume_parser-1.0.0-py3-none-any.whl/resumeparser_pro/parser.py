...
class ResumeParserPro:
    ...
    def _create_parsing_prompt(self, text: str) -> str:
        ...
        return f"""You are an expert resume parser. Extract information with maximum accuracy.

CRITICAL REQUIREMENTS:
1. ALL duration values must be INTEGER MONTHS (e.g., "1.5 years" = 18 months)
2. Categorize skills properly (Programming, Frameworks, Tools, etc.)
3. Separate responsibilities from quantifiable achievements
4. Extract exact dates and calculate durations precisely
5. Include all contact information and social profiles

DURATION EXAMPLES:
- "2 years" → 24 months
- "1 year 6 months" → 18 months  
- "6 months" → 6 months
- "Present" → calculate to current date

RESUME TEXT:
{text}

Extract complete, accurate information. Return structured data with integer month values for all durations."""
