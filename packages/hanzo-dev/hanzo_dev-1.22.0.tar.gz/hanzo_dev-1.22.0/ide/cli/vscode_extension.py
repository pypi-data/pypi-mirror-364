...
def download_latest_vsix_from_github() -> str | None:
    ...
    """
    api_url = 'https://api.github.com/repos/hanzoai/ide/releases'
    try:
...
def attempt_vscode_extension_install():
    """
    Checks if running in a supported editor and attempts to install the IDE companion extension.
    ...
    flag_dir = pathlib.Path.home() / '.ide'
    flag_file = flag_dir / f'.{flag_suffix}_extension_installed'
    extension_id = 'ide.ide-vscode'
    ...
    if _is_extension_installed(editor_command, extension_id):
        print(f'INFO: IDE {editor_name} extension is already installed.')
    ...
    print(
        f'INFO: First-time setup: attempting to install the IDE {editor_name} extension...'
    )
    ...
    print(f'INFO: Will retry installation next time you run IDE in {editor_name}.')
...
def _is_extension_installed(editor_command: str, extension_id: str) -> bool:
    """
    Check if the IDE extension is already installed.
...
def _attempt_github_install(editor_command: str, editor_name: str) -> bool:
    ...
    try:
        ...
        if process.returncode == 0:
            print(
                f'INFO: IDE {editor_name} extension installed successfully from GitHub.'
            )
            github_success = True
        else:
            logger.debug(
...
def _attempt_bundled_install(editor_command: str, editor_name: str) -> bool:
    ...
    Uses the VSIX file packaged with the IDE installation.
    ...
    """
    try:
        vsix_filename = 'ide-vscode-0.0.1.vsix'
        with importlib.resources.as_file(
            importlib.resources.files('ide').joinpath(
...
def _attempt_marketplace_install(
    ...
    This method is currently unused as the IDE extension is not yet published
