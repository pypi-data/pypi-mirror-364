...
def display_banner(session_id: str) -> None:
    ...
    print_formatted_text(HTML(f'<grey>IDE CLI v{__version__}</grey>'))
...
def display_event(event: Event, config: IDEConfig) -> None:
...
def display_command_output(output: str) -> None:
    ...
    formatted_lines = []
    for line in lines:
        if line.startswith('[Python Interpreter') or line.startswith('ide@'):
...
def display_help() -> None:
    ...
    print_formatted_text(
        HTML(
            f'\n<grey>IDE CLI v{__version__}</grey>\n'
            '<gold>IDE CLI lets you interact with the IDE agent from the command line.</gold>\n'
        )
    )
    ...
    # Tips section
    print_formatted_text(
        'Some tips to get the most out of IDE:\n'
    ...
    # Footer
    print_formatted_text(
        HTML('<grey>Learn more at: https://docs.hanzo.ai/usage/getting-started</grey>')
    )
...
def create_prompt_session(config: IDEConfig) -> PromptSession[str]:
...
async def read_prompt_input(
    config: IDEConfig, agent_state: str, multiline: bool = False
) -> str:
    try:
...
async def read_confirmation_input(config: IDEConfig) -> str:
    try:
        prompt_session = create_prompt_session(config)
