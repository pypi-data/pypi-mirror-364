...
_LOCAL_CONFIG_FILE_PATH = Path.home() / '.ide' / 'config.toml'
...
def extract_model_and_provider(model: str) -> ModelInfo:
    ...
    if len(split) == 1:
        ...
        if split[0] in VERIFIED_IDE_MODELS:
            return ModelInfo(provider='ide', model=split[0], separator='/')
...
VERIFIED_PROVIDERS = ['ide', 'anthropic', 'openai', 'mistral']
