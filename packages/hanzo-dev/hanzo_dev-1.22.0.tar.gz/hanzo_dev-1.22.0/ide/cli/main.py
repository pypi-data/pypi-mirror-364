...
import ide.cli.suppress_warnings  # noqa: F401
...
from ide.cli.vscode_extension import attempt_vscode_extension_install
from ide.controller import AgentController
...
from ide.events import EventSource, EventStreamSubscriber
...
from ide.io import read_task
from ide.mcp.utils import add_mcp_tools_to_agent
from ide.memory.condenser.impl.llm_summarizing_condenser import (
    LLMSummarizingCondenserConfig,
)
from ide.microagent.microagent import BaseMicroagent
from ide.runtime import get_runtime_cls
...
async def run_session(
    ...
    if agent.config.enable_mcp:
        # Add IDE' MCP server by default
        _, ide_mcp_stdio_servers = IDEMCPConfigImpl.create_default_mcp_server_config(
            config.mcp_host, config, None
        )

        runtime.config.mcp.stdio_servers.extend(ide_mcp_stdio_servers)
    ...
    # Show IDE banner and session ID if not skipped
    if not skip_banner:
...
async def run_setup_flow(config: IDEConfig, settings_store: FileSettingsStore):
...
def run_alias_setup_flow(config: IDEConfig) -> None:
    """Run the alias setup flow to configure shell aliases.

    Prompts the user to set up aliases for 'ide' and 'oh' commands.
    Handles existing aliases by offering to keep or remove them.
    """
    print_formatted_text('')
    print_formatted_text(HTML('<gold>🚀 Welcome to IDE CLI!</gold>'))
    print_formatted_text('')
    ...
    if aliases_exist_in_shell_config():
        print_formatted_text(
            HTML(
                '<grey>We detected existing IDE aliases in your shell configuration.</grey>'
            )
        )
        print_formatted_text('')
        print_formatted_text(
            HTML('<grey>  • <b>ide</b> → uvx --python 3.12 --from hanzo-dev hanzo-dev</grey>')
        )
        print_formatted_text(
            HTML('<grey>  • <b>oh</b> → uvx --python 3.12 --from hanzo-dev hanzo-dev</grey>')
        )
        print_formatted_text('')
        print_formatted_text(
...
async def main_with_loop(loop: asyncio.AbstractEventLoop) -> None:
    ...
    config: IDEConfig = setup_config_from_args(args)
