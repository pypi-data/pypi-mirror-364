...
const outputChannel = vscode.window.createOutputChannel("IDE Debug");
...
/**
 * Creates a new IDE terminal with timestamp
 * @returns vscode.Terminal
 */
function createNewIDETerminal(): vscode.Terminal {
  const timestamp = new Date().toLocaleTimeString("en-US", {
    hour12: false,
    hour: "2-digit",
    minute: "2-digit",
  });
  const terminalName = `IDE ${timestamp}`;
  return vscode.window.createTerminal(terminalName);
}

/**
 * Finds an existing IDE terminal or creates a new one using safe detection
 * @returns vscode.Terminal
 */
function findOrCreateIDETerminal(): vscode.Terminal {
  const openHandsTerminals = vscode.window.terminals.filter((terminal) =>
    terminal.name.startsWith("IDE"),
  );
...
/**
 * Executes an IDE command using Shell Integration when available
 * @param terminal The terminal to execute the command in
 * @param command The command to execute
 */
function executeIDECommand(
      ...
      if (event.execution === execution) {
        if (event.exitCode === 0) {
          outputChannel.appendLine(
            "DEBUG: IDE command completed successfully",
          );
          ...
          outputChannel.appendLine(
            `DEBUG: IDE command exited with code ${event.exitCode}`,
          );
...
/**
 * Builds the IDE command with proper sanitization
...
function buildIDECommand(
  ...
  let commandToSend = `${activationCommand}ide`;
  ...
  if (options.filePath) {
    ...
    const safeFilePath = options.filePath.includes(" ")
      ? `"${options.filePath}"`
      : options.filePath;
    commandToSend = `${activationCommand}ide --file ${safeFilePath}`;
    ...
    const sanitizedTask = options.task
      .replace(/`/g, "\\`")
      .replace(/"/g, '\\"');
    commandToSend = `${activationCommand}ide --task "${sanitizedTask}"`;
  }
...
/**
 * Main function to start IDE in terminal with safe terminal reuse
 * @param options Command options
 */
function startIDEInTerminal(options: {
  task?: string;
  filePath?: string;
}): void {
  try {
    // Find or create terminal using safe detection
    const terminal = findOrCreateIDETerminal();
    ...
    // Build command
    const commandToSend = buildIDECommand(options, activationCommand);
    ...
    executeIDECommand(terminal, commandToSend);
  } catch (error) {
    vscode.window.showErrorMessage(`Error starting IDE: ${error}`);
  }
}
  ...
  const startWithFileContextDisposable = vscode.commands.registerCommand(
    ...
    () => {
      ...
      if (editor.document.isUntitled) {
        ...
        startIDEInTerminal({ task: contextualTask });
      } else {
