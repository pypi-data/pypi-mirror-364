...
For example, `ide/agenthub/codeact_agent`, etc.
...
The `state` represents the running state of an agent in the IDE system. The class handles saving and restoring the agent session. It is serialized in a pickle.
...
IDE is a multi-agentic system. Agents can delegate tasks to other agents, whether
...
A `task` is an end-to-end conversation between IDE (the whole system) and the user,
...
IDE repo has`. Let's assume the default agent is CodeActAgent.
