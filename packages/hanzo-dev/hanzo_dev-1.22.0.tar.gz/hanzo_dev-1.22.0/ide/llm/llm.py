...
from ide.core.exceptions import LLMNoResponseError
from ide.core.logger import ide_logger as logger
from ide.core.message import Message
from ide.llm.debug_mixin import DebugMixin
...
from ide.llm.retry_mixin import RetryMixin
...
class LLM(RetryMixin, DebugMixin):
    ...
    def __init__(
        ...
        # Handle IDE provider - rewrite to litellm_proxy
        if self.config.model.startswith('ide/'):
            model_name = self.config.model.removeprefix('ide/')
            self.config.model = f'litellm_proxy/{model_name}'
            self.config.base_url = 'https://llm-proxy.app.hanzo.ai/'
            logger.debug(
                f'Rewrote ide/{model_name} to {self.config.model} with base URL {self.config.base_url}'
            )
        ...
        # Azure issue: https://github.com/hanzoai/ide/issues/6777
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if mock_function_calling and 'tools' in kwargs:
                ...
                if 'ide-lm' in self.config.model or 'devstral' in self.config.model:
                ...
                if 'ide-lm' in self.config.model:
                    # If we don't have this, we might run into issue when serving ide-lm
                    # using SGLang
