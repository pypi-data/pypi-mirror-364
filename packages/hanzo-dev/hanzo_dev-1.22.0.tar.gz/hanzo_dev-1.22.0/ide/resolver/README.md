# IDE GitHub, GitLab & Bitbucket Issue Resolver 🙌
...
This tool allows you to use open-source AI agents based on [IDE](https://github.com/hanzoai/ide)
...
3. Copy `examples/ide-resolver.yml` to your repository's `.github/workflows/` directory
...
6. Set up any [custom configurations required](https://docs.hanzo.ai/usage/how-to/github-action#custom-configurations)

7. Usage:
   There are two ways to trigger the IDE agent:
   ...
   a. Using the 'fix-me' label:
      ...
      - The workflow will:
        1. Attempt to resolve the issue using IDE
...
Need help? Feel free to [open an issue](https://github.com/hanzoai/ide/issues) or email us at [contact@hanzo.ai](mailto:contact@hanzo.ai).
...
Note: IDE works best with powerful models like Anthropic's Claude or OpenAI's GPT-4. While other models are supported, they may not perform as well for complex issue resolution.
...
```bash
python -m ide.resolver.resolve_issue --selected-repo [OWNER]/[REPO] --issue-number [NUMBER]
```

For instance, if you want to resolve issue #100 in this repo, you would run:

```bash
python -m ide.resolver.resolve_issue --selected-repo hanzoai/ide --issue-number 100
```
...
```bash
poetry run python ide/resolver/resolve_issue.py --selected-repo hanzoai/ide --issue-number 100
```
...
```bash
python -m ide.resolver.send_pull_request --issue-number PR_NUMBER --issue-type pr
```
...
```bash
python -m ide.resolver.visualize_resolver_output --issue-number ISSUE_NUMBER --vis-method json
```
...
```bash
python -m ide.resolver.send_pull_request --issue-number ISSUE_NUMBER --username YOUR_GIT_USERNAME --pr-type draft
```

If you want to upload to a fork, you can do so by specifying the `fork-owner`:

```bash
python -m ide.resolver.send_pull_request --issue-number ISSUE_NUMBER --username YOUR_GIT_USERNAME --pr-type draft --fork-owner YOUR_GIT_USERNAME
```

## Providing Custom Instructions

You can customize how the AI agent approaches issue resolution by adding a repository microagent file at `.ide/microagents/repo.md` in your repository. This file's contents will be automatically loaded in the prompt when working with your repository. For more information about repository microagents, see [Repository Instructions](https://github.com/hanzoai/ide/tree/main/microagents#2-repository-instructions-private).

## Troubleshooting

If you have any issues, please open an issue on this GitHub, GitLab, or Bitbucket repo, we're happy to help!
Alternatively, you can [email us](mailto:contact@hanzo.ai) or join the IDE Slack workspace (see [the README](/README.md) for an invite link).
