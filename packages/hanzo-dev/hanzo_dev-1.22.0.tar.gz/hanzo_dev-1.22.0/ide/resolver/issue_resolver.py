...
from ide.core.config import AgentConfig, IDEConfig, SandboxConfig
from ide.core.config.utils import load_ide_config
...
from ide.resolver.utils import (
...
from ide.utils.async_utils import GENERAL_TIMEOUT, call_async_from_sync
...
class IssueResolver:
    ...
    def __init__(self, args: Namespace) -> None:
        ...
        self.app_config = self.update_ide_config(
            load_ide_config(),
            self.max_iterations,
            self.workspace_base,
    ...
    def update_sandbox_config(
        ...
        if (
            ...
            runtime_container_image = (
                f'ghcr.io/hanzoai/runtime:{ide.__version__}-nikolaik'
            )
        ...
        ide_config.sandbox.base_container_image = sandbox_config.base_container_image
        ide_config.sandbox.runtime_container_image = (
            sandbox_config.runtime_container_image
        )
        ide_config.sandbox.enable_auto_lint = sandbox_config.enable_auto_lint
        ide_config.sandbox.use_host_network = sandbox_config.use_host_network
        ide_config.sandbox.timeout = sandbox_config.timeout
        ide_config.sandbox.local_runtime_url = sandbox_config.local_runtime_url
        ide_config.sandbox.user_id = sandbox_config.user_id
    ...
    def initialize_runtime(
        ...
        logger.info('Checking for .ide/setup.sh script...')
        runtime.maybe_run_setup_script()

        # Setup git hooks if they exist
        logger.info('Checking for .ide/pre-commit.sh script...')
    ...
    async def resolve_issue(
        ...
        if self.repo_instruction is None:
            # Check for .ide_instructions file in the workspace directory
            ide_instructions_path = os.path.join(repo_dir, '.ide_instructions')
            if os.path.exists(ide_instructions_path):
                with open(ide_instructions_path, 'r') as f:  # noqa: ASYNC101
