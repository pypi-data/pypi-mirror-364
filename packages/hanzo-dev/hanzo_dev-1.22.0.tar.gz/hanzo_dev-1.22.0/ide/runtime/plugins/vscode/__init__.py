...
from ide.runtime.utils.system import check_port_available
...
class VSCodePlugin(Plugin):
    ...
    async def initialize(self, username: str) -> None:
        ...
        if username not in ['root', 'ide']:
            self.vscode_port = None
            self.vscode_connection_token = None
            logger.warning(
                'VSCodePlugin is only supported for root or ide user. '
        ...
        cmd = (
            ...
            f'sudo chown -R {username}:{username} /ide/.openvscode-server\n'
            f'cd {workspace_path}\n'
            f'exec /ide/.openvscode-server/bin/openvscode-server --host 0.0.0.0 --connection-token {self.vscode_connection_token} --port {self.vscode_port} --disable-workspace-trust\n'
            'EOF'
        )
