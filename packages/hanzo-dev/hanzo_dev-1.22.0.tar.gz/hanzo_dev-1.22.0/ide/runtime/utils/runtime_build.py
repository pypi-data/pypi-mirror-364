...
from ide.core.exceptions import AgentRuntimeBuildError
from ide.core.logger import ide_logger as logger
from ide.runtime.builder import DockerRuntimeBuilder, RuntimeBuilder
...
def build_runtime_image(
    ...
    If dry_run is False, it will also build the IDE runtime Docker image using the docker build folder.
    ...
    See https://docs.hanzo.ai/usage/architecture/runtime for more details.
    """
...
def prep_build_folder(
    ...
    ide_source_dir = Path(ide.__file__).parent
    project_root = ide_source_dir.parent
    logger.debug(f'Building source distribution using project root: {project_root}')

    # Copy the 'ide' directory (Source code)
    shutil.copytree(
        ide_source_dir,
        Path(build_folder, 'code', 'ide'),
...
def get_hash_for_lock_files(base_image: str) -> str:
    ...
    for file in ['pyproject.toml', 'poetry.lock']:
        src = Path(ide_source_dir, file)
        if not src.exists():
            src = Path(ide_source_dir.parent, file)
