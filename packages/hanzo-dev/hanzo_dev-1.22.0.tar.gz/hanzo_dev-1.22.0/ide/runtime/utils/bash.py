...
from ide.utils.shutdown_listener import should_continue
...
class BashSession:
    ...
    def initialize(self) -> None:
        ...
        session_name = f'ide-{self.username}-{uuid.uuid4()}'
        ...
        # Set up shell options to avoid issues
        self.pane.send_keys('set +H')  # Disable history expansion to avoid '!' issues
        time.sleep(0.1)
        
        # Use a heredoc to set PS1 to avoid complex escaping
        self.pane.send_keys("cat > /tmp/ps1_prompt.txt << 'EOF'")
        time.sleep(0.1)
        self.pane.send_keys(self.PS1)
        time.sleep(0.1)
        self.pane.send_keys('EOF')
        time.sleep(0.2)
        
        # Now set PS1 from the file
        self.pane.send_keys('export PS1="$(cat /tmp/ps1_prompt.txt)"')
        time.sleep(0.1)
        ...
        # Ensure PATH includes basic system directories
        self.pane.send_keys(
    ...
    def _clear_screen(self) -> None:
        ...
        # Only clear the tmux history, don't send C-l which can interfere with output
    ...
    def execute(self, action: CmdRunAction) -> CmdOutputObservation | ErrorObservation:
        ...
        if command != '':
            ...
            if is_input:
                ...
                logger.debug(f'ORIGINAL COMMAND: {command!r}')
                logger.debug(f'ESCAPED COMMAND: {escaped_command!r}')
                logger.debug(f'SENDING COMMAND: {escaped_command!r}')
                self.pane.send_keys(
                    escaped_command,
                    enter=not is_special_key,
                )
