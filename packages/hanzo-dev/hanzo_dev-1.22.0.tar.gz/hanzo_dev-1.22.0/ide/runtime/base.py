...
from ide.core.exceptions import AgentRuntimeDisconnectedError
...
from ide.events.serialization.action import ACTION_TYPE_TO_CLASS
...
from ide.runtime.plugins import (
    JupyterRequirement,
    PluginRequirement,
    VSCodeRequirement,
)
from ide.runtime.runtime_status import RuntimeStatus
from ide.runtime.utils.edit import FileEditRuntimeMixin
...
class Runtime(FileEditRuntimeMixin):
    ...
    This is an extension point in IDE that allows applications to customize how
    ...
    async def clone_or_init_repo(
        ...
        ide_workspace_branch = f'ide-workspace-{random_str}'
        ...
        checkout_command = (
            ...
            if selected_branch
            else f'git checkout -b {ide_workspace_branch}'
        )
    ...
    def maybe_run_setup_script(self):
        """Run .ide/setup.sh if it exists in the workspace or repository."""
    ...
    def maybe_setup_git_hooks(self):
        """Set up git hooks if .ide/pre-commit.sh exists in the workspace or repository."""
        pre_commit_script = '.ide/pre-commit.sh'
        ...
        if not isinstance(read_obs, ErrorObservation):
            # If the existing hook wasn't created by IDE, preserve it
            if 'This hook was installed by IDE' not in read_obs.content:
...
# This hook was installed by IDE
# It calls the pre-commit script in the .ide directory
...
"""
    ...
    def _is_gitlab_repository(self, repo_name: str) -> bool:
        ...
        try:
            ...
            # This is a safe fallback since we'll just use the default .ide
            return False
    ...
    def get_microagents_from_org_or_user(
        ...
        github.com/acme-co/.ide exists. If it does, it will clone it and load
        the microagents from the ./microagents/ folder.

        For GitLab repositories, it will use ide-config instead of .ide
        ...
        # For GitLab, use ide-config (since .ide is not a valid repo name)
        # For other providers, use .ide
        if is_gitlab:
            org_ide_repo = f'{org_name}/ide-config'
        else:
            org_ide_repo = f'{org_name}/.ide'

        self.log(
            'info',
            f'Checking for org-level microagents at {org_ide_repo}',
        )

        # Try to clone the org-level repo
        try:
            # Create a temporary directory for the org-level repo
            org_repo_dir = self.workspace_root / f'org_ide_{org_name}'
            self.log(
                'debug',
            ...
            try:
                ...
                self.log(
                    'error',
                    f'Failed to get authenticated URL for {org_ide_repo}: {str(e)}',
                )
            ...
            if isinstance(obs, CmdOutputObservation) and obs.exit_code == 0:
                self.log(
                    'info',
                    f'Successfully cloned org-level microagents from {org_ide_repo}',
                )
                ...
                self.log(
                    'info',
                    f'Loaded {len(loaded_microagents)} microagents from org-level repository {org_ide_repo}',
                )
                ...
                self.log(
                    'info',
                    f'No org-level microagents found at {org_ide_repo} (exit_code: {exit_code})',
                )
                self.log(
                    'debug',
            ...
            self.log(
                'debug',
                f'Error loading org-level microagents from {org_ide_repo}: {str(e)}',
            )
    ...
    def get_microagents_from_selected_repo(
        ...
        github.com/acme-co/.ide and load microagents from there if it exists.
        ...
        """
        loaded_microagents: list[BaseMicroagent] = []
        microagents_dir = self.workspace_root / '.ide' / 'microagents'
        repo_root = None
        ...
        if selected_repository:
            ...
            microagents_dir = repo_root / '.ide' / 'microagents'
        ...
        # Check for legacy .ide_instructions file
        obs = self.read(
            FileReadAction(path=str(self.workspace_root / '.ide_instructions'))
        )
        if isinstance(obs, ErrorObservation) and repo_root is not None:
            ...
            self.log(
                'debug',
                f'.ide_instructions not present, trying to load from repository {microagents_dir=}',
            )
            obs = self.read(FileReadAction(path=str(repo_root / '.ide_instructions')))

        if isinstance(obs, FileReadObservation):
            self.log('info', 'ide_instructions microagent loaded.')
            loaded_microagents.append(
                BaseMicroagent.load(
                    path='.ide_instructions',
                    microagent_dir=None,
    ...
    @classmethod
    def setup(cls, config: IDEConfig, headless_mode: bool = False):
        """Set up the environment for runtimes to be created."""

    @classmethod
    def teardown(cls, config: IDEConfig):
