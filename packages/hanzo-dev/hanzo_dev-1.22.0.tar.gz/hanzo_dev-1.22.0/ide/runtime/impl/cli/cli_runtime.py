...
if sys.platform == 'win32':
    try:
        from ide.runtime.utils.windows_bash import WindowsPowershellSession
        from ide.runtime.utils.windows_exceptions import DotNetMissingError
...
class CLIRuntime(Runtime):
    ...
    def _sanitize_filename(self, filename: str) -> str:
        ...
        if filename == '/workspace':
            ...
            # For absolute paths, we need to check against the resolved workspace path
            resolved_workspace = os.path.realpath(self._workspace_path)
            resolved_filename = os.path.realpath(filename)
            if not (filename.startswith(self._workspace_path) or resolved_filename.startswith(resolved_workspace)):
        ...
        # Also resolve the workspace path to handle symlinks consistently
        resolved_workspace = os.path.realpath(self._workspace_path)
    ...
    async def delete(cls, conversation_id: str) -> None:
        ...
        prefix = f'ide_workspace_{conversation_id}_'
