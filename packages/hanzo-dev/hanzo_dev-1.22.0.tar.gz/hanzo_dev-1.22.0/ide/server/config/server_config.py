...
from ide.server.types import AppMode, ServerConfigInterface
from ide.utils.import_utils import get_impl


class ServerConfig(ServerConfigInterface):
    config_cls = os.environ.get('IDE_CONFIG_CLS', None)
    ...
    settings_store_class: str = (
        'ide.storage.settings.file_settings_store.FileSettingsStore'
    )
    secret_store_class: str = (
        'ide.storage.secrets.file_secrets_store.FileSecretsStore'
    )
    conversation_store_class: str = (
        'ide.storage.conversation.file_conversation_store.FileConversationStore'
    )
    ...
    monitoring_listener_class: str = 'ide.server.monitoring.MonitoringListener'
    user_auth_class: str = (
        'ide.server.user_auth.default_user_auth.DefaultUserAuth'
    )
