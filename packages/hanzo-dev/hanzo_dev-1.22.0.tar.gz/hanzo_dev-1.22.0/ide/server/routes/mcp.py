...
from ide.integrations.bitbucket.bitbucket_service import BitBucketServiceImpl
from ide.integrations.github.github_service import GithubServiceImpl
from ide.integrations.gitlab.gitlab_service import GitLabServiceImpl
from ide.integrations.provider import ProviderToken
from ide.integrations.service_types import GitService, ProviderType
from ide.server.dependencies import get_dependencies
from ide.server.shared import ConversationStoreImpl, config, server_config
from ide.server.types import AppMode
from ide.server.user_auth import (
    get_access_token,
    get_provider_tokens,
    get_user_id,
)
...
HOST = f'https://{os.getenv("WEB_HOST", "app.hanzo.ai").strip()}'
CONVO_URL = HOST + '/conversations/{}'


async def get_convo_link(service: GitService, conversation_id: str, body: str) -> str:
    """
    Appends a followup link, in the PR body, to the IDE conversation that opened the PR
    """
...
async def create_pr(
    ...
    logger.info('Calling IDE MCP create_pr')

    request = get_http_request()
    headers = request.headers
    conversation_id = headers.get('X-IDE-ServerConversation-ID', None)
...
async def create_mr(
    ...
    logger.info('Calling IDE MCP create_mr')
...
async def create_bitbucket_pr(
    ...
    logger.info('Calling IDE MCP create_bitbucket_pr')
