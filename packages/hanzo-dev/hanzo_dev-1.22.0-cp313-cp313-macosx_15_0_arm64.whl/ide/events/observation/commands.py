    ...
    def matches_ps1_metadata(cls, string: str) -> list[re.Match[str]]:
        ...
        for match in CMD_OUTPUT_METADATA_PS1_REGEX.finditer(string):
            try:
                ...
                # Don't try to parse if it contains excessive escaping (likely the initial prompt setup)
                if metadata_str.count('\\\\') > 10:
                    continue
                json.loads(metadata_str)  # Try to parse as JSON
