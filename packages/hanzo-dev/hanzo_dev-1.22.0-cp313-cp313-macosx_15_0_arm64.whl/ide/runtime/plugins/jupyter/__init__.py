...
from ide.events.action import Action, IPythonRunCellAction
from ide.events.observation import IPythonRunCellObservation
from ide.runtime.plugins.jupyter.execute_server import JupyterKernel
...
class JupyterPlugin(Plugin):
    ...
    async def initialize(
        self, username: str, kernel_id: str = 'ide-default'
    ) -> None:
        ...
        if not is_local_runtime:
            ...
            poetry_prefix = (
                'cd /ide/code\n'
                'export POETRY_VIRTUALENVS_PATH=/ide/poetry;\n'
                'export PYTHONPATH=/ide/code:$PYTHONPATH;\n'
                'export MAMBA_ROOT_PREFIX=/ide/micromamba;\n'
                '/ide/micromamba/bin/micromamba run -n ide '
            )
        else:
            # LocalRuntime
            prefix = ''
            code_repo_path = os.environ.get('IDE_REPO_PATH')
            if not code_repo_path:
                raise ValueError(
                    'IDE_REPO_PATH environment variable is not set. '
