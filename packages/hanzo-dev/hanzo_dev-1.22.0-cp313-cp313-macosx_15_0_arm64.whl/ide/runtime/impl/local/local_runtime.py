...
from ide.events.serialization import event_to_dict, observation_from_dict
from ide.integrations.provider import PROVIDER_TOKEN_TYPE
from ide.runtime.impl.action_execution.action_execution_client import (
    ActionExecutionClient,
)
from ide.runtime.impl.docker.docker_runtime import (
    APP_PORT_RANGE_1,
    APP_PORT_RANGE_2,
...
from ide.runtime.plugins.vscode import VSCodeRequirement
from ide.runtime.runtime_status import RuntimeStatus
from ide.runtime.utils import find_available_tcp_port
from ide.runtime.utils.command import get_action_execution_server_startup_command
from ide.utils.async_utils import call_sync_from_async
from ide.utils.tenacity_stop import stop_if_should_exit
...
def check_dependencies(code_repo_path: str, check_browser: bool) -> None:
    ERROR_MESSAGE = 'Please follow the instructions in https://github.com/hanzoai/ide/blob/main/Development.md to install IDE.'
    ...
    if check_browser:
        ...
        from ide.runtime.browser.browser_env import BrowserEnv
...
class LocalRuntime(ActionExecutionClient):
    ...
    Args:
        config (IDEConfig): The application configuration.
    ...
    def __init__(
        ...
        logger.warning(
            ...
            'This is an experimental feature, please report issues to https://github.com/hanzoai/ide/issues. '
            '`run_as_ide` will be ignored since the current user will be used to launch the server. '
        ...
        if self.sid in _RUNNING_SERVERS:
            ...
            if self.config.workspace_base is not None:
                ...
                self._temp_workspace = tempfile.mkdtemp(
                    prefix=f'ide_workspace_{self.sid}',
                )
    ...
    def setup(cls, config: IDEConfig, headless_mode: bool = False):
        ...
        if should_check_dependencies:
            code_repo_path = os.path.dirname(os.path.dirname(ide.__file__))
...
def _create_server(
    ...
    temp_workspace = tempfile.mkdtemp(
        prefix=f'ide_workspace_{workspace_prefix}',
    )
    ...
    env['IDE_REPO_PATH'] = code_repo_path
...
def _get_plugins(config: IDEConfig) -> list[PluginRequirement]:
