...
It is responsible for executing actions received from IDE backend and producing observations.
...
from ide.core.exceptions import BrowserUnavailableException
...
from ide.runtime.browser import browse
from ide.runtime.browser.browser_env import BrowserEnv
from ide.runtime.file_viewer_server import start_file_viewer_server

# Import our custom MCP Proxy Manager
from ide.runtime.mcp.proxy import MCPProxyManager
from ide.runtime.plugins import ALL_PLUGINS, JupyterPlugin, Plugin, VSCodePlugin
from ide.runtime.utils import find_available_tcp_port
from ide.runtime.utils.bash import BashSession
from ide.runtime.utils.files import insert_lines, read_lines
from ide.runtime.utils.memory_monitor import MemoryMonitor
from ide.runtime.utils.runtime_init import init_user_and_working_directory
from ide.runtime.utils.system_stats import get_system_stats
from ide.utils.async_utils import call_sync_from_async, wait_all
    ...
    async def ainit(self):
        ...
        if 'agent_skills' in self.plugins and 'jupyter' in self.plugins:
            obs = await self.run_ipython(
                IPythonRunCellAction(
                    code='from ide.runtime.plugins.agent_skills.agentskills import *\n'
                )
            )
    ...
    async def _init_bash_commands(self):
        ...
        if is_local_runtime:
            if is_windows:
                ...
                INIT_COMMANDS.append('git config --file ./.git_config user.name "ide"')
                INIT_COMMANDS.append(
                    'git config --file ./.git_config user.email "ide@hanzo.ai"'
                )
                INIT_COMMANDS.append(
                ...
                # Linux/macOS, local
                base_git_config = (
                    'git config --file ./.git_config user.name "ide" && '
                    'git config --file ./.git_config user.email "ide@hanzo.ai" && '
            ...
            base_git_config = (
                'git config --global user.name "ide" && '
                'git config --global user.email "ide@hanzo.ai"'
            )
            INIT_COMMANDS.append(base_git_config)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--username', type=str, help='User to run as', default='ide')
