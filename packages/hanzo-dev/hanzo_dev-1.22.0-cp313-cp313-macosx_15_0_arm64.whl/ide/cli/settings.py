...
from ide.core.config.utils import OH_DEFAULT_AGENT
...
from ide.utils.llm import get_supported_llm_models


def display_settings(config: IDEConfig) -> None:
...
def save_settings_confirmation(config: IDEConfig) -> bool:
    return (
        cli_confirm(
            config,
...
async def modify_llm_settings_basic(
    config: IDEConfig, settings_store: FileSettingsStore
) -> None:
    ...
    try:
        ...
        if provider == 'ide':
            provider_models = [
                m for m in provider_models if m not in VERIFIED_IDE_MODELS
            ]
            provider_models = VERIFIED_IDE_MODELS + provider_models
        ...
        elif provider == 'ide' and VERIFIED_IDE_MODELS:
            # Use the first model in the VERIFIED_IDE_MODELS list as it's the best/newest
            default_model = VERIFIED_IDE_MODELS[0]
        else:
        ...
        # For IDE provider, directly show all verified models without the "use default" option
        if provider == 'ide':
            print_formatted_text(HTML('\n<grey>Available IDE models:</grey>'))

            # Create a list of models for the cli_confirm function
            model_choices = VERIFIED_IDE_MODELS
        ...
        if provider == 'ide':
            print_formatted_text(
                HTML(
                    '\nYou can find your IDE LLM API Key in the <a href="https://app.hanzo.ai/settings/api-keys">API Keys</a> tab of IDE Cloud: https://app.hanzo.ai/settings/api-keys'
                )
            )
