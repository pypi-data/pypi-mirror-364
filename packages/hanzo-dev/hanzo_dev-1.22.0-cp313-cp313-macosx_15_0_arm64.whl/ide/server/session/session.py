...
from ide.core.config.mcp_config import MCPConfig, IDEMCPConfigImpl
...
from ide.events.action import MessageAction, NullAction
...
class Session:
    ...
    def __init__(
        ...
        self.logger = IDELoggerAdapter(extra={'session_id': sid})
    ...
    async def initialize_agent(
        ...
        ide_mcp_server, ide_mcp_stdio_servers = (
            IDEMCPConfigImpl.create_default_mcp_server_config(
                self.config.mcp_host, self.config, self.user_id
            )
        )
        if ide_mcp_server:
            self.config.mcp.shttp_servers.append(ide_mcp_server)
        self.config.mcp.stdio_servers.extend(ide_mcp_stdio_servers)
