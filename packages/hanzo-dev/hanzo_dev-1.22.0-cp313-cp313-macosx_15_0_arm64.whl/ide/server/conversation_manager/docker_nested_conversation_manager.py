...
from ide.integrations.provider import PROVIDER_TOKEN_TYPE, ProviderHandler
...
from ide.server.config.server_config import ServerConfig
from ide.server.conversation_manager.conversation_manager import (
    ConversationManager,
)
from ide.server.data_models.agent_loop_info import AgentLoopInfo
...
from ide.server.session.session import ROOM_KEY, Session
...
from ide.storage.locations import get_conversation_dir
...
class DockerNestedConversationManager(ConversationManager):
    ...
    async def get_running_agent_loops(
        ...
        conversation_ids = {
            name[len('ide-runtime-') :]
            for name in names
            if name.startswith('ide-runtime-')
        }
    ...
    async def close_session(self, sid: str):
        ...
        try:
            container = self.docker_client.containers.get(f'ide-runtime-{sid}')
    ...
    async def get_agent_loop_info(
        ...
        for container in containers:
            if not container.name or not container.name.startswith(
                ...
                continue
            conversation_id = container.name[len('ide-runtime-') :]
    ...
    def get_nested_url_for_container(self, container: Container) -> str:
        ...
        conversation_id = container_name[len('ide-runtime-') :]
    ...
    async def _create_runtime(
        ...
        env_vars['CONVERSATION_MANAGER_CLASS'] = (
            'ide.server.conversation_manager.standalone_conversation_manager.StandaloneConversationManager'
        )
        ...
        if config.file_store == 'local':
            ...
            volumes.append(
                f'{file_store_path}/{conversation_dir}:/root/.ide/{conversation_dir}:rw'
            )
