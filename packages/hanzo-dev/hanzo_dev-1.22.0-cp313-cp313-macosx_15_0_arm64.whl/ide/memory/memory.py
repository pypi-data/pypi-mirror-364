...
from ide.events.event import Event, EventSource, RecallType
from ide.events.observation.agent import (
    MicroagentKnowledge,
    RecallObservation,
)
from ide.events.observation.empty import NullObservation
from ide.events.stream import EventStream, EventStreamSubscriber
...
GLOBAL_MICROAGENTS_DIR = os.path.join(
    os.path.dirname(os.path.dirname(ide.__file__)),
    'microagents',
)

USER_MICROAGENTS_DIR = Path.home() / '.ide' / 'microagents'
...
class Memory:
    ...
    def __init__(
        ...
        # from typically IDE/microagents (i.e., the PUBLIC microagents)
        self._load_global_microagents()

        # Load user microagents from ~/.ide/microagents/
    ...
    def _load_user_microagents(self) -> None:
        """
        Loads microagents from the user's home directory (~/.ide/microagents/)
