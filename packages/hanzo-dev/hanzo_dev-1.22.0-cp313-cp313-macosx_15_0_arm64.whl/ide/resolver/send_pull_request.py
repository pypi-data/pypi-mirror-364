...
from ide.resolver.io_utils import (
    load_single_resolver_output,
)
from ide.resolver.patching import apply_diff, parse_patch
from ide.resolver.resolver_output import ResolverOutput
from ide.resolver.utils import identify_token
...
def make_commit(repo_dir: str, issue: Issue, issue_type: str) -> None:
    ...
    if not result.stdout.strip():
        ...
        subprocess.run(
            f'git -C {repo_dir} config user.name "ide" && '
            f'git -C {repo_dir} config user.email "ide@hanzo.ai" && '
            f'git -C {repo_dir} config alias.git "git --no-pager"',
            shell=True,
            check=True,
        )
        logger.info('Git user configured as ide')
...
def send_pull_request(
    ...
    base_branch_name = f'ide-fix-issue-{issue.number}'
...
def update_existing_pull_request(
    ...
    if not comment_message and additional_message:
        try:
            ...
            if explanations:
                comment_message = (
                    'IDE made the following changes to resolve the issues:\n\n'
                )
            ...
            comment_message = f'A new IDE update is available, but failed to parse or summarize the changes:\n{additional_message}'
