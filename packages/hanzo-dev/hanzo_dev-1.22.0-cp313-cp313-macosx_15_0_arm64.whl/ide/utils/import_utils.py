...
def import_from(qual_name: str):
    ...
    from its fully qualified name. For example, 'ide.server.user_auth.UserAuth' would
    import the UserAuth class from the ide.server.user_auth module.

    Args:
        qual_name: A fully qualified name in the format 'module.submodule.name'
                  e.g. 'ide.server.user_auth.UserAuth'
...
def get_impl(cls: type[T], impl_name: str | None) -> type[T]:
    ...
    This function is an extensibility mechanism in IDE that allows runtime substitution
    of implementations. It enables applications to customize behavior by providing their own
    implementations of IDE base classes.
    ...
    Args:
        ...
        impl_name: Fully qualified name of the implementation class, or None to use the base class
                  e.g. 'ide.server.conversation_manager.StandaloneConversationManager'
