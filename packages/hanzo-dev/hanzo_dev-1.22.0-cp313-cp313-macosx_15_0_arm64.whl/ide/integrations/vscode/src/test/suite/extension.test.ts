...
suite("Extension Test Suite", () => {
  ...
  test("Commands should be registered", async () => {
    ...
    const expectedCommands = [
      "ide.startConversation",
      "ide.startConversationWithFileContext",
      "ide.startConversationWithSelectionContext",
    ];
  ...
  test("ide.startConversation should send correct command to terminal", async () => {
    ...
    await vscode.commands.executeCommand("ide.startConversation");
  ...
  test("ide.startConversationWithFileContext (saved file) should send --file command", async () => {
    ...
    assert.deepStrictEqual(sendTextSpy.lastArgs, [
      `ide --file ${testFilePath.includes(" ") ? `"${testFilePath}"` : testFilePath}`,
      true,
    ]);
  ...
  test("ide.startConversationWithFileContext (untitled file) should send contextual --task command", async () => {
...
Please ask the user what they want to do with this file.`;
  ...
  test("ide.startConversationWithFileContext (no editor) should start conversation without context", async () => {
    const originalActiveTextEditor = Object.getOwnPropertyDescriptor(
      vscode.window,
      "activeTextEditor",
    );
    ...
    assert.ok(sendTextSpy.called, "terminal.sendText should be called");
    assert.deepStrictEqual(sendTextSpy.lastArgs, ["ide", true]);
  ...
  test("ide.startConversationWithSelectionContext should send contextual --task with selection", async () => {
    const selectedText = "selected text for ide";
...
Please ask the user what they want to do with this selection.`;
  ...
  test("ide.startConversationWithSelectionContext (no selection) should start conversation without context", async () => {
    const originalActiveTextEditor = Object.getOwnPropertyDescriptor(
      vscode.window,
      "activeTextEditor",
    );
  ...
  test("ide.startConversationWithSelectionContext should handle multi-line selections", async () => {
...
Please ask the user what they want to do with this selection.`;
  ...
  test("Terminal reuse should work when existing IDE terminal exists", async () => {
  ...
  test("Shell Integration should use executeCommand for IDE commands", async () => {
    const executeCommandSpy = createManualSpy();

    // Mock execution for IDE command
    const mockExecution = {
      read: () => ({
        async *[Symbol.asyncIterator]() {
          yield "IDE started successfully";
        },
      }),
      exitCode: Promise.resolve(0),
      commandLine: {
    ...
    // Should have called executeCommand for IDE command
    assert.ok(
      executeCommandSpy.called,
      "Shell Integration executeCommand should be called for IDE command",
    );

    // Check that the command was an IDE command
    const ideCall = executeCommandSpy.argsHistory.find(
      (args: any[]) => args[0] && args[0].includes("ide"),
    );
    assert.ok(
      ideCall,
      `Should execute IDE command. Actual calls: ${JSON.stringify(executeCommandSpy.argsHistory)}`,
    );
  ...
  test("Shell Integration should use executeCommand when available", async () => {
    ...
    // The IDE command should be executed via Shell Integration
    const ideCommand = executeCommandSpy.argsHistory.find(
      (args: any[]) => args[0] && args[0].includes("ide"),
    );
    assert.ok(
      ideCommand,
      "Should execute IDE command via Shell Integration",
    );
  });
  ...
  test("Shell Integration fallback should work when Shell Integration unavailable", async () => {
    ...
    assert.ok(
      sendTextSpy.lastArgs[0].includes("ide"),
      "Should send IDE command",
    );
  });
});
