...
class IDEMCPConfig:
    @staticmethod
    def add_search_engine(app_config: 'IDEConfig') -> MCPStdioServerConfig | None:
        ...
        # Do not add search engine to MCP config in SaaS mode since it will be added by the IDE server
        return None

    @staticmethod
    def create_default_mcp_server_config(
        host: str, config: 'IDEConfig', user_id: str | None = None
        ...
        # Add hanzo-mcp as the primary MCP server
        hanzo_mcp_server = MCPStdioServerConfig(
            name='hanzo-mcp',
            command='hanzo-mcp',
            args=['--enable-all-tools'],
            env={}
        )
        stdio_servers.append(hanzo_mcp_server)
        
        search_engine_stdio_server = IDEMCPConfig.add_search_engine(config)
...
ide_mcp_config_cls = os.environ.get(
    'IDE_MCP_CONFIG_CLS',
    'ide.core.config.mcp_config.IDEMCPConfig',
)

IDEMCPConfigImpl = get_impl(IDEMCPConfig, ide_mcp_config_cls)
