# Configuration Management in IDE

## Overview

IDE uses a flexible configuration system that allows settings to be defined through environment variables, TOML files, and command-line arguments. The configuration is managed through a package structure in `ide/core/config/`.
...
````python
from ide.core.config import load_app_config
...
While this document focuses on environment variable configuration, IDE also supports:
...
The IDE configuration system provides a flexible and type-safe way to manage application settings. By following the naming conventions and utilizing the provided functions, developers can easily customize the behavior of IDE components through environment variables and other configuration sources.
