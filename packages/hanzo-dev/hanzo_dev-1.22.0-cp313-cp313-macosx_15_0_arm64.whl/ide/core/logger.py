...
def get_file_handler(
    ...
    file_name = f'ide_{timestamp}.log'
...
ide_logger = logging.getLogger('ide')
current_log_level = logging.INFO

if LOG_LEVEL in logging.getLevelNamesMapping():
    current_log_level = logging.getLevelNamesMapping()[LOG_LEVEL]
ide_logger.setLevel(current_log_level)

if DEBUG:
    ide_logger.addFilter(StackInfoFilter())

if current_log_level == logging.DEBUG:
    LOG_TO_FILE = True
    ide_logger.debug('DEBUG mode enabled.')

if LOG_JSON:
    ide_logger.addHandler(json_log_handler(current_log_level))
else:
    ide_logger.addHandler(get_console_handler(current_log_level))

ide_logger.addFilter(SensitiveDataFilter(ide_logger.name))
ide_logger.propagate = False
ide_logger.debug('Logging initialized')

LOG_DIR = os.path.join(
    # parent dir of ide/core (i.e., root of the repo)
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
    'logs',
)

if LOG_TO_FILE:
    ide_logger.addHandler(
        get_file_handler(LOG_DIR, current_log_level)
    )  # default log to project root
    ide_logger.debug(f'Logging to file in: {LOG_DIR}')
...
class LlmFileHandler(logging.FileHandler):
        ...
        if not DEBUG:
            ...
            for file in os.listdir(self.log_directory):
                ...
                try:
                    ...
                    ide_logger.error('Failed to delete %s. Reason: %s', file_path, e)
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        super().emit(record)
        self.stream.close()
        ide_logger.debug('Logging to %s', self.baseFilename)
        self.message_counter += 1
...
class IDELoggerAdapter(logging.LoggerAdapter):
    extra: dict

    def __init__(
        self, logger: logging.Logger = ide_logger, extra: dict | None = None
    ) -> None:
        self.logger = logger
        self.extra = extra or {}
