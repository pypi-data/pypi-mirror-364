...
class FrameModel:
    ...
    def __init__(self,
        ...
        if node_marker_scale is not None:
    ...
    def from_dict(sam:dict,
        ...
        for i,node in enumerate(sam.get("nodes", [])):
            m["StructuralAnalysisModel"]["geometry"]["nodes"].append({
                "tag": i,
                "crd": node
            })
        for i,elem in enumerate(sam.get("cells", [])):
            if len(elem) == 3:
        ...
        return FrameModel(m,
                          shift=shift,
                          frame_outlines=frame_outlines,
                          node_marker=node_marker,
                          node_marker_scale=node_marker_scale,
                          xmodel=xmodel,
                          **kwds)
