...
def write_requirements_file(content: str, output_path: Optional[Path] = None,
    ...
    """Write requirements content to file"""
    if output_path is None:
        ...
        if (project_root / f"{base_name}{suffix}").exists():
            ...
            if interactive:
                ...
                console.print(f"\n[yellow]requirements.txt already exists.[/yellow]")
                use_generated = Confirm.ask(
                    f"Save as [cyan]{output_path.name}[/cyan]?",
                    default=True
                )
                ...
                if not use_generated:
                    ...
                    custom_name = Prompt.ask(
                        ...
                        default=f"requirements_{counter}"
                    )
                    output_path = project_root / f"{custom_name}.txt"
            # In non-interactive mode, just use the generated name
        else:
            output_path = project_root / f"{base_name}{suffix}"
