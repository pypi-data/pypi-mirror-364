...
def validate_app_name(app_name):
    ...
    if not re.match(r'^[a-zA-Z_\-\s]+$', app_name):
        raise ValueError("Le nom de l'application ne doit contenir que des lettres, tirets, underscores ou espaces.")
...
def custom_app(app_name):
    try:
        ...
        # Fichiers de base
        with open(f"{app_name}/migrations/__init__.py", "w"):
            pass
        with open(f"{app_name}/__init__.py", "w"):
            pass
