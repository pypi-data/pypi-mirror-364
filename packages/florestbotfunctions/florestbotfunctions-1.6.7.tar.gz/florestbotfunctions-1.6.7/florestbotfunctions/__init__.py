    ...
    def detect_new_kworks(self, category: int = 11, pages: int = 1, delay: int = 300, message: str = 'Новый кворк!\nИмя кворка: $name$\nЦена: $amout$\nСсылка: $url$\nАвтор: $username$\nВремя активности: $time$.', telegram_bot_token: str = None, telegram_chat_id: int = None):
        ...
        while True:
            ...
            for kwork in new_kworks:
                if kwork.url in new:
                    formatted_message = message.replace('$name$', kwork.name).replace('$amout$', kwork.possible_price_limit).replace('$url$', kwork.url).replace('$username$', kwork.username).replace('$time$', kwork.date_active)
