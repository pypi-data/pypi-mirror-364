...
if hasattr(sys, "_MEIPASS"):
    base_dir = os.path.join(sys._MEIPASS)  # type: ignore
...
processes = {}
task_files: Dict[str, Task] = {}
schedules: Dict[str, Task] = {}
loq_queue = deque([], 1000)
task_log_queues = {}
telemetry_data = []
...
logging_handler_path = ("./log",)
logging_handler_rotate_when = ("midnight",)
logging_handler_rotate_interval = (1,)
logging_handler_backup_count = (5,)
    ...
    if not config_cache:
        if os.path.exists("suvi.toml") and os.path.isfile("suvi.toml"):
            with open("suvi.toml", "rb") as toml_pt:
                config_cache = {**config_defaults, **load_toml(toml_pt)}
        else:
            config_cache = config_defaults
...
@auth.user_required
def app_ps():
    return render_template("ps.html", processes=processes.copy())


@app.route("/ps_inner")
@auth.user_required
def app_ps_inner():
    filter = str(request.args.get("filter", "")).lower()

    if filter == "":
        filtered_processes = processes.copy()
    else:
        filtered_processes = {}
        for task_name, proc in processes.copy().items():
            if filter in str(task_name).lower() or filter in "\n".join(proc.args).lower():
                filtered_processes[task_name] = proc

    return render_template("ps_inner.html", processes=filtered_processes, filter=filter)
...
def app_ps_json():
    psList = []
    for task_name, proc in processes.items():
        psList.append((task_name, proc.pid, proc.args))
...
@auth.user_required
def app_tasks():
    return render_template("tasks.html", **create_task_lists(), cycle_wait=cycle_wait)
...
def app_tasks_inner():
    ...
    if filter == "":
        ...
        filtered_lists = {list_name: {} for list_name in lists.keys()}
        for list_name, task_dict in lists.items():
            for task_name, task in task_dict.items():
                if filter in str(task_name).lower() or filter in "\n".join(flatten(task.args)).lower():
                    filtered_lists[list_name][task_name] = task

    return render_template("tasks_inner.html", **filtered_lists, cycle_wait=cycle_wait)
...
def fetch_infos() -> dict:
    ...
    try:
        ...
        log("Manage: {} ({})".format(type(ex).__name__, ex.args), is_error=True)
...
@auth.admin_required
def app_manage():
    return render_template("manage.html", message=None, infos=fetch_infos())


@app.route("/about")
@auth.user_required
def app_about():
    return render_template("about.html", about=__about__, title="suvi", version=__version__)
...
def app_run(task_name):
    task: Task = task_files[task_name]
    if not task_name in processes:
...
def app_run_hook(token):
    ...
    time.sleep(1)
    tasks = {task_name: task for task_name, task in task_files.items() if task != None and token in task.tokens}
    result = 0
...
@auth.user_required
def app_log():
    return render_template("log.html", log=loq_queue.copy())


@app.route("/log_inner")
@auth.user_required
def app_log_inner():
    return render_template("log_inner.html", log=loq_queue.copy())


@app.route("/log.json")
@auth.user_required
def app_log_json():
    return jsonify(list(loq_queue))


@app.route("/log.txt")
@auth.user_required
def app_log_txt():
    log = "\n".join(["\t".join(line) for line in list(loq_queue)])
...
@auth.user_required
def app_task_logs():
    return render_template("tasklogs.html")
...
def app_task_logs_inner():
    ...
    if filter == "":
        logs = task_log_queues

    else:
        logs = {}
        for log_name, queue in task_log_queues.items():
            if filter in str(log_name).lower():
                logs[log_name] = queue

    return render_template("tasklogs_inner.html", logs=logs, filter=filter)


@app.route("/tasklog.json")
@auth.user_required
def app_task_logs_json():
    return jsonify(list(task_log_queues))
...
def app_task_log(task_name):
    return render_template(
        "tasklog.html",
        log=task_log_queues.get(task_name, deque([], 1)).copy(),
        task_name=task_name,
        proc=processes.get(task_name, None),
    )
...
def app_task_log_json(task_name):
    task_log = list(task_log_queues.get(task_name, deque([], 1)))
...
def app_task_log_txt(task_name):
    log = "\n".join(["\t".join(line) for line in list(task_log_queues.get(task_name, deque([], 1)))])
    return Response(log, mimetype="text/plain")
...
def app_task_info(task_name):
    task = task_files.get(task_name, None)
    if task != None:
...
def app_clean_task_logs():
    global task_log_queues
    task_log_queues = {}
    return render_template("manage.html", message="All tasklogs cleared.", infos=fetch_infos())
...
def app_shutdown():
    shutdown()
    return render_template("manage.html", message="All tasks and schedules stopped.", infos={})
...
def telemetry_json():
    ...
    for telemetry_item in telemetry_data.copy():
    ...
    return jsonify(
        {
            "version": __version__,
            "startup": int(startup.timestamp()),
            "now": int(datetime.now().timestamp()),
            "continue": continue_threads,
...
def add_file_log_handler(logger: logging.Logger, format_string: str):
    logging_formatter = logging.Formatter(format_string)
    file_handler = TimedRotatingFileHandler(
        os.path.join(str(logging_handler_path), f"{logger.name}.log"),
        when=str(logging_handler_rotate_when),
        interval=int(str(logging_handler_rotate_interval)),
        backupCount=int(str(logging_handler_backup_count)),
    )
...
def create_task_lists() -> Dict[str, Any]:
    class TaskState:
        def __init__(self, task: Task, active_state: bool):
            self.task = task
            self.active = active_state

    keepalive = {task_name: task for task_name, task in task_files.items() if task.type == TaskType.KEEPALIVE}
    for task_name, task in keepalive.items():
        task.runtime["active"] = task_name in processes

    ondemand = {task_name: task for task_name, task in task_files.items() if task.type == TaskType.ONDEMAND}
...
def runner():
            ...
            try:
                for telemetry_item in telemetry_data:
                    ...
                    if item_time < min_time:
                        telemetry_data.remove(telemetry_item)
                pass
            except Exception as ex:
                log(
                    "Telemetry truncation: {} ({})".format(type(ex).__name__, ex.args),
                    is_error=True,
                )
    ...
    def run_keepalives():
        global continue_threads
        time.sleep(1)
        log(f"Watching {tasks_path} ({os.path.abspath(tasks_path)}) for changes every {cycle_wait} seconds.")
        iteration = 0
        while continue_threads:
            telemetry("cycle")
            log(f"Checking for task file updates, iteration {iteration}")
            new_listing = dict([(Path(f).stem, None) for f in os.listdir(tasks_path) if f.endswith(".json") and f != "suvi.json"])
            removed = [f for f in task_files if not f in new_listing]
            added = [f for f in new_listing if not f in task_files]
            for task_name in removed:
                task_name = Path(task_name).stem
                telemetry("task-removed")
                log(f"{task_name} removed.")
                task_files.pop(task_name)
                task_log_queues.pop(task_name, "")
                schedules.pop(task_name, None)
                kill(task_name)
    ...
    thread_main = threading.Thread(name="run_keepalives", target=run_keepalives, args=())
    thread_main.start()
                    ...
                    if task.schedule_matches(now):
                        if task_name in processes:
                            ...
                            try:
                                task: Task = task_files[task_name]
                                telemetry(
                                    f"task-scheduled-{'start' if task.exec in [TaskExec.SYSTEM, TaskExec.SHELL] else task.exec.value}-{task_name}"
                                )
                                telemetry(f"scheduled-{'start' if task.exec in [TaskExec.SYSTEM, TaskExec.SHELL] else task.exec.value}")
...
def kill(task_name: str):
    proc = processes.pop(task_name, None)
    if proc:
...
def load_task(task_name: str):
    if not os.path.exists(f"{tasks_path}/{task_name}.json"):
    ...
    try:
        with warnings.catch_warnings(record=True) as caught_warnings:
            new_task = Task(f"{tasks_path}/{task_name}.json")
    ...
    task_files.update({task_name: new_task})
    ...
    if new_task.type == TaskType.KEEPALIVE:
        ...
        schedules.update({task_name: new_task})
...
def exec_task(task_name: str, task: Task, send_to_stdin: Optional[bytes] = None):
    def run_in_thread(task_name, args, env, cwd: str, shell: bool = False):
        ...
        def task_log(pipe, level):
            ...
            try:
                for line in iter(pipe.readline, b""):
                    ...
                    task_log_queues.setdefault(task_logger.name, deque([], 1000)).append(
                        (
                            task_logger.name,
        ...
        try:
            ...
            for sub_args in args:
                ...
                processes.update({task_name: proc})
                log(
                    f"Started process #{sub_proc_no} of {len(args)} with PID {proc.pid} for {task_name} ({exec_uuid}){' in shell' if shell else ''}."
                )
                ...
                if sub_proc_no == 1 and send_to_stdin:
                    log(
                        f"Sending stdin text with {len(send_to_stdin)} bytes to #{sub_proc_no} of {len(args)} with PID {proc.pid} for {task_name}."
                    )
            ...
            if proc:
                ...
                task_files.pop(task_name, None)
                schedules.pop(task_name, None)

        except FileNotFoundError as e:
            log(f"{task_name}: {e.strerror}: '{e.filename}'", is_error=True)
            proc = task_files.pop(task_name, None)
        except OSError as e:
    ...
    if task.exec in [TaskExec.SYSTEM, TaskExec.SHELL]:
        ...
        try:
            ...
            task_files.update({task_name: task})

            log(
                f"Talend task '{task_name}' contains package '{task.runtime['talend']['job']}', version '{task.runtime['talend']['jobVersion']}' build on '{task.runtime['talend']['date']}' with default context '{task.runtime['talend']['contextName']}'"
            )

            if os.name == "nt":
                java_platform_delimiter = ";"
            else:
                java_platform_delimiter = ":"
            ...
            args = [
                ...
                f"{task.runtime['talend']['project'].lower()}.{task.runtime['talend']['job'].lower()}_{task.runtime['talend']['jobVersion'].replace('.', '_')}.{task.runtime['talend']['job']}",
            ] + task.args
            thread = threading.Thread(
                name=f"task_{task_name}",
                target=run_in_thread,
...
def log(text: str, is_error: bool = False):
    ...
    loq_queue.append((dt, user, level, text))


def telemetry(event_name: str):
    telemetry_data.append((datetime.now(), event_name))
...
@cmd.command()
def main(
    bind: str = typer.Option(get_config("bind"), envvar="SUVI_BIND"),
    port: int = typer.Option(get_config("port"), envvar="SUVI_PORT"),
    enforce: bool = typer.Option(get_config("enforce"), envvar="SUVI_ENFORCE"),
    admin_login: str = typer.Option(get_config("admin_login"), envvar="SUVI_ADMIN_LOGIN"),
    admin_password: Union[str, None] = typer.Option(get_config("admin_password"), envvar="SUVI_ADMIN_PASSWORD"),
    user_login: str = typer.Option(get_config("user_login"), envvar="SUVI_USER_LOGIN"),
    user_password: Union[str, None] = typer.Option(get_config("user_password"), envvar="SUVI_USER_PASSWORD"),
    tasks: str = typer.Option(get_config("tasks"), envvar="SUVI_TASKS"),
    cycle: int = typer.Option(get_config("cycle"), envvar="SUVI_CYCLE"),
    logs: str = typer.Option(get_config("logs"), envvar="SUVI_LOGS"),
    log_rotate_when: str = typer.Option(get_config("log_rotate_when"), envvar="SUVI_LOG_ROTATE_WHEN"),
    log_rotate_interval: int = typer.Option(get_config("log_rotate_interval"), envvar="SUVI_LOG_ROTATE_INTERVAL"),
    log_backup_count: int = typer.Option(get_config("log_backup_count"), envvar="SUVI_LOG_BACKUP_COUNT"),
):
    global tasks_path
    global cycle_wait
    global logging_handler_path
    global logging_handler_rotate_when
    global logging_handler_rotate_interval
    global logging_handler_backup_count

    for dir in [tasks, logs]:
        if not os.path.isdir(dir):
            print(f'Directory "{dir}" missing. Trying to create it.')
            os.makedirs(dir, exist_ok=True)

    logging_handler_path = logs
    logging_handler_rotate_when = log_rotate_when
    logging_handler_rotate_interval = log_rotate_interval
    logging_handler_backup_count = log_backup_count
    tasks_path = tasks
    cycle_wait = cycle
