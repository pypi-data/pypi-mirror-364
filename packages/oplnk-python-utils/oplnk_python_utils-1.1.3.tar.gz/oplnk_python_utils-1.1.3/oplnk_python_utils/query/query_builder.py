        ...
        try:
            ...
            for field in distinct_params:
                try:
                    ...
                    # Create nested structure for fields with dot notation
                    parts = field.split(".")
                    d = group_id
                    for i, part in enumerate(parts[:-1]):
                        # If a non-dict value is at the path, it can't be extended
                        if part in d and not isinstance(d[part], dict):
                            # To avoid overwriting, you could add a unique suffix, or handle as an error
                            # For now, let's just print a warning and continue
                            print(
                                f"Warning: conflicting path in distinct fields for '{field}' at part '{part}'."
                            )
                            d = d.setdefault(f"{part}_{i}", {})
                        else:
                            d = d.setdefault(part, {})

                    d[parts[-1]] = field_expression
