...
class TestCreateKeyPair:
    ...
    @patch("awslabs.ec2_mcp_server.modules.key_pairs.aws_client")
    @pytest.mark.asyncio
    async def test_create_key_pair_without_storage(self, mock_aws_client, mock_validate, mock_key_storage, mock_create_key_pair_response):
        """Test key pair creation without storing private key."""
        mock_validate.return_value = True
        mock_client = MagicMock()
        mock_aws_client.get_client.return_value = mock_client
        mock_client.create_key_pair.return_value = mock_create_key_pair_response
        ...
        assert result["status"] == "success"
        assert "private_key_material" in result
        assert "warning" in result
        assert "Private key not stored" in result["warning"]
        
        # Storage should not be called
        mock_key_storage.store_private_key.assert_not_called()
...
class TestDeleteKeyPair:
    ...
    @patch("awslabs.ec2_mcp_server.modules.key_pairs.aws_client")
    @pytest.mark.asyncio
    async def test_delete_key_pair_success_without_stored_key(self, mock_aws_client, mock_validate):
        """Test successful key pair deletion without stored key deletion."""
        mock_validate.return_value = True
        mock_client = MagicMock()
        mock_aws_client.get_client.return_value = mock_client
