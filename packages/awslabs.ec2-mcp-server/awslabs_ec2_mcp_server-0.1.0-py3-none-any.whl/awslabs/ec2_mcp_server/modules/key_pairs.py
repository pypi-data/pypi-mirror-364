...
async def create_key_pair(
    ...
    """
    create_key_pair
            Creates an EC2 key pair and optionally stores the private key securely using the specified method (Secrets Manager, S3, or Parameter Store).
            Required:
                key_name: Unique name for the key pair
                storage_method: Storage backend ("secrets_manager", "s3_encrypted", or "parameter_store")
            Optional:
                key_type: Key algorithm ("rsa" or "ed25519", default "rsa")
                tags: Tags to associate with the key pair
                store_private_key: Whether to store the private key (default True)
                storage_kwargs: Extra parameters for the selected storage method
            Returns: 
                Dict with key pair creation details.
    """
    try:
        ...
        # Store the private key securely (storage_method is mandatory)
        if store_private_key:
            ...
            if storage_result.get("status") == "success":
                ...
                result["message"] += f" and private key stored using {key_storage.STORAGE_METHODS[storage_method]}"
            else:
            ...
            # Return private key material if not storing
            result["private_key_material"] = response["KeyMaterial"]
            result["warning"] = "Private key not stored - ensure you save it securely"
...
async def delete_key_pair(
    ...
    Args:
        ...
        delete_stored_key: Whether to also delete stored private key
    ...
    try:
        ...
        # Optionally delete stored private key
        if delete_stored_key and storage_method and storage_location:
