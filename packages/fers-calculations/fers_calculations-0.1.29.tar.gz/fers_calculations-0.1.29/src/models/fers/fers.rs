...
impl FERS {
   ...
   pub fn assemble_load_vector_for_combination(
    ...
    pub fn solve_for_load_case(&mut self, load_case_id: u32) -> Result<Results, String> {
        ...
        // f) pull out the result‐writer
        let results = self.build_and_store_results(format!("Load Combination {}", load_case_id), ResultType::Loadcase(load_case_id),  &u, &reaction)?;
        Ok(results.clone())
    }
  ...
  pub fn solve_for_load_combination(
        ...
        let results = self.build_and_store_results(
            format!("Load Combination {}", combination_id),
    ...
    pub fn build_and_store_results(
        ...
        match result_type {
                ResultType::Loadcase(_) => {
                    if bundle.loadcases.contains_key(&key) {
                        return Err(format!("Duplicate load‐case name `{}`", key));
                    }
                    bundle.loadcases.insert(key.clone(), res);
                    Ok(bundle.loadcases.get(&key).unwrap())
                }
                ResultType::Loadcombination(_) => {
                    if bundle.loadcombinations.contains_key(&key) {
                        return Err(format!("Duplicate load‐combination name `{}`", key));
                    }
                    bundle.loadcombinations.insert(key.clone(), res);
                    Ok(bundle.loadcombinations.get(&key).unwrap())
                }
            }
        }
