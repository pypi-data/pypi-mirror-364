...
class MeteringManagement(ToolBase):
                ...
                for i, transaction_result in enumerate(result["results"], 1):
                    ...
                    if transaction_result["found"]:
                        ...
                        if return_transaction_data in ["summary", "full"] and "transaction_data" in transaction_result:
                            ...
                            # Always show core fields for summary and full modes
                            response_text += f"- **Model**: {data.get('model', 'N/A')}\n"
                            response_text += f"- **Provider**: {data.get('provider', 'N/A')}\n"
                            response_text += (
                                f"- **Input Tokens**: {data.get('inputTokenCount', 'N/A')}\n"
                            )
                            response_text += (
                                f"- **Output Tokens**: {data.get('outputTokenCount', 'N/A')}\n"
                            )
    ...
    def _format_full_transaction_details(self, data: Dict[str, Any]) -> str:
        ...
        details += self._format_subscriber_details(data)
    ...
    def _format_attribution_details(self, data: Dict[str, Any]) -> str:
        """Format attribution section with nested object safety."""
        attr_fields = ['taskType', 'agent', 'organization', 'product', 'subscriptionId']
        if not any(data.get(field) for field in attr_fields):
            return ""
    ...
    def _format_subscriber_details(self, data: Dict[str, Any]) -> str:
        ...
        if isinstance(subscriber, dict):
            if subscriber.get('email'):
                details += f"  - **Email**: {subscriber.get('email')}\n"

            if subscriber.get('id'):
                details += f"  - **ID**: {subscriber.get('id')}\n"

            if subscriber.get('credential'):
                cred = subscriber['credential']
                if isinstance(cred, dict):
                    cred_name = cred.get('name', 'N/A')
                    cred_value = cred.get('value', '')
                    # Obfuscate credential value showing only last 3 characters
                    if cred_value and len(cred_value) > 3:
                        obfuscated = f"***{cred_value[-3:]}"
                    else:
                        obfuscated = "***"
                    details += f"  - **Credential**: {cred_name} ({obfuscated})\n"
...
```"""
        ...
        text += "2. Use `lookup_transactions()` to find and verify transactions\n"
        text += "3. Use detailed capability actions above for specific guidance\n"
        text += "4. Review examples with `get_examples()`"
