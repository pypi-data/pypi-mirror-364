...
from fused._options import StorageStr, _get_cache_dir
...
def _cache(
    ...
    **kwargs,
    ...
    cache_interface = _get_cache_interface()
...
def _get_cache_base_path(sub_path, storage: StorageStr = "auto") -> Path:
    ...
    if storage != "auto":
        base_path = _get_cache_dir(OPTIONS.cache_in_object_storage, storage)
...
def _get_cache_interface():
    if OPTIONS.cache_in_object_storage:
        # e.g. s3 or gs
        return urllib.parse.urlparse(OPTIONS.fd_prefix).scheme
    return "filesystem"
