...
class Future(BaseFuture):
    ...
    def exception(self) -> Optional[Exception]:
        try:
            ...
            # CancelledError is BaseException, convert to Exception for consistency
            return Exception(f"Task was cancelled: {e}")
...
class AsyncJobPool(BaseJobPool):
    ...
    async def _run_task(self, index, args):
    ...
    def _create_future(self, index, args) -> AsyncFuture:
        task = asyncio.create_task(self._run_task(index, args))
        return AsyncFuture(task, index, args)
    ...
    async def _start_jobs(self):
        ...
        for index, args in enumerate(self.arg_list):
            ...
            future = self._create_future(index, args)
    ...
    def cancel(self, wait: bool = False):
        ...
        for future in self._futures:
            if not future._task.done():
                if future._task.cancel():
                    cancelled_count += 1
                else:
                    running_count += 1

        if not wait:
            message = f"{cancelled_count} task(s) cancelled successfully"
            if running_count:
                message += f", {running_count} task(s) already in progress and can not be cancelled."
            print(message)
