...
def _create_execution_function(
    ...
    sync_mode, exec_type, storage_type, engine = dispatch_params
    ...
    if storage_type == "token" and engine == "remote":
        ...
        if exec_type == "tile":
            func = run_shared_tile_async if sync_mode == "async" else run_shared_tile
            return partial(func, udf_resolution.token, **tile_params)
        else:  # file
            func = run_shared_file_async if sync_mode == "async" else run_shared_file
            return partial(func, udf_resolution.token, **file_params)
    ...
    # Local job step
        ...
        if engine == "remote":
            if exec_type == "tile":
                func_name = "run_tile_async" if sync_mode == "async" else "run_tile"
                return lambda: getattr(job_step, func_name)(**tile_params)
            else:  # file
                func_name = "run_file_async" if sync_mode == "async" else "run_file"
                # Async file doesn't use cache_max_age for some reason
                params = parameters if sync_mode == "async" else file_params
                return lambda: getattr(job_step, func_name)(**params)
        else:  # local engine
            if exec_type == "tile":
                ...
                return lambda: job_step.run_local(
                    local_tile_bbox,
                ...
                return lambda: job_step.run_local(**local_params)
...
def _create_execution_function_async(
    ...
    # Local job step
        ...
        if engine == "remote":
            if exec_type == "tile":
                return lambda: job_step.run_tile_async(**tile_params)
            else:  # file
                return lambda: job_step.run_file_async(**file_params)
        else:  # local engine
            ...
            if exec_type == "tile":
                ...
                async def _async_file_local():
                    return job_step.run_local(**local_params)
...
async def run_async(
    ...
    **kw_parameters,
    ...
    # Create and execute the function (always async)
    dispatch_params = (
        "async",  # Always async for run_async
        type,
        resolved_udf.storage_type,
        engine,
    )
