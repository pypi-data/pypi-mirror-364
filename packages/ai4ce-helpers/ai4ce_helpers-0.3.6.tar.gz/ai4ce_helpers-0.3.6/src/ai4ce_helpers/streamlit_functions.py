...
def st_generated_design_selector(columns: [int, int, int, int] = [5, 1, 2, 2]):
    ...
    if status_code == 200:
        selected_sysgen = st.selectbox("Select a system generator",
    ...
    if 'sysgen_description' in st.session_state and 'generated_design_comp_list_uid' in st.session_state:
