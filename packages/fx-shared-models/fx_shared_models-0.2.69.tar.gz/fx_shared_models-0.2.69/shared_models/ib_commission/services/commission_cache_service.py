...
class CommissionCacheService:
    ...
    def get_account(cls, mt5_login):
        ...
        account_dict = get_cache().get(cache_key)
        if account_dict:
            logger.info(f"Account cache hit for {cache_key}")
            # Reconstruct model instance from cached dict
            from shared_models.accounts.models import Account
        ...
        if account:
            ...
            get_cache().set(cache_key, account_dict, cls.ACCOUNT_CACHE_EXPIRY)
    ...
    def get_ib_hierarchy(cls, customer_id):
        ...
        # Try to get from cache first
        hierarchy_dict = get_cache().get(cache_key)
        if hierarchy_dict:
            logger.info(f"Cache hit for {cache_key}")
            # Reconstruct model instance from cached dict
            from shared_models.ib_commission.models import IBHierarchy
            return IBHierarchy(**hierarchy_dict)
        ...
        if hierarchy:
            ...
            logger.info(f"Storing in cache: {cache_key}")
            # Convert model to dict for JSON serialization
            hierarchy_dict = model_to_dict(hierarchy)
            get_cache().set(cache_key, hierarchy_dict, cls.HIERARCHY_CACHE_EXPIRY)
    ...
    def get_client_mapping(cls, mt5_login, customer_id):
        ...
        # Try to get from cache first
        mapping_dict = get_cache().get(cache_key)
        if mapping_dict:
            logger.info(f"Cache hit for {cache_key}")
            # Reconstruct model instance from cached dict
            from shared_models.ib_commission.models import ClientIBMapping
            return ClientIBMapping(**mapping_dict)
        ...
        if mapping:
            ...
            logger.info(f"Storing in cache: {cache_key}")
            # Convert model to dict for JSON serialization
            mapping_dict = model_to_dict(mapping)
            get_cache().set(cache_key, mapping_dict, cls.CLIENT_MAPPING_CACHE_EXPIRY)
