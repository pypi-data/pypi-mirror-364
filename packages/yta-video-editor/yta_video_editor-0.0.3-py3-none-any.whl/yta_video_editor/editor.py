"""
TODO: Code migrated from 'yta_video_base' and
has to be refactored and the module renamed
maybe...
"""
"""
...
class VideoEditor:
    ...
    def zoom(
        ...
        """
        Apply zoom on the 'video' provided. A factor of 1 means
        x0.01 zoom, which is a zoom out. A factor of 200 means
        x2.00 zoom, which is a zoom in.
        """
        ParameterValidator.validate_mandatory_number_between('factor', factor, ZOOM_LIMIT[0], ZOOM_LIMIT[1])
        video = VideoParser.to_moviepy(video)
        #video = VideoParser.to_moviepy(video, do_include_mask = True, do_calculate_real_duration = True)
        
        factor = int(factor)

        new_size = (
            factor / 100 * video.size[0],
            factor / 100 * video.size[1]
        )

        return put_video_over_black_background(video.resized(new_size))
    ...
    def move(
        ...
        """
        Apply a movement on the 'video' provided, which means
        that it will be not centered if 'x_variation' and/or
        ...
        return put_video_over_black_background(video, position = (x_variation, y_variation))
    ...
    def rotate(
        ...
        """
        Apply a rotation in the 'video' provided. A positive
        rotation will rotate it clockwise, and a negative one,
        ...
        return put_video_over_black_background(video.rotated(factor))
