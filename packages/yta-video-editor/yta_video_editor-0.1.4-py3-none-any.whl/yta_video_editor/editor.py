...
class _Color:
    ...
    def temperature(
        ...
        """
        if (
            not NumberValidator.is_positive_number(factor, do_include_zero = True) and
            not PythonValidator.is_list_of_positive_numbers(factor, do_include_zero = True)
        ):
            raise Exception('The "factor" parameter provided is not a positive number nor a list of positive numbers.')
    
        factor = (
            [factor] * self._video.n_frames
            if not PythonValidator.is_list(factor) else
            factor
        )

        # TODO: Validate each factor, please
        for f in factor:
        ...
        self.editor._video = self.editor._video.transform(
            lambda get_frame, t:
            ImageEditor(get_frame(t)).color.temperature(factor[T.frame_time_to_frame_index(t, self.editor._video.fps)]).image
        )
    ...
    def hue(
        ...
        self.editor._video = self.editor._video.transform(
            lambda get_frame, t:
            ImageEditor(get_frame(t)).color.hue(factor).image
        )
    ...
    def brightness(
        ...
        self.editor._video = self.editor._video.transform(
            lambda get_frame, t:
            ImageEditor(get_frame(t)).color.brightness(factor).image
        )
    ...
    def contrast(
        ...
        self.editor._video = self.editor._video.transform(
            lambda get_frame, t:
            ImageEditor(get_frame(t)).color.contrast(factor).image
        )
    ...
    def sharpness(
        ...
        self.editor._video = self.editor._video.transform(
            lambda get_frame, t:
            ImageEditor(get_frame(t)).color.sharpness(factor).image
        )
    ...
    def white_balance(
        ...
        self.editor._video = self.editor._video.transform(
            lambda get_frame, t:
            ImageEditor(get_frame(t)).color.white_balance(factor).image
        )
