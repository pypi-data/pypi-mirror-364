    ...
    def process_test(
        ...
        class PositionChange:
            ...
            def __init__(
                ...
                # TODO: This 'subclass' validation is not working
                # ParameterValidator.validate_subclass_of('position_absolute', position_absolute, PositionAbsoluteChange)
                # TODO: Create this validation method below
                # ParameterValidator.validate_list_of_subclasses_of()
                # ParameterValidator.validate_instance_of('position_absolute', position_absolute, PositionAbsoluteChange)
                # ParameterValidator.validate_mandatory_list_of_these_instances('position_offsets', position_offsets, PositionOffsetChange)
        ...
        def fp(t):
            return video_position.get_position(t)

        # Put black background to be able to position it
        put_video_over_black_background(self.video, fp).write_videofile(filename)

        return filename

        # Apply the position
        self._video = self.video.with_position(
            lambda t: video_position.get_position(t)
