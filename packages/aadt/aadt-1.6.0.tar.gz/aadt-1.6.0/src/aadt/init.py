    ...
    def _collect_project_info(self, existing_config: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        optional_answers = questionary.form(
            ankiweb_id=questionary.text(
                ...
                default=existing_config.get("ankiweb_id") if existing_config else None
            ),
            contact=questionary.text(
                "Contact email (optional):",
                default=existing_config.get("contact") if existing_config else None
            ),
            homepage=questionary.text(
                "Homepage URL (optional):",
                default=existing_config.get("homepage") if existing_config else None
            ),
            tags=questionary.text(
                "Tags (space-separated, optional):",
                default=existing_config.get("tags") if existing_config else None
            ),
            min_anki_version=questionary.text(
                "Minimum Anki version (optional, e.g., '24.04'):",
                default=existing_config.get("min_anki_version") if existing_config else None
            ),
        ).ask()
