...
def observee_usage_logger(func: Callable[..., Awaitable[Any]]) -> Callable[..., Awaitable[Any]]:
    ...
    async def wrapper(*args, **kwargs):
        ...
        # Extract the actual tool/prompt name and server name
        usage_name = func.__name__
        arguments_dict = None
        server_name = None
        
        # Special handling for MCP call_tool and get_prompt patterns
        if func.__name__ == "call_tool" and len(args) >= 1:
            usage_name = args[0] if args else kwargs.get("name", func.__name__)
            arguments_dict = args[1] if len(args) >= 2 else kwargs.get("arguments")
        elif func.__name__ == "get_prompt" and len(args) >= 1:
            usage_name = args[0] if args else kwargs.get("name", func.__name__)
            arguments_dict = args[1] if len(args) >= 2 else kwargs.get("arguments")
        ...
        # Try to get server name from config first
        try:
            server_name = ObserveeConfig.get_mcp_server_name()
        except ValueError:
            # If no server name is set, try to extract from usage_name (for call_tool case)
            if "__" in usage_name:
                parts = usage_name.split("__", 1)
                server_name = parts[0]
                # Update the usage_name to be the part after __
                usage_name = parts[1]
            # Otherwise try function name
            elif "__" in func.__name__:
                parts = func.__name__.split("__", 1)
                server_name = parts[0]
                # Update the usage_name to be the part after __ for non-MCP functions
                if len(parts) > 1 and func.__name__ not in ["call_tool", "get_prompt"]:
                    usage_name = parts[1]
        
        # Final check - we must have a server name by now
        if not server_name:
            raise ValueError("MCP server name not set and function name doesn't contain '__' separator")
