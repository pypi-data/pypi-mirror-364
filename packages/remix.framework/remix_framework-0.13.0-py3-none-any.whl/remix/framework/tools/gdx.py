...
def select_from_dataframe(df: pd.DataFrame, sel_dict: dict) -> pd.DataFrame:
    ...
    for idx, sel in sel_dict.items():
        try:
            ...
            missing_elements: list[str] = list(
                set(sel) - set(df.index.get_level_values(idx).to_list())
            )
            warnings.warn(
                f"Missing data encountered, due to {missing_elements} not in all data.",
                UserWarning,
            )
            del missing_elements
            continue
    return df
...
class GDXEval:
    ...
    def get_symbol(
        ...
        if self._gdx_dict:
            assert all(
                len(self._scen_names) <= len(t) if not isinstance(t, str) else 1
                for t in self._gdx
