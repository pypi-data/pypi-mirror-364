...
class HypertensionRequest(BaseModel):
    ...
    history_sbp: Optional[int] = Field(
        None,
        description="既往最高收缩压值，单位 mmHg。若提供，将优先用于分级和诊断。",
        ge=0,
        le=300
    )
    history_dbp: Optional[int] = Field(
        None,
        description="既往最高舒张压值，单位 mmHg。若提供，将优先用于分级和诊断。",
        ge=0,
        le=200
    )
...
class HypertensionResponse(BaseModel):
    ...
    diagnosis: bool = Field(
        ...,
        description="是否达到当前测量场景下的高血压诊断标准"
    )
    explanation: str = Field(
        ...,
        description="详细的诊断与分级依据说明"
    )
    guideline: str = Field(
        ...,
        description="本次诊断所参考的权威临床指南名称"
    )
    service_version: str = Field(
        "1.0.0",
        description="服务版本号，用于追踪与兼容性管理"
    )


# 获取血压分级（符合中国高血压指南“或”判断逻辑）
def get_level(sbp: int, dbp: int, history_sbp: int = None, history_dbp: int = None) -> str:
    """
    根据血压值进行临床分级，优先使用既往最高血压。
    分级标准基于“收缩压或舒张压任一达标”原则。
    """
...
@mcp.tool(
    ...
    description=(
        "根据患者血压值和测量场景（诊室/动态/家庭），"
        "依据中国高血压防治指南进行分级与诊断。"
        "若未提供测量场景，默认使用【诊室测量】（≥140/90 mmHg）。"
        "返回是否确诊、分级结果、诊断依据和参考指南。"
        "适用于健康咨询、慢病管理等AI辅助决策场景。"
    )
)
async def hypertension_diagnosis(
    ...
    高血压诊断主函数
    """
    # 获取测量场景（Pydantic 已校验，但做兜底）
    context = request.measurement_context

    # 防御性 fallback
    if context not in DIAGNOSIS_THRESHOLDS:
        logger.warning(
            f"无效或不支持的 measurement_context: '{context}'，"
            f"将 fallback 到默认值 '诊室测量'"
        )
        context = "诊室测量"
...
def run():
    ...
    logger.info("服务将通过 stdio 通信，等待 Agent 调用...")
