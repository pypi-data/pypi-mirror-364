...
class Neo4jAdapter(VectorStoreAdapter):
    ...
    def _convert_to_neo4j_triplet(self, graph_triplet: GraphTriplet):
        ...
        # Create edge with only edge_id and metadata
        edge = GraphEdge(
            edge_id=graph_triplet.edge.edge_id if hasattr(graph_triplet.edge, 'edge_id') else f"{graph_triplet.edge.subject}_{graph_triplet.edge.relationship}_{graph_triplet.edge.object}",
            metadata=metadata
        )
        
        # Set graph data separately
        edge.set_graph_data(
        ...
        return Neo4jGraphTriplet(edge=edge)
