...
class GraphDBExamples:
    ...
    def example_2_optimized_entity_exploration(self):
        ...
        print("\n2.1 Single-hop exploration for 'Tor' (optimized with caching):")
        ...
        # 2.2 Cached query (should be much faster)
        print(f"\n2.2 Same query with caching (should be faster):")
        start_time = datetime.now()
        
        cached_triplets = self.graph_db.get_entity_relationships_optimized(
            entity="Tor",
            filter_obsolete=True,
            max_depth=1,
            limit=10
        ...
        duration = (datetime.now() - start_time).total_seconds() * 1000
        print(f"   ⚡ Cached query completed in {duration:.2f}ms")
        print(f"   📊 Found {len(cached_triplets)} relationships (from cache)")
    ...
    def example_7_advanced_queries(self):
        ...
        print(f"\n7.4 Cache performance statistics:")
        ...
        print(f"      TTL: {cache_stats['cache_ttl_seconds']} seconds")
        print(f"      Indexes created: {cache_stats['indexes_created']}")
    ...
    def example_8_optimization_features(self):
        ...
        # 8.3 Query cache demonstration
        print(f"\n8.3 Query cache demonstration:")
        
        # Clear cache first
        self.graph_db.clear_query_cache()
        print("   🧹 Cache cleared")
        ...
        # Second query (cached)
        start_time = datetime.now()
        cached_conflicts = self.graph_db.detect_relationship_conflicts_optimized(
            entity_name="Tor",
            confidence_threshold=0.5
        )
        cached_duration = (datetime.now() - start_time).total_seconds() * 1000
        
        print(f"   ⏱️  First query (not cached): {first_duration:.2f}ms")
        print(f"   ⚡ Second query (cached): {cached_duration:.2f}ms")
        
        if first_duration > 0:
            speedup = first_duration / cached_duration if cached_duration > 0 else float('inf')
