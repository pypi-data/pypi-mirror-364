...
from ..models import (
    ...
    SearchResult, QueryResponse, RelationshipStatus, EdgeData
)
...
class KnowledgeGraphEngineV2:
    ...
    - Optimized Neo4j operations with caching
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = DEFAULT_MODEL,
        ...
        print("🚀 Knowledge Graph Engine v2 (Optimized)")
        print(f"   - Vector store: {vector_collection} (Neo4j)")
        print(f"   - Graph database: Neo4j with optimizations")
    ...
    def _add_new_edge(self, edge_data: EdgeData) -> Dict[str, Any]:
        """Add new edge using optimized graph operations"""
        try:
            success = self.graph_db.add_edge_data(edge_data)
            if success:
                # Create edge for vector store
