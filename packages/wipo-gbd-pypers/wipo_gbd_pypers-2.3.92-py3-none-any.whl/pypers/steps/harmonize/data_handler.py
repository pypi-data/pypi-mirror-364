...
class GBDFormat(BaseHandler):
    ...
    def process(self, data_file, appnum):
        ...
        # there's a mess between pypers and gbd_etl_transform, type name is not consistent...
        pipeline_type = self.pipeline_type
        if self.pipeline_type == "design":
            pipeline_type = "designs"

        if not parsers.get(collection_name, None):
            print("create parser:", collection_name, pipeline_type)
            parsers[collection_name] = Parser(collection_name, type=pipeline_type)
