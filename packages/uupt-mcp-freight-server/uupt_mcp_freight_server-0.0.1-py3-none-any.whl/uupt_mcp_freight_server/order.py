...
OPENAPI_URL_BASE = "https://freight-open.uupt.com/openapi/v1/"
#OPENAPI_URL_BASE = "http://freight-open.test.uupt.com/openapi/v1/"
# 线测：http://freight-open.test.uupt.com
# ab：http://freight-open-ab.uupt.co

# 创建MCP服务器实例
mcp = FastMCP("mcp-server-uupt-freight-orders")


@mcp.tool(name="货运智能发单-订单询价", description="这是货运的计价接口，包含搬家，拉货等关键词时优先用这个。查询订单价格，需要需要输入开始地址，结束地址。")
async def order_price(from_address: str = Field(description="开始地址，要求完整地址信息。必要字段"),  # 开始地址，例如：阳光城5号楼6层6号
    ...
    biz = {
        'startAddress': from_address,
        'endAddress': to_address,
        'transportType': 2090,
        'orderCityName': city_name,
        'specialChannel': 1
    }
...
async def order_create(price_token: str = Field(description="计算订单价格接口返回的price_token,必填字段"),
    ...
    url = f"{OPENAPI_URL_BASE}order/createOrder"
    return post_send(biz, url)


@mcp.tool(name="货运智能发单-获取订单详情", description="获取订单详情，需要需要输入订单编号：订单编号order_code")
...
@mcp.tool(name="货运智能发单-跑男信息查询,查询跑腿实时信息", description="跑男信息查询，需要需要输入订单编号")
