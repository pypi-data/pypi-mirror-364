...
def preprocess_data():
    df= pd.read_csv("/Users/xiaoql/Downloads/是否感知移网上网速度慢-l4lbor.csv")
    # columns 768
    df['vector']=df['vector'].apply(lambda x: eval(x))
    # 将vector数组转换为单独的列
    vector_df = pd.DataFrame(df['vector'].tolist(), columns=[f'v_{i}' for i in range(len(df['vector'].iloc[0]))])
    
    # 将新列添加到原始数据框
    df = pd.concat([df.drop('vector', axis=1), vector_df], axis=1)
    df['label'] = df['label'].map({'是': 1, '否': 0})
    print(df.head())

    df_train = df.sample(frac=0.8)
    df_val = df.drop(df_train.index)

    df_train.to_csv("sample/binary_cls_train.csv",index=False)
    df_val.to_csv("sample/binary_cls_val.csv",index=False)
...
def generate_multi_label_data(n_samples=2000, n_features=768, n_labels=4, random_state=42):
    ...
    # 保存数据
    df_train.to_csv("sample/multi_label_train.csv", index=False)
    df_val.to_csv("sample/multi_label_val.csv", index=False)
    ...
    print(f"\n数据已保存到:")
    print(f"- sample/multi_label_train.csv")
    print(f"- sample/multi_label_val.csv")
    ...
    def forward(self, x):
        if self.dependent_mode:
            ...
            for i, network in enumerate(self.label_networks):
                ...
                current_input = torch.cat([current_input, current_prediction], dim=1)
...
class MultiLabelBinaryClsModel:
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        if X_val is not None and y_val is not None:
            ...
            y_val_tensor = torch.FloatTensor(y_val.values if isinstance(y_val, pd.DataFrame) else y_val)
...
def train_single_label_binary_cls():
...
def train_multi_label_binary_cls(dependent_mode=False):
    """训练多标签分类模型"""
    print(f"\n{'='*50}")
    ...
    # 读取多标签数据
    df_train = pd.read_csv("sample/multi_label_train.csv")
    df_val = pd.read_csv("sample/multi_label_val.csv")
    
    # 分离特征和标签
    label_columns = ['技术问题', '服务质量', '价格敏感', '用户体验']
    ...
    for label_name, scores in label_scores.items():
        print(f"  {label_name}: 准确率={scores['accuracy']:.4f}, F1={scores['f1']:.4f}")
    ...
    for i in range(5):
        ...
        for j, label in enumerate(label_columns):
            print(f"    {label}: 真实={int(y_val.iloc[i, j])}, 预测={int(sample_predictions[i, j])}, 概率={sample_probabilities[i, j]:.3f}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "multi_label":
        # 生成多标签数据
        print("正在生成多标签分类数据...")
        generate_multi_label_data(n_samples=2000, n_features=768, n_labels=4)
        ...
        # 训练独立模式的多标签模型
        print("\n1. 训练独立模式的多标签模型:")
        independent_model = train_multi_label(dependent_mode=False)
        
        # 训练相关模式的多标签模型  
        print("\n2. 训练相关模式的多标签模型:")
        dependent_model = train_multi_label(dependent_mode=True)
        
        print("\n" + "="*80)
        print("多标签分类测试完成！")
        
    elif len(sys.argv) > 1 and sys.argv[1] == "generate_data":
        # 仅生成数据
        print("正在生成多标签分类数据...")
        generate_multi_label_data(n_samples=2000, n_features=768, n_labels=4)
        print("数据生成完成！")
