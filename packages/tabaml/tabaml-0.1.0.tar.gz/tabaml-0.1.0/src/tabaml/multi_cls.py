...
class MultiClassifierModel:
    ...
    def __init__(self, hyperparameters=None):
        ...
        self.use_early_stopping = self.hyperparameters.get('use_early_stopping', True)
        self.f1_average = self.hyperparameters.get('f1_average', 'macro')  # F1计算方式
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        for epoch in range(self.num_epochs):
            ...
            if val_loader is not None and X_val_df is not None:
                ...
                print(f"Epoch [{epoch+1}/{self.num_epochs}], Loss: {avg_loss:.4f}, Val Acc: {val_acc:.4f}, Val F1: {val_f1:.4f}")
...
def create_sample_multiclass_data():
    ...
    y[noise_indices] = np.random.choice(n_classes, len(noise_indices))
    ...
    # 保存数据
    df_train.to_csv("sample/multi_cls_train.csv", index=False)
    df_val.to_csv("sample/multi_cls_val.csv", index=False)
    
    print(f"训练集大小: {len(df_train)}")
    print(f"验证集大小: {len(df_val)}")
    print(f"类别分布:")
    print("训练集:", dict(df_train['label'].value_counts().sort_index()))
    print("验证集:", dict(df_val['label'].value_counts().sort_index()))
...
class MultiLabelMultiClassifierModel:
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        for i in range(self.num_labels):
            ...
            print(f"标签 '{self.label_names[i]}' 有 {len(unique_classes)} 个类别: {unique_classes}")
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch_data in train_loader:
                ...
                batch_y_list = [batch_data[i+1].to(self.device) for i in range(self.num_labels)]
            ...
            if val_loader is not None:
                ...
                print(f"Epoch [{epoch+1}/{self.num_epochs}], Loss: {avg_loss:.4f}, Val F1 (macro): {val_f1:.4f}")
    ...
    def score(self, X, y, metric='f1_macro'):
        ...
        if metric == 'f1_macro':
            ...
            for i in range(self.num_labels):
                f1 = multi_class_f1_score(y_array[:, i], predictions[:, i], average='macro')
                f1_scores.append(f1)
            return np.mean(f1_scores)
        elif metric == 'f1_micro':
            # 所有标签的微平均F1
            ...
            y_array = y.values if isinstance(y, pd.DataFrame) else y
            for i in range(self.num_labels):
                f1 = multi_class_f1_score(y_array[:, i], predictions[:, i], average='micro')
                f1_scores.append(f1)
            return np.mean(f1_scores)
    ...
    def get_label_scores(self, X, y):
        ...
        for i, label_name in enumerate(self.label_names):
            acc = accuracy_score(y_array[:, i], predictions[:, i])
            f1_macro = multi_class_f1_score(y_array[:, i], predictions[:, i], average='macro')
            f1_micro = multi_class_f1_score(y_array[:, i], predictions[:, i], average='micro')
            f1_weighted = multi_class_f1_score(y_array[:, i], predictions[:, i], average='weighted')
...
def generate_multi_label_multi_class_data(n_samples=1500, n_features=50, n_labels=3, random_state=42):
    """
    生成多标签多分类的模拟数据
    ...
    # 保存数据
    df_train.to_csv("sample/multi_label_multi_cls_train.csv", index=False)
    df_val.to_csv("sample/multi_label_multi_cls_val.csv", index=False)
    ...
    print(f"\n数据已保存到:")
    print(f"- sample/multi_label_multi_cls_train.csv")
    print(f"- sample/multi_label_multi_cls_val.csv")
    
    return df_train, df_val


def train_multi_label_multi_class(shared_layers=True):
    """训练多标签多分类模型"""
    print(f"\n{'='*60}")
    ...
    # 读取多标签多分类数据
    df_train = pd.read_csv("sample/multi_label_multi_cls_train.csv")
    df_val = pd.read_csv("sample/multi_label_multi_cls_val.csv")
    
    # 分离特征和标签
    label_columns = ['产品类型', '服务等级', '用户群体']
    feature_columns = [col for col in df_train.columns if col not in label_columns]
...
def train_single_label_multi_class():
    """训练多分类模型"""
    # 创建示例数据（如果不存在）
    try:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "multi_label_multi_class":
        # 生成多标签多分类数据
        print("正在生成多标签多分类数据...")
        generate_multi_label_multi_class_data(n_samples=1500, n_features=50, n_labels=3)
        ...
        # 训练共享层的多标签多分类模型
        print("\n1. 训练共享层的多标签多分类模型:")
        shared_model = train_multi_label_multi_class(shared_layers=True)
        
        # 训练独立层的多标签多分类模型
        print("\n2. 训练独立层的多标签多分类模型:")
        independent_model = train_multi_label_multi_class(shared_layers=False)
        
        print("\n" + "="*80)
        print("多标签多分类测试完成！")
        
    elif len(sys.argv) > 1 and sys.argv[1] == "generate_multi_label_multi_class":
        # 仅生成多标签多分类数据
        print("正在生成多标签多分类数据...")
        generate_multi_label_multi_class_data(n_samples=1500, n_features=50, n_labels=3)
        print("数据生成完成！")
        ...
        # 默认运行单标签多分类训练
        train_single_label_multi_class()