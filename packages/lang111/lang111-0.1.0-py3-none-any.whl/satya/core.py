# Satya Language Core Module
# Contains all the data structures and interpreter functionality
...
class SatyaGraph:
    ...
    def add_edge(self, u, v):
        ...
        return f"Added edge from {u} to {v}"
    ...
    def dfs(self, start):
        if start not in self.graph:
            return f"Node {start} not in graph"
        visited = set()
        result = []
        ...
        dfs_util(start)
        return f"DFS from {start}: {result}"
    ...
    def bfs(self, start):
        ...
        return f"BFS from {start}: {result}"
