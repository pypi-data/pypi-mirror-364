        ...
        *,
        ...
        if provider in ["AZURE_MONITOR"]:
            if not live_endpoint or not application_id:
                raise InvalidArgumentError("Must provide args 'live_endpoint' or 'application_id' with provider")
        ...
        if provider in ["KRATOS_THANOS", "PROMETHEUS"] and not client_cert:
            raise InvalidArgumentError("Must provide arg 'client_cert' with provider")
        ...
        if provider in ["DATADOG", "SERVICENOW"]:
            value = key
