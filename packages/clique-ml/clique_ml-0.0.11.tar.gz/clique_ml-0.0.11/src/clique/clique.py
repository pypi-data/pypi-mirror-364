...
class EvaluationWarning(Warning):
    '''Specific for errors preventing Clique evaluation.'''
    pass
...
class Clique(dict):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        if self.inputs is None or self.targets is None or len(self) == 0: 
            warn(message='Clique has been instantiated but cannot be trained. Ensure inputs and targets are present.', 
                 category=EvaluationWarning,
                 stacklevel=2)
    ...
    def prune(self, limit:int|None=None, skip_evaluate:bool=False) -> Self:
        ...
        if isnan(mean_score): raise EvaluationError('Some models have not been scored. Call `evaluate` first.')
        clone = self.copy()
    ...
    def predict(self, X:ndarray|DataFrame|Series, ignore_errors:bool=False, **kwargs) -> ndarray:
        '''
        Makes predictions for all models in the ensemble, then returns the consensus (mean) for all results.
        Supports `kwargs` as part of the `IModel` protocol, but is only recommended if all models are of the same type.
        Raises an `EvaluationError` if the model returns a constant value for all inputs, but this will be output as a
            warning if `ignore_errors` is set to `True`.
        Returns the instance for method chaining.
        '''
        ...
        for model_id, model in self.items():
            ...
            if len(unique(predictions)) == 1: 
                message = f'Model {model_id} is guessing a constant value.'
                if ignore_errors: warn(message=message[:-1]+' and is excluded from consensus.', category=EvaluationWarning, stacklevel=2)
                else: raise EvaluationError(message)
