...
class TestReadBOJ:
    ...
    @patch("bojdata.core.pd.read_csv")
    @patch("bojdata.core.requests.get")
    def test_read_multiple_series(self, mock_get, mock_read_csv):
        ...
        mock_get.side_effect = [search_resp1, search_resp2]
        
        # Mock CSV data for both series
        df1 = pd.DataFrame({
    ...
    @patch("bojdata.core.pd.read_csv")
    @patch("bojdata.core.requests.get")
    def test_date_filtering(self, mock_get, mock_read_csv):
        ...
        # Mock CSV data
        mock_df = pd.DataFrame({
            'Date': ['2023-12-01', '2023-11-01', '2023-10-01', '2023-09-01'],
...
class TestPrivateFunctions:
    ...
    @patch("bojdata.core.pd.read_csv")
    @patch("bojdata.core.requests.get")
    def test_download_with_frequency_resample(self, mock_get, mock_read_csv):
        ...
        # Mock CSV data with daily data
        dates = pd.date_range('2023-01-01', '2023-01-10', freq='D')
        mock_df = pd.DataFrame({
            'Date': dates.strftime('%Y-%m-%d'),
            'TEST': range(len(dates))
        })
        mock_read_csv.return_value = mock_df
        
        # Test with monthly frequency
        df = _download_single_series("TEST", frequency="M")
    ...
    @patch("bojdata.core.pd.read_csv")
    @patch("bojdata.core.requests.get")
    def test_download_invalid_frequency(self, mock_get, mock_read_csv):
        ...
        with pytest.warns(UserWarning, match="Invalid frequency"):
            df = _download_single_series("TEST", frequency="INVALID")
        
        assert len(df) == 1  # Original data unchanged
    
    @patch("bojdata.core.requests.get")
    def test_download_no_csv_link(self, mock_get):
        ...
        # Test
        with pytest.raises(BOJDataError, match="Could not find CSV file"):
            _download_single_series("TEST")
