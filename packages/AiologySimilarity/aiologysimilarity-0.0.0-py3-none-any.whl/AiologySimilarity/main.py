...
💡 Tip : This supports any language !!
...
```
    """
    def check_words(self,words : tuple[str] , detect_words : tuple[str] , sensivity : int):
        """
...
`check_words(self,words : tuple[str] , detect_words : tuple[str] , sensivity : int)`
        ...
        for word in words:
            for detect_word in detect_words:
                ...
                if (sensivity * len(detect_word) / 100) <= len(word):
                    for letter in word:
                        if letter in detect_word:
                            count += 1

                if count != 0:
                    if (count * 100 / len(word)) >= sensivity:
                        yield word , count * 100 / len(word)
