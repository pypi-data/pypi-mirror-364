...
class JamaUTManager:
    ...
    def find_or_create_unit_test(self, test_name: str, module_folder: Dict, covers_list: List[str] = None, test_content: Dict = None) -> Dict:
        ...
        try:
            ...
            logging.debug(f"covers_list before UT ID check: {covers_list}")
            ut_id_used = False
            ut_id = None
            if covers_list and is_jama_ut_id(covers_list[0]):
                ...
                if ut_item:
                    ...
                    if clean_current != clean_expected:
                        ...
                        logging.info(f"   Current (normalized): {clean_current}")
                        logging.info(f"   Expected (normalized): {clean_expected}")
