#!/usr/bin/env python3
"""
FARScore Command Line Interface
"""
...
from scripts.utils.mol.AttfpMPNN import AttentiveFPPredictor
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog='''Examples:
  CSV file mode:
    farscore --input_file example.csv
    farscore --input_file molecules.csv --smiles_field "SMILES"
  
  Direct SMILES mode:
    farscore --smiles "CCO"
    farscore --smiles "CCO" "CC(=O)O" "c1ccccc1"
    farscore --smiles "CCO,CC(=O)O,c1ccccc1"'''
    )
    ...
    parser.add_argument(
        "--version", 
        action="version", 
        version=f"FARScore {__version__}"
    )
    ...
    print("🧬 FARScore: Molecular Synthesis Accessibility Predictor")
    print("=" * 60)
