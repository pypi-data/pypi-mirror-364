...
class TestFunc:
    ...
    def test_func_nested(self, session):
        ...
        task = FuncTask(
            func_nested, execution="process", name="unpicklable", session=session
        )
        with pytest.raises(AttributeError):
            pickle_dump_read(task)
        # This should not raise (though still not pickleable)
        task = FuncTask(
            func_nested, execution="thread", name="picklable", session=session
        )
    ...
    def test_unpicklable_start_cond(self, session):
        ...
        task = FuncTask(
            func_on_main_level,
            execution="process",
            start_cond=TaskFailed(task=unpkl_task),
            session=session,
        )
    ...
    def test_unpicklable_end_cond(self, session):
        ...
        task = FuncTask(
            func_on_main_level,
            execution="process",
            end_cond=TaskFailed(task=unpkl_task),
            session=session,
        )
    ...
    def test_unpicklable_session(self, session):
        ...
        unpkl_task = FuncTask(
            func_nested, execution="thread", name="unpicklable", session=session
        )
        task = FuncTask(
            func_on_main_level, execution="process", name="picklable", session=session
        )
    ...
    def test_unpicklable_session_params(self, session):
        session.parameters["unpicklable"] = FuncTask(
            lambda: None, execution="main", name="unpicklable", session=session
        )
