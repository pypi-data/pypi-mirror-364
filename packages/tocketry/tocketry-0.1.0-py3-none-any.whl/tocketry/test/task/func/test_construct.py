...
def test_description(session):
    ...
    task = FuncTask(
        myfunc, name="has desc 3", description="But this is preferred", session=session
    )
    ...
    task = FuncTask(
        myfunc, name="has desc 4", description="This is func", session=session
    )
    assert task.description == "This is func"
...
def test_construct_delayed(session, execution, tmpdir):
    ...
    with tmpdir.as_cwd():
        ...
        task = FuncTask(
            func_name="myfunc", path="myfile.py", execution=execution, session=session
        )
        assert task.status is None
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(AlwaysTrue(), None, AlwaysTrue(), id="AlwaysTrue"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            lambda: AlwaysTrue() & AlwaysTrue(),
            id="always true & always true",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            lambda s: FuncTask(
                lambda: None, name="task", execution="not valid", session=s
            ),
            ValueError,
