...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "scripts/syntax_error_script.py", "fail", ImportError, id="Import failure"
        ),
    ],
)
def test_run(
...
def test_additional_sys_paths(tmpdir, session):
    ...
    task_dir.mkdir("subfolder").mkdir("another").join("utils.py").write(
        dedent("""
    value = 5
    """)
    )
