...
from tocketry import Tocketry, Grouper
...
def test_simple(session):
    ...
    assert app.session.tasks == {
        app.session["do_things"],
        app.session["do_other_things"],
    }
...
def test_execution(session):
    ...
    app = Tocketry(config=dict(execution="process"))
