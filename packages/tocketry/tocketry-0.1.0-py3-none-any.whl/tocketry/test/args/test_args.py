...
def test_env_arg():
    os.environ["TOCKETRY_TEST_VARIABLE"] = "a value"
    arg = EnvArg("TOCKETRY_TEST_VARIABLE")
    assert arg.get_value() == "a value"

    arg = EnvArg("TOCKETRY_NOT_SET", default="a value 2")
    assert arg.get_value() == "a value 2"

    with pytest.raises(KeyError):
        arg = EnvArg("TOCKETRY_NOT_SET")
        arg.get_value()
...
def test_task_logger(session):
    ...
    assert logger.name == "tocketry.task"
...
def test_scheduler_logger(session):
    ...
    assert logger.name == "tocketry.scheduler"
