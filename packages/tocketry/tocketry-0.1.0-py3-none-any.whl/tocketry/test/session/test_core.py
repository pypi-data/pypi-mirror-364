...
def test_tasks_attr(session):
    task1 = FuncTask(lambda: None, name="example 1", execution="main", session=session)
    task2 = FuncTask(lambda: None, name="example 2", execution="main", session=session)
...
def test_getitem(session):
    task_1 = FuncTask(lambda: None, name="task 1", execution="main", session=session)
    task_2 = FuncTask(lambda: None, name="task 2", execution="main", session=session)
...
def test_create(session):
    assert session.tasks == set()
    session.create_task(
        name="do_command", start_cond="daily", command="echo 'hello world'"
    )
...
def test_remove(session):
    task_1 = FuncTask(lambda: None, name="task 1", execution="main", session=session)
    task_2 = FuncTask(lambda: None, name="task 2", execution="main", session=session)
    task_3 = FuncTask(lambda: None, name="task 3", execution="main", session=session)
...
def test_get_task(session):
    task = FuncTask(lambda: None, name="example", execution="main", session=session)
