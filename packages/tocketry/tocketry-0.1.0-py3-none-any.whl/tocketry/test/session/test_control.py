...
def test_restart(execution, session):
    ...
    FuncTask(
        on_startup, name="startup", on_startup=True, execution="main", session=session
    )
    FuncTask(
        call_restart,
        name="restart",
    ...
    session.config.shut_cond = (TaskStarted(task="shutdown") >= 2) | ~SchedulerStarted(
        period=TimeDelta("60 second")
    )
