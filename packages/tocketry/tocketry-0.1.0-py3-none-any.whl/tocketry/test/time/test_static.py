...
def test_always():
    ...
    assert always.rollforward(datetime(2022, 1, 1)) == Interval(
        datetime(2022, 1, 1, 0, 0), datetime(2260, 1, 1, 0, 0), closed="left"
    )
    assert always.rollback(datetime(2022, 1, 1)) == Interval(
        datetime(1970, 1, 3, 2, 0), datetime(2022, 1, 1, 0, 0), closed="left"
    )
...
def test_never():
    ...
    assert never.rollforward(datetime(2022, 1, 1)) == Interval(
        datetime(2260, 1, 1, 0, 0), datetime(2260, 1, 1, 0, 0), closed="left"
    )
    assert never.rollback(datetime(2022, 1, 1)) == Interval(
        datetime(1970, 1, 3, 2, 0), datetime(1970, 1, 3, 2, 0), closed="left"
    )
