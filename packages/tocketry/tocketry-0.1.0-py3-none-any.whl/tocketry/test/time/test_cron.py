...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            Cron("30", "*", "*", "*", "*"),
            every_minute & TimeOfHour.at(30),
            id="30 * * * *",
        ),
        pytest.param(
            Cron("*", "12", "*", "*", "*"),
            every_minute & TimeOfDay.at(12),
            id="* 12 * * *",
        ),
        pytest.param(
            Cron("*", "*", "28", "*", "*"),
            every_minute & TimeOfMonth.at(28),
            id="* * 28 * *",
        ),
        pytest.param(
            Cron("*", "*", "*", "6", "*"),
            every_minute & TimeOfYear.at(6),
            id="* * * 6 *",
        ),
        pytest.param(
            Cron("*", "*", "*", "*", "0"),
            every_minute & TimeOfWeek.at("Sunday"),
            id="* * * * 0",
        ),
        # Test at synonyms
        pytest.param(
            Cron("*", "*", "*", "JUN", "*"),
            every_minute & TimeOfYear.at("June"),
            id="* * * JUN *",
        ),
        pytest.param(
            Cron("*", "*", "*", "*", "SUN"),
        ...
        pytest.param(
            Cron("45-59", "*", "*", "*", "*"),
            every_minute & TimeOfHour(45, 59),
            id="45-59 * * * *",
        ),
        pytest.param(
            Cron("*", "10-13", "*", "*", "*"),
            every_minute & TimeOfDay(10, 13),
            id="* 10-13 * * *",
        ),
        pytest.param(
            Cron("*", "*", "28-30", "*", "*"),
            every_minute & TimeOfMonth(28, 30),
            id="* * 28-30 * *",
        ),
        pytest.param(
            Cron("*", "*", "*", "FEB-MAR", "*"),
            every_minute & TimeOfYear("feb", "mar"),
            id="* * * FEB-MAR *",
        ),
        pytest.param(
            Cron("*", "*", "*", "*", "FRI-SUN"),
            every_minute & TimeOfWeek("fri", "sun"),
            id="* * * * FRI-SUN",
        ),
        # Test list
        pytest.param(
        ...
        pytest.param(
            ...
            & TimeOfDay(10, 13)
            & TimeOfYear("feb", "mar")
            & (TimeOfMonth(28, 30) | TimeOfWeek("fri", "sun")),
        ...
        pytest.param(
            ...
            (TimeOfHour(0, 29) | TimeOfHour(45, 59))
            & (TimeOfDay(0, 10) | TimeOfDay(20, 23))
            & every_minute,
        ...
        pytest.param(
            Cron("*", "*", "*", "*", "Tue-Fri/2"),
            every_minute & (TimeOfWeek.at("Tue") | TimeOfWeek.at("Thu")),
            id="* * * * Tue-Fri/2",
        ),
        pytest.param(
            Cron("*", "*", "*", "Feb-Aug/3", "*"),
