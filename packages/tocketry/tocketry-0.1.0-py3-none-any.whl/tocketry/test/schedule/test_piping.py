...
def test_dependent(tmpdir, execution, session):
    with tmpdir.as_cwd():
        ...
        task_after_all = FuncTask(
            run_succeeding,
            name="After all",
            start_cond=DependSuccess(depend_task="After A")
            & DependSuccess(depend_task="After B"),
            execution=execution,
            session=session,
        )

        session.config.shut_cond = (
            TaskStarted(task="After all") >= 1
        ...
        after_a_start = (
            repo.filter_by(task_name="After A", action="run").first().created
        )
        after_b_start = (
            repo.filter_by(task_name="After B", action="run").first().created
        )
        after_all_start = (
            repo.filter_by(task_name="After all", action="run").first().created
        )
