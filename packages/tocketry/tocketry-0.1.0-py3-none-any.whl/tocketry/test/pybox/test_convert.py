...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "1 days 16:05:01.00003",
            timedelta(days=1, hours=16, minutes=5, seconds=1, microseconds=30),
        ),
        ...
        ("1d 5h 10m 20s", timedelta(days=1, hours=5, minutes=10, seconds=20)),
        (
            "1days 5hours 10minutes 20seconds",
            timedelta(days=1, hours=5, minutes=10, seconds=20),
        ),
        (
            "1 day 5 hour 10 minute 20 second",
            timedelta(days=1, hours=5, minutes=10, seconds=20),
        ),
        (
            "1 days 5 hours 10 minutes 20 seconds",
            timedelta(days=1, hours=5, minutes=10, seconds=20),
        ),
        (
            "-1 days 5 hours 10 minutes 20 seconds",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("ns", 1e9, timedelta(seconds=1), id="nanoseconds"),
        pytest.param("μs", 1e6, timedelta(seconds=1), id="microseconds"),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            {"include": "all"},
            "1 years, 2 months, 1 weeks, 3 days, 12 hours, 30 minutes, 59 seconds, 500 milliseconds, 500 microseconds",
            id="default (all)",
        ),
        pytest.param(
            timedelta(
        ...
        pytest.param(
            ...
            {"format": "semishort"},
            "2 days, 12 hrs, 30 mins, 59 secs, 500 ms, 500 μs",
            id="semishort",
        ),
        pytest.param(
            timedelta(days=16, hours=6),
            {"include": ("weeks", "days", "hours")},
            "2 weeks, 2 days, 6 hours",
            id="include week",
        ),
        pytest.param(
            timedelta(days=62, hours=6),
            {"include": ("months", "days", "hours"), "days_in_month": 30},
            "2 months, 2 days, 6 hours",
            id="include month",
        ),
        pytest.param(
            timedelta(days=365 * 2 + 2, hours=6),
