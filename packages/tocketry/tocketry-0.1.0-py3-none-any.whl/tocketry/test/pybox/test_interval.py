...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            Interval(
                to_datetime("2022-07-01"), to_datetime("2022-07-15"), closed="neither"
            ),
            Interval(
                to_datetime("2022-07-14"), to_datetime("2022-07-30"), closed="neither"
            ),
        ...
        pytest.param(
            Interval(
                to_datetime("2022-07-01"), to_datetime("2022-07-12"), closed="both"
            ),
            Interval(
                to_datetime("2022-07-12"), to_datetime("2022-07-15"), closed="both"
            ),
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            Interval(
                to_datetime("2022-07-01"), to_datetime("2022-07-12"), closed=l_closed
            ),
            Interval(
                to_datetime("2022-07-12"), to_datetime("2022-07-30"), closed=r_closed
            ),
...
def test_empty():
    assert not Interval(
        datetime(2022, 1, 1), datetime(2022, 1, 1), closed="both"
    ).is_empty
    for closed in ("left", "right", "both", "neither"):
        assert not Interval(
            datetime(2022, 1, 1), datetime(2022, 1, 2), closed=closed
        ).is_empty
    for closed in ("left", "right", "neither"):
        assert Interval(
            datetime(2022, 1, 1), datetime(2022, 1, 1), closed=closed
        ).is_empty
