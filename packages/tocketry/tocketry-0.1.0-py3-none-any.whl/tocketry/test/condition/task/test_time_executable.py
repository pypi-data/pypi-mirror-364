...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            lambda: TaskExecutable(
                task="the task", period=TimeOfDay("07:00", "08:00"), retries=1
            ),
            [
                ("2020-01-01 07:10", "run"),
                ("2020-01-01 07:20", "terminate"),
        ...
        pytest.param(
            ...
            "2020-01-01 07:30",
            False,
            id="Do not run (has inacted)",
        ),  # One would not want the task to try to run every millisecond if inacted.
        pytest.param(
            lambda: TaskExecutable(task="the task", period=TimeOfDay("07:00", "08:00")),
            [
                ("2020-01-01 07:10", "run"),
            ...
            "2020-01-01 07:30",
            False,
            id="Do not run (is already running)",
            marks=pytest.mark.xfail(
                reason="Bug but does not affect if no multiple schedulers nor multilaunch allowed"
            ),
        ),  # One would not want the task to try to run every millisecond if inacted.
        # Do run
        pytest.param(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            lambda: TaskExecutable(
                task="the task", period=TimeOfDay() & TimeOfMinute()
            ),
            [
...
def test_periods(
    mock_datetime_now, logs, time_after, get_condition, outcome, session, from_logs
):
