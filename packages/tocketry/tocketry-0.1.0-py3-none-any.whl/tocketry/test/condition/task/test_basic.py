...
def test_task_status_race(tmpdir, session, execution_number):
    ...
    task = FuncTask(run_task, name="runned task", execution="main", session=session)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(TaskFinished, True, True, id="TaskFinished Success"),
        pytest.param(TaskFinished, False, True, id="TaskFinished Failure"),
        pytest.param(TaskSucceeded, True, True, id="TaskSucceeded Success"),
        pytest.param(TaskSucceeded, False, False, id="TaskSucceeded Failure"),
        pytest.param(TaskFailed, True, False, id="TaskFailed Success"),
        pytest.param(TaskFailed, False, True, id="TaskFailed Failure"),
    ],
)
def test_task_status(tmpdir, session, cls, succeeding, expected):
    ...
    with tmpdir.as_cwd():
        ...
        # time.sleep(0.02)
        assert (
            condition.observe(task=task)
            if expected
            else not condition.observe(task=task)
        )
...
@pytest.mark.parametrize(
    "cls,expected",
    [
        pytest.param(DependFinish, True, id="DependFinish"),
        pytest.param(DependSuccess, False, id="DependSuccess"),
        pytest.param(DependFailure, True, id="DependFailure"),
    ],
)
def test_task_depend_fail(tmpdir, session, cls, expected):
    ...
    with tmpdir.as_cwd():
        ...
        depend_task = FuncTask(
            run_task, name="prerequisite task", execution="main", session=session
        )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(DependFinish, True, id="DependFinish"),
        pytest.param(DependSuccess, True, id="DependSuccess"),
        pytest.param(DependFailure, False, id="DependFailure"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            DependFinish,
            "task 'mydep' finished before 'mytask' started",
            id="DependFinish",
        ),
        pytest.param(
            DependFailure,
            "task 'mydep' failed before 'mytask' started",
            id="DependFailure",
        ),
        pytest.param(
            DependSuccess,
            "task 'mydep' succeeded before 'mytask' started",
            id="DependSuccess",
        ),
    ],
)
