...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            query.Any(query.Key("mykey1") == 1, query.Key("mykey2") == 2),
            "((<mykey1> == 1) | (<mykey2> == 2))",
            id="any",
        ),
        pytest.param(
            query.All(query.Key("mykey1") != 1, query.Key("mykey2") != 2),
            "((<mykey1> != 1) & (<mykey2> != 2))",
            id="all",
        ),
        ...
        pytest.param(
            query.Regex(query.Key("mykey"), r".+ is good"),
            "re.match('.+ is good', <mykey>)",
            id="regex",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            query.Key("mykey") >= 1, {"mykey": 1}, True, id="greater-equal true"
        ),
