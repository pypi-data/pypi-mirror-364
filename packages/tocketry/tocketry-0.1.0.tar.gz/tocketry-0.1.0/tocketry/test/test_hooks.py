...
from tocketry.tasks import FuncTask, _DummyTask
...
def test_task_init(session):
    ...
    assert session.hooks.task_init == [
        myhook,
        mygenerhook,
    ]  # The func is in different namespace thus different
...
def test_scheduler_startup(session):
    ...
    FuncTask(
        lambda: timeline.append("ran TASK (startup)"),
        name="start",
        on_startup=True,
        execution="main",
        session=session,
    ...
    task1 = FuncTask(
        lambda: timeline.append("ran TASK (normal 1)"),
        name="1",
        execution="main",
        start_cond=true,
        priority=1,
    ...
    task2 = FuncTask(
        lambda: timeline.append("ran TASK (normal 2)"),
        name="2",
        execution="main",
        start_cond=DependSuccess(depend_task=task1),
        priority=0,
        session=session,
    )
    FuncTask(
        lambda: timeline.append("ran TASK (shutdown)"),
        name="shut",
        on_shutdown=True,
        execution="main",
        session=session,
    ...
    assert session.hooks.scheduler_startup == [
        my_startup_hook,
        my_startup_hook_generator,
    ]
    assert session.hooks.scheduler_cycle == [my_cycle_hook, my_cycle_hook_generator]
    assert session.hooks.scheduler_shutdown == [
        my_shutdown_hook,
        my_shutdown_hook_generator,
    ]
...
@pytest.mark.parametrize(
    "func,exc_type,exc",
    [
        pytest.param(do_success, None, None, id="success"),
        pytest.param(do_fail, RuntimeError, RuntimeError("Deliberate fail"), id="fail"),
    ],
)
