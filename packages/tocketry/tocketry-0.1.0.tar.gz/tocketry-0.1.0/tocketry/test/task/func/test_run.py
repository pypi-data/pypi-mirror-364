...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(run_successful_func, "success", None, id="Success"),
        pytest.param(run_failing_func, "fail", RuntimeError, id="Failure"),
        pytest.param(run_inaction, "inaction", None, id="Inaction"),
    ],
)
def test_run(task_func, expected_outcome, exc_cls, execution, session):
    task = FuncTask(task_func, name="a task", execution=execution, session=session)
...
def test_parametrization_runtime(session):
    task = FuncTask(run_parametrized, name="a task", execution="main", session=session)
    ...
    task(
        params={
            ...
            "string": "X",
            "optional_float": 1.1,
            "extra_parameter": "Should not be passed",
        }
    )
