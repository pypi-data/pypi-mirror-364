import datetime
import pytest
from tocketry.time import TimeDelta, TimeOfDay
...
def test_repr():
    ...
    assert (
        str(TimeDelta(past="1 days", future="2 days"))
        == "past 1 day, 0:00:00 to next 2 days, 0:00:00"
    )
    assert (
        repr(TimeDelta("2 days"))
        == "TimeDelta(past=datetime.timedelta(days=2), future=datetime.timedelta(0))"
    )
