...
def run_with_func_arg(
    arg=FuncArg(get_some_value), arg2=FuncArg(get_some_value_with_arg, pass_yes="yes")
):
...
def test_return(execution, session):
    input_task = FuncTask(
        func=run_with_output,
        name="task_with_output",
        start_cond=(TaskStarted(task="task_with_output") == 0),
        execution=execution,
        session=session,
    )
    task = FuncTask(
        func=run_with_return,
        name="task_use_output",
        start_cond=DependSuccess(depend_task=input_task),
        execution=execution,
        session=session,
    )

    session.config.shut_cond = (
        TaskStarted(task="task_use_output") >= 1
    ) | ~SchedulerStarted(period=TimeDelta("10 seconds"))
    session.start()
...
def test_termination_flag_as_arg(execution, session):
    if execution == "process":
        pytest.skip(
            "For some reason CI fails on process. Termination flag should not be used with process tasks anyways."
        )

    task = FuncTask(
        func=run_with_termination_flag,
        name="my_task",
        start_cond=AlwaysTrue(),
        execution=execution,
        session=session,
    ...
    @FuncTask(
        name="terminator",
        execution="main",
        start_cond="task 'my_task' has started",
        session=session,
    )
    def task_terminate(session=Session()):
        session["my_task"].terminate()

    session.config.shut_cond = (TaskStarted(task="my_task") >= 1) | ~SchedulerStarted(
        period=TimeDelta("20 seconds")
    )
