...
def test_without_timeout(tmpdir, execution, session):
    ...
    with tmpdir.as_cwd():
        ...
        session.config.shut_cond = (
            TaskFinished(task="slow task but passing") >= 2
...
def test_task_terminate(tmpdir, execution, session):
    ...
    with tmpdir.as_cwd():
        ...
        FuncTask(
            terminate_task,
            name="terminator",
            start_cond=TaskRunning(task=task),
            execution="main",
            session=session,
        )
        session.config.shut_cond = (
...
def test_task_terminate_end_cond(tmpdir, execution, session):
    ...
    with tmpdir.as_cwd():
        ...
        task = FuncTask(
            ...
            name="slow task",
            start_cond=AlwaysTrue(),
            end_cond=TaskStarted(task="slow task"),
            execution=execution,
            session=session,
        )

        session.config.shut_cond = (
            TaskStarted(task="slow task") >= 1
        ) | ~SchedulerStarted(period=TimeDelta("20 seconds"))
        session.start()
...
def test_permanent_task(tmpdir, execution, session):
    ...
    with tmpdir.as_cwd():
        ...
        session.config.shut_cond = (
            TaskStarted(task="slow task but passing") >= 3
        ) | ~SchedulerStarted(period=TimeDelta("20 seconds"))
        session.config.timeout = 0.1
        session.start()
        ...
        assert (
            1 == logger.filter_by(action="terminate").count()
        )  # The last run is terminated
        assert 2 == logger.filter_by(action="success").count()
        assert 0 == logger.filter_by(action="fail").count()

        assert (
            logger.filter_by(action="terminate").last().created
            >= logger.filter_by(action="success").last().created
        )
