...
class WSNOCLIB : 
    ...
    def kafka_connect( self , 
        ...
        if new_release :
            self.kafka_nsp_os_name = 'nspos-kafka'
        ...
        if not self.external_nsp :
            self.ssh.ssh_execute(f"docker cp {self.kafka_nsp_os_name}:/opt/nsp/os/ssl/certs/nsp/nsp.pem /tmp/nsp.pem")
            self.ssh.ssh_execute(f"docker cp {self.kafka_nsp_os_name}:/opt/nsp/os/ssl/nsp.key /tmp/nsp.key")
            ...
            containerId = f"ctr -n k8s.io containers list | grep -i nspos-tomcat | awk '{{print $1}}'"
            containerId = self.ssh.ssh_execute(containerId).replace('\n','')
            self.logger.debug(f"containerId: {containerId}")
