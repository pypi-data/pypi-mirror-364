...
class ApiYamlBase(RequestBase, metaclass=SingletonMeta):
    ...
    def _request(self, method, url, **kwargs):
        ...
        if response.headers.get("Content-Type", "").startswith("application/json") and isinstance(response.json(),
                                                                                                  dict):
    ...
    def _apply_response(self, response, kwargs):
        ...
        elif response.headers.get("Content-Type") == "application/octet-stream" or response.headers.get(
                'Content-Disposition'):
...
class OperationApi(metaclass=SingletonMeta):
    ...
    def statistic_api(self, service_name=None):
        ...
        for service in os.listdir(self.api_path):
            ...
            number_map[service] = {"api_num": total}
        if service_name:
            total = sum([v['api_num'] for v in number_map.values()])
            number_map = {service_name: {"api_num": total}}
