...
from localstack.pro.core.eventstudio.utils import get_eventstudio_logger
from localstack.pro.core.runtime.plugin import ProPlatformPlugin
LOG=get_eventstudio_logger(__name__)
class EventStudioPlugin(ProPlatformPlugin):
	name='eventstudio'
	def should_load(A):
		if not config.EVENTSTUDIO_DEV_ENABLE:return False
		LOG.debug('EventStudioPlugin is enabled via EVENTSTUDIO_DEV_ENABLE');return super().should_load()
