...
from ..common import ColumnName, mk_attr_columns, mk_header_file_columns
...
class Header(pydantic.BaseModel):
    ...
    def typed_df_columns(self) -> dict[ConceptName, dict[TypeName, tuple[list[str], dict[str, MITMDataType]]]]:
        return {
            c: {tp: mk_typed_df_columns(self.mitm, he) for tp, he in tps.items()} for c, tps in self.as_dict.items()
        }
