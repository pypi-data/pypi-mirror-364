...
def test_add_dependency(pyproject: Pyproject, toml_parser: BaseToml) -> None:
    ...
    expected_dependencies = data["project"]["dependencies"] + packages
    pyproject.manage_dependencies("add", packages)
...
def test_remove_dependency(
    ...
    packages = ["foo", "baz"]
    pyproject.manage_dependencies("remove", packages)
    result = toml_parser.load()
...
def test_remove_dependency_with_constraint(
    ...
    packages = [f"foo{constraint}1.0.0", f"baz{constraint}2.0.0"]
...
def test_add_dependency_with_group(
    ...
    pyproject.manage_dependencies("add", packages, group)
    result = toml_parser.load()
    assert (
...
def test_remove_dependency_with_group(
    ...
    group = "dev"
    packages = ["developing", "nonexistent"]
    pyproject.manage_dependencies("remove", packages, group)
    result = toml_parser.load()
