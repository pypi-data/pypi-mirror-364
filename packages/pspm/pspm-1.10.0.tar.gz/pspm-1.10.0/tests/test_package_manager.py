...
class DummyPyproject(BasePyproject):
    ...
    def manage_dependencies(
        self, action: str, packages: list[str], group: str | None = None
    ) -> None:
        ...
        for package in packages:
            if not group:
                ...
                elif package in data["project"]["dependencies"]:
                ...
                if action == "add":
                    ...
                    self.added_group_dependencies[group] = (
                        self.added_group_dependencies.get(group, [])
                        + [package]
                    )
                else:
                    data["project"]["optional-dependencies"][group].remove(
                        package
                    )
...
def test_add_dependency(
    ...
    package_manager.manage_dependencies("add", packages)
    for package in packages:
...
def test_remove_dependency(
    ...
    packages = ["foo"]
    package_manager.manage_dependencies("remove", packages)
    for package in packages:
...
def test_manage_dependencies_compiles_all_files(
    action: Literal["add", "remove"],
    package_manager: PackageManager,
    resolver: DummyResolver,
) -> None:
    packages = ["bla"] if action == "add" else ["foo"]
    package_manager.manage_dependencies(action, packages)
...
def test_add_dependency_with_group(
    ...
    group = "test"
    package_manager.manage_dependencies("add", packages, group)
...
def test_remove_dependency_with_group(
    ...
    packages = ["testing"]
    group = "test"
    package_manager.manage_dependencies("remove", packages, group)

    for package in packages:
        assert package in pyproject.uninstalled_group_dependencies.get(
            group, []
        )
        assert package not in installer.installed_packages
