...
This positions us as an innovation leader while solving traditional remote work challenges."""
    ...
    async def test_full_discussion_workflow(
        ...
        mock_rich_console.return_value = realistic_task_result
        ...
        )  # Should be a boolean (realistic mixed opinions = no consensus)
        assert len(result.final_recommendation) > 100  # Should have substantial content
        ...
        # The synthesis uses the last comprehensive message if it's > 200 chars
    ...
    async def test_partial_agent_participation(
        self, mock_group_chat, mock_rich_console, mock_create_agent, test_configs
    ):
        ...
        mock_rich_console.return_value = task_result
    ...
    async def test_quick_consensus_integration(self, mock_create_agent, test_configs):
        ...
        with patch.object(panel, "discuss") as mock_discuss:
            from agent_expert_panel.panel import PanelResult, DiscussionPattern
