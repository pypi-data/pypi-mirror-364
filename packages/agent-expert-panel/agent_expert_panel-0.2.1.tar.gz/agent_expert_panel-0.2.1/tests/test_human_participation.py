...
class TestHumanParticipation:
    ...
    async def test_discuss_with_human_enabled(self, panel):
        ...
        with patch.object(panel, "_run_round_robin_discussion") as mock_discussion:
            ...
            agent_names = args[2]  # Third argument is the agent names list

            assert len(agents_list) > 5  # Should have more than the 5 AI agents
            assert "test_human" in agent_names
            assert kwargs.get("with_human", False) is True
    ...
    async def test_discuss_without_human(self, panel):
        ...
        with patch.object(panel, "_run_round_robin_discussion") as mock_discussion:
            ...
            assert len(agents_list) == 5  # Should have exactly 5 AI agents
            assert len(agent_names) == 5
            assert kwargs.get("with_human", False) is False
