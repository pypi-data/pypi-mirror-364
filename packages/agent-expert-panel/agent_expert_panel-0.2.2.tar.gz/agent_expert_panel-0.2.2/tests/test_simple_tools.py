...
class TestSimpleTools:
    ...
    def test_import_function_errors(self):
        ...
        with pytest.raises(ImportError, match="Could not import 'nonexistent.module'"):
            import_function("nonexistent.module.function")

        # Test with non-existent function in existing module
        with pytest.raises(
            AttributeError, match="Module 'json' has no attribute 'nonexistent'"
        ):
    ...
    async def test_create_library_tool_function_error(self):
        ...
        with patch("agent_expert_panel.tools.import_function", return_value=error_func):
    ...
    @patch("agent_expert_panel.tools.import_function")
