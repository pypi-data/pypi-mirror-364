...
def _extract_text(resp) -> str:
    ...
    try:
        ...
        # If response has no text (finish_reason=2 with no content), return empty JSON array
        log.debug(f"Response has no text content: {e}")
        return "[]"
...
def identify_evolution_locations(
    ...
    *,
    ...
    # Prepare to load manuscript PDF directly
    manuscript_pdf_data = None
    ...
    if pdf_paths:
        ...
        # For enzyme lineage, load both manuscript and SI PDFs since mutations can be in either
        manuscript_pdf_data = None
        si_pdf_data = None
        
        if len(pdf_paths) >= 1:
            manuscript_pdf = pdf_paths[0]
            log.info(f"Loading manuscript PDF: {manuscript_pdf.name}")
            
            try:
                with open(manuscript_pdf, 'rb') as pdf_file:
                    manuscript_pdf_data = pdf_file.read()
                log.info(f"Successfully loaded manuscript PDF ({len(manuscript_pdf_data)} bytes)")
            except Exception as e:
                log.warning(f"Failed to load manuscript PDF: {e}")
        
        if len(pdf_paths) >= 2:
            si_pdf = pdf_paths[1]
            log.info(f"Loading SI PDF: {si_pdf.name}")
            
            try:
                with open(si_pdf, 'rb') as pdf_file:
                    si_pdf_data = pdf_file.read()
                log.info(f"Successfully loaded SI PDF ({len(si_pdf_data)} bytes)")
            except Exception as e:
                log.warning(f"Failed to load SI PDF: {e}")
    ...
    locs: List[dict] = []
    try:
        if manuscript_pdf_data or si_pdf_data:
            # Use vision API with PDFs
            log.info("Using vision API with PDFs (manuscript: %s, SI: %s)", 
                    manuscript_pdf_data is not None, si_pdf_data is not None)
            
            # Build multimodal prompt with both PDFs
            multimodal_prompt = [prompt]
            
            # Add manuscript PDF if available
            if manuscript_pdf_data:
                multimodal_prompt.extend([
                    "\n\n=== MANUSCRIPT PDF ===\n",
                    {
                        "mime_type": "application/pdf",
                        "data": manuscript_pdf_data
                    }
                ])
            
            # Add SI PDF if available
            if si_pdf_data:
                multimodal_prompt.extend([
            ...
            if debug_dir:
                ...
                debug_info = f"=== VISION PROMPT FOR LOCATE ===\nTimestamp: {time.strftime('%Y-%m-%d %H:%M:%S')}\n"
                if manuscript_pdf_data:
                    debug_info += f"Manuscript PDF size: {len(manuscript_pdf_data)} bytes\n"
                if si_pdf_data:
                    debug_info += f"SI PDF size: {len(si_pdf_data)} bytes\n"
                debug_info += f"{'='*80}\n\n{prompt}\n\n"
                if manuscript_pdf_data:
                    debug_info += "[Manuscript PDF attached]\n"
                if si_pdf_data:
                    debug_info += "[SI PDF attached]\n"
                _dump(debug_info, prompt_file)
            ...
            # Fall back to text-only mode with just captions/TOC
            log.warning("No PDFs available for location identification, using minimal text")
            # Only include TOC and first few pages for context
            limited_text = toc_text + "\n\n" + text[:10000] if toc_text else text[:10000]
            prompt += "\n\nTEXT (Limited to table of contents and first pages):\n" + limited_text
    ...
    if campaigns:
        ...
        for campaign in campaigns:
            ...
            if not extracted_variants and location_type in ['table', 'text', 'section']:
                    ...
                    # Determine which text to use based on document
                    location_document = primary_location.get('document', 'manuscript')
                    if location_document in ['supplementary', 'si'] and si_text:
                        text_to_search = si_text
                        log.info(f"Using SI text for location {location_str}")
                    elif location_document == 'manuscript' and manuscript_text:
                    ...
                    caption_hint = primary_location.get('caption', '')
    ...
    for entry in raw:
        ...
        if dna:
            ...
            # Check if DNA sequence length is divisible by 3
                ...
                log.info(f"DNA sequence for {vid}: length {len(dna)} not divisible by 3 (may be partial sequence)")
                # Don't skip - many sequences in papers are partial or have incomplete codons
            else:
...
def get_sequences(text: str, model, *, pdf_paths: List[Path] = None, debug_dir: str | Path | None = None, lineage_variants: List[Variant] = None) -> list[SequenceBlock]:
    ...
    if locations:
        ...
        if best_loc_id >= 0 and best_loc_id < len(locations):
            # Log best location info
            best_location = locations[best_loc_id]
            log.info("Validation identified best location: %s (reason: %s)", 
                     loc_strs[best_loc_id] if best_loc_id < len(loc_strs) else str(best_location),
                     validation.get("reason", ""))
            
            # Extract text from ALL locations, not just the best one
            log.info("Extracting text from all %d identified sequence locations", len(locations))
            ...
            # Collect text from all locations
            all_extracted_texts = []
            ...
            for idx, location in enumerate(locations):
                ...
                if pdf_paths and isinstance(location, dict) and 'page' in location:
                    ...
                    if all_pages:
                        location_text = "\n".join(all_pages)
                        log.info("Location %d: Extracted %d chars from pages %s through %d more pages", 
                                 idx, len(location_text), page_num, len(all_pages) - 1)
                
                # Fallback to text search if page extraction didn't work
                if not location_text:
                    log.info("Location %d: Page extraction did not return text, falling back to text search", idx)
                    location_text = _extract_text_at_locations(
                        text, [location], 
                ...
                if location_text:
                    all_extracted_texts.append(location_text)
                    log.info("Location %d: Added %d chars of text", idx, len(location_text))
            
            # Combine all extracted texts
            focused_text = "\n\n--- NEXT SEQUENCE LOCATION ---\n\n".join(all_extracted_texts) if all_extracted_texts else ""
            
            # Use focused text if we got any content, regardless of size
            if focused_text:
                if len(focused_text) < len(text):
                    log.info("Combined text from all %d locations: reduced from %d to %d chars", 
                             len(all_extracted_texts), len(text), len(focused_text))
                else:
                    log.info("Combined text from all %d locations: %d chars (full text: %d chars)", 
                             len(all_extracted_texts), len(focused_text), len(text))
                ...
                log.warning("Failed to extract focused text from any of the %d locations, will use full text", len(locations))
        else:
