...
class ReactionExtractor:
    ...
    def _validate_response(self, data: Dict, enzyme_list: List[str], ref: str) -> bool:
        ...
        for key, enzyme_data in data.items():
            ...
            if matched and isinstance(enzyme_data, dict) and enzyme_data:
    ...
    def extract_metrics_batch(self, enzyme_list: List[str], ref: Union[str, Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        if image_b64 or pdf_data:
            ...
            if pdf_data:
                location_context = f"\n\nIMPORTANT: The performance data is located at {location_str} in the {pdf_type.upper()}. The attached PDF contains the complete {pdf_type}. Please analyze the PDF to find the performance data at the specified location.\n"
                LOGGER.info("Gemini Vision: extracting metrics for %d enzymes from FULL %s PDF (location: %s)…", len(enzyme_list), pdf_type.upper(), location_str)
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                for key, enzyme_data in data.items():
                    ...
                    if matched_enzyme and isinstance(enzyme_data, dict):
