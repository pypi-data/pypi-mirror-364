...
from IMTreatment import Profile, ScalarField, VectorField
...
class System:
    ...
    def __init__(self, u_inf=0.0, alpha=0.0):
        """
        Parameters
        ----------
    ...
    def solving_sigma(self):
        ...
        for i, p_i in enumerate(solid_panels):
            for j, p_j in enumerate(solid_panels):
                ...
                A[i, j] = (
                    0.5
                    / np.pi
                    * integral(p_i.xc, p_i.yc, p_j, p_i.cosbeta, p_i.sinbeta)
                )
        ...
        # computes the RHS of the linear system
        b = self.u_inf * np.sin(
            [self.alpha - panel.beta for panel in solid_panels]
        )
    ...
    def compute_velocity(self, x, y):
        ...
        for pan in solid_panels:
            if pan.on_panel(x, y):
                return pan.vector
        # compute u
        u = self.u_inf * np.cos(self.alpha) + 0.5 / np.pi * sum(
            [p.sigma * integral(x, y, p, 1, 0) for p in solid_panels]
        )
        # compute v
        v = self.u_inf * np.sin(self.alpha) + 0.5 / np.pi * sum(
            [p.sigma * integral(x, y, p, 0, 1) for p in solid_panels]
        )
    ...
    def compute_velocity_on_grid(
        self, grid_x, grid_y, raw=False, remove_solid=False
    ):
        """
        ...
        for i in range(Nx):
            for j in range(Ny):
                ...
                if remove_solid:
                    in_solid = np.any(
                        [
                            path.contains_point((x, y), radius=-1e-10)
                            for path in paths
                        ]
                    )
                    if in_solid:
    ...
    def compute_velocity_on_line(
        self, xya, xyb, res=100, raw=False, remove_solid=False
    ):
        """
        ...
        if isinstance(obj, VectorField):
            ...
            if isinstance(obj[0], Profile):
                ...
                if raw:
                    ...
                    prof = Profile(
                        obj[0].x, cp, mask=mask, unit_x=obj[0].unit_x, unit_y=""
                    )
                    return prof
            else:
                raise TypeError()
...
class object_1D:
    ...
    def _actualize_panels(self):
        ...
        self.panels = np.empty(shape=(self.nmb_panels,), dtype=object)
...
class Panel:
    ...
    def __init__(self, xya, xyb, sigma=0.0):
        ...
        if self.xa == self.xb and self.ya == self.yb:
            raise ValueError()
        self.xc, self.yc = (self.xa + self.xb) / 2.0, (self.ya + self.yb) / 2.0
        self.length = np.sqrt(
            (self.xb - self.xa) ** 2 + (self.yb - self.ya) ** 2
        )
        self.length2 = self.length**2
        # orientation of the panel (angle between x-axis and panel's normal)
        if self.xb - self.xa <= 0.0:
            self.beta = np.arccos((self.yb - self.ya) / self.length)
        elif self.xb - self.xa > 0.0:
        ...
        if not self.vertical and not self.horizontal:
            ...
            self.coef_C1 = (1 + self.coef_a**2) ** 0.5
...
def integral(x, y, panel, dxdz, dydz):
    ...
    res = integrate.quad(lambda s: func(s), 0.0, panel.length, limit=50)
    if res[1] > 1e-5:
        print(
            "integration pb, check your geometry"
...
def get_separation_position(
    ...
    """
    # creating system
    syst = System(u_inf=Vd, alpha=0.0)
    syst.add_object(
        2,
        [
            [-D / 2.0, -D / 2.0],
    ...
    x_f = -D / 2.0
    x_0 = x_f - dx
    prof_vit = syst.compute_velocity_on_line(
        [x_f, 0], [x_0, 0], res=res_int, remove_solid=True
    )[0]
    # compute theta**2
    U = prof_vit.y
    theta_0 = 0.0
...
def get_gradP_length(
    D=1.0, Vd=1.0, perc=0.1, nu=1e-6, res_pot=20, res_int=500, eps=1e-6
):
    """
