...
from ..core import Points, TemporalVectorFields, VectorField
from ..utils import make_unit
from ..utils.types import ARRAYTYPES, NUMBERTYPES
...
class SolidVortex(Vortex):
    ...
    def __init__(self, x0=0.0, y0=0.0, omega=1.0, movable=True, idi=0):
        """
        Parameters
        ----------
...
class FreeVortex(Vortex):
    ...
    def __init__(self, x0=0.0, y0=0.0, gamma=1.0, movable=True, idi=0):
        """
        Parameters
        ----------
        x0, y0 : numbers, optional
...
class BurgerVortex(Vortex):
    ...
    def __init__(
        ...
        self.C1 = -self.alpha / (4.0 * self.viscosity)
        self.C2 = self.ksi / (2 * np.pi)
        self.C3 = -1 / 2.0 * self.alpha
    ...
    def get_vector(self, x, y):
        ...
        if r == 0:
            ...
            Vphi = (self.C2 / r) * (1 - np.exp(self.C1 * r**2)) * self.rot_dir
        # get velocity in the cartesian refenrentiel
        Vx, Vy = self._cyl_to_cart(theta, Vr, Vphi)
        # returning
        return Vx, Vy
...
class LambOseenVortex(Vortex):
    ...
    def __init__(
        self, x0=0, y0=0, ksi=1.0, t=1.0, viscosity=1e-6, movable=True, idi=0
    ):
        """
        Parameters
        ----------
    ...
    def get_vector(self, x, y):
        ...
        if r == 0:
            ...
            Vphi = (
                self.ksi
                / (2 * np.pi * r)
                * (1 - np.exp(-(r**2) / (4.0 * self.viscosity * self.t)))
                * self.rot_dir
            )
        # get velocity in the cartesian refenrentiel
        Vx, Vy = self._cyl_to_cart(theta, Vr, Vphi)
...
class RankineVortex(Vortex):
    ...
    def __init__(self, x0=0.0, y0=0.0, circ=1.0, rv=1.0, movable=True, idi=0):
        """
        Parameters
        ----------
        x0, y0 : numbers, optional
...
class LambChaplyginVortex(Vortex):
    ...
    def __init__(
        self, x0=0, y0=0, U=1.0, rv=1.0, Bessel_root_nmb=1, movable=True, idi=0
    ):
        """
        Parameters
        ----------
        ...
        """
        super(LambChaplyginVortex, self).__init__(
            x0, y0, movable=movable, idi=idi
        )
        self.U = U
        self.rv = rv
...
class PersoVortex(Vortex):
    ...
    def __init__(
        ...
        self.C1 = -self.alpha / (4.0 * self.nu)
...
class CustomField:
    ...
    def __init__(self, funct, unit_values="", idi=0):
        ...
        try:
            ...
            if not isinstance(Vx, NUMBERTYPES) or not isinstance(
                Vy, NUMBERTYPES
            ):
                raise TypeError()
        # store
...
class VortexSystem:
    ...
    def display_compounded_vector(self, x, y, scale=1.0, detailed=False):
        """
        Display the compounded vector in 'x', 'y' position.
        """
        # get data
        colors = [plt.cm.jet(i) for i in np.linspace(0, 1, len(self.vortex))]
        vort_vects, vort_ids, cf_vects, cf_ids = self.get_compounded_vector(
            x, y, detailed=detailed
        )
    ...
    def get_compounded_vector(self, x, y, detailed=False):
        ...
        if detailed:
            ...
            for idi in vortex_ids:
                vects.append(
                    self.get_vector(x, y, vortex_ids=[idi], cf_ids="none")
                )
            # loop on cf ids
            for idi in cf_ids:
                vects_cf.append(
                    self.get_vector(x, y, vortex_ids="none", cf_ids=[idi])
                )
        # return
    ...
    def get_vector_field(self, axe_x, axe_y, unit_x="", unit_y=""):
        ...
        # add vortex participation
        for vort in self.vortex:
            VF += vort.get_vector_field(
                axe_x, axe_y, unit_x=unit_x, unit_y=unit_y
            )
        # add imaginary vortex participation
        for ivort in self.im_vortex:
            VF += ivort.get_vector_field(
                axe_x, axe_y, unit_x=unit_x, unit_y=unit_y
            )
        # add custom fields
        for cst_field in self.custfields:
            VF += cst_field.get_vector_field(
                axe_x, axe_y, unit_x=unit_x, unit_y=unit_y
            )
        # returning
        return VF

    def get_evolution(self, dt=1.0):
        """
    ...
    def get_temporal_vector_field(
        self, dt, axe_x, axe_y, nmb_it, unit_x="", unit_y="", unit_time=""
    ):
        """ """
        # create tvf
        time = 0.0
        tmp_tvf = TemporalVectorFields()
        tmp_vf = self.get_vector_field(
            axe_x, axe_y, unit_x=unit_x, unit_y=unit_y
        )
        ...
        for i in np.arange(nmb_it):
            ...
            tmp_vf = tmp_vs.get_vector_field(
                axe_x, axe_y, unit_x=unit_x, unit_y=unit_y
            )
            tmp_tvf.add_field(tmp_vf, time=time, unit_times=unit_time)
            time += dt
    ...
    def get_vortex_evolution(self, dt, nmb_it, unit_time=""):
        ...
        for vort in self.vortex:
            tmp_pts.append(
                Points(xy=[[vort.x0, vort.y0]], v=[time], unit_v=unit_time)
            )
        # make time iterations
        tmp_vs = self.copy()
        for i in np.arange(nmb_it):
...
class HsvSystem(VortexSystem):
    ...
    def _get_velocity_slowdown(D):
        ...
        PS = pf.System(u_inf=1.0, alpha=0.0)
        PS.add_object(
            2, [[0, -D / 2], [0, D / 2], [D, D / 2], [D, -D / 2]], res=20
        )
        PS.objects_2D[0].inverse_normals()
        Vx, _ = PS.compute_velocity_on_line(
            [-5 * D, 0], [0, 0], res=200, raw=False, remove_solid=False
        )
        Vx.x -= 5 * D
        sd_funct = spinterp.interp1d(
            Vx.x,
            Vx.y,
            kind="linear",
            axis=0,
...
class StepSystem(VortexSystem):
    ...
    def _get_velocity_deviation(H, h, u_D):
        # create potential flow
        PS = pf.System(u_inf=u_D, alpha=0.0)
        PS.add_object(2, [[1.0 * H, -H], [0, -H], [0, H], [1.0 * H, H]], res=20)
        PS.objects_2D[0].inverse_normals()
        axe_x = np.linspace(-3 * H, 0, 31)
        axe_y = np.arange(0, h + H / 10.0, H / 10.0)
        VF = PS.compute_velocity_on_grid(axe_x, axe_y, remove_solid=True)
        interp_x = spinterp.RegularGridInterpolator(
            (axe_x, axe_y), np.transpose(VF.comp_x), fill_value=1.0
        )
        interp_y = spinterp.RegularGridInterpolator(
            (axe_x, axe_y), np.transpose(VF.comp_y), fill_value=0.0
        )
