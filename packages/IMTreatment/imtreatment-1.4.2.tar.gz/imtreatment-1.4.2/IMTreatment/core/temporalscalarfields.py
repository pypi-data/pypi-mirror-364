...
from ..utils.types import STRINGTYPES
...
class TemporalScalarFields(tf.TemporalFields):
    ...
    def get_min_field(self, nmb_min=1):
        ...
        for field in self.fields[1::]:
            new_min_mask = np.logical_and(
                field.values < result_f.values, np.logical_not(field.mask)
            )
    ...
    def get_background(self, noise_level=10, verbose=False):
        ...
        if verbose:
            pg = ProgressCounter(
                init_mess="Computing background image",
        ...
        for i, j in np.ndindex(self.shape):
            ...
            while True:
                vals = vals[
                    np.logical_and(vals > mean - std, vals < mean + std)
                ]
                old_std = std
                std = np.std(vals)
                mean = np.mean(vals)
    ...
    def substract_background(
        self, bg, noise_level=10, blend=True, filling_value=None, verbose=False
    ):
        """
        ...
        if verbose:
            pg = ProgressCounter(
                init_mess="Substracting background image",
        ...
        for n in range(len(ntf)):
            ...
            if blend:
                ...
                values = (
                    values * blend_field + (1 - blend_field) * filling_value
                )
    ...
    def get_phase_map(self, freq, tf=None, check_spec=None, verbose=True):
        ...
        if verbose:
            pg = ProgressCounter(
                init_mess="Computing phase maps",
                nmb_max=self.shape[0] * self.shape[1],
    ...
    def get_spectral_filtering(self, fmin, fmax, order=2, inplace=False):
        ...
        ftsf = self._get_comp_spectral_filtering(
            "values", fmin=fmin, fmax=fmax, order=order
        )
    ...
    def fill(
        self, tof="spatial", kind="linear", value=0.0, inplace=False, crop=False
    ):
        """
            ...
            for i, j in np.argwhere(super_mask):
                ...
                if kind == "value":
                    ...
                    interp = spinterp.interp1d(
                        prof.x[prof_filt], prof.y[prof_filt], kind="linear"
                    )

                elif kind == "cubic":
                    prof_filt = np.logical_not(prof.mask)
                    interp = spinterp.interp1d(
                        prof.x[prof_filt], prof.y[prof_filt], kind="cubic"
                    )
                else:
