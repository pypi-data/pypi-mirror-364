...
class TemporalPoints:
    ...
    def times(self, values):
        ...
        if len(self.point_sets) != len(values):
            raise ValueError(
                f"New number of time ({len(values)}) do not corespond to "
                f"the number of fields ({len(self.point_sets)})"
            )
        self.__times = values
    ...
    def scale(
        self, scalex=None, scaley=None, scalev=None, scalet=None, inplace=False
    ):
        """
    ...
    def add_pts(self, pt, time=0.0, unit_times=""):
        """
        ...
        if not isinstance(time, NUMBERTYPES):
            raise TypeError(f"'time' should be a number, not {type(time)}")
