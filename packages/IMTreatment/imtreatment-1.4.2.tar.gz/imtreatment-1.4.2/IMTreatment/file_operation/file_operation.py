    ...
    if not path.exists(filepath):
        ...
        while True:
            ...
            if not path.exists(new_tested_path):
                ...
                err_mess = (
                    r"'{}' directory/file not found in '{}' directory.".format(
                        new_dir, valid_path
                    )
                )
...
def find_file_in_path(regs, dirpath, ask=False):
    ...
    if ask and len(dir_paths) > 1:
        print(f"{len(dir_paths)} folders found :")
...
def matlab_parser(obj, name):
    ...
    if obj is None:
        ...
        if simple:
            ...
            return {
                name: [
                    matlab_parser(obj[i], f"obj{i}") for i in range(len(obj))
                ]
            }
            raise OSError("Matlab can't handle this kind of variable")
        ...
        dic.update(matlab_parser(obj.unit_y.strUnit()[1:-1], "unit_y"))
        dic.update(
            matlab_parser(obj.unit_values.strUnit()[1:-1], "unit_values")
        )
        return {name: dic}
        ...
        raise OSError(f"Can't parser that : \n {obj}")
...
def import_from_file(filepath, **kw):
    ...
    if extension == ".imt":
        ...
        raise OSError(
            "File is not readable " f"(unknown extension : {extension})"
        )
    # For backward support
    attr_name = "_values_dtype"
...
def import_from_matlab(filepath, obj, show_struct=False, **kwargs):
    ...
    if show_struct:
        if is_h5py:
            ...
            print(f"Available keys in {filepath}:" f"\n{keys}")
        return data
...
def _get_imx_buffers(filename):
    ...
    try:
        ...
        raise Exception(
            "You need the 'ReadIM' module to import from davis" " files"
        )
...
def import_from_IM7(filename, infos=False):
    ...
    if not (ext == ".im7" or ext == ".IM7"):
        raise ValueError(
            f"I need the file to be an IM7 file (not a {ext} file)"
        )
...
def import_from_IM7s(fieldspath, kind="TSF", fieldnumbers=None, incr=1):
    ...
    if isinstance(fieldspath, ARRAYTYPES):
        if not isinstance(fieldspath[0], STRINGTYPES):
            raise TypeError(
                "'fieldspath' must be a string or a tuple of" " string"
            )
        ...
        paths = np.asarray(
            [
                f
                for f in glob(path.join(fieldspath, "*"))
        ...
        if len(paths) == 0:
            poss_paths = find_file_in_path(
                [".*.im7", ".*.IM7"], fieldspath, ask=True
            )
            if len(poss_paths) == 0:
                raise ValueError()
            paths = np.asarray(
    ...
    if fieldnumbers is not None:
        ...
        if not len(fieldnumbers) == 2:
            raise TypeError("'fieldnumbers' must be a 2x1 array")
        if not isinstance(fieldnumbers[0], int) or not isinstance(
            fieldnumbers[1], int
        ):
...
def import_from_VC7(filename, infos=False, add_fields=False):
    ...
    if fmt == 2:
        ...
        Vx = v_array[1]
        Vy = v_array[2]
    mask = np.logical_or(mask, np.logical_and(Vx == 0.0, Vy == 0.0))
...
def import_from_VC7s(
    ...
    pc = ProgressCounter(
        init_mess=f"Begin importation of {nmb_files} VC7 files",
        nmb_max=nmb_files,
    ...
    for i, p in enumerate(paths[start:end:incr]):
        if verbose:
            pc.print_progress()
        if add_fields:
            tmp_vf, infos, add_fields = import_from_VC7(
                p, infos=True, add_fields=True
            )
            dt = infos["FrameDt0"].split()
            unit_time = make_unit(dt[1])
            dt = float(dt[0])
    ...
    if path.isdir(im7_path):
        if kind in ["SSF", "TSF"]:
            ...
            for tmp_path in paths:
                ...
                export_to_file(SF, imt_path + f"/{name}", compressed=compressed)
    ...
    if path.isdir(vc7_path):
        if kind in ["SVF", "TVF"]:
            ...
            for tmp_path in paths:
                ...
                export_to_file(VF, imt_path + f"/{name}", compressed=compressed)
...
def import_from_picture(
    ...
    if ext not in usable_ext:
        raise ValueError(
            "I need the file to be an supported picture file"
            f"(not a {ext} file)"
        )
    # importing from file
    values = imageio.imread(filepath, mode="F").transpose()[:, ::-1]
...
def export_to_pictures(SFs, filepath):
    ...
    for i, val in enumerate(values):
        ...
        val = val.convert("L")
        imageio.imwrite(f"{filepath}_{i:0>5}.png", val)
...
def export_to_video(SFs, filepath, fps=24, colormap=None):
    ...
    vid = cv2.VideoWriter(
        ...
        fps=fps,
        frameSize=(values[0].shape[1], values[0].shape[0]),
    )
    for val in values:
        if colormap is None:
            tmpval = cv2.cvtColor(
                (val * 255).astype("uint8"), cv2.COLOR_GRAY2RGB
            )
        else:
            tmpval = np.array(colormap(val)[:, :, 0:3] * 255, dtype=np.uint8)
            tmpval = tmpval[:, :, ::-1]
        # # TEMP
...
def import_pts_from_ascii(
    ...
    **kwargs :
    ...
    if v_col is None:
        v_col = 0
    if (
        not isinstance(x_col, int)
        or not isinstance(y_col, int)
...
def import_sf_from_ascii(
    ...
    **kwargs :
    ...
    if (
        ...
        raise TypeError(
            "'x_col', 'y_col', 'vx_col' and 'vy_col' must " "be integers"
        )
    ...
    **kwargs :
    ...
    for i in np.arange(interval[0], interval[1] + 1, incr):
        ...
        if len(path) != ref_path_len:
            raise Warning(
                "You should check your files names,"
                "i may have taken them in the wrong order."
            )
        tmp_vf = VectorField()
        tmp_vf.import_from_ascii(
            path,
            x_col,
            y_col,
            vx_col,
    ...
    if isinstance(obj, VectorField):
        ...
        for i, x in enumerate(obj.axe_x):
            for j, y in enumerate(obj.axe_y):
                f.write(f"{x}\t{y}\t{obj.comp_x[i, j]}\t{obj.comp_y[i, j]}\n")
        f.close()
    elif isinstance(obj, ScalarField):
        # open file
        f = open(filepath, "w")
        ...
        for i, x in enumerate(obj.axe_x):
            for j, y in enumerate(obj.axe_y):
                f.write(f"{x}\t{y}\t{obj.values[i, j]}\n")
        f.close()
    elif isinstance(obj, Profile):
        # open file
        f = open(filepath, "w")
        # write header
        header = f"# X {obj.unit_x.strUnit()}\tY {obj.unit_y.strUnit()}\n"
        f.write(header)
        # write data
        ...
        if len(obj.v) == 0:
            ...
            # write header
            header = "# X {}\tY {}\tV {}\n".format(
                obj.unit_x.strUnit(), obj.unit_y.strUnit(), obj.unit_v.strUnit()
            )
            f.write(header)
            # write data
        ...
        for typ in obj.cp_types:
            ...
            for t, pt in zip(obj.times, pts):
                export_to_ascii(pt, f"{basename}/{typ}_t={t}{t_unit}.txt")
    else:
        raise TypeError()
