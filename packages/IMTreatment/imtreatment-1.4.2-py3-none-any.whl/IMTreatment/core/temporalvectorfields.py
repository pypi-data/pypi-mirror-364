...
class TemporalVectorFields(tf.TemporalFields):
    ...
    def get_mean_kinetic_energy(self):
        ...
        final_sf = 1.0 / 2 * (values_x**2 + values_y**2)
        return final_sf
    ...
    def get_turbulent_intensity(self):
        ...
        """
        turb_int = (2.0 / 3.0 * self.get_tke()) ** (0.5) / self.magnitude_as_sf
        return turb_int
    ...
    def get_reynolds_stress(self, nmb_val_min=1):
        ...
        rs_xx_sf.import_from_arrays(
            axe_x, axe_y, rs_xx, mask_rs, unit_x, unit_y, unit_values**2
        )
        rs_yy_sf = sf.ScalarField()
        rs_yy_sf.import_from_arrays(
            axe_x, axe_y, rs_yy, mask_rs, unit_x, unit_y, unit_values**2
        )
        rs_xy_sf = sf.ScalarField()
        rs_xy_sf.import_from_arrays(
            axe_x, axe_y, rs_xy, mask_rs, unit_x, unit_y, unit_values**2
        )
    ...
    def get_spectral_filtering(self, fmin, fmax, order=2, inplace=False):
        ...
        ftsfx = self._get_comp_spectral_filtering(
            "Vx", fmin=fmin, fmax=fmax, order=order
        )
        ftsfy = self._get_comp_spectral_filtering(
            "Vy", fmin=fmin, fmax=fmax, order=order
        )
    ...
    def fill(
        ...
        if len(self.fields) < 3 and tof == "temporal":
            raise ValueError(
                "Not enough fields to fill with temporal" " interpolation"
            )
        ...
        if tof == "temporal":
            ...
            super_mask = np.logical_and(
                sum_masks > 0, sum_masks < len(self.fields) - 2
            )
            # loop on each field position
            for i, j in np.argwhere(super_mask):
                # get time profiles
                prof_x = self.get_time_profile(
                    component="comp_x", pt=[i, j], ind=True
                )
                prof_y = self.get_time_profile(
                    component="comp_y", pt=[i, j], ind=True
                )
                ...
                if kind == "value":
                    ...
                    interp_x = spinterp.interp1d(
                        prof_x.x[prof_filt], prof_x.y[prof_filt], kind="linear"
                    )
                    prof_filt = np.logical_not(prof_y.mask)
                    interp_y = spinterp.interp1d(
                        prof_y.x[prof_filt], prof_y.y[prof_filt], kind="linear"
                    )
                elif kind == "cubic":
                    prof_filt = np.logical_not(prof_x.mask)
                    interp_x = spinterp.interp1d(
                        prof_x.x[prof_filt], prof_x.y[prof_filt], kind="cubic"
                    )
                    prof_filt = np.logical_not(prof_y.mask)
                    interp_y = spinterp.interp1d(
                        prof_y.x[prof_filt], prof_y.y[prof_filt], kind="cubic"
                    )
                else:
        ...
        # spatial interpolation
            ...
            raise ValueError(f"Unknown parameter for 'tof' : {tof}")
        # returning
        if inplace:
            self.fields = fields
        else:
