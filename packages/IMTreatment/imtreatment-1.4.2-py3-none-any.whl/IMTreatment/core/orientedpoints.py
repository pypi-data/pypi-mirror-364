...
class OrientedPoints(pts.Points):
    ...
    def __init__(
        ...
        if len(xy) != 0 and not orientations.ndim == 3:
            raise ValueError(
                "'orientations' must have 3 dimensions, not {}".format(
                    orientations.ndim
                )
            )
    ...
    def orientations(self, new_ori):
        ...
        if len(self.xy) != 0 and new_ori.shape[0:3:2] != (len(self.xy), 2):
            raise ValueError(
                "'orientations' shape must be (n, d, 2)  (with n"
                f" the number of points ({len(self.xy)}) and d the number of "
                f"directions), not {new_ori.shape}"
            )
        self.__orientations = new_ori

    def get_streamlines(
        self, vf, delta=0.25, interp="linear", reverse_direction=False
    ):
        """
    ...
    def get_streamlines_from_orientations(
        self, vf, delta=0.25, interp="linear", reverse_direction=False
    ):
        """
        ...
        if reverse_direction.shape != (nmb_dir,):
            raise ValueError()
        # get coef
        coefx = (vf.axe_x[1] - vf.axe_x[0]) * 0.25
        coefy = (vf.axe_x[1] - vf.axe_x[0]) * 0.25
        # get streamlines
        streams = []
        ...
        for i, pt in enumerate(self.xy):
            for n in np.arange(nmb_dir):
                ...
                tmp_stream = get_streamlines(vf, [pt1, pt2], reverse=reverse)
    ...
    def add(self, pt, orientations, v=None):
        ...
        if len(self.orientations) == 0:
            ...
            self.orientations = np.append(
                self.orientations, [orientations], axis=0
            )
    ...
    def crop(self, intervx=None, intervy=None, intervv=None, inplace=True):
        ...
        if intervv is not None and len(self.v) != 0:
            out_zone = np.logical_or(self.v < intervv[0], self.v > intervv[1])
            mask = np.logical_or(mask, out_zone)
        # use inheritance
        super(OrientedPoints, tmp_pts).crop(
            intervx=intervx, intervy=intervy, intervv=intervv, inplace=True
        )
