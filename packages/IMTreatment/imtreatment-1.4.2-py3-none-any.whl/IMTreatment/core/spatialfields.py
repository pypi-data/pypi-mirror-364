...
class SpatialFields(flds.Fields):
    ...
    def __mul__(self, other):
        if isinstance(other, (NUMBERTYPES, unum.Unum)):
            ...
            raise TypeError(
                "You can only multiply a temporal velocity field " "by numbers"
            )
    ...
    def __truediv__(self, other):
        if isinstance(other, (NUMBERTYPES, unum.Unum)):
            ...
            raise TypeError(
                "You can only divide a temporal velocity field " "by numbers"
            )
    ...
    def get_value(self, x, y, unit=False, error=True):
        ...
        if len(inter_ind) == 0:
            ...
            values = self.fields[inter_ind[0]].get_value(
                x, y, ind=False, unit=False
            )
        else:
            values = self.fields[inter_ind[0]].get_value(
                x, y, ind=False, unit=False
    ...
    def get_profile(self, direction, position, component=None):
        ...
        if self.fields_type == vf.VectorField:
            if component is None:
                ...
                try:
                    comp = self.__getattribute__(f"{component}_as_sf")
        ...
        if direction == 1:
            for i, field in enumerate(self.fields):
                if np.any(position < field.axe_x[-1]) and np.any(
                    position > field.axe_x[0]
                ):
                    inter_ind.append(i)
        elif direction == 2:
            for i, field in enumerate(self.fields):
                if np.any(position < field.axe_y[-1]) and np.any(
                    position > field.axe_y[0]
                ):
                    inter_ind.append(i)
        # get profiles
        if len(inter_ind) == 0:
            ...
            return comp[inter_ind[0]].get_profile(
                direction=direction, position=position, ind=False
            )[0]
        else:
            # get profiles
            x = np.array([])
            ...
            x = x[ind_sort]
            y = y[ind_sort]
            fin_prof = prof.Profile(
                x, y, mask=False, unit_x=tmp_prof.unit_x, unit_y=tmp_prof.unit_y
            )
    ...
    def _display(self, compo=None, **plotargs):
        ...
        if self.fields_type == vf.VectorField:
            if compo == "V" or compo is None:
                ...
                try:
                    comp = self.__getattribute__(f"{compo}_as_sf")
                except AttributeError:
                    raise ValueError()
        elif self.fields_type == sf.ScalarField:
            if compo is None:
        ...
        if plotargs["kind"] == "stream":
            if "density" not in list(plotargs.keys()):
                plotargs["density"] = 1.0
            plotargs["density"] = (
                plotargs["density"] / (len(self.fields)) ** 0.5
            )
        # display
        for field in comp:
