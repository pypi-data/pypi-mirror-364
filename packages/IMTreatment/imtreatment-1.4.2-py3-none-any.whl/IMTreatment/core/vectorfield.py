...
class VectorField(field.Field):
    ...
    def __add__(self, other):
        if isinstance(other, VectorField):
            ...
            if np.all(self.axe_x == other.axe_x) and np.all(
                self.axe_y == other.axe_y
            ):
                tmpvf = self.copy()
            ...
            # different shape, partially same axis
                ...
                new_ind_x = np.array(
                    [
                        np.any(np.abs(val - other.axe_x) < np.abs(val) * 1e-4)
                        for val in self.axe_x
                    ]
                )
                new_ind_y = np.array(
                    [
                        np.any(np.abs(val - other.axe_y) < np.abs(val) * 1e-4)
                        for val in self.axe_y
                    ]
                )
                new_ind_xo = np.array(
                    [
                        np.any(np.abs(val - self.axe_x) < np.abs(val) * 1e-4)
                        for val in other.axe_x
                    ]
                )
                new_ind_yo = np.array(
                    [
                        np.any(np.abs(val - self.axe_y) < np.abs(val) * 1e-4)
                ...
                new_comp_x = (
                    self.comp_x[new_ind_value]
                    + other.comp_x[new_ind_valueo] * fact.asNumber()
                )
                new_comp_y = (
                    self.comp_y[new_ind_value]
                    + other.comp_y[new_ind_valueo] * fact.asNumber()
                )
                new_comp_x = new_comp_x.reshape(
                    (len(new_axe_x), len(new_axe_y))
                )
                new_comp_y = new_comp_y.reshape(
                    (len(new_axe_x), len(new_axe_y))
                )
                new_mask = np.logical_or(
                    self.mask[new_ind_value], other.mask[new_ind_valueo]
                )
            ...
            raise TypeError(
                "You can only add a velocity field "
    ...
    def __truediv__(self, other):
        if isinstance(other, ARRAYTYPES):
            ...
            raise TypeError("You can only divide a vector field " "by numbers")
    ...
    def __mul__(self, other):
        if isinstance(other, ARRAYTYPES):
            ...
            raise TypeError(
                "You cannot multiply Vectorfields object "
                f"with {type(other)} objects"
            )
    ...
    def __pow__(self, number):
        if not isinstance(number, NUMBERTYPES):
            raise TypeError(
                "You only can use a number for the power " "on a Vectorfield"
            )
        tmpvf = self.copy()
    ...
    def mask(self, new_mask):
        ...
        if np.any(np.logical_not(new_mask[self.mask])):
            raise Warning(
                "This mask reveal masked values, maybe you should"
    ...
    def import_from_arrays(
        ...
        if (
            len(axe_x) == comp_x.shape[1]
            and len(axe_y) == comp_x.shape[0]
    ...
    def get_value(self, x, y, ind=False, unit=False):
        ...
        return np.array(
            [
                self.comp_x_as_sf.get_value(x, y, ind=ind, unit=unit),
                self.comp_y_as_sf.get_value(x, y, ind=ind, unit=unit),
            ]
        )

    def get_profile(
        self, component, direction, position, ind=False, interp="linear"
    ):
        """
        ...
        """
        if component == "vx":
            return self.comp_x_as_sf.get_profile(
                direction, position, ind, interp=interp
            )
        elif component == "vy":
            return self.comp_y_as_sf.get_profile(
                direction, position, ind, interp=interp
            )
        else:
    ...
    def get_norm(self, norm=2, normalized="perpoint"):
        ...
        values = np.concatenate(
            (self.comp_x[~self.mask], self.comp_y[~self.mask])
        )
        res = (np.sum(np.abs(values) ** norm)) ** (1.0 / norm)
    ...
    def rotate(self, angle, inplace=False):
        ...
        comp_x2 = (
            np.cos(angle / 180.0 * np.pi) * comp_x
            - np.sin(angle / 180.0 * np.pi) * comp_y
        )
        comp_y2 = (
            np.cos(angle / 180.0 * np.pi) * comp_y
            + np.sin(angle / 180.0 * np.pi) * comp_x
        )
    ...
    def fill(
        ...
        new_comp_x = comp_x.fill(
            kind=kind, value=value[0], inplace=False, reduce_tri=reduce_tri
        )
        new_comp_y = comp_y.fill(
            kind=kind, value=value[1], inplace=False, reduce_tri=reduce_tri
        )
        # returning
        if inplace:
    ...
    def crop(self, intervx=None, intervy=None, ind=False, inplace=False):
        ...
        """
        if inplace:
            indmin_x, indmax_x, indmin_y, indmax_y = field.Field.crop(
                self, intervx, intervy, full_output=True, ind=ind, inplace=True
            )
            self.__comp_x = self.comp_x[
                indmin_x : indmax_x + 1, indmin_y : indmax_y + 1
            ]
            self.__comp_y = self.comp_y[
                indmin_x : indmax_x + 1, indmin_y : indmax_y + 1
            ]
            ...
            cropfield.__comp_x = self.comp_x[
                indmin_x : indmax_x + 1, indmin_y : indmax_y + 1
            ]
            cropfield.__comp_y = self.comp_y[
                indmin_x : indmax_x + 1, indmin_y : indmax_y + 1
            ]
        ...
        if hard:
            ...
            while np.any(tmp_sf.mask):
                ...
                if more_masked == 0:
                    ...
                    tmp_sf.crop(intervy=[1, len_y], ind=True, inplace=True)
                elif more_masked == 3:
    ...
    def extend(
        self, nmb_left=0, nmb_right=0, nmb_up=0, nmb_down=0, inplace=False
    ):
        """
    ...
    def _display(self, component=None, kind=None, axis="image", **plotargs):
        ...
        if kind == "stream" and self.xy_scale.asNumber() != 1.0:
            magn = self.magnitude
            new_comp_x = self.comp_x * self.xy_scale.asNumber()
            new_magn = (new_comp_x**2 + self.comp_y**2) ** 0.5
            values = [
                new_comp_x / new_magn * magn,
                self.comp_y / new_magn * magn,
            ]
        ...
        dp = pplt.Displayer(
            x=self.axe_x, y=self.axe_y, values=values, kind=kind, **plotargs
        )
