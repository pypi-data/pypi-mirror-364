...
class Files:
        ...
        if hide_top:
            ...
            while True:
                if isinstance(curr_fold, dict):
                    if (
                        len(list(curr_fold.keys())) == 1
                        and list(curr_fold.keys())[0] != "files"
                    ):
            ...
            tree = curr_fold
        # display the tree
        for thing in get_info_for_folder(tree, tab1=tab1, tab2=tab2):
            text += thing
        return text
    ...
    def load_files_from_regex(
        self, rootpath, regex, load_files=True, load_dirs=True, depth="all"
    ):
        """
        ...
        for root, dirs, files in os.walk(rootpath, topdown=True):
            # check depth
            if depth != "all":
                tmp_depth = root[len(rootpath) + len(os.path.sep) :].count(
                    os.path.sep
                )
                if depth < tmp_depth:
                    break
    ...
    def delete_existing_files(self, recursive=False):
        ...
        print(
            f"+++ Ready to remove {nmb_files} files and {nmb_dir} directories "
        )
        while True:
...
def remove_files_in_dirs(
    ...
    # ask before deletion
    print("")
    print(f"+++ Checked {nmb_tot_files} files")
    if np.sum(nmb_files) == 0:
        print("+++ Nothing to delete")
        return None
    print(f"+++ Ready to remove {np.sum(nmb_files)} files in directories :")
    for i in range(len(dir_paths)):
        print(f"+++    [{nmb_files[i]} files] {dir_paths[i]}")
    while True:
        rep = input("+++ Okay with that ? ('o', 'n') \n+++ ")
        if rep in [
            "o",
