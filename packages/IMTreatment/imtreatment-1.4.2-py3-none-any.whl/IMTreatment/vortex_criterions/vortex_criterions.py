...
def get_angle_deviation(
    ...
    (
        ...
        mask_surr,
        motif,
    ) = _non_local_criterion_precomputation(
        vectorfield, mask, radius, ind, dev_pass=False
    )
    # Computing criterion
    # creating reference dispersion functions
    best_fun = np.array([2.0 * np.pi / nmbpts] * nmbpts)
    worse_fun = np.array([0] * (nmbpts - 1) + [2.0 * np.pi])
    worse_value = (np.sum(np.abs(worse_fun - best_fun) ** order)) ** (
        1.0 / order
    )
    # Loop on points
    ...
    for inds, pos, _ in vectorfield:
        ...
        deviation[ind_x, ind_y] = (
            1
            - (np.sum(np.abs(d_angles - best_fun) ** order)) ** (1.0 / order)
            / worse_value
        )
    # Applying masks
    ...
    if raw:
        ...
        deviation_sf.import_from_arrays(
            axe_x,
            axe_y,
            deviation,
            mask,
...
def get_gamma(
    ...
    if isinstance(vectorfield, TemporalVectorFields):
        ...
        for time, field in zip(vectorfield.times, vectorfield.fields):
            ...
            gammas.add_field(
                gamma, time=time, unit_times=vectorfield.unit_times
            )
        return gammas
    ...
    (
        ...
        vectorfield, mask, radius, ind, dev_pass
    )
    ...
    norm_vect_a = (vector_a_x**2 + vector_a_y**2) ** (0.5)
    # Loop on points
    ...
    for inds, pos, _ in vectorfield:
        ...
        if kind in ["gamma2b"]:
            v_mean2[0] = np.mean(
                (Vx[indsaround[:, 0], indsaround[:, 1]] - v_mean[0]) ** 2
            )
            v_mean2[1] = np.mean(
                (Vy[indsaround[:, 0], indsaround[:, 1]] - v_mean[1]) ** 2
            )
            fact = np.sqrt(v_mean2[0] + v_mean2[1]) / np.sqrt(
                v_mean[0] ** 2 + v_mean[1] ** 2
            )
            if np.abs(fact) > 1:
                fact = 1.0
...
def get_NL_residual_vorticity(
    vectorfield, radius=None, ind=False, mask=None, raw=False
):
    """
    ...
    s = np.sqrt(4 * Exx**2 + (Exy + Eyx) ** 2) / 2.0
...
def get_kappa(
    ...
    for inds, pos, _ in vectorfield:
        ...
        for i, indaround in enumerate(indsaround):
            ...
            if denom != 0:
                kappa += (
                    vector_a_x[i] * vector_b_x + vector_a_y[i] * vector_b_y
                ) / denom
...
def get_iota(vectorfield, mask=None, radius=None, ind=False, raw=False):
    ...
    iota = 1 / 2.0 * np.sqrt(theta_x**2 + theta_y**2)
    # getting mask
...
def get_enstrophy(vectorfield, radius=None, ind=False, mask=None, raw=False):
    """
    ...
    if raw:
        ...
        enstrophy_sf.import_from_arrays(
            axe_x,
            axe_y,
            enstrophy,
            mask,
...
def _non_local_criterion_precomputation(
    vectorfield, mask, radius, ind, dev_pass
):
    """ """
    ...
    axe_x, axe_y = vectorfield.axe_x, vectorfield.axe_y
    indcentral = [int(len(axe_x) / 2.0), int(len(axe_y) / 2.0)]
    if ind:
    ...
    if ind:
        ...
        border_x = np.logical_or(
            indx <= indx[0] + (int(radius) - 1),
            indx >= indx[-1] - (int(radius) - 1),
        )
        border_y = np.logical_or(
            indy <= indy[0] + (int(radius) - 1),
        ...
        border_x = np.logical_or(
            axe_x <= axe_x[0] + (radius - delta),
            axe_x >= axe_x[-1] - (radius - delta),
        )
        border_y = np.logical_or(
            axe_y <= axe_y[0] + (radius - delta),
    ...
    if dev_pass:
        ...
        mask_dev = dev < 0.1
    # returning
    return (mask, nmbpts, mask_dev, mask_border, mask_surr, motif)
...
def get_divergence(vf, raw=False):
    ...
    if isinstance(vf, VectorField):
        ...
        if raw:
            div_tsf = np.empty(
                (len(vf.fields), vf.shape[0], vf.shape[1]), dtype=float
            )
            ...
            for i, field in enumerate(vf.fields):
                ...
                div_tsf.add_field(
                    tmp_div, time=vf.times[i], unit_times=vf.unit_times
                )
        return div_tsf
    else:
        raise TypeError()
    ...
    if isinstance(vf, VectorField):
        ...
        if raw:
            ...
            for i, field in enumerate(vf.fields):
                ...
                vort_tsf.add_field(
                    tmp_vort, time=vf.times[i], unit_times=vf.unit_times
                )
        return vort_tsf
    else:
        raise TypeError()
...
def get_stokes_vorticity(vf, window_size=2, raw=False):
    ...
    # creating new axis
    new_axe_x = np.arange(
        np.mean(axe_x[0:window_size]),
        np.mean(axe_x[-window_size::] + dx * 0.9),
        dx,
    )
    new_axe_y = np.arange(
        np.mean(axe_y[0:window_size]),
        np.mean(axe_y[-window_size::] + dy * 0.9),
        dy,
    )
    # Loop on field
    ...
    for i in np.arange(len(axe_x) - window_size + 1):
        for j in np.arange(len(axe_y) - window_size + 1):
            ...
            tmp_vort += np.trapz(bord_vec, dx=dx)
            # adding coefficients
            tmp_vort *= 1.0 / (dx * dy * window_size**2)
            # storing
            vort[i, j] = tmp_vort
    # returning
    if raw:
...
def get_q_criterion(vectorfield, mask=None, raw=False):
    ...
    # calcul de Qcrit
    norm_rot2 = 1 / 2.0 * (Exy - Eyx) ** 2
    norm_shear2 = Exx**2 + 1.0 / 2.0 * (Exy + Eyx) ** 2 + Eyy**2
    qcrit = 0.5 * (norm_rot2) - norm_shear2
...
def get_Nk_criterion(vectorfield, mask=None, raw=False):
    ...
    Exx, Exy, Eyx, Eyy = get_gradients(vectorfield, raw=True)
    # calcul de Nk
    norm_rot = 1.0 / 2.0**0.5 * np.abs(Exy - Eyx)
    norm_shear = (Exx**2 + 1.0 / 2.0 * (Exy + Eyx) ** 2 + Eyy**2) ** 0.5
    ...
    for i in np.arange(lambda2.shape[0]):
        for j in np.arange(lambda2.shape[1]):
            ...
            Omega = (
                ...
                / 2.0
                * np.array(
                    [[0, Udy[i, j] - Vdx[i, j]], [Vdx[i, j] - Udy[i, j], 0]]
                )
            )
            # getting S^2 + Omega^2
...
def get_residual_vorticity(vf, raw=False):
    ...
    if isinstance(vf, VectorField):
        ...
        sign_omega[omega_abs == 0] = 1.0
        sign_omega[omega_abs != 0] = (
            omega[omega_abs != 0] / omega_abs[omega_abs != 0]
        )
        filt = s < omega_abs
        # getting the residual vorticity
        ...
        if raw:
            ...
            for i, field in enumerate(vf.fields):
                tvf.add_field(
                    get_residual_vorticity(field, raw=False),
                    time=vf.times[i],
...
def get_shear_vorticity(vf, raw=False):
    ...
    if isinstance(vf, VectorField):
        ...
        for i, field in enumerate(vf.fields):
            ...
            ret.add_field(tmp_ret, time=vf.times[i], unit_times=vf.unit_times)
        return ret
    else:
        raise TypeError()
