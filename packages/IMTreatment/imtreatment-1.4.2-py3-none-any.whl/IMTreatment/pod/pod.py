...
class ModalFields(Field):
    ...
    def __init__(
        ...
        if eigvects is not None:
            ...
            if not eigvects.shape == (
                len(temporal_evolutions[0].x),
                len(modes),
            ):
                raise ValueError()
    ...
    def modes_as_tf(self):
        if self.field_class == VectorField:
            ...
            for i in np.arange(len(self.modes)):
                tmp_tf.add_field(self.modes[i], time=i + 1, unit_times="")
            return tmp_tf
    ...
    def crop_modal_base(
        self, modes_to_keep=None, modes_to_remove=None, inplace=True
    ):
        """
        ...
        if modes_to_keep is not None:
            ...
            if isinstance(modes_to_remove, int):
                modes_to_remove = np.arrange(
                    len(self.modes) - modes_to_remove, len(self.modes)
                )
        ...
        if self.decomp_type == "pod":
            new_eigvals = Profile(
                unit_x=self.eigvals.unit_x, unit_y=self.eigvals.unit_y
            )
            new_eigvects = []
        elif self.decomp_type == "dmd":
            new_growth_rate = Profile(
                unit_x=self.eigvals.unit_x, unit_y=self.eigvals.unit_y
            )
            new_mode_norms = Profile(
                unit_x=self.eigvals.unit_x, unit_y=self.eigvals.unit_y
            )
            new_pulsation = Profile(
                unit_x=self.eigvals.unit_x, unit_y=self.eigvals.unit_y
            )
            new_ritz_vals = Profile(
                unit_x=self.eigvals.unit_x, unit_y=self.eigvals.unit_y
            )
        else:
            raise Exception(f"Not implemented for {self.decomp_type}")
        # loop on wanted modes
        for ind in modes_to_keep:
            new_modes.append(tmp_pod.modes[ind])
            new_temp_evo.append(tmp_pod.temp_evo[ind])
            if self.decomp_type == "pod":
                new_eigvals.add_point(
                    tmp_pod.eigvals.x[ind], tmp_pod.eigvals.y[ind]
                )
                tmp_vect = []
            ...
            if self.decomp_type == "dmd":
                new_growth_rate.add_point(
                    tmp_pod.growth_rate.x[ind], tmp_pod.growth_rate.y[ind]
                )
                new_mode_norms.add_point(
                    tmp_pod.mode_norms.x[ind], tmp_pod.mode_norms.y[ind]
                )
                new_pulsation.add_point(
                    tmp_pod.pulsation.x[ind], tmp_pod.pulsation.y[ind]
                )
                new_ritz_vals.add_point(
                    tmp_pod.ritz_vals.x[ind], tmp_pod.ritz_vals.y[ind]
                )
        # store
    ...
    def crop(
        self, intervx=None, intervy=None, intervt=None, ind=False, inplace=False
    ):
        """
        ...
        if intervx is not None or intervy is not None:
            ...
            tmp_mf.mean_field.crop(
                intervx=intervx, intervy=intervy, ind=ind, inplace=True
            )
            # loop on modes
    ...
    def scale(
        self, scalex=1.0, scaley=1.0, scalet=1.0, scalev=1.0, inplace=False
    ):
        """ """
        #
        if inplace:
            ...
            tmp_mf = self.copy()
        # scale mean field
        tmp_mf.mean_field.scale(
            scalex=scalex, scaley=scaley, scalev=scalev, inplace=True
        )
        ...
        for i in range(len(tmp_mf.temp_evo)):
            tmp_mf.temp_evo[i].scale(scalex=scalet, scaley=scalev, inplace=True)
        ...
        for i in range(len(tmp_mf.modes)):
            tmp_mf.modes[i].scale(scalex=scalex, scaley=scaley, inplace=True)
        # return
        if not inplace:
            return tmp_mf

    def smooth_temporal_evolutions(
        self, tos="uniform", size=None, inplace=True
    ):
        """
    ...
    def augment_temporal_resolution(
        self, fact=2, interp="linear", inplace=True, verbose=False
    ):
        """
        ...
        if not isinstance(interp, STRINGTYPES):
            raise TypeError()
        if interp not in ["linear", "nearest", "slinear", "quadratic", "cubic"]:
            raise ValueError()
    ...
    def get_modes_energy(self, cum=False, raw=False):
        ...
        for n in np.arange(len(self.modes)):
            if isinstance(self.modes[0], ScalarField):
                magnitude = 1.0 / 2.0 * np.real(self.modes[n].values) ** 2
            elif isinstance(self.modes[0], VectorField):
                magnitude = (
                    1.0
                    / 2.0
                    * (
                        np.real(self.modes[n].comp_x) ** 2
        ...
        if raw:
            ...
            modes_nrj = Profile(
                np.arange(len(self.modes)),
                modes_nrj,
                mask=False,
                unit_x="",
                unit_y=self.modes[0].unit_values ** 2,
                name="",
            )
        return modes_nrj
    ...
    def get_temporal_coherence(self, raw=False):
        ...
        if self._ModalFields__temp_coh is not None:
            ...
            for n in np.arange(len(self.modes)):
                ...
                spec = prof.get_spectrum(scaling="spectrum") ** 0.5
    ...
    def get_spatial_coherence(self, raw=False):
        ...
        if self._ModalFields__spat_coh is not None:
            ...
            center_x = int(len(axe_x) / 2.0)
            center_y = int(len(axe_y) / 2.0)
            ...
            for i in np.arange(20):
                min_std_field = np.random.rand(*shape) * 2.0 - 1.0
                min_std += get_field_spec_std(min_std_field)
            min_std /= 20.0
            # computing maximal variance
            max_std_spec = np.zeros(shape)
            max_std_spec[center_x : center_x + 2, center_y : center_y + 2] = 1.0
    ...
    def display(self):
        ...
        plot3 = pplt.Displayer(
            tmp_x, tmp_y, data_type="profile", kind="loglog", color="k"
        )
    ...
    def display_recap(self, figsize=(15, 10)):
        ...
        if self.decomp_type in ["pod", "bpod"]:
            ...
            delta_omega = np.mean(
                np.abs(sorted_omega[1::] - sorted_omega[0:-1:])
            )
            width = delta_omega / 2.0
            plt.bar(
                self.pulsation.y - width / 2.0, self.mode_norms.y, width=width
            )
            ...
            tmp_sf.display()
            plt.title(
                "More stable mode (pulsation={:.2f})\n"
                "(Real representation)".format(self.pulsation.y[stab_sort[-0]])
            )
            plt.subplot(2, 3, 4)
            ...
            tmp_sf.display()
            plt.title(
                "Mode with the bigger norm (pulsation={:.2f})\n"
                "(Real representation)".format(self.pulsation.y[norm_sort[-1]])
            )
        plt.tight_layout()
...
def _tsf_to_POD(tsf):
    ...
    del tsf
    snaps = [modred.VecHandleInMemory(values[t]) for t in ind_fields]
    del values
    # return
    return snaps, props
...
def _tvf_to_POD(tvf):
    ...
    # Linking to snaps
    values = [[tvf.fields[t].comp_x, tvf.fields[t].comp_y] for t in ind_fields]
    del tvf
    snaps = [
        modred.VecHandleInMemory(np.transpose(values[i], (1, 2, 0)))
        for i in ind_fields
    ]
    del values
    # return
...
def modal_decomposition(
    ...
    pulsation = None
    if kind == "pod":
        my_decomp = modred.PODHandles(
            np.vdot, max_vecs_per_node=max_vecs_per_node, verbosity=verbose
        )
        ...
        eigvals = Profile(
            wanted_modes,
            eigvals[wanted_modes],
            mask=False,
            unit_x=props["unit_times"],
            unit_y="",
        )
    elif kind == "bpod":
        my_decomp = modred.BPODHandles(
            np.vdot, max_vecs_per_node=max_vecs_per_node, verbosity=verbose
        )
        ...
        lambd_arg[filt] = 2 * np.arctan(
            lambd_i[filt] / (lambd_r[filt] + lambd_mod[filt])
        )
        ...
        raise ValueError(f"Unknown kind of decomposition : {kind}")
    ...
    if kind in ["pod", "dmd"]:
        modes = [
            modred.VecHandleInMemory(np.zeros(f_shape))
    ...
    if kind in ["pod"]:
        for n in np.arange(len(modes)):
            tmp_prof = eigvect[:, n] * eigvals.y[n] ** 0.5
            tmp_prof = Profile(
                times,
                tmp_prof,
                mask=False,
