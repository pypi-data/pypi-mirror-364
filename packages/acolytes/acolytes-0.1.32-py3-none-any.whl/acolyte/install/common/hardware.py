...
class ModelRecommender:
    ...
    MODELS = {
        "0.5b": {
            ...
            "context": 32768,
            "size": "7B-optimized",
            "ollama_model": "ticlazau/qwen2.5-coder-7b-instruct",
        },
        "1.5b": {
            "ram_min": 8,
            "vram_min": 6,
            "context": 32768,
            "size": "14B-q3_K_S",
            "ollama_model": "qwen2.5-coder:14b-instruct-q3_K_S",
        },
        "3b": {
            "ram_min": 10,
            "vram_min": 8,
    ...
    def recommend(cls, ram_gb: int, gpu_info: Optional[Dict]) -> str:
        ...
        if gpu_info and isinstance(vram_mb, int) and vram_mb >= 4096:
            ...
            if vram_mb < 1024 or vram_mb > 131072:
                ...
                if vram_mb >= 12288:  # 12GB+ VRAM
                    logger.info("Recommending full 7B model based on GPU VRAM")
                    return "14b"
                elif vram_mb >= 10240:  # 10GB VRAM
                    logger.info("Recommending 14B-q4_K_M model based on GPU VRAM")
                    return "7b"
                elif vram_mb >= 8192:  # 8GB VRAM
                    logger.info("Recommending 7B-q8_0 model based on GPU VRAM")
                    return "3b"
                elif vram_mb >= 6144:  # 6GB VRAM
                    logger.info("Recommending 14B-q3_K_S model based on GPU VRAM")
                    return "1.5b"
                else:  # 4GB+ VRAM
                    logger.info("Recommending 7B-optimized model based on GPU VRAM")
                    return "0.5b"

        # Based on RAM
        if ram_gb >= 16:
            logger.info("Recommending full 7B model based on RAM")
            return "14b"
        elif ram_gb >= 12:
            logger.info("Recommending 14B-q4_K_M model based on RAM")
            return "7b"
        elif ram_gb >= 10:
            logger.info("Recommending 7B-q8_0 model based on RAM")
            return "3b"
        elif ram_gb >= 8:
            logger.info("Recommending 14B-q3_K_S model based on RAM")
            return "1.5b"
        else:
            logger.info("Recommending 7B-optimized model based on RAM")
            return "0.5b"
...
def warn_if_gpu_missing(expected_gpu: bool, detected_gpu: bool):
    ...
    if expected_gpu and not detected_gpu:
        logger.warning("GPU not detected. Selected model will run on CPU.")
        logger.info("Performance will be slower without GPU acceleration.")
        logger.info("Consider using a lighter quantized model for better CPU performance.")
