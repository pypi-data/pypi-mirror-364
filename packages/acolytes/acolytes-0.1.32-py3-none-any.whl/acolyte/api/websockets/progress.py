...
async def websocket_progress(websocket: WebSocket, task_id: str):
    ...
    def handle_progress_event(event: Event):
        ...
        if isinstance(event, ProgressEvent) and hasattr(event, 'task_id'):
            # Accept events for this task_id (including related events)
            # This allows discovery phase events to be captured
            if event.task_id and (
                event.task_id == task_id or event.task_id.startswith(f"{task_id}_")
            ):
                    ...
                    if isinstance(event, ProgressEvent):
                        ...
                        progress_tracker.update_task(
                            ...
                            stats={
                                ...
                                "errors": event.errors,
                                "phase": (
                                    event.metadata.get("phase", "indexing")
