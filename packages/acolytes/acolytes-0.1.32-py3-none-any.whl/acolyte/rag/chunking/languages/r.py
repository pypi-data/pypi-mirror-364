...
class RChunker(LanguageChunker, PatternDetectionMixin, SecurityAnalysisMixin):
    ...
    async def chunk(self, content: str, file_path: str) -> List[Chunk]:
        ...
        logger.info("[TRACE] r.chunk override called")
        ...
        if not chunks and content.strip():
            logger.info("[TRACE] r creating module chunk for unchunked content")
            lines = content.split('\n')
    ...
    def _detect_security_issues_in_text(self, text: str) -> List[Dict[str, Any]]:
        ...
        logger.info("[TRACE] r._detect_security_issues_in_text called")
        issues = []
    ...
    def _create_chunk_from_node(
            ...
            if chunk and node.start_point[0] > 0:
                ...
                while check_line >= 0:
                    ...
                    if line.startswith("#'"):
                        ...
                        check_line -= 1
                        logger.info("[TRACE] r roxygen check backwards")
                    elif (
                        line == ''
                        and check_line > 0
                ...
                if roxygen_lines:
                    logger.info("[TRACE] r roxygen lines found")
                    roxygen_content = '\n'.join(roxygen_lines)
        ...
        # Handle S4 class definitions and imports via calls
            ...
            logger.info("[TRACE] r call node type check")
        ...
        if chunk:
            logger.info("[TRACE] r adding quality metrics")
            quality = {
    ...
    def _check_function_assignment(self, assignment_node) -> ChunkType:
        ...
        logger.info("[TRACE] r._check_function_assignment called")
    ...
    def _get_call_name(self, call_node) -> str:
        ...
        logger.info("[TRACE] r._get_call_name called")
        ...
        if node.type == 'assignment':
            ...
            if node.children and len(node.children) > 0:
                ...
                # Sometimes the identifier might be nested
                    ...
                    logger.info("[TRACE] r nested identifier search")
                ...
                if child.type == 'parameters':
                    ...
                    for param in child.children:
                        if param.type == 'identifier':
                            ...
                            logger.info("[TRACE] r default parameter extraction")
                        ...
                        if child.type == 'argument_list':
                            for arg in child.children:
                                if arg.type == 'string':
                                    ...
                                    logger.info("[TRACE] r source dependency found")
    ...
    def _get_decision_node_types(self):
        ...
        logger.info("[TRACE] r._get_decision_node_types called")
    ...
    def _check_r_eval(self, node, text: str) -> Optional[Dict[str, Any]]:
        ...
        logger.info("[TRACE] r._check_r_eval called")
    ...
    def _extract_imports(
        ...
        if import_nodes:
            ...
            while start_line > 0 and (
                ...
                logger.info("[TRACE] r including leading comments in imports")
                start_line -= 1
