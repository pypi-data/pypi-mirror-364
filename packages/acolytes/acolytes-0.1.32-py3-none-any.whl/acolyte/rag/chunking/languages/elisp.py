...
class ElispChunker(BaseChunker):
    ...
    def _get_tree_sitter_language(self) -> Any:
        ...
        logger.info("[TRACE] elisp._get_tree_sitter_language called")
    ...
    def _extract_node_name(self, node: Any) -> Optional[str]:
        ...
        logger.info("[TRACE] elisp not enough symbols for name")
        return None
            ...
            if child.type == 'string':
                ...
                for subchild in child.children:
                    if subchild.type == 'symbol' and subchild.text.decode('utf8') == 'interactive':
                        logger.info("[TRACE] elisp interactive function")
    ...
    def _extract_variable_metadata(self, var_node: Any) -> Dict[str, Any]:
        ...
        for child in var_node.children:
            if child.type == 'string':
                logger.info("[TRACE] elisp variable has docstring")
                metadata['has_docstring'] = True
                break
    ...
    def _extract_imports(
        ...
        def find_imports(node: Any, depth: int = 0) -> None:
            logger.info("[TRACE] elisp find_imports function")
        ...
        if not import_nodes:
            logger.info("[TRACE] elisp no import nodes found")
            return []
        ...
        for node in import_nodes:
            ...
            if already_done:
                logger.info("[TRACE] elisp import already processed")
                continue
