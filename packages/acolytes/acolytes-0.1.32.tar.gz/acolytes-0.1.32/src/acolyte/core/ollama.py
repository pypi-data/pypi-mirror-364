...
class OllamaClient:
    ...
    async def generate(
        ...
        3. Retry con backoff exponencial (solo para no-streaming)
        ...
        - NOTA: Streaming NO tiene retry automático, errores deben manejarse en el consumidor
        """
        ...
        # STREAMING: Retornar directamente sin retry_async
        if stream:
            return self._stream_generate(payload, cache_key)

        # NO-STREAMING: Usar retry_async como antes
        ...
        async def _do_generate():
            ...
            async with self.session.post(
                ...
                timeout=aiohttp.ClientTimeout(
                    total=600, connect=300
                ),  # 10 min timeout con 300s connect (fix NP2)
            ) as response:
    ...
    async def generate_sync(
        ...
        """
        Generate response synchronously (non-streaming only).
        This is the old generate() method for backward compatibility.
        """
        # ALWAYS use acolyte:latest regardless of what is requested
        payload = {
            "model": "acolyte:latest",
    ...
    async def _stream_generate(
        ...
        async with self.session.post(
            f"{self.base_url}/api/generate",
            json=payload,
            timeout=aiohttp.ClientTimeout(total=600, connect=300),  # fix NP2
        ) as response:
            response.raise_for_status()
    ...
    def generate_streaming(
        ...
        """
        Generate response in streaming mode only.
        Returns AsyncIterator[str] directly.
        """
        # Check cache if cache_key is provided
        if cache_key and cache_key in self.cache:
            # Move to end (most recent) for LRU
        ...
        # Retornar directamente el AsyncIterator
        return self._stream_generate(payload, cache_key)
