...
class ChunkMetadata(AcolyteBaseModel):
    ...
    git_metadata: Optional[GitMetadata] = Field(None, description="Git metadata (author, commit)")

    # --- Collection-Specific Properties ---
    # These fields are used by specific collections in Weaviate.
    # They are all Optional because a chunk only belongs to one collection.

    # Fields for the 'DocChunk' collection
    document_type: Optional[str] = Field(None, description="Type of document (e.g., MARKDOWN, RST)")
    title: Optional[str] = Field(None, description="Extracted title for the document chunk")

    # Fields for the 'ConfigChunk' collection
    config_format: Optional[str] = Field(
        None, description="Configuration format (e.g., YAML, TOML)"
    )
    key_path: Optional[str] = Field(None, description="Full key path for a configuration value")
    value: Optional[str] = Field(None, description="Value associated with the configuration key")

    # Language-specific metadata (e.g., for CodeChunk)
    language_specific: Optional[Dict[str, Any]] = Field(
        None, description="Language-specific metadata (e.g., complexity, patterns)"
    )
