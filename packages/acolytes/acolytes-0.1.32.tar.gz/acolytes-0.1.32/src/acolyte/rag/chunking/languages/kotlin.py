...
class KotlinChunker(BaseChunker, SecurityAnalysisMixin, PatternDetectionMixin):
    ...
    def _create_chunk_from_node(
        ...
        if chunk:
            if chunk.metadata.chunk_type in [
                ...
                logger.info("[TRACE] kotlin imports chunk")
    ...
    def _extract_function_metadata(self, func_node) -> Dict[str, Any]:
        ...
        for child in func_node.children:
            ...
            if child.type == 'suspend':
                logger.info("[TRACE] kotlin suspend modifier node")
                metadata['is_suspend'] = True
                modifiers = metadata['modifiers']
                assert isinstance(modifiers, list)
                modifiers.append('suspend')
            elif child.type == 'inline':
                logger.info("[TRACE] kotlin inline modifier node")
                metadata['is_inline'] = True
                modifiers = metadata['modifiers']
                assert isinstance(modifiers, list)
                modifiers.append('inline')
            elif child.type == 'operator':
                logger.info("[TRACE] kotlin operator modifier node")
                metadata['is_operator'] = True
                modifiers = metadata['modifiers']
                assert isinstance(modifiers, list)
                modifiers.append('operator')
            elif child.type == 'infix':
                logger.info("[TRACE] kotlin infix modifier node")
                metadata['is_infix'] = True
                modifiers = metadata['modifiers']
                assert isinstance(modifiers, list)
                modifiers.append('infix')
            elif child.type == 'abstract':
                logger.info("[TRACE] kotlin abstract modifier node")
                metadata['is_abstract'] = True
                modifiers = metadata['modifiers']
                assert isinstance(modifiers, list)
                modifiers.append('abstract')
                        ...
                        if c == child and i > 0:
                            ...
                            while j >= 0:
                                if children[j].type in [
                                    ...
                                    logger.info("[TRACE] kotlin extension function detection stop")
                                    break
                                j -= 1
            # Extract parameters
    ...
    def _extract_class_metadata(self, class_node) -> Dict[str, Any]:
        ...
        if first_line.startswith('data class'):
            logger.info("[TRACE] kotlin data class")
        ...
        for child in class_node.children:
            ...
            if child.type == 'data':
                logger.info("[TRACE] kotlin data modifier node")
                metadata['is_data_class'] = True
                modifiers = metadata['modifiers']
                assert isinstance(modifiers, list)
                modifiers.append('data')
            elif child.type == 'sealed':
                logger.info("[TRACE] kotlin sealed modifier node")
                metadata['is_sealed_class'] = True
                modifiers = metadata['modifiers']
                assert isinstance(modifiers, list)
                modifiers.append('sealed')
                                ...
                                if p.type in ['val', 'var']:
                                    ...
                                    for sibling in param_child.children:
                                        if sibling.type == 'simple_identifier':
                                            ...
                                            logger.info(
                                                "[TRACE] kotlin property from constructor param"
                                            )
                                            break
            ...
            # Extract supertype list
                ...
                logger.info("[TRACE] kotlin delegation specifiers")
    ...
    def _extract_property_metadata(self, prop_node) -> Dict[str, Any]:
        ...
        for child in prop_node.children:
            ...
            # Check for custom getter/setter
                ...
                logger.info("[TRACE] kotlin property getter")
                metadata['has_getter'] = True
            elif child.type == 'setter':
                logger.info("[TRACE] kotlin property setter")
    ...
    def _check_unsafe_null_assertion(self, node, text: str) -> Optional[Dict[str, Any]]:
        ...
        if '!!' in text and node.type in [
            ...
            logger.info("[TRACE] kotlin unsafe null assertion")
            return {
                            ...
                            if line.startswith('import '):
                                ...
                                if import_path:
                                    ...
                                    if root_package in [
                                        ...
                                        # Likely internal
                                        logger.info("[TRACE] kotlin internal import")
                ...
                if child.type == 'import_header':
                    ...
                    if import_path:
                        ...
                        if root_package in [
                            ...
                            # Likely internal
                            logger.info("[TRACE] kotlin internal import from list")
                            deps.add(import_path)
