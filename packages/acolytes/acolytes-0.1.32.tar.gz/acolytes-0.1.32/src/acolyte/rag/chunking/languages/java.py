...
class JavaChunker(BaseChunker, PatternDetectionMixin):
    ...
    def _get_tree_sitter_language(self) -> Any:
        ...
        logger.info("[TRACE] java._get_tree_sitter_language")
    ...
    def _create_chunk_from_node(
        ...
        if annotations:
            logger.info("[TRACE] java annotations detected")
    ...
    def _extract_class_metadata(self, node) -> Dict[str, Any]:
        ...
        while parent:
            if parent.type == 'class_body':
                logger.info("[TRACE] java inner class detected")
                    ...
                    for member in child.children:
                        if member.type == 'constructor_declaration':
                            ...
                            if 'static' in mods:
                                ...
                                logger.info("[TRACE] java static field for singleton")
                                has_static_instance = True  # Simplified

            if has_private_constructor and has_static_instance:
                logger.info("[TRACE] java singleton pattern detected")
        ...
        if name:
            if chunk_type == ChunkType.CLASS:
                ...
                if not name[0].isupper():
                    logger.info("[TRACE] java class naming convention violation")
    ...
    def _detect_security_issues(self, node, lines: List[str]) -> List[Dict[str, Any]]:
        ...
        if 'PreparedStatement' not in node_text and any(
            ...
            if '+' in node_text or 'concat' in node_text:
                logger.info("[TRACE] java SQL injection risk")
                issues.append(
                    {
            ...
            for var in suspicious_vars:
                if var in line.lower() and '=' in line and ('"' in line or "'" in line):
                    ...
                    if not any(
                        ...
                        logger.info("[TRACE] java hardcoded credential")
                        issues.append(
                            {
        ...
        if 'java.util.Random' in node_text and any(
            ...
            logger.info("[TRACE] java weak random")
            issues.append(
                {
                    'type': 'weak_random',
                    'severity': 'medium',
    ...
    def _analyze_dependencies(self, node) -> Dict[str, List[str]]:
        ...
        for ann in annotations:
            if 'springframework' in ann:
                logger.info("[TRACE] java spring framework")
                frameworks.add('spring-framework')
            elif 'junit' in ann:
                logger.info("[TRACE] java junit")
                frameworks.add('junit')
            elif 'lombok' in ann:
                logger.info("[TRACE] java lombok")
                frameworks.add('lombok')
            elif 'jackson' in ann:
                logger.info("[TRACE] java jackson")
