...
class TestLoadConfig:
    ...
    def test_load_config_invalid_yaml(self):
        ...
        with patch("pathlib.Path.exists") as mock_exists:
            mock_exists.return_value = True
            with patch("builtins.open", mock_open(read_data=invalid_yaml)), patch("click.echo") as mock_echo:
                config = load_config()
    ...
    def test_load_config_env_override(self):
        ...
        with patch("pathlib.Path.exists") as mock_exists:
            mock_exists.return_value = True
            with patch("builtins.open", mock_open(read_data=yaml_content)), patch.dict(os.environ, env_vars):
                config = load_config()
