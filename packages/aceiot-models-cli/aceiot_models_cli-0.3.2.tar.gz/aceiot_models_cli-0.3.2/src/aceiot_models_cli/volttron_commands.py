...
@click.option("--keep-archive", is_flag=True, help="Keep temporary archive file for troubleshooting")
@click.pass_context
def upload_agent(
    ctx: Context,
    path: str,
    ...
    if interactive:
        ...
        try:
            ...
            config_choice = click.prompt(
                "\nConfiguration",
                type=click.Choice(["default", "custom"]),
                default="default"
            )
...
def list_packages(ctx: Context, name: str, page: int, per_page: int) -> None:
    ...
    try:
        ...
        result = client.get_client_volttron_agent_package_list(client_name, page=page, per_page=per_page)
