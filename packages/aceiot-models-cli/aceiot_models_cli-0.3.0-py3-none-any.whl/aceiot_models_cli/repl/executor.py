...
class ReplCommandExecutor:
    ...
    def execute(self, parsed_cmd: ParsedCommand, context: ReplContext) -> Any:
        ...
        if (context.current_frame and
            context.current_frame.type == ContextType.VOLTTRON and
            command_name in ["deploy", "upload", "list", "status"]):
    ...
    def _execute_cli_command(self, parsed_cmd: ParsedCommand, context: ReplContext) -> Any:
        ...
        if (parsed_cmd.command_path == ["points", "list"] and
            context.current_gateway and
            "site" not in parsed_cmd.options):
        ...
        if (parsed_cmd.command_path == ["volttron", "list-packages"] and
            len(parsed_cmd.arguments) == 0):  # No name argument provided
        ...
        if parsed_cmd.command_path in volttron_gateway_commands:
            if parsed_cmd.command_path == ["volttron", "upload-agent"]:
                ...
                if len(parsed_cmd.arguments) == 0:  # No arguments provided
                    raise click.ClickException("Missing required argument: PATH. Usage: volttron upload-agent <path>")
                    ...
                    if context.current_gateway:
                        ...
                        raise click.ClickException("Missing required argument: NAME. Usage: volttron upload-agent <path> <name>\nTip: Use 'use client <name>' or 'use gateway <name>' to set a context first.")
            elif parsed_cmd.command_path == ["volttron", "create-config"]:
                # create-config: file, gateway
                if len(parsed_cmd.arguments) == 0:  # No arguments provided
                    raise click.ClickException("Missing required argument: FILE. Usage: volttron create-config <file>")
                elif len(parsed_cmd.arguments) == 1:  # Only file provided
                    if context.current_gateway:
                        parsed_cmd.arguments.append(context.current_gateway)
                    else:
                        raise click.ClickException("Missing required argument: GATEWAY. Usage: volttron create-config <file> <gateway>\nTip: Use 'use gateway <name>' to set a gateway context first.")
                ...
                if len(parsed_cmd.arguments) == 0:  # No gateway provided
                    if context.current_gateway:
                        ...
                        raise click.ClickException("Missing required argument: GATEWAY. Usage: volttron deploy <gateway>\nTip: Use 'use gateway <name>' to set a gateway context first.")
            elif parsed_cmd.command_path == ["volttron", "quick-deploy"]:
                # quick-deploy: path, config, gateway
                if len(parsed_cmd.arguments) == 0:  # No arguments provided
                    raise click.ClickException("Missing required arguments: PATH CONFIG. Usage: volttron quick-deploy <path> <config>")
                elif len(parsed_cmd.arguments) == 1:  # Only path provided
                    raise click.ClickException("Missing required argument: CONFIG. Usage: volttron quick-deploy <path> <config>")
                elif len(parsed_cmd.arguments) == 2:  # Path and config provided
                    if context.current_gateway:
                        parsed_cmd.arguments.append(context.current_gateway)
                    else:
                        raise click.ClickException("Missing required argument: GATEWAY. Usage: volttron quick-deploy <path> <config> <gateway>\nTip: Use 'use gateway <name>' to set a gateway context first.")
            elif parsed_cmd.command_path == ["volttron", "get-config-package"] and len(parsed_cmd.arguments) == 0:
                # get-config-package: gateway - No gateway provided
                if context.current_gateway:
                    parsed_cmd.arguments = [context.current_gateway]
                else:
                    raise click.ClickException("Missing required argument: GATEWAY. Usage: volttron get-config-package <gateway>\nTip: Use 'use gateway <name>' to set a gateway context first.")
...
"""
    ...
    def _switch_to_context(self, context_type: ContextType, name: str, context: ReplContext) -> str:
        ...
        if context_type == ContextType.CLIENT and context.current_gateway:
            ...
            return_msg = "Exited gateway context. " + self._switch_to_context(context_type, name, context)
            return return_msg
        elif context_type == ContextType.GATEWAY and context.current_client:
            # Exit client context first
            context.reset_context()
            return_msg = "Exited client context. " + self._switch_to_context(context_type, name, context)
            return return_msg
        ...
        if command == "deploy":
            ...
            if args and args[0] == "packages":
                ...
                if context.current_client or context.current_gateway:
                    return "Usage: list packages (client/gateway will be auto-detected from context)"
                else:
