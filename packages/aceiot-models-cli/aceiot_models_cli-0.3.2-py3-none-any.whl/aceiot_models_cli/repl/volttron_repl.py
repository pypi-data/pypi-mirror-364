...
class VolttronReplHandler:
    ...
    def handle_deploy_wizard(self, context: ReplContext) -> str:
        ...
        agent_identity = click.prompt("Agent identity", default=package_name.lower().replace(" ", "."))
    ...
    def _upload_package_to_gateway(self, gateway: str) -> str | None:
        ...
        if selection == 1:
            ...
            try:
                ...
                if not client_name:
                    self.console.print(f"[red]Could not determine client for gateway '{gateway}'[/red]")
                    return None
    ...
    def _create_config_on_gateway(self, gateway: str, agent_identity: str, config_name: str) -> dict | None:
        ...
        if selection == 1:
            # Create new config
            config_type = click.prompt("Configuration type", type=click.Choice(["file", "inline"]), default="file")

            if config_type == "file":
                path = click.prompt(
                    "Enter path to configuration file (JSON or YAML)",
    ...
    def _select_gateway(self) -> str | None:
        ...
        try:
            ...
            selection = click.prompt(
                f"\nEnter number (1-{len(all_gateways)}) or gateway name",
                type=str
            )
    ...
    def _select_existing_package(self, gateway: str) -> str | None:
        ...
        try:
            ...
            result = self.api_client.get_client_volttron_agent_package_list(client_name, per_page=100)
            items = result.get("items", [])
            ...
            selection = click.prompt(
                f"\nEnter number (1-{len(items)}) or package name",
                type=str
            )
