...
# 单独生成透明背景版本
result = generate_clean_interpolation_map(data, 'cod', 'output.png', transparent_bg=True, output_format='png')
"""
import os
import logging
...
def enhanced_interpolation_with_neighborhood(all_data, grid_resolution=200, method='linear', 
    ...
    if boundary_method == 'kml' and kml_boundary_path and os.path.exists(kml_boundary_path):
        ...
        if kml_boundary_points is not None:
            ...
            # 使用扩展的数据集进行插值
            grid_values = griddata(extended_points, extended_values, (grid_lon, grid_lat), method=method)
        else:
            # 如果无法获取KML边界点，使用原始插值
