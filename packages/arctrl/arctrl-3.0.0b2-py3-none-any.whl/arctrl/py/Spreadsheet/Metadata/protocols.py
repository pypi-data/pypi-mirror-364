...
def from_sparse_table(matrix: SparseTable) -> FSharpList[Protocol]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow958(i: int, matrix: Any=matrix) -> Protocol:
        ...
        return initialize(matrix.ColumnCount, _arrow958)
...
def to_sparse_table(protocols: FSharpList[Protocol]) -> SparseTable:
    ...
    class ObjectExpr961:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow960(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr961())), matrix.ColumnCount)
