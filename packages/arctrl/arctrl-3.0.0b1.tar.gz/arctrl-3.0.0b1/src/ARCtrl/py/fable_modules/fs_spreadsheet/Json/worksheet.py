...
decode_rows: Decoder_1[FsWorksheet] = object(_arrow331)
...
def encode_columns(no_numbering: bool, sheet: FsWorksheet) -> IEncodable:
    ...
    def _arrow335(__unit: None=None, no_numbering: Any=no_numbering, sheet: Any=sheet) -> IEnumerable_1[IEncodable]:
        def _arrow334(c: int) -> IEncodable:
            def _arrow333(__unit: None=None) -> IEnumerable_1[FsCell]:
                def _arrow332(r: int) -> IEnumerable_1[FsCell]:
                    match_value: FsCell | None = FsCellsCollection__TryGetCell_Z37302880(sheet.CellCollection, r, c)
                    return singleton(FsCell("")) if (match_value is None) else singleton(match_value)

                return collect(_arrow332, range_big_int(1, 1, sheet.MaxRowIndex))

            return encode_no_numbers_1(to_list(delay(_arrow333)))

        return map(_arrow334, range_big_int(1, 1, sheet.MaxColumnIndex))

    def mapping(col_1: FsColumn, no_numbering: Any=no_numbering, sheet: Any=sheet) -> IEncodable:
        return encode_2(col_1)

    j_columns: IEncodable = seq(to_list(delay(_arrow335))) if no_numbering else seq(map(mapping, sheet.Columns))
...
decode_columns: Decoder_1[FsWorksheet] = object(_arrow343)
