...
def _arrow2679(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2680(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2681(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ROCrate_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2679, ROCrate_decoder_1), map(_arrow2680, ROCrate_decoder_2), map(_arrow2681, ROCrate_decoder_3)]))
...
def _arrow2685(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2686(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2687(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ISAJson_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2685, ISAJson_decoder_1), map(_arrow2686, ISAJson_decoder_2), map(_arrow2687, ISAJson_decoder_3)]))
