...
def encode_columns(no_numbering: bool, sheet: FsWorksheet) -> IEncodable:
    ...
    def _arrow339(__unit: None=None, no_numbering: Any=no_numbering, sheet: Any=sheet) -> IEnumerable_1[tuple[str, IEncodable]]:
        ...
        def _arrow338(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
            ...
            return append(singleton(("tables", seq(map(encode_1, sheet.Tables)))) if (not is_empty(sheet.Tables)) else empty(), delay(_arrow337))

        return append(singleton(("name", _arrow335())), delay(_arrow338))

    values_1: IEnumerable_1[tuple[str, IEncodable]] = to_list(delay(_arrow339))
