...
class ProgramResources:
    ...
    def update(self) -> None:
        ...
        self.dotnet_binary = DotNetInstaller(self.root).get_home_binary()
...
class GeneratorData:
    ...
    def get_url_zip(self, platform: IPlatform) -> IRepoZipData:
        match platform:
            case "osx":
                return self.osx
            case "linux":
...
class PkgManager:
    ...
    def get_generator_url_zip(
        self, version: Version, platform: IPlatform
    ) -> IRepoZipData:
        try:
            return self.get_data().generators[version].get_url_zip(platform)
        except (KeyError, ValueError):
            raise ValueError(
                f"Generator version {version} for {platform} is not installed. "
    ...
    def install_generator(
        self, root: str, version: Version, platform: IPlatform
    ) -> None:
        url = self.get_generator_url_zip(version, platform)
        GeneratorDownloader(
            repo=url,
    ...
    def check_generators(self) -> None:
        ...
        for name, pkg in self.resources.packages.iter_packages():
            if not self.resources.generators.has_generator(pkg.generator, platform):
                ...
                self.pkg_mgr.install_generator(self.resources.root, version, platform)
                installed = True
        if installed:
            self.resources.update()
...
class DotNetInstaller:
    ...
    def get_home_binary(self) -> str | None:
        dotnet_binary = os.path.join(os.environ.get("HOME",""), ".dotnet", "dotnet")
        if os.path.exists(dotnet_binary):
    ...
    def install(self) -> str | None:
        if platform.system() != "Linux":
            if self.explicit:
                log.error(
                    "DotNet can only be installed dynamically on linux platform. "
                    "Please install .net manually"
                )
            return None

        dotnet_binary = self.get_home_binary()

        if dotnet_binary and not self.overrides:
            log.info(f"Dotnet binary already installed at {dotnet_binary}")
            return dotnet_binary

        dotnet_path = _make_tree(self.root, "dotnet")
        dotnet_script = os.path.join(dotnet_path, "dotnet-install.sh")

        req = get_request(self.script_url)
        with open(dotnet_script, "wb") as fd:
            for chunk in req.iter_content(chunk_size=128):
                fd.write(chunk)

        os.chmod(dotnet_script, 0o755)
        os.system(f"{dotnet_script} --channel 7.0  --runtime aspnetcore")
        return self.get_home_binary()
...
class ExposedMethod:
    ...
    def from_dict(cls, data: dict[str, Any]) -> ExposedMethod:
        ...
        inputs = ExposedProperties.from_list(data.pop("inputs", []))
        outputs = ExposedProperties.from_list(data.pop("outputs", []))
...
class ExposedClass:
    ...
    def from_dict(cls, data: dict[str, Any]) -> ExposedClass:
        ...
        properties = ExposedProperties.from_list(data.pop("properties", []))
...
class ExposedEnumMember:
    ...
    @staticmethod
    def from_dict(data: dict[str, Any]) -> ExposedEnumMember:
        data = data.copy()
        try:
            label = data["label"]
            value = data["value"]
        ...
        return ExposedEnumMember(
            label=label, value=value, comment=data.get("comment", "")
        )
...
class ExposedEnum:
    ...
    @staticmethod
    def from_dict(data: dict[str, Any]) -> ExposedEnum:
        data = data.copy()
        try:
            name = data["name"]
        except KeyError:
            raise ValueError("Missing name keyword for enum declaration")
        if extras := (set(data) - {"name", "entries"}):
            raise ValueError(f"Extra keyword in Enum definition: {extras}")
        entries = [ExposedEnumMember.from_dict(d) for d in data.get("entries", [])]
        return ExposedEnum(name=name, entries=entries)

    def export(self) -> dict[str, Any]:
        return {"Name": self.name, "Entries": [e.export() for e in self.entries]}
...
class Model:
    ...
    def from_dict(cls, data: dict[str, Any]) -> Model:
        data = data.copy()
        classes = [ExposedClass.from_dict(d) for d in data.pop("classes", [])]
        properties = ExposedProperties.from_list(data.pop("properties", []))
        methods = ExposedMethods.from_list(data.pop("methods", []))
        enums = ExposedEnums.from_list(data.pop("enums", []))
        return Model(
    ...
    def export_to_scriban(self) -> str:
        ...
        if self.classes:
            ...
            scriban.append(f"ExposedClasses = {json_classes}")

        if self.properties.size:
            json_properties = json.dumps(self.properties.export())
            scriban.append(f"ExposedProperties = {json_properties}")
            scriban.append("ExposedObjects = ExposedProperties")

        if self.methods.size:
            json_methods = json.dumps(self.methods.export())
            scriban.append(f"ExposedMethods = {json_methods}")

        if self.enums.size:
            json_enums = json.dumps(self.enums.export())
            scriban.append(f"Enums = {json_enums}")
...
class InstallDotnetCommand:
    ...
    def execute(self, argv: list[str]) -> None:
        args = self.parse(argv)
        _init_logger(log, "INFO", True)
        DotNetInstaller(overrides=args.force, explicit=True).install()
...
class InstallGeneratorCommand:
    ...
    def execute(self, argv: list[str]) -> None:
        ...
        pkg_mgr.install_generator(CONFIG_DIR, version, platform)
...
class InstallAllCommand:
    ...
    def execute(self, argv: list[str]) -> None:
        ...
        pkg_mgr = PkgManager()
        platform = find_platform()
        DotNetInstaller(overrides=args.force, explicit=False).install()
...
class InfoCommand:
    ...
    def get_argument_parser(self, *subcommands: str) -> argparse.ArgumentParser:
        ...
        arg_parser.add_argument(
            "category", choices=["template", "model", "config", "custom_model"], nargs="?",
        )
        arg_parser.add_argument(
            "subargs", nargs=argparse.REMAINDER, help=argparse.SUPPRESS
        )
