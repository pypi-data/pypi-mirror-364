    ...
    passport.use(
        new GoogleStrategy(
            ...
            async (issuer, profile, done) => {
                console.log('Logged in with Google. Issuer:', issuer, 'Profile:', profile);
                const email  = profile.emails[0].value;
                // make sure email address is in stackgen.com domain
                if (!email.endsWith('@stackgen.com')) {
                    done(new Error(`${email} is not allowed to login`));
                    return;
                }
