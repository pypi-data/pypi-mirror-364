...
from ai_app.config.external import GitLabParameters, JiraParameters, SonarQubeParameters
...
from ai_app.tools import GitLabToolkit, JiraToolkit, SonarQubeToolkit, SqlToolkit
...
def get_jira_toolkit():
    return JiraToolkit(**get_jira_kwargs())
...
def get_sql_toolkit(connection_context_name: str) -> SqlToolkit:
    ...
    schema, tables = next(
        iter(connection_context.get_tables_by_schema().items())
    )  # Todo: support multiple schemas.
    if engine.dialect.name.lower() == "postgresql" and schema and schema.lower() == "public":
        # Due to a bug in langchain_community SqlToolkit, need to avoid errors
        # from attempting a parametrized "SET search path %s" statement.
        schema = None

    model = get_chat_model("gpt-4.1")
    toolkit = SqlToolkit(
        engine=engine,
        sql_checker_model=model,
        schema=schema,
