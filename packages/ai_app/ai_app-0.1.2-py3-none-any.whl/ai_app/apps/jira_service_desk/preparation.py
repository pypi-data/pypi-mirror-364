...
from ai_app.apps.jira_service_desk.data import (
    ...
    get_it_service_desk_id,
    get_saved_request_type_primary_keys,
)
from ai_app.apps.jira_service_desk.tools import get_postgres_vector_store
from ai_app.config import get_ai_postgres_engine, get_chat_model, get_jira, get_service_desk
...
def get_request_types_to_ignore():
    ignore_requests = {
        3095: "Eldf bot automatic requests",
    }
    return ignore_requests
...
def update_jira_requests():
    f"""Updates {JiraRequest} and {InputRequestField} tables."""
    engine = get_ai_postgres_engine()
    service_desk_id = get_it_service_desk_id()
    service_desk = get_service_desk()
    service_desks_request_types = get_service_desks_request_types(
        service_desk=service_desk, service_desk_id=service_desk_id, verbose=True
    )

    with open_session(engine) as session:
        for request_type_id, request_type in tqdm.auto.tqdm(service_desks_request_types.items()):
            if request_type_id in get_request_types_to_ignore():
                continue
...
def generate_requests(
    ...
    jira = get_jira()
    service_desk = get_service_desk()
    fields = fields or get_request_field_names(service_desk, service_desk_id, request_type_id)
...
def update_ai_jira_request_guides(**kwargs):
    f"""Updates {AiJiraRequestGuide} table."""
    with open_session(get_ai_postgres_engine()) as session:
        for request in tqdm.auto.tqdm(
            session.exec(sqlmodel.select(JiraRequest)),
...
def summarize_request_type(
    ...
    summarizer = Summarizer(
        model=model or get_chat_model("gpt-4.1-mini"),
