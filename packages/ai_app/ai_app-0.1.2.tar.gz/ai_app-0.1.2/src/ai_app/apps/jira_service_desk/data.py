...
from ai_app.sql import (
    build_foreign_key_constraint,
    get_cosine_distance_embedding_sqlmodel_mixin,
    open_session,
)
from ai_app.utils import get_utc_now


def get_it_service_desk_id():
    return 21
...
class JiraRequestGuideEmbedding(
    JiraRequestCommonColumns,
    get_cosine_distance_embedding_sqlmodel_mixin(1024),
    # table=True,
):
    """
    Table for manual storing embeddings of Jira request guides.
    Deprecated in favor of purpose built packages, like LangChain, LlamaIndex or Haystack.
    """
...
def fetch_use_case_guides_for_requests(primary_keys: list[JiraRequestPrimaryKey]) -> list[dict]:
    guides = fetch_guides(primary_keys, only_recommended=True)
    guides = [
        dict(
