...
def build_custom_analyzer(
    ...
    # Score is the confidence value from 0 to 1 that the pattern matched something worth anonymizing.
    patterns = [
        presidio_analyzer.Pattern(
            name="AZ_PASSPORT", regex=r"\b(AR|AZ|AZE|AA|MYI|MYİ|MOM|QV|DYI|DYİ)\d{6,}\b", score=0.9
        ),
        # presidio_analyzer.Pattern(name="NUMERIC_ID", regex=r"\b\d{7,}\b", score=0.7), # Too many false positives.
        presidio_analyzer.Pattern(name="KOD", regex=r"\bKOD ?\d{6,}\b", score=0.7),
        presidio_analyzer.Pattern(
            ...
            regex=(
                ...
                r"(?=(?:[A-Z]*[0-9]){2})"  # Look ahead for at least 2 numbers using a non-capturing group.
...
class AnonCodec:
    ...
    def deanonymize(self, text: str, codec: EntityTypeCodec | None = None) -> str:
        codec = codec or self.codec
