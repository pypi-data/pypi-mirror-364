...
from ai_app.utils import get_number_of_batches
...
def fetch_all_jira_usernames(jira: atlassian.Jira) -> list[str]:
    ...
    users = []
    while True:
        batch_users = jira.user_find_by_user_string(
            username=".",
            start=start,
            limit=1000,
        )
...
def extract_issue_field_values(issue: dict) -> dict:
...
def get_request_fields(
    ...
    df = df.with_columns(
        pl.col("jiraSchema")
        .struct["type"]
        .replace_strict(can_be_passed_as_url_parameter)
        .alias("can_be_passed_as_url_parameter"),
    )
    try:
