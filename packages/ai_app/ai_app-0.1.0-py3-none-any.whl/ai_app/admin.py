...
@prefect.flow
def sync_dwh_tables_flow():
    assert False, "Do not sync, use source"
    get_config().setup_logging()
    logger = prefect.get_run_logger()
    connection_schema_tables = {
        "DWH": {
            "analytics_coe": ["employees", "hiring_metrics", "structure_metrics_with_predictions"]
        }
    }
    logger.info(f"Syncing DWH tables {connection_schema_tables}")
    sync_dwh_tables(connection_schema_tables)
...
def update_ai_jira_request_summaries_and_embeddings():
    ...
    prefect.task(update_ai_jira_request_guides)(
        model=get_chat_model("gpt-4.1-mini"),
        limit_requests=100,
    )
