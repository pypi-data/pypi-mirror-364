...
class SummarizationStateBase(pydantic.BaseModel):
    items_summarized: int = pydantic.Field(
        default=0,
        description="Number of items summarized, a meta field which automatically updates.",
    )


class Summarizer:
    def __init__(self, model, max_tokens: int, is_json: bool = False, context: str | None = None):
        self.model = model
    ...
    def build_prompt(
        self, state: SummarizationStateBase | type[SummarizationStateBase], items: list[str | dict]
    ) -> str:
        current_state = (
            ""
...
def render_tool_call(
    name: str, args: dict, response: str | None = None, background_color: str = "#f6f8fa"
) -> str:
    args = to_formatted_markdown(json.dumps(args, indent=2))
    if not response:
        status = "Calling tool"
        response = ""
    else:
        ...
        try:
            ...
            response = to_formatted_markdown(json.dumps(response, indent=2))
    ...
    html = textwrap.dedent(
        """
        <details style="background-color: {background_color};">
...
async def astream_response_messages(
    ...
    # TODO: need to check the order of updates and values, if update is emitted before values,
    # which it probably is, then the AstreamResponse may have new formatted message,
    # but stale state.
    state = {}
        ...
        match stream_mode:
            ...
            case "updates":
                for update_value in payload.values():
                    new_messages.extend(update_value.get("messages", []))
