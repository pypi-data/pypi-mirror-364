    ...
    def build_gradio_chat_interface(
        ...
        **kwargs,
        ...
        chat_interface = gr.ChatInterface(
            fn=self._respond,
            type="messages",
            additional_inputs=[model_choice, state] + (additional_inputs or []),
            additional_outputs=(additional_outputs or []) + [state],
            api_name=self.get_chat_api_name(),
            **get_config().common_chat_interface_parameters,
            **kwargs,
        )
        chat_interface.load(State.from_request, outputs=state)
